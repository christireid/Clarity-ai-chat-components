...
export class ApiKeyManager {
    ...
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel('clarityApiKeys', 'Clarity Chat API Keys', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async saveApiKey(context, provider, key) {
        await context.secrets.store(`clarity-chat.${provider}-key`, key);
    }
    async deleteApiKey(context, provider) {
    ...
    async getHtmlForWebview(webview, context) {
        const providers = ['openai', 'anthropic', 'google'];
        const keys = await Promise.all(providers.map(async (p) => ({
            provider: p,
