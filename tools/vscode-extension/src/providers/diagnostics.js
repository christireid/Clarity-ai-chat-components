...
export class DiagnosticsProvider {
    ...
    updateDiagnostics(document) {
        ...
        if (this.hasProviderImport(text) && !this.hasApiKeyConfig(text)) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), 'API key configuration missing. Add your API key to environment variables.', vscode.DiagnosticSeverity.Warning);
            ...
            for (const match of matches) {
                if (match.index !== undefined) {
                    ...
                    diagnostic.relatedInformation = [
                        new vscode.DiagnosticRelatedInformation(new vscode.Location(document.uri, range), `Replace with: ${replacement}`),
                    ];
                    diagnostics.push(diagnostic);
                }
            }
        ...
        if (this.hasAsyncChatCall(text) && !this.hasTryCatch(text)) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), 'Consider adding error handling for AI API calls', vscode.DiagnosticSeverity.Information);
        ...
        if (this.hasMultipleSequentialCalls(text)) {
            const diagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), 'Multiple sequential API calls detected. Consider batching or using streaming.', vscode.DiagnosticSeverity.Hint);
        ...
        if (this.hasHardcodedApiKey(text)) {
            ...
            for (const match of matches) {
                if (match.index !== undefined) {
                    const position = document.positionAt(match.index);
                    const range = new vscode.Range(position, position.translate(0, match[0].length));
                    const diagnostic = new vscode.Diagnostic(range, '⚠️ Hardcoded API key detected! Use environment variables instead.', vscode.DiagnosticSeverity.Error);
...
export class QuickFixProvider {
    provideCodeActions(document, range, context) {
        ...
        for (const diagnostic of context.diagnostics) {
            if (diagnostic.source !== 'clarity-chat')
                continue;
    ...
    createDeprecationFix(document, diagnostic) {
        const fix = new vscode.CodeAction('Update to new API', vscode.CodeActionKind.QuickFix);
    ...
    createErrorHandlingFix(document, diagnostic) {
        const fix = new vscode.CodeAction('Add error handling', vscode.CodeActionKind.QuickFix);
        fix.diagnostics = [diagnostic];
        fix.command = {
    ...
    createApiKeyFix(document, diagnostic) {
        const fix = new vscode.CodeAction('Move API key to environment variable', vscode.CodeActionKind.QuickFix);
        fix.diagnostics = [diagnostic];
        fix.edit = new vscode.WorkspaceEdit();
    ...
    createAddApiKeyFix(document, diagnostic) {
        const fix = new vscode.CodeAction('Add API key configuration', vscode.CodeActionKind.QuickFix);
        fix.diagnostics = [diagnostic];
        fix.command = {
