...
const HOOKS = [
  ...
  warningThreshold: \${3:0.8},
    ...
    },
    {
        ...
        category: 'Token Optimization',
        imports: "import { useTokenOptimization } from '@clarity-chat/react'",
        code: `const {
  optimizedMessages,
  compressionRatio,
  applyOptimization,
...
export async function addHookCommand(context) {
    ...
    const recentNames = context.globalState.get(RECENT_KEY, []);
    const pinnedNames = context.globalState.get(PINNED_KEY, []);
    ...
    const buildItems = () => {
        ...
        const recentItems = HOOKS.filter((h) => recentNames.includes(h.value) && !pinnedNames.includes(h.value))
    ...
    picker.onDidAccept(async () => {
        ...
        const hasImport = text.includes(originalHook.value) && text.includes('@clarity-chat/react');
        ...
        const action = await vscode.window.showInformationMessage(`✓ Added ${originalHook.value}`, 'Pin Hook', 'View Docs');
        if (action === 'Pin Hook') {
            const newPinned = [
                ...new Set([...pinnedNames, originalHook.value]),
            ].slice(0, 5);
            context.globalState.update(PINNED_KEY, newPinned);
            vscode.window.showInformationMessage(`${originalHook.value} pinned for quick access!`);
        }
        else if (action === 'View Docs') {
            vscode.env.openExternal(vscode.Uri.parse(`https://docs.claritychat.dev/hooks/${originalHook.value.toLowerCase()}`));
        }
    });
    picker.show();
}
//# sourceMappingURL=addHook.js.map