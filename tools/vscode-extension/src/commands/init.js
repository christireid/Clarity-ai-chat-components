...
export async function initProjectCommand() {
    ...
    await vscode.window.withProgress({
        ...
        vscode.window.showInformationMessage('Clarity Chat project initialized!', 'View Files', 'Install Dependencies').then(selection => {
...
function generateEnvTemplate(provider) {
    ...
    if (provider === 'openai' || provider === 'all') {
        lines.push('# OpenAI API Key', '# Get from: https://platform.openai.com/api-keys', 'OPENAI_API_KEY=sk-...', '');
    }
    if (provider === 'anthropic' || provider === 'all') {
        lines.push('# Anthropic API Key', '# Get from: https://console.anthropic.com/settings/keys', 'ANTHROPIC_API_KEY=sk-ant-...', '');
    }
    if (provider === 'google' || provider === 'all') {
        lines.push('# Google AI API Key', '# Get from: https://makersuite.google.com/app/apikey', 'GOOGLE_API_KEY=...', '');
    }
    return lines.join('\n');
}
function generateInstallInstructions(provider, framework) {
    const packages = [];
    if (provider === 'openai' || provider === 'all') {
