...
const IMPORT_CONVERSIONS: ConversionPattern[] = [
  {
...
export async function convertToClarityCommand() {
  ...
  if (!hasVercelAI && !hasUseChat) {
    const action = await vscode.window.showInformationMessage(
      'No Vercel AI SDK usage detected in this file.',
  ...
  if (hasAlreadyConverted) {
    const proceed = await vscode.window.showWarningMessage(
      'This file already uses @clarity-chat/react. Continue with conversion?',
      'Continue',
      'Cancel'
    )
  ...
  const action = await vscode.window.showInformationMessage(
    `Converted ${changes.length} patterns to Clarity Chat!`,
...
function analyzeChanges(text: string): ConversionPattern[] {
  const changes: ConversionPattern[] = []
...
async function applyChanges(editor: vscode.TextEditor, originalText: string) {
...
async function showPreview(document: vscode.TextDocument, changes: ConversionPattern[]) {
  ...
  await vscode.commands.executeCommand(
    ...
    originalUri,
    modifiedUri,
    `Migration Preview: ${document.fileName.split('/').pop()}`
  )

  // Show apply option
  const action = await vscode.window.showInformationMessage(
    'Review the changes above. Apply them?',
    'Apply Changes',
    'Cancel'
  )
