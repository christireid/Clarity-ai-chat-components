...
export async function addComponentCommand(context) {
    ...
    const buildItems = () => {
        ...
        const recentItems = COMPONENTS.filter((c) => recentNames.includes(c.value) && !pinnedNames.includes(c.value))
    ...
    picker.onDidAccept(async () => {
        ...
        const hasImport = text.includes(originalComponent.value) &&
        ...
        const action = await vscode.window.showInformationMessage(`✓ Added ${originalComponent.value}`, 'Pin Component', 'View Docs');
        if (action === 'Pin Component') {
            const newPinned = [
                ...new Set([...pinnedNames, originalComponent.value]),
            ].slice(0, 5);
            context.globalState.update(PINNED_KEY, newPinned);
            vscode.window.showInformationMessage(`${originalComponent.value} pinned for quick access!`);
        }
        else if (action === 'View Docs') {
            vscode.env.openExternal(vscode.Uri.parse(`https://docs.claritychat.dev/components/${originalComponent.value.toLowerCase()}`));
        }
    });
    picker.show();
}
