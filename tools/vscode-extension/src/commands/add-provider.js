...
export async function addProviderCommand() {
    ...
    const installAction = await vscode.window.showInformationMessage(`Add ${provider.label} to your project?`, 'Install Package', 'Setup Instructions', 'Get API Key');
    if (installAction === 'Install Package') {
        vscode.window.showInformationMessage(`Run: npm install ${provider.package}`, 'Copy Command').then(selection => {
...
function showSetupInstructions(provider) {
    const panel = vscode.window.createWebviewPanel('claritySetup', `Setup ${provider.label}`, vscode.ViewColumn.One, {});
    panel.webview.html = `
    <!DOCTYPE html>
    <html>
    <head>
...
function getProviderExample(provider) {
    const examples = {
