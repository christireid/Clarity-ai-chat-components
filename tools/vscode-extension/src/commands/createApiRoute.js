...
export async function createApiRouteCommand(_context, targetUri) {
    ...
    if (fileExists) {
        const overwrite = await vscode.window.showWarningMessage(`File ${framework.fileName} already exists. Overwrite?`, 'Overwrite', 'Cancel');
    ...
    const action = await vscode.window.showInformationMessage(`API route created at ${framework.directory}/${framework.fileName}`, 'Add Environment Variables', 'View Documentation');
...
function generateApiRouteCode(provider, framework, streaming) {
    switch (framework) {
        case 'nextjs-app':
...
function generateNextJsAppRoute(provider, streaming) {
    if (provider === 'multi') {
...
function generateNextJsPagesRoute(provider, streaming) {
...
function generateExpressRoute(provider, streaming) {
...
function generateHonoRoute(provider, streaming) {
