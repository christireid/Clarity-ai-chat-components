...
suite('Import Utils Test Suite', () => {
    ...
    suite('Import regex patterns', () => {
        ...
        const importRegex = /import\s+\{([^}]+)\}\s+from\s+['"]@clarity-chat\/react['"]/;
        ...
        test('should match multiline import', () => {
            const multilineRegex = /import\s+\{([^}]+)\}\s+from\s+['"]@clarity-chat\/react['"]/s;
            const code = `import {
  ClarityChat,
  ChatInput,
  MessageList
    ...
    suite('Import merging logic', () => {
        function mergeImports(existingImports, newImports) {
...
//# sourceMappingURL=import-utils.test.js.map