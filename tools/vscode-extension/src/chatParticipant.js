 ...
 */
export function registerChatParticipant(context) {
    // Check if chat API is available (VS Code 1.90+)
    if (!vscode.chat) {
        console.log('Chat API not available - Copilot features disabled');
        return;
    }
    const participant = vscode.chat.createChatParticipant('clarity-chat.assistant', handleChatRequest);
...
async function handleChatRequest(request, _context, response, _token) {
    ...
    else if (prompt.includes('migrate') ||
...
async function handleComponentRequest(request, response) {
...
async function handleHookRequest(request, response) {
    ...
    response.markdown('| `useTokenOptimization` | Advanced optimization |\n');
...
async function handleMemoryRequest(response) {
...
async function handleOptimizationRequest(response) {
    response.markdown('# Token Optimization\n\n');
    ...
    response.markdown('```tsx\n');
    response.markdown('const { optimizedMessages, compressionRatio } = useTokenOptimization({\n');
    response.markdown('  messages,\n');
...
async function handleMigrationRequest(response) {
...
async function handleStreamingRequest(response) {
...
async function handleApiRouteRequest(response) {
...
async function handleDocsRequest(request, response) {
...
async function handleExamplesRequest(response) {
...
async function handleGeneralRequest(prompt, response) {
