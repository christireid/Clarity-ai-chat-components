 ...
 */
function findMatchIndices(text, query, caseSensitive) {
    const indices = [];
 ...
 */
function fuzzyMatch(text, query, caseSensitive) {
    const searchText = caseSensitive ? text : text.toLowerCase();
    const searchQuery = caseSensitive ? query : query.toLowerCase();
    const indices = [];
    let queryIndex = 0;
...
export class SearchEngine {
    ...
    constructor(items = [], defaultOptions) {
        this.items = items;
        if (defaultOptions) {
    ...
    search(query, options) {
        ...
        const { threshold, limit, fields, fieldBoosts, caseSensitive: _caseSensitive, } = opts;
     ...
     */
    getSearchableValues(item, fields) {
     ...
     */
    matchValue(value, query, options) {
 ...
 */
export function highlightMatches(text, indices, startTag = '**', endTag = '**') {
    if (indices.length === 0)
        return text;
 ...
 */
export function scoreMultiWordMatch(text, query, options) {
 ...
 */
export function createSearchableString(obj) {
    const parts = [];
 ...
 */
export const componentSearchOptions = {
    threshold: 0.3,
    limit: 20,
 ...
 * Hook search options
 */
export const hookSearchOptions = {
    threshold: 0.3,
    limit: 20,
