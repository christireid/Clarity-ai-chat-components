...
import { getServerHealth, getDiagnostics, metrics, formatBytes, formatDuration, } from '../utils/health.js';
...
import { searchComponents, searchHooks, getComponent, getHook, COMPONENTS, HOOKS as _HOOKS, } from '../data/component-registry.js';
...
// =============================================================================
export async function handleEnhancedToolCall(name, args) {
...
async function handleHealth(args) {
    ...
    const health = await getServerHealth(SERVER_VERSION, COMPONENTS.length + pluginStats.tools, totalResources, totalPrompts, { total: pluginStats.total, enabled: pluginStats.enabled });
    const result = {
...
function handleMetrics() {
    ...
    const totalResources = BASE_RESOURCE_COUNT + pluginStats.resources;
    const totalPrompts = BASE_PROMPT_COUNT + pluginStats.prompts;
    const serverMetrics = metrics.getMetrics(COMPONENTS.length + pluginStats.tools, totalResources, totalPrompts, pluginStats);
    return {
        ...
        tools: {
            ...
            errorRates: Object.entries(serverMetrics.tools.errors).map(([name, errors]) => ({
                name,
                errors,
...
function handleBatchGetDocs(args) {
...
function handleBatchGenerateCode(args) {
    const components = args.components;
    const variant = args.variant || 'typescript';
...
function handleSmartSuggest(args) {
    const context = args.context;
    const currentComponents = args.currentComponents || [];
    if (!context) {
    ...
    for (const core of coreComponents) {
        if (!currentComponents.includes(core) &&
...
function handlePluginInfo(args) {
...
function handleAdvancedSearch(args) {
    const query = args.query;
    const types = args.types || ['component', 'hook', 'model'];
...
function handleComponentTree(args) {
    const rootName = args.rootComponent || 'ClarityChat';
    const maxDepth = args.depth || 3;
    function buildTree(componentName, depth, visited) {
...
function handleCompareComponents(args) {
    ...
    const comparison = {
        ...
        summary: {
            mostProps: components.reduce((prev, curr) => prev.props.length > curr.props.length ? prev : curr).name,
...
function handleFeatureMatrix(args) {
