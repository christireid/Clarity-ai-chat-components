...
describe('handleToolCall', () => {
    ...
    describe('get_example', () => {
        ...
        it('should throw NotFoundError when example not found', async () => {
            await expect(handleToolCall('get_example', { exampleName: 'nonexistent' })).rejects.toThrow();
        });
        it('should validate required parameter', async () => {
            await expect(handleToolCall('get_example', {})).rejects.toThrow();
        });
    });
    describe('get_model_info', () => {
        ...
        it('should throw NotFoundError when model not found', async () => {
            await expect(handleToolCall('get_model_info', { modelName: 'gpt-999' })).rejects.toThrow();
        });
        it('should validate required parameter', async () => {
            await expect(handleToolCall('get_model_info', {})).rejects.toThrow();
        });
    });
    describe('calculate_cost', () => {
        ...
        it('should validate required parameters', async () => {
            await expect(handleToolCall('calculate_cost', { modelName: 'gpt-4-turbo' })).rejects.toThrow();
        });
        it('should validate token counts are non-negative', async () => {
            await expect(handleToolCall('calculate_cost', {
                modelName: 'gpt-4-turbo',
                promptTokens: -1,
    ...
    describe('init_project', () => {
        ...
        it('should validate required parameters', async () => {
            await expect(handleToolCall('init_project', {})).rejects.toThrow();
        });
    ...
    describe('validate_config', () => {
        ...
        it('should validate required parameter', async () => {
            await expect(handleToolCall('validate_config', {})).rejects.toThrow();
        });
    });
    describe('analyze_project', () => {
        beforeEach(() => {
            vi.mocked(fs.readdir).mockResolvedValue(['package.json', '.env.local', 'tsconfig.json']);
            vi.mocked(fs.access).mockResolvedValue(undefined);
            vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify({
        ...
        it('should throw NotFoundError when directory does not exist', async () => {
            ...
            await expect(handleToolCall('analyze_project', {
