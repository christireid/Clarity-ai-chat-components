...
import { validateInput, DiscoverComponentsSchema, GetComponentDocsSchema, DiscoverHooksSchema, GetHookDocsSchema, GetAccessibilitySchema, GenerateCodeSchema, GetRelatedComponentsSchema, } from '../utils/schemas.js';
import { searchComponents, searchHooks, getComponent, getHook, getRelatedComponents, getCategoryStats, COMPONENTS, HOOKS, } from '../data/component-registry.js';
import { getModel, calculateCost as calculateModelCost, getAllModels, formatPricing, } from '../data/model-registry.js';
/**
 * Available tools
 */
export const tools = [
    ...
    { "name": "ChatHeader", "description": "Header with title and actions", "category": "chat" }
        ...
        inputSchema: {
            ...
            properties: {
                query: {
                    type: 'string',
                    description: 'Search query for components (e.g., "chat", "message", "streaming", "input")',
                },
                category: {
                    type: 'string',
                    enum: [
  ...
  "accessibility": { "wcagLevel": "AA", "keyboardSupport": ["Enter to send", "Shift+Enter for newline"] }
        ...
        inputSchema: {
            ...
            properties: {
                componentName: {
                    type: 'string',
                    description: 'Name of the component (e.g., "ClarityChat", "ChatInput", "MessageList", "TypingIndicator")',
                },
            },
    ...
    { "name": "useStreamParser", "description": "Parse streaming responses in multiple formats" }
        ...
        inputSchema: {
            ...
            properties: {
                query: {
                    type: 'string',
                    description: 'Search query for hooks (e.g., "chat", "streaming", "voice", "storage")',
                },
                limit: {
                    type: 'number',
                    description: 'Maximum number of results (default: 10, max: 50)',
  ...
  "examples": [{ "title": "Basic Chat", "code": "const { messages, sendMessage } = useClarityChat({ api: '/api/chat' })" }]
        ...
        inputSchema: {
            ...
            properties: {
                hookName: {
                    type: 'string',
                    description: 'Name of the hook (e.g., "useClarityChat", "useVoiceInput", "useTokenTracker")',
                },
            },
            required: ['hookName'],
        },
  ...
  "focusManagement": "Auto-focuses after message sent. Maintains focus during streaming responses."
        ...
        inputSchema: {
            ...
            properties: {
                componentName: {
                    type: 'string',
                    description: 'Name of the component to get accessibility guidance for',
                },
            },
            required: ['componentName'],
        },
        ...
        inputSchema: {
            ...
            properties: {
                componentNames: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'List of component names to check. If empty, checks all main components.',
                },
                wcagLevel: {
                    type: 'string',
                    enum: ['A', 'AA', 'AAA'],
    ...
    try {
        switch (name) {
            case 'init_project': {
                ...
                const provider = validateEnum(args.provider, ['openai', 'anthropic', 'google', 'all'], 'provider');
                const framework = validateEnum(args.framework, ['nextjs', 'express', 'hono', 'standalone'], 'framework');
 ...
 */
async function initProject(provider, framework, projectPath) {
 ...
 */
async function validateConfig(projectPath) {
    const issues = [];
    const warnings = [];
    try {
        // Check package.json
...
async function getModelInfo(modelName) {
    ...
    if (!model) {
        ...
        const availableModels = getAllModels()
            ...
            .map((m) => m.id)
            .join(', ');
        throw new NotFoundError('Model', `${modelName}. Available models include: ${availableModels}`);
    }
    const result = {
        id: model.id,
        name: model.name,
 ...
 */
async function calculateCost(modelName, promptTokens, completionTokens) {
...
async function analyzeProject(projectPath) {
    ...
    try {
        ...
        if (files.length > MAX_FILES_TO_ANALYZE) {
            throw new ValidationError(`Project contains too many files (>${MAX_FILES_TO_ANALYZE}). This may not be a valid project directory.`, { fileCount: files.length, maxAllowed: MAX_FILES_TO_ANALYZE });
        }
        ...
        if (analysis.hasPackageJson) {
            try {
                ...
                if (deps['openai'])
                    analysis.providers.push('OpenAI');
                if (deps['@anthropic-ai/sdk'])
                    analysis.providers.push('Anthropic');
                if (deps['@google/generative-ai'])
                    analysis.providers.push('Google AI');
                // Detect framework
                if (deps['next'])
...
function generateExampleCode(provider, framework) {
    if (framework === 'nextjs') {
...
function generatePackageJson(provider, framework) {
    const deps = {};
    if (provider === 'openai' || provider === 'all')
        deps['openai'] = '^4.0.0';
    if (provider === 'anthropic' || provider === 'all')
 ...
 */
async function handleDiscoverComponents(args) {
 ...
 */
async function handleGetComponentDocs(args) {
    ...
    if (!component) {
        ...
        throw new NotFoundError('Component', `${input.componentName}. Did you mean: ${suggestions.map((c) => c.name).join(', ') || 'ClarityChat, ChatInput, MessageList'}?`);
    }
    return {
        name: component.name,
 ...
 */
async function handleDiscoverHooks(args) {
 ...
 */
async function handleGetHookDocs(args) {
    ...
    if (!hook) {
        ...
        throw new NotFoundError('Hook', `${input.hookName}. Did you mean: ${suggestions.map((h) => h.name).join(', ') || 'useClarityChat, useVoiceInput, useTokenTracker'}?`);
    }
    return {
        name: hook.name,
 ...
 */
async function handleGetAccessibility(args) {
    ...
    if (!component) {
        const suggestions = searchComponents(input.componentName, { limit: 3 });
        throw new NotFoundError('Component', `${input.componentName}. Did you mean: ${suggestions.map((c) => c.name).join(', ')}?`);
    }
    return {
 ...
 */
async function handleGetRelatedComponents(args) {
...
function handleListCategories() {
    ...
    const categoryDescriptions = {
        'top-level': 'Main container components for building complete chat interfaces',
    ...
    return {
        ...
        categories: stats.map((s) => ({
            ...
            description: categoryDescriptions[s.category] ||
...
function handleCheckAccessibility(args) {
    ...
    const targetLevel = args.wcagLevel || 'AA';
    ...
    for (const name of componentsToCheck) {
        ...
        if (component) {
            ...
            if (componentLevelIndex < targetLevelIndex) {
                issues.push(`Component only meets WCAG ${accessibility.wcagLevel}, target is ${targetLevel}`);
                componentScore -= 15;
                criticalIssues++;
            }
            ...
            if (accessibility.ariaAttributes.length < 2) {
                recommendations.push('Add more ARIA attributes for better screen reader support');
                componentScore -= 5;
            }
            // Check screen reader notes
            if (!accessibility.screenReaderNotes ||
    ...
    const overallScore = auditResults.length > 0 ? Math.round(totalScore / auditResults.length) : 0;
    return {
