...
export function getModel(modelId) {
    ...
    if (aliasKey)
        return MODELS[aliasKey];
    return undefined;
}
/**
 * Calculate cost for token usage
 */
export function calculateCost(modelId, inputTokens, outputTokens, cachedInputTokens = 0) {
...
export function getModelsByProvider(provider) {
    return Object.values(MODELS).filter((m) => m.provider === provider && !m.deprecated);
}
/**
