...
import { globalRateLimiter, toolRateLimiter, createRateLimitError, } from './utils/rate-limiter.js';
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (config.plugins) {
            try {
                processedArgs = await pluginRegistry.executeBeforeToolCallHooks(name, processedArgs);
            }
            catch (hookError) {
        ...
        const executeToolWithTimeout = async () => {
            if (isCoreToolName) {
        ...
        result = await withTimeout(executeToolWithTimeout(), TOOL_TIMEOUT_MS, `tool:${name}`);
        ...
        logger.error('Tool call failed', error instanceof Error ? error : undefined, { tool: name, duration });
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    try {
        ...
        let content;
        const pluginHandler = config.plugins && pluginRegistry.getResourceHandler(uri);
        if (pluginHandler) {
        ...
        else if (uri.includes('/examples/list') ||
            uri.includes('/models/') ||
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    try {
        ...
        let result;
        const pluginHandler = config.plugins && pluginRegistry.getPromptHandler(name);
        if (pluginHandler) {
...
async function main() {
    try {
        ...
        const CONNECTION_TIMEOUT_MS = parseInt(process.env.MCP_CONNECTION_TIMEOUT || '30000', 10);
