...
function inferPriority(file, changeType) {
    const { path, status } = file;
...
function getAffectedAPIs(file) {
    ...
    const componentMatch = path.match(/components\/([A-Za-z]+)\/([A-Za-z]+)\.tsx$/);
    if (componentMatch) {
...
function classifyChange(file) {
    ...
    const isDocsRelevant = matchesAny(path, DOCS_RELEVANT_PATTERNS) &&
...
function createSummary(files, classifications) {
    const packagesAffected = new Set();
...
export async function detectChanges(options) {
    const { base, head } = options;
    const cwd = process.cwd();
    // Get actual commit SHAs
    const baseCommit = base === 'HEAD~1' ? getMergeBase(head, 'HEAD~1', cwd) : base;
    const headCommit = head === 'HEAD' ? getCurrentCommit(cwd) : head;
    // Get changed files
    let changedFiles = getChangedFiles(baseCommit, headCommit, cwd);
    // Apply include patterns
    if (options.includePatterns?.length) {
        changedFiles = changedFiles.filter((file) => matchesAny(file.path, options.includePatterns ?? []));
    }
    // Apply exclude patterns
    if (options.excludePatterns?.length) {
        changedFiles = changedFiles.filter((file) => !matchesAny(file.path, options.excludePatterns ?? []));
    }
    // Skip test files if requested
    if (options.skipTests) {
        changedFiles = changedFiles.filter((file) => !file.path.includes('.test.') &&
            !file.path.includes('.spec.') &&
            !file.path.includes('__tests__'));
    }
    // Skip internal files if requested
    if (options.skipInternal) {
        changedFiles = changedFiles.filter((file) => !file.path.includes('/internal/'));
    }
    // Classify each file
...
export function filterDocsRelevant(result) {
    ...
    const docsRelevantClassifications = new Map();
...
export function formatAsGitHubOutput(result) {
    const lines = [];
...
export function formatAsText(result) {
    const lines = [];
