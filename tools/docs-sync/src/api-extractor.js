...
import { createProgram, formatTypeForDisplay, getExportedSymbols, getFunctionParameters, getJSDocComment, getJSDocExamples, getJSDocParams, getJSDocReturns, getJSDocTags, getLineNumber, getReturnType, getTypeProperties, getTypeString, isExternalLibrary, isHookName, } from './utils/typescript.js';
import { getFileHash, isCacheValid, getCachedAPIs, setCacheEntry, } from './utils/cache.js';
/** Determine API kind from symbol and type */
function determineKind(symbol, type, checker, name) {
...
function extractParameters(signature, checker, paramDocs) {
...
function extractReturnInfo(signature, checker, returnDoc) {
...
function extractComponentProps(type, checker) {
    ...
    if (!propsDeclaration)
        return [];
    const propsType = checker.getTypeOfSymbolAtLocation(propsParam, propsDeclaration);
...
function extractTypeParameters(declaration) {
    let typeParams;
    if (ts.isFunctionDeclaration(declaration) ||
...
function getDisplaySignature(symbol, type, checker, kind) {
    const declaration = symbol.valueDeclaration ?? symbol.declarations?.[0];
...
function extractAPIFromSymbol(symbol, checker, sourceFile, packageName) {
...
function extractFromProgram(program, config) {
    const checker = program.getTypeChecker();
    const apis = [];
    const errors = [];
    for (const sourceFile of program.getSourceFiles()) {
        // Skip external libraries
        if (isExternalLibrary(sourceFile))
            continue;
        ...
        if (!config.entryPoints.some((ep) => relativePath.endsWith(ep.replace(/^\.\//, '')))) {
            continue;
        }
        try {
            ...
            for (const symbol of exportedSymbols) {
                try {
                    const api = extractAPIFromSymbol(symbol, checker, sourceFile, config.packageName);
                    if (api) {
                    ...
                    errors.push({
                        ...
                        message: `Failed to extract ${symbol.getName()}: ${err.message}`,
                    });
                }
            }
        }
...
export async function extractAPIs(config, cache) {
...
export async function extractMultiplePackages(packages, cache) {
...
export async function findSourceFiles(packagePath) {
...
export function getDefaultConfig(packagePath, packageName) {
    return {
        packagePath,
        packageName,
...
/** Filter APIs by kind */
export function filterByKind(apis, kind) {
    return apis.filter((api) => api.kind === kind);
}
/** Group APIs by kind */
export function groupByKind(apis) {
...
export function formatExtractionAsJSON(result) {
