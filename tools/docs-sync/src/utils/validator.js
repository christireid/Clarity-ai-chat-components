...
export const DocsSyncConfigSchema = z.object({
    ...
    repository: z
        .string()
        .regex(/^[a-zA-Z0-9-]+\/[a-zA-Z0-9-]+$/, 'Repository must be in owner/repo format')
        .optional(),
...
export function validateDocsSyncConfig(config) {
...
export function validatePackageConfig(pkg) {
...
export function validateConfigSemantics(config) {
    const errors = [];
...
export function validateConfigFull(config) {
    ...
    const semanticErrors = validateConfigSemantics(config);
    return {
