...
export function createProgram(entryPoints, options) {
...
export function isExternalLibrary(sourceFile) {
...
export function getJSDocParams(node) {
...
export function getJSDocReturns(node) {
    const tags = ts.getJSDocTags(node);
    for (const tag of tags) {
...
export function getJSDocExamples(node) {
    const examples = [];
    const tags = ts.getJSDocTags(node);
    for (const tag of tags) {
...
export function getTypeString(type, checker) {
    return checker.typeToString(type, undefined, ts.TypeFormatFlags.NoTruncation |
        ts.TypeFormatFlags.WriteArrowStyleSignature |
        ts.TypeFormatFlags.WriteTypeArgumentsOfSignature);
}
/** Get signature string for a function/method */
export function getSignatureString(signature, checker) {
    return checker.signatureToString(signature, undefined, ts.TypeFormatFlags.NoTruncation |
        ts.TypeFormatFlags.WriteArrowStyleSignature);
}
/** Check if a type is a React component */
export function isReactComponent(type, checker) {
    const typeString = getTypeString(type, checker);
    return (typeString.includes('React.FC') ||
...
export function getTypeProperties(type, checker) {
    const properties = [];
...
export function getFunctionParameters(signature, checker) {
    const params = [];
    for (const param of signature.getParameters()) {
        ...
        const optional = !!(param.flags & ts.SymbolFlags.Optional) ||
...
export function getReturnType(signature, checker) {
    const returnType = signature.getReturnType();
    return getTypeString(returnType, checker);
}
/** Find all exported symbols from a source file */
export function getExportedSymbols(sourceFile, checker) {
...
export function formatTypeForDisplay(type) {
    return type
