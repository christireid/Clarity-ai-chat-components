...
function createMockAPI(overrides: Partial<ExtractedAPI>): ExtractedAPI {
  return {
    name: 'TestAPI',
    kind: 'function',
    signature: '() => void',
...
describe('API Differ', () => {
  describe('diffAPIs', () => {
    it('should detect added APIs', () => {
      const oldAPIs: ExtractedAPI[] = []
      const newAPIs: ExtractedAPI[] = [createMockAPI({ name: 'NewAPI' })]
    ...
    it('should detect removed APIs', () => {
      const oldAPIs: ExtractedAPI[] = [createMockAPI({ name: 'OldAPI' })]
      const newAPIs: ExtractedAPI[] = []
    ...
    it('should ignore internal APIs when configured', () => {
      const oldAPIs: ExtractedAPI[] = [createMockAPI({ name: '_internalAPI' })]
      const newAPIs: ExtractedAPI[] = []
  ...
  describe('formatDiffForChangelog', () => {
    it('should format breaking changes section', () => {
      const oldAPIs: ExtractedAPI[] = [createMockAPI({ name: 'RemovedAPI' })]
      const newAPIs: ExtractedAPI[] = []
