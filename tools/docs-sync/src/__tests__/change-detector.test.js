...
import { detectChanges, filterDocsRelevant, formatAsJSON, formatAsText, formatAsGitHubOutput, getAffectedPackages, hasBreakingChanges, groupByPackage, } from '../change-detector.js';
...
describe('Change Detector', () => {
    ...
    describe('detectChanges', () => {
        it('should detect docs-relevant changes in source files', async () => {
            ...
            const hookClassification = result.classifications.get('packages/react/src/hooks/useChat.ts');
            expect(hookClassification?.isDocsRelevant).toBe(true);
            const testClassification = result.classifications.get('packages/react/src/__tests__/useChat.test.ts');
        ...
        it('should exclude internal files', async () => {
            ...
            const classification = result.classifications.get('packages/react/src/internal/utils.ts');
        ...
        it('should detect breaking changes on deleted files', async () => {
            ...
            const classification = result.classifications.get('packages/react/src/hooks/useOldHook.ts');
        ...
        it('should skip test files when skipTests is true', async () => {
            ...
            expect(result.changedFiles[0]?.path).toBe('packages/react/src/hooks/useChat.ts');
        });
    });
    describe('filterDocsRelevant', () => {
        it('should filter to only docs-relevant files', async () => {
            ...
            expect(filtered.changedFiles[0]?.path).toBe('packages/react/src/hooks/useChat.ts');
        });
    });
