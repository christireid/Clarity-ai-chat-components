...
import { detectChanges, filterDocsRelevant, formatAsGitHubOutput, formatAsJSON, formatAsText, } from './change-detector.js';
import { extractAPIs, getDefaultConfig } from './api-extractor.js';
import { generateDocs, writeDocs, getDefaultGeneratorConfig, formatGenerationResult, } from './doc-generator.js';
import { generateChangelog, updateChangelogFile, getChangelogSinceRef, formatChangelogResult, } from './changelog-generator.js';
import { diffAPIs, formatDiffAsText, formatDiffAsJSON, formatDiffForChangelog, } from './api-differ.js';
import { loadCache, saveCache, createEmptyCache } from './utils/cache.js';
import { configureGitUser, createCommit, stageFiles, isWorkingDirClean, } from './utils/git.js';
import { configureLogger, info, success, warn, error, debug, startSpinner, succeedSpinner, failSpinner, ProgressTracker, printSummary, formatDuration, } from './utils/logger.js';
import { validateConfigFull, formatValidationErrors, } from './utils/validator.js';
...
async function withRetry(fn, options = {}) {
    const { maxRetries = 3, baseDelay = 1000, description = 'operation', } = options;
    let lastError = null;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries) {
                ...
                warn(`${description} failed (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${delay}ms...`);
...
function loadConfig(configPath, skipValidation = false) {
...
program
    ...
    try {
        ...
        const result = await withRetry(() => detectChanges({
            base: options.base,
        ...
        succeedSpinner(`Detected ${filtered.summary.docsRelevantFiles} docs-relevant changes`);
...
program
    ...
    .option('--packages <names>', 'Comma-separated list of package names to process')
    .option('--full', 'Force full extraction (ignore cache)')
    .option('--parallel', 'Extract packages in parallel')
    .option('--concurrency <number>', 'Max parallel extractions (default: 3)', '3')
    ...
    try {
        ...
        if (options.packages) {
            ...
            packages = packages.filter((p) => requestedPkgs.some((rp) => p.name.includes(rp)));
        }
        const results = {};
        const progress = new ProgressTracker(packages.length, 'Extracting APIs');
        progress.start();
        const extractPackage = async (pkg) => {
            const extractionConfig = getDefaultConfig(join(config.rootDir, pkg.path), pkg.name);
            extractionConfig.entryPoints = pkg.entryPoints;
            const result = await withRetry(() => extractAPIs(extractionConfig, cache), { description: `extract ${pkg.name}`, maxRetries: 2 });
        ...
        const totalAPIs = Object.values(results).reduce((sum, r) => sum + (r.apis?.length ?? 0), 0);
    ...
    try {
        ...
        if (options.packages) {
            ...
            for (const [pkgName, data] of Object.entries(apiData)) {
                if (requestedPkgs.some((rp) => pkgName.includes(rp))) {
        ...
        const result = await withRetry(() => generateDocs(allAPIs, generatorConfig), { description: 'documentation generation', maxRetries: 2 });
...
program
    ...
    try {
        ...
        const { entry, markdown, hasBreakingChanges } = await withRetry(() => generateChangelog(sinceRef, version, {
...
program
    ...
    try {
        ...
        const changeResult = await withRetry(() => detectChanges({
            base: options.base,
            head: options.head ?? 'HEAD',
            includePatterns: config.docsRelevantPatterns,
            excludePatterns: config.excludePatterns,
        ...
        succeedSpinner(`Found ${filtered.summary.docsRelevantFiles} docs-relevant files`);
        // Step 2: Extract APIs
        ...
        const affectedPackages = options.full
            ? config.packages
            : config.packages.filter((p) => filtered.summary.packagesAffected.includes(p.name));
        const pkgProgress = new ProgressTracker(affectedPackages.length, 'Processing packages');
        pkgProgress.start();
        ...
        const generatorConfig = getDefaultGeneratorConfig(config.rootDir);
        generatorConfig.dryRun = options.dryRun ?? false;
        const docResult = await withRetry(() => generateDocs(allAPIs, generatorConfig), { description: 'documentation generation', maxRetries: 2 });
        if (!options.dryRun) {
            ...
            try {
                ...
                for (const [pkgName, data] of Object.entries(apiData)) {
                    for (const api of data.apis) {
                        ...
                        const docPath = join(config.docsDir, `${api.name.toLowerCase()}.mdx`);
                ...
                const coverage = totalAPIs > 0 ? Math.round((documentedAPIs / totalAPIs) * 100) : 0;
                if (coverage < 80) {
                    issues.push({
                        type: 'warning',
                ...
                succeedSpinner(`API coverage: ${coverage}% (${documentedAPIs}/${totalAPIs})`);
            }
            catch (err) {
                failSpinner('Failed to check coverage');
                issues.push({
                    type: 'error',
                    message: `Coverage check failed: ${err.message}`,
                });
            }
        }
            ...
            try {
                ...
                for (const file of mdxFiles) {
                    ...
                    while ((match = linkRegex.exec(content)) !== null) {
                        ...
                        if (linkTarget) {
                            const targetPath = join(config.docsDir, linkTarget.replace(/^\//, ''));
                ...
                issues.push({
                    type: 'error',
                    message: `Link check failed: ${err.message}`,
                });
            }
        }
        // Print results
...
program
    ...
    try {
        ...
        startSpinner('Loading API data...');
        const currentData = JSON.parse(readFileSync(apiDataPath, 'utf-8'));
        ...
        // Load baseline data
        let baselineAPIs = [];
        const baselinePath = options.baseline ?? join(config.cacheDir, 'api-baseline.json');
        if (existsSync(baselinePath)) {
            ...
            succeedSpinner(`Loaded ${currentAPIs.length} current and ${baselineAPIs.length} baseline APIs`);
        }
        else {
...
program
    ...
    if (existsSync(configPath) && !options.force) {
        handleError(new ConfigError('Configuration file already exists', [
