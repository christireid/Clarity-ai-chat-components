...
import { Check, Copy, Bot, User, Sparkles, Zap, Brain, Code, Terminal, Play } from 'lucide-react'
...
const ChatMessage = ({ role, content, isStreaming = false }: { role: 'user' | 'assistant', content: string, isStreaming?: boolean }) => (
  <motion.div 
...
const DemoSteps = [
  {
    ...
    lines: [
      { text: "import { ClarityChat } from '@clarity-chat/react'", type: 'import' },
      { text: "", type: 'normal' },
    ...
    chatState: (
      <>
        ...
        <ChatMessage role="assistant" content="I am a basic instance of Clarity Chat. I process text input using the standard provider configuration." />
      </>
    )
  },
  {
    ...
    lines: [
      { text: "import { ClarityChat, useMemory } from '@clarity-chat/react'", type: 'import' },
      { text: "", type: 'normal' },
      { text: "export default function App() {", type: 'function' },
    ...
    chatState: (
      <>
        ...
        <ChatMessage role="assistant" content="Nice to meet you, Sarah! I've saved that to my memory." />
        <motion.div 
      ...
      <div className="relative rounded-xl overflow-hidden bg-[#0d1117] border border-surface-700 shadow-2xl shadow-black/50 flex flex-col h-[450px] group">
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-[#161b22] border-b border-surface-700">
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={handleCopy} className="text-gray-400 hover:text-white transition-colors p-1 hover:bg-surface-700 rounded">
                    ...
                    >
                        {DemoSteps[activeStep].lines.map((line, idx) => (
                            <div key={idx} className="whitespace-pre">
                                <span style={{ 
                                    color: line.type === 'import' ? COLORS.keyword : 
                                           line.type === 'function' ? COLORS.function : 
                                           line.type === 'component' ? COLORS.component : 
                                           line.type === 'prop' ? COLORS.prop : 
                                           COLORS.normal 
                                }}>
                                    {line.text}
                                </span>
        ...
        <div className="bg-[#161b22] border-t border-surface-700 px-3 py-1 flex items-center justify-between text-[10px] text-gray-400 font-mono select-none">
            <div className="flex gap-3">
                <div className="flex items-center gap-1"><Terminal className="w-3 h-3" /> READY</div>
                <div>master*</div>
            </div>
            <div className="flex gap-3">
                <div>Ln 9, Col 1</div>
      ...
      <div className="relative rounded-xl overflow-hidden bg-surface-950 border border-surface-700 shadow-2xl shadow-clarity-500/10 flex flex-col h-[450px]">
        ...
        <div className="h-14 border-b border-surface-800 flex items-center px-4 bg-surface-900/80 backdrop-blur-md justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="text-sm font-semibold text-white">Clarity Assistant</div>
