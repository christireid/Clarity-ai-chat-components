...
import { getContextualPrompt } from './prompts';
...
export async function retrieveRelevantDocs(query, options = {}) {
    ...
    try {
        ...
        if (keywordResults.length > 0) {
            ...
            const searchResults = keywordResults.map((result) => ({
                id: result.chunk.id,
 ...
 */
export function buildContext(sources, maxLength = 4000) {
    if (sources.length === 0) {
...
export async function generateRAGContext(query, options = {}) {
    const { currentPath = '/', maxContextLength = 4000, includeSources = true, } = options;
 ...
 */
export function formatRAGPrompt(ragContext) {
 ...
 */
export function getSuggestedFollowUps(sources) {
    const suggestions = [];
 ...
 */
export function calculateRelevanceScore(query, source) {
