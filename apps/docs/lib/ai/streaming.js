 ...
 */
export async function* streamFromOpenAI(messages, options = {}) {
    const { model = 'gpt-4-turbo-preview', temperature = 0.7, maxTokens = 2000, } = options;
 ...
 */
export async function* streamFromClaude(messages, options = {}) {
    const { model = 'claude-3-5-sonnet-20241022', temperature = 0.7, maxTokens = 2000, } = options;
 ...
 */
export async function* streamFromGemini(messages, options = {}) {
    ...
    try {
        ...
        if (systemPrompt) {
            history.unshift({ role: 'user', parts: [{ text: `System instructions: ${systemPrompt}` }] }, { role: 'model', parts: [{ text: 'Understood. I will follow these instructions.' }] });
        }
 ...
 */
export async function* streamFromDemo(messages) {
 ...
 */
export function validateRequest(messages, maxTokens = 10000) {
...
export function formatResponse(content, sources) {
    return {
        content,
        metadata: {
