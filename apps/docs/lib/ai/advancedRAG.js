 ...
 */
export function isFollowUpQuestion(query, conversationHistory) {
    ...
    const startsWithPronoun = pronouns.some(p => lowerQuery.startsWith(p + ' ') || lowerQuery.startsWith(p + '?'));
 ...
 */
export function buildConversationContext(messages, maxMessages = 5) {
 ...
 */
export function enhanceQueryWithContext(query, context) {
 ...
 */
export function rerankWithConversationContext(query, results, context) {
    ...
    const recentlyShownUrls = new Set(context.recentMessages
        .flatMap(m => m.sources || [])
 ...
 */
export async function retrieveWithContext(query, conversationHistory, options = {}) {
 ...
 */
export function generateContextualSystemPrompt(basePrompt, context, isFollowUp) {
    if (!isFollowUp || context.recentMessages.length === 0) {
        return basePrompt;
    }
 ...
 */
export function formatConversationHistory(messages, maxLength = 1000) {
 ...
 */
export function detectTopicShift(currentQuery, conversationHistory) {
    if (conversationHistory.length === 0) {
