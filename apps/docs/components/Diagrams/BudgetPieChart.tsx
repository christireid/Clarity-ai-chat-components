...
export function BudgetPieChart({ monthlyIncome = 5000 }: { monthlyIncome?: number }) {
  ...
  const getPieSlicePath = (startPercent: number, percent: number) => {
    ...
    <div className="not-prose my-12">
      <div className="bg-gradient-to-br from-slate-50 to-green-50 dark:from-slate-900 dark:to-green-950 p-8 rounded-2xl border-2 border-slate-200 dark:border-slate-700">
            ...
            <svg width="240" height="240" viewBox="0 0 240 240" className="drop-shadow-xl">
              {segments.map((segment, index) => {
                ...
                return (
                  <motion.path
                    key={segment.id}
                    d={getPieSlicePath(startPercent, segment.percentage / 100)}
                    fill={segment.color}
              ...
              <text
                ...
                y={centerY - 10}
                textAnchor="middle"
                className="fill-gray-900 dark:fill-gray-100 font-bold text-xl"
              >
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-sm">{segment.label}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">
                      {segment.percentage}% • ${segment.amount.toLocaleString()}
                    </div>
                  </div>
                </div>
        ...
        <motion.div
          ...
          className="mt-8 p-4 bg-blue-50 dark:bg-blue-950/30 border-2 border-blue-200 dark:border-blue-800 rounded-xl text-sm"
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-blue-800 dark:text-blue-200">
                Start with needs (50%), then allocate wants (30%), and always save 20%. Adjust percentages based on your situation, but keep savings a priority.
              </div>
            </div>
          </div>
        </motion.div>
