...
function createAsyncIterable<T>(items: T[], delay: number = 100): AsyncIterable<T> {
  return {
      ...
      <div className="space-y-4">
        ...
        {values.length > 0 && (
          <div className="space-y-1">
            <p className="text-sm font-semibold">All Values ({values.length}):</p>
...
const useStreamableUIOptionsProps: Prop[] = [
  {
    ...
    type: '"append" | "replace"',
    default: '"append"',
    description: 'How to handle new values: append to array or replace the array with single value.',
  },
  {
    name: 'transform',
    type: '(value: unknown) => T | null | undefined',
    description: 'Transform incoming payloads (e.g., decode Uint8Array, parse JSON). Return null/undefined to skip value.',
  },
  {
    name: 'completeWhen',
    type: '(value: unknown) => boolean',
    description: 'Function to determine when streaming should be marked complete. Called with each value.',
  },
  {
    name: 'onUpdate',
    type: '(value: T) => void',
  ...
  },
  {
    ...
    description: 'Callback invoked when stream completes. Receives final value or latest value if no final value.',
  },
  {
    name: 'onError',
...
const useStreamableUIStateProps: Prop[] = [
  {
    name: 'values',
    type: 'T[]',
    description: 'Array of all received values (accumulated if mode is "append", single value if mode is "replace").',
  },
  {
    name: 'latest',
    type: 'T | null',
  ...
  return (
    <ToastProvider>
      <>
        ...
        <p className="lead">
          A flexible hook for managing UI state from various streaming sources including StreamableValue,
          AsyncIterable, Promise, and ReadableStream. Perfect for handling incremental updates from AI responses.
        </p>

        <Callout type="info">
          <p>
            This hook is designed to work with Vercel AI SDK's <code>StreamableValue</code> and other
            streaming sources. It automatically handles subscription, transformation, and completion detection.
          </p>
        </Callout>
  ...
  return (
    <div>
      ...
      <p>All: {values.join('')}</p>
        ...
        <p>
          Use with Vercel AI SDK's StreamableValue:
        </p>
  ...
  return (
    <div>
      ...
      <p>{latest || ''}</p>
        ...
        <p>
          Handle a Promise that resolves to a value:
        </p>
  ...
  return (
    <div>
      ...
      {error && <p>Error: {error.message}</p>}
        ...
        <p>
          Stream from a ReadableStream (e.g., from fetch):
        </p>
  ...
  return (
    <div>
      ...
      <p>{values.join('')}</p>
        ...
        <p>
          Control how values are accumulated:
        </p>
...
function ReplaceMode() {
  ...
  return <p>{values.join(', ')}</p> // "c"
        ...
        <p>
          Transform incoming values (e.g., decode binary, parse JSON):
        </p>
  ...
  return (
    <div>
      ...
      <p>{values.join('')}</p>
        ...
        <p>
          useStreamableUI supports the following source types:
        </p>

        <ul>
          <li>
            <strong>StreamableValue:</strong> Vercel AI SDK's streamable value with subscribe/onDone
          </li>
          <li>
            <strong>AsyncIterable:</strong> Any async iterable (generators, async generators)
          </li>
          <li>
        ...
        <p>
          For StreamableValue-like objects, the hook expects:
        </p>
  ...
  return (
    <div className="space-y-4">
      ...
      {values.length > 0 && (
        ...
        <p>
          Streaming status can be one of:
        </p>
        ...
        <ul>
          <li>
            <strong>Use append mode for tokens:</strong> When streaming text tokens, use <code>mode: 'append'</code>
          </li>
          <li>
            <strong>Use replace mode for updates:</strong> When streaming complete objects that replace each other
          </li>
          <li>
            <strong>Transform binary data:</strong> Use <code>transform</code> to decode Uint8Array chunks
          </li>
          <li>
            <strong>Handle completion:</strong> Use <code>completeWhen</code> or rely on stream completion
          </li>
          <li>
            <strong>Clean up:</strong> The hook automatically cleans up subscriptions when source changes
          </li>
          <li>
            <strong>Error handling:</strong> Always provide <code>onError</code> callback
          </li>
        </ul>
        ...
        <ul>
          ...
          <li>
            <a href="/reference/hooks/use-clarity-chat">useClarityChat</a> - Chat hook with streaming support
          </li>
          <li>
            <a href="/reference/components/streaming-message">StreamingMessage</a> - Display streaming content
          </li>
          <li>
            <a href="/guides/streaming">Streaming Guide</a> - Streaming patterns and best practices
          </li>
        </ul>
