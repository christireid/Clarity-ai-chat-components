...
import { createSSEStream, getStreamingFunction, checkRateLimit, validateRequest, handleStreamError, } from '@/lib/ai/streaming';
import { SYSTEM_PROMPT, RATE_LIMIT_PROMPT } from '@/lib/ai/prompts';
import { getOrCreateSessionForRequest, updateSessionWithMessages, } from '@/lib/ai/sessionStore';
...
export async function POST(request) {
    try {
        ...
        // Check rate limit
        const rateLimit = checkRateLimit(identifier, parseInt(process.env.RATE_LIMIT_MAX_REQUESTS || '100'), parseInt(process.env.RATE_LIMIT_WINDOW_MS || '60000'));
        ...
        if (sessionId) {
            try {
                ...
                session = await getOrCreateSessionForRequest(sessionId, body.userId, request.headers.get('user-agent') || undefined);
            }
            catch (error) {
...
async function* streamWithRAG(userMessage, messages, currentPath, sessionId) {
    ...
    try {
        ...
        const { enhancedMessage, ragContext } = await enhanceMessageWithRAG(userMessage, {
            currentPath,
            topK: 5,
            minScore: 0.7,
        });
 ...
 */
async function* streamWithoutRAG(userMessage, messages, sessionId) {
    let assistantResponse = '';
    try {
