...
import { MessageSquare, Database, Zap, GitBranch, Command, BarChart, Shield } from 'lucide-react'
    ...
    <div className="docs-content">
      ...
      <section className="docs-section mt-12">
        <ScrollReveal delay={0.2}>
          ...
          <p className="mb-6">
            Our components are organized into logical families. You can mix and match them
            to build anything from a simple chatbot to a full-scale AI workspace.
          </p>
        </ScrollReveal>
        ...
        <ScrollReveal stagger staggerDelay={0.1} className="grid md:grid-cols-2 gap-4">
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold m-0">Conversation Core</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">The essential elements of any chat interface.</p>
              <div className="flex flex-wrap gap-2">
                {['ChatWindow', 'Message', 'MessageList', 'ChatInput'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
          </ScrollRevealItem>
          ...
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold m-0">Context &amp; Knowledge</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">Display RAG context and documents.</p>
              <div className="flex flex-wrap gap-2">
                {['ContextCard', 'KnowledgeBaseViewer', 'MultiModalPreview'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
          ...
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold m-0">Streaming &amp; Status</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">Handle real-time AI responses.</p>
              <div className="flex flex-wrap gap-2">
                {['StreamingMessage', 'ThinkingIndicator', 'StreamCancellation'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
          ...
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold m-0">Branching &amp; History</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">Navigate complex conversation trees.</p>
              <div className="flex flex-wrap gap-2">
                {['ConversationBranchVisualizer', 'ConversationList'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
          ...
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              ...
              <p className="text-sm text-muted-foreground mb-4">Power user tools and shortcuts.</p>
              <div className="flex flex-wrap gap-2">
                {['CommandPalette', 'PromptLibrary', 'FollowUpSuggestions'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
          ...
          <ScrollRevealItem>
            <div className="p-6 rounded-xl border border-border bg-card h-full">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold m-0">Enterprise &amp; Analytics</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">Monitor usage, costs, and performance.</p>
              <div className="flex flex-wrap gap-2">
                {['UsageDashboard', 'PerformanceDashboard', 'TokenCounter'].map(c => (
                  <code key={c} className="px-2 py-1 bg-muted rounded text-xs font-mono">{c}</code>
                ))}
              </div>
            </div>
