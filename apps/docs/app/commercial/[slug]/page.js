...
export default async function CommercialPage({ params }) {
    const { slug } = await params;
    const page = commercialPages[slug];
    if (!page) {
        notFound();
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Breadcrumbs, {}), _jsxs("div", { className: "docs-content", children: [_jsxs(Link, { href: "/commercial", className: "inline-flex items-center gap-2 text-text-secondary hover:text-text-primary mb-6 transition-colors", children: [_jsx(ArrowLeft, { className: "w-4 h-4" }), "Back to Commercial"] }), _jsx("article", { children: _jsx("div", { className: "prose prose-lg max-w-none dark:prose-invert", children: _jsx(MDXRemote, { source: mdxContent, components: mdxComponents }) }) })] })] }));
}
