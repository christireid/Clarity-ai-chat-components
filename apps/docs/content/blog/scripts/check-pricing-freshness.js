...
// CORE FUNCTIONS
// =============================================================================
function extractPricingMentions(filePath) {
...
function generateChecksum(mentions) {
...
function saveBaseline(mentions) {
    const baseline = {
...
function compareMentions(baseline, current) {
    const report = {
    ...
    if (report.newMentions.length > 0) {
        report.recommendations.push(`Review ${report.newMentions.length} new pricing mention(s) for accuracy`);
    }
    if (report.changedMentions.length > 0) {
        report.recommendations.push(`Verify ${report.changedMentions.length} changed pricing value(s)`);
    }
    // Check baseline age
    const baselineAge = Date.now() -
        new Date(baseline[0]?.file ? loadBaseline()?.generatedAt || Date.now() : Date.now()).getTime();
    const daysOld = Math.floor(baselineAge / (1000 * 60 * 60 * 24));
    if (daysOld > 90) {
        report.recommendations.push(`Baseline is ${daysOld} days old. Consider verifying all pricing against current provider rates.`);
    }
    // Check for specific high-risk patterns
    const modelMentions = current.filter((m) => /gpt-4|claude|embedding/i.test(m.value));
    if (modelMentions.length > 0) {
        report.recommendations.push('Verify model names and versions are still current');
    }
    return report;
}
...
function printPricingReference() {
    logger.debug('\n' + '='.repeat(60));
...
main();
//# sourceMappingURL=check-pricing-freshness.js.map