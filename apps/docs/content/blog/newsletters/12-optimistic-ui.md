...
It doesn't wait for server confirmation. It doesn't show a spinner. It just *appears*.
...
**Critical rule:** Failed messages stay visible. The user's words are precious—never silently delete them.

The psychology is simple: 0ms perceived latency feels instant. 500ms actual latency with a spinner feels slow.

---

[Read the full article →](/blog/optimistic-ui)

*Clarity Chat's `useOptimisticMessage` hook handles status transitions, retry logic, and offline queuing.*
