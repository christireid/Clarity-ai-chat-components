  ...
  const announce = useCallback(
    (
      ...
      if (options.duration) {
        setTimeout(() => {
          setAnnouncements((prev) =>
            prev.filter((_, index) => index !== prev.indexOf(message))
          )
        }, options.duration)
      }
...
export function useKeyboardNavigation(options: KeyboardNavigationOptions) {
  const {
    containerRef,
    itemSelector = '[role="menuitem"], button:not([disabled]), a[href]',
    loop = true,
    horizontal = false,
    onActivate,
  } = options
  ...
  const focusItem = useCallback(
    (index: number) => {
      ...
      if (item) {
        ...
        const label =
          item.getAttribute('aria-label') || item.textContent || 'Item'
        announceToScreenReader(
          `${label} ${normalizedIndex + 1} of ${items.length}`
        )
      }
    },
    [loop]
      ...
      switch (event.key) {
        ...
        case 'Escape':
          ...
          if (containerRef.current) {
            const trigger =
              containerRef.current.querySelector('[aria-expanded]')
 ...
 */
export function manageFocusForDynamicContent(
  ...
  setTimeout(() => {
    const target =
      focusTarget ||
      (container.querySelector('[data-autofocus]') as HTMLElement)
