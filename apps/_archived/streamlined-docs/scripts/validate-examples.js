 ...
 */
function extractCodeExamples(filePath) {
    const content = fs.readFileSync(filePath, 'utf-8');
    const examples = [];
    // Match code blocks in template literals (common pattern in docs)
    // Pattern: code={`...`} or code="..."
    const codeBlockRegex = /code=\{`([\s\S]*?)`\}|code="([^"]+)"|<CodeBlock[^>]*code=\{`([\s\S]*?)`\}/g;
    // Also match EnhancedCodeBlock
    const enhancedCodeBlockRegex = /<EnhancedCodeBlock[\s\S]*?code=\{`([\s\S]*?)`\}[\s\S]*?language="(\w+)"/g;
    let match;
    let lineNumber = 1;
    ...
    const simpleCodeBlockRegex = /<CodeBlock[\s\S]*?language="(\w+)"[\s\S]*?code=\{`([\s\S]*?)`\}/g;
 ...
 */
function validateTypeScriptSyntax(code, language) {
    const errors = [];
    // Skip non-TypeScript code
    if (!['ts', 'tsx', 'typescript', 'javascript', 'js', 'jsx'].includes(language)) {
  ...
  `;
    try {
        // Parse the code
        const sourceFile = ts.createSourceFile(fileName, wrappedCode, ts.ScriptTarget.Latest, true, language.includes('x') ? ts.ScriptKind.TSX : ts.ScriptKind.TS);
 ...
 */
function checkDeprecatedAPIs(code) {
    const warnings = [];
...
function checkImports(code) {
    ...
    const clarityImportRegex = /import\s*{([^}]+)}\s*from\s*['"]@clarity-chat\/react(?:\/[^'"]+)?['"]/g;
    let match;
    while ((match = clarityImportRegex.exec(code)) !== null) {
        ...
        for (const imp of imports) {
            ...
            if (cleanImport.includes('Clearity') ||
...
function validateExamples() {
    ...
    for (const pagePath of pages) {
        ...
        for (const example of examples) {
            ...
            const syntaxResult = validateTypeScriptSyntax(example.code, example.language);
            ...
            for (const warning of deprecationWarnings) {
                result.warnings.push(`${relativePath}:${example.lineNumber} - ${warning}`);
            }
            // Check imports
    ...
    result.info.push(`${result.stats.validExamples} valid, ${result.stats.invalidExamples} with syntax errors`);
    return result;
}
/**
 * CLI entry point
