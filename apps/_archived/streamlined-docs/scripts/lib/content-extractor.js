...
export function extractCodeBlocks(content) {
    ...
    const playgroundRegex = new RegExp(`<CodePlayground[^>]*code=\\\{\\` +
        '`(' +
    ...
    const enhancedRegex = new RegExp(`<EnhancedCodeBlock[^>]*code=\\\{\\` +
        '`(' +
        templateLiteralPattern +
        ')' +
        '`\\}[^>]*language=["\']([^"\']+)["\'][^>]*(?:filename=["\']([^"\']+)["\'])?[^>]*\\/>', 'gs');
    ...
    const enhancedRegex2 = new RegExp(`<EnhancedCodeBlock[^>]*language=["\']([^"\']+)["\'][^>]*code=\\\{\\` +
        '`(' +
        templateLiteralPattern +
        ')' +
        '`\\}[^>]*(?:filename=["\']([^"\']+)["\'])?[^>]*\\/>', 'gs');
    ...
    const preCodeRegex = new RegExp(`<pre[^>]*><code[^>]*>\\\{\\` +
        '`(' +
        templateLiteralPattern +
        ')' +
        '`\\}<\\/code><\\/pre>', 'gs');
...
export function extractTextContent(content) {
    ...
    const calloutRegex = /<Callout[^>]*type=["'](\w+)["'][^>]*>([^<]*(?:<[^>]+>[^<]*)*)<\/Callout>/gs;
    while ((match = calloutRegex.exec(content)) !== null) {
        const type = match[1];
        const text = stripTags(match[2]);
        if (text.trim()) {
            const prefix = type === 'tip'
                ? 'Tip'
                : type === 'info'
                    ? 'Info'
                    : type === 'warning'
 ...
 */
export async function extractPageContent(filePath) {
    const content = await readFile(filePath, 'utf-8');
    const metadata = extractMetadata(content);
    const textContent = extractTextContent(content);
    const codeBlocks = extractCodeBlocks(content);
    // Check for draft status
    const isDraft = content.includes('draft: true') || content.includes("'draft': true");
    return {
...
function extractTitleFromContent(content) {
    ...
    const gradientTitleMatch = content.match(/<h1[^>]*className="[^"]*text-4xl[^"]*"[^>]*>\s*([^<]+)\s*<\/h1>/);
 ...
 */
export function contentToMarkdown(extracted) {
    const lines = [];
    // Add title
    if (extracted.title) {
