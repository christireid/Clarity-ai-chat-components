...
function addFeedbackWidget(filePath) {
    ...
    try {
        ...
        const footerPattern = /(\s*<div className="mt-8 pt-8 border-t[\s\S]*?<\/div>)\s*(<\/div>\s*\))/;
        ...
        if (footerMatch) {
            ...
            content = content.replace(footerPattern, `$1${feedbackCode}\n    $2`);
        }
        else {
...
main();
//# sourceMappingURL=add-feedback-widget.js.map