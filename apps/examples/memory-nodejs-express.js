...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // Store user message
        await memoryService.addMemory(message, 'episodic', 'session', {
            userId,
            sessionId,
            role: 'user',
        ...
        await memoryService.addMemory(llmResponse, 'episodic', 'session', {
            userId,
            sessionId,
            role: 'assistant',
            timestamp: new Date().toISOString(),
...
app.post('/api/preferences/:userId', async (req, res) => {
    try {
        ...
        await memoryService.addMemory(`User preference: ${key} = ${value}`, 'semantic', 'user', {
            userId,
            preferenceKey: key,
