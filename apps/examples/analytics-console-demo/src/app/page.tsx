...
import { ThemeProvider, themes, TokenCounter, Progress } from '@clarity-chat/react'
import { Card } from '@clarity-chat/primitives'
import { BarChart3, Coins, Zap, Activity, TrendingUp, Clock, RefreshCw } from 'lucide-react'
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <TokenCounter
                  currentTokens={stats?.totalTokens || 0}
                  maxTokens={1000000}
                  showCost
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <Card className="p-6">
              ...
              {dailyData.length > 0 ? (
                  ...
                  {dailyData.slice(-10).map((day) => (
                    <div key={day.date} className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <Progress
                          value={(day.totalRequests / Math.max(...dailyData.map(d => d.totalRequests))) * 100}
                        />
                      </div>
                      <div className="flex items-center gap-3 text-sm">
                        <span className="text-slate-700 font-medium">{day.totalRequests}</span>
                ...
                <div className="text-slate-400 text-center py-12">
                  ...
                  <p className="text-sm mt-1">Start making API calls to see analytics</p>
                </div>
              )}
            </Card>
            ...
            <Card className="p-6">
              <h2 className="text-lg font-semibold text-slate-900 mb-4">Provider Distribution</h2>
                    ...
                    return (
                      <div key={provider}>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span className="text-sm text-slate-500">{data.requests} requests</span>
                        </div>
          ...
          <Card className="p-6 mb-8">
            <h2 className="text-lg font-semibold text-slate-900 mb-4">Model Statistics</h2>
            {stats && Object.keys(stats.byModel).length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-slate-200">
                      <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600">Model</th>
                      <th className="text-right py-3 px-4 text-sm font-semibold text-slate-600">Requests</th>
                      <th className="text-right py-3 px-4 text-sm font-semibold text-slate-600">Tokens</th>
                      <th className="text-right py-3 px-4 text-sm font-semibold text-slate-600">Cost</th>
                      <th className="text-right py-3 px-4 text-sm font-semibold text-slate-600">Avg Response</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(stats.byModel).map(([model, data]) => (
                      <tr key={model} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <td className="py-3 px-4">
                          <span className="font-medium text-slate-900">{model}</span>
                        </td>
                        <td className="py-3 px-4 text-right text-slate-700">{data.requests}</td>
                        <td className="py-3 px-4 text-right text-slate-700">{data.tokens.toLocaleString()}</td>
                        <td className="py-3 px-4 text-right font-mono text-green-600">${data.cost.toFixed(4)}</td>
                        <td className="py-3 px-4 text-right text-slate-700">{Math.round(data.avgResponseTime)}ms</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {recentEntries.length > 0 ? (
              <div className="space-y-3">
                {recentEntries.map((entry) => (
                  <div key={entry.id} className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-slate-900">{entry.model}</span>
                    ...
                    <div className="text-right">
                      <div className="font-mono text-green-600">${entry.cost.toFixed(6)}</div>
  ...
  return (
    <Card className="p-6 relative overflow-hidden">
      <div className="flex items-start justify-between">
        ...
        <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${colorClasses[color as keyof typeof colorClasses]} flex items-center justify-center text-white`}>
          {icon}
        </div>
      </div>
    </Card>
