 ...
 */
export function chunkDocument(content, config = RAG_CONFIG) {
    ...
    while (index < words.length) {
        ...
        if (index >= words.length - overlapWords)
            break;
    }
    return chunks;
}
 ...
 */
export function searchChunks(query, documents, topK = RAG_CONFIG.topK, documentIds) {
 ...
 */
function calculateRelevanceScore(queryTerms, text) {
    let score = 0;
 ...
 */
export function buildContext(searchResults, maxTokens = RAG_CONFIG.maxContextTokens) {
    let context = '';
    let tokenCount = 0;
 ...
 */
export function extractSources(searchResults) {
