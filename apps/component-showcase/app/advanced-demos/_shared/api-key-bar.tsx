...
import { Button, Badge, Input, cn } from '@clarity-chat/primitives'
import { Key, Eye, EyeOff, ChevronDown, Check, Zap, Shield } from 'lucide-react'
...
export const ApiKeyBar = memo(function ApiKeyBar({
  provider,
  onProviderChange,
  model,
  onModelChange,
  ...
  const currentProvider =
    MODEL_PROVIDERS.find((p) => p.id === provider) || MODEL_PROVIDERS[0]
  const currentModel =
    currentProvider.models.find((m) => m.id === model) ||
