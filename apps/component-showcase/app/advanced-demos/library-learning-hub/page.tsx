...
import { KNOWLEDGE_BASE, type KnowledgeEntry } from './components/knowledge-base'
...
function searchKnowledgeBase(query: string): KnowledgeEntry[] {
  ...
  return KNOWLEDGE_BASE.filter(entry => {
    const searchText = `${entry.title} ${entry.tags.join(' ')} ${entry.content}`.toLowerCase()
...
const slashCommands: SlashCommand[] = [
  { id: 'components', label: '/components', description: 'List all components' },
...
const mentionItems: MentionItem[] = [
  ...knowledgeBase.filter(e => e.category === 'component').map(e => ({ id: e.id, label: e.title, type: 'component' as const })),
  ...knowledgeBase.filter(e => e.category === 'hook').map(e => ({ id: e.id, label: e.title, type: 'hook' as const })),
]

// Default conversations
function createDefaultConversation(): Conversation {
  return { id: generateId(), title: 'New Chat', messages: [], createdAt: new Date(), updatedAt: new Date() }
}

// Default prompts
const defaultPrompts: SavedPrompt[] = [
  { id: '1', text: 'How do I set up streaming?', category: 'setup' },
  { id: '2', text: 'Show me ChatInput with file upload', category: 'component' },
...
function generateLibraryResponse(query: string, matches: KnowledgeEntry[]): { response: string; citations: Citation[] } {
...
function generateSlashResponse(commandId: string): { response: string; citations: Citation[] } {
  switch (commandId) {
    case 'components': {
    ...
    case 'adapters': {
      ...
      return {
        ...
        citations: adapters.map((a, i) => ({ id: `cit-${i}`, title: a.title, snippet: a.content.slice(0, 100), type: 'library' as const, relevance: 0.9 })),
      }
    }
    case 'memory': {
      const memEntries = knowledgeBase.filter(e => e.category === 'memory')
      return {
        response: memEntries.length > 0
    ...
    case 'tokens': {
      ...
      return {
        response: tokenEntries.length > 0
    ...
    default:
      return { response: 'Command not recognized. Available commands: /components, /hooks, /adapters, /example, /memory, /tokens', citations: [] }
  }
}
...
export default function LibraryLearningHubPage() {
  ...
  const [memorySettings, setMemorySettings] = useState<MemorySettings>({
    enabled: true, strategy: 'semantic-chunks', maxTokens: 4096, usage: 1247,
  })
  const [tokenSettings, setTokenSettings] = useState<TokenSettings>({
    ...
    budget: 8000, used: { input: 1245, output: 892, total: 2137 },
  ...
  const handleSend = async (overrideText?: string, slashCommandId?: string) => {
    ...
    if (slashCommandId) {
      ...
      if (matches.length > 0) {
        addThinkingStep(`Found ${matches.length} relevant entries: ${matches.slice(0, 3).map(m => m.title).join(', ')}`)
        await new Promise(r => setTimeout(r, 400))
        addThinkingStep('Composing response with code examples...')
      } else {
        addThinkingStep('No direct matches found, generating general guidance...')
      }
      await new Promise(r => setTimeout(r, 400))
    ...
    const cancelStream = simulateStreaming(
      ...
      () => {
        ...
        setMemorySettings(prev => ({ ...prev, usage: Math.min(prev.usage + inputTokens + outputTokens, prev.maxTokens) }))
      },
      12
    )
  }
  ...
  const handleEditSave = (msgId: string) => {
    const msgIndex = messages.findIndex(m => m.id === msgId)
    updateMessages(messages.slice(0, msgIndex).concat({ ...messages[msgIndex], content: editingText }))
              ...
              if (boldEnd > 0) {
                return (
                  <div key={j} className="flex gap-2 py-0.5">
                    ...
                    <span><strong>{line.slice(4, boldEnd)}</strong>{line.slice(boldEnd + 2)}</span>
                  </div>
                )
              }
            }
  ...
  return (
    <div>
        ...
        <div className="flex h-[calc(100%-3rem)]">
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto">
              {messages.length === 0 && !isStreaming ? (
                <LibraryWelcomeScreen onSuggestionClick={(text) => { setInput(text); inputRef.current?.focus() }} />
              ) : (
                    ...
                    <div key={msg.id} className={cn('group flex gap-3', msg.role === 'user' && 'justify-end')}>
                      ...
                      <div className={cn('max-w-[75%] min-w-0', msg.role === 'user' ? 'order-first' : '')}>
                        ...
                        {msg.citations && msg.citations.length > 0 && (
                          <RagCitations entries={msg.citations.map(c => knowledgeBase.find(k => k.title === c.title)).filter(Boolean) as KnowledgeEntry[]} />
                        )}
                  ...
                  {isThinking && (
                    <div className="flex gap-3">
                      ...
                      <div className="max-w-[75%]">
                        <ThinkingDisplay steps={activeThinking} isActive={true} />
                      </div>
                    </div>
                  )}
            ...
            <div className="border-t p-4 bg-card/50 relative">
              ...
              <div className="flex items-end gap-2">
                <div className="flex-1 relative">
                  ...
                  <div className="absolute right-2 bottom-1.5 flex items-center gap-1">
                    <button
                      onClick={() => { setShowSlashMenu(!showSlashMenu); setShowMentionMenu(false) }}
                    ...
                    <button
                      onClick={() => { setShowMentionMenu(!showMentionMenu); setShowSlashMenu(false) }}
                      className="p-1.5 rounded-md hover:bg-muted transition-colors text-muted-foreground"
                      title="Mention"
                    >
              ...
              <div className="flex items-center justify-between mt-2 text-[10px] text-muted-foreground">
                <div className="flex items-center gap-3">
                  <span>Type <kbd className="px-1 py-0.5 rounded bg-muted text-[9px]">/</kbd> for commands</span>
                  <span>Type <kbd className="px-1 py-0.5 rounded bg-muted text-[9px]">@</kbd> for mentions</span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => setShowExport(true)} className="flex items-center gap-1 hover:text-foreground transition-colors">
                    <Download className="h-3 w-3" /> Export
                  </button>
                  <button onClick={() => setShowSettings(true)} className="flex items-center gap-1 hover:text-foreground transition-colors">
                    <Settings className="h-3 w-3" /> Settings
                  </button>
                  <button onClick={() => setRightPanelOpen(!rightPanelOpen)} className="flex items-center gap-1 hover:text-foreground transition-colors">
          ...
          {rightPanelOpen && (
            <div className="w-72 border-l bg-card/30 overflow-y-auto p-4 space-y-6 shrink-0">
              ...
              {/* Memory */}
              <MemoryPanel settings={memorySettings} onUpdate={setMemorySettings} />
      ...
      {/* Dialogs */}
      <ChatExportDialog open={showExport} onClose={() => setShowExport(false)} onExport={() => {}} />
      <SettingsDialog
        open={showSettings}
