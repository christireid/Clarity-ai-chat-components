...
import { ARTIFACT_TEMPLATES as artifactTemplates, type ArtifactTemplate } from './components/artifact-data'
...
function createConversation(): Conversation {
  return { id: generateId(), title: 'New Project', messages: [], createdAt: new Date(), updatedAt: new Date(), artifactCount: 0 }
}

const defaultMCPServers: MCPServer[] = [
  { id: '1', name: 'Code Runner MCP', endpoint: 'https://code.mcp.local:3001', status: 'connected', enabled: true, tools: ['run_code', 'lint_code', 'format_code'] },
  { id: '2', name: 'Design Tool MCP', endpoint: 'https://design.mcp.local:3002', status: 'connected', enabled: true, tools: ['generate_svg', 'create_layout'] },
  { id: '3', name: 'Data Viz MCP', endpoint: 'https://viz.mcp.local:3003', status: 'disconnected', enabled: false, tools: ['create_chart', 'plot_data'] },
]
...
export default function ArtifactStudioPage() {
  ...
  const [memorySettings, setMemorySettings] = useState<MemorySettings>({
    enabled: true, strategy: 'sliding-window', maxTokens: 4096, usage: 890,
    trackPatterns: true, rememberPreferences: true,
  })
  const [tokenSettings, setTokenSettings] = useState<TokenSettings>({
    ...
    budget: 12000, used: { input: 1800, output: 1200, total: 3000 },
  ...
  const [settingsResponseLength, setSettingsResponseLength] = useState('balanced')
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    const isUpdate = text.toLowerCase().includes('update') || text.toLowerCase().includes('modify') || text.toLowerCase().includes('change')
    ...
    if (template && !isUpdate) {
      ...
      const tool: ToolExecution = { id: toolId, name: template.toolName, status: 'running', input: `Creating ${template.artifact.type} artifact...` }
      setActiveTools([tool])
      await new Promise(r => setTimeout(r, 1200))
      setActiveTools([{ ...tool, status: 'completed', output: `${template.artifact.title} created`, duration: '1.2s' }])
      ...
      setToolUsageCounts(prev => ({ ...prev, [toolKey]: (prev[toolKey] || 0) + 1 }))
      ...
      const newArtifact: Artifact = {
        ...
        versions: [{ id: generateId(), content: template.artifact.content, timestamp: new Date(), label: 'v1' }],
        createdAt: new Date(),
        updatedAt: new Date(),
      }
      ...
      for (let i = 0; i < lines.length; i += 3) {
        ...
        setArtifacts(prev => prev.map(a => a.id === newArtifact.id ? { ...a, content: partial } : a))
      }
      ...
      setConversations(prev => prev.map(c =>
        c.id === activeConvId ? { ...c, artifactCount: (c.artifactCount || 0) + 1 } : c
      ))
      ...
      simulateStreaming(
        ...
        () => {
          ...
          const assistantMessage: ChatMessage = {
            ...
            tools: [{ ...tool, status: 'completed', output: `${template.artifact.title} created`, duration: '1.2s' }],
            artifacts: [{ id: newArtifact.id, title: newArtifact.title, type: newArtifact.type, version: 1 }],
            status: 'delivered',
          }
          ...
          setTokenSettings(prev => ({ ...prev, used: { input: prev.used.input + inT, output: prev.used.output + outT, total: prev.used.total + inT + outT } }))
        },
        12
      )
      ...
      setActiveThinking(prev => [...prev, { id: generateId(), content: 'Understanding modification request...', timestamp: new Date() }])
      await new Promise(r => setTimeout(r, 400))
      setActiveThinking(prev => [...prev, { id: generateId(), content: `Updating artifact: ${latest.title}...`, timestamp: new Date() }])
      ...
      const newVersion = { id: generateId(), content: updatedContent, timestamp: new Date(), label: `v${latest.versions.length + 1}` }
      setArtifacts(prev => prev.map(a => a.id === latest.id ? { ...a, content: updatedContent, versions: [...a.versions, newVersion], updatedAt: new Date() } : a))
      ...
      simulateStreaming(
        ...
        () => {
          setIsStreaming(false)
          updateMessages([...newMessages, {
            id: generateId(), role: 'assistant', content: updateResponse, timestamp: new Date(),
            artifacts: [{ id: latest.id, title: latest.title, type: latest.type, version: latest.versions.length + 1 }],
            status: 'delivered',
          }])
          setStreamingText('')
        },
      ...
      // Normal conversation
      await new Promise(r => setTimeout(r, 400))
      setActiveThinking(prev => [...prev, { id: generateId(), content: 'Processing your request...', timestamp: new Date() }])
      await new Promise(r => setTimeout(r, 500))
      setActiveThinking(prev => [...prev, { id: generateId(), content: 'Generating response...', timestamp: new Date() }])
      setIsThinking(false)
      ...
      simulateStreaming(
        ...
        () => {
          setIsStreaming(false)
          updateMessages([...newMessages, {
            id: generateId(), role: 'assistant', content: response, timestamp: new Date(),
            thinking: [...activeThinking], status: 'delivered',
          }])
          setStreamingText('')
          const inT = estimateTokens(text)
  ...
  const handleDeleteMessage = (msgId: string) => { updateMessages(messages.filter(m => m.id !== msgId)) }
      ...
      return (
        <div key={i}>
          {part.split('\n').map((line, j) => {
            ...
            const rendered = line.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code class="px-1 py-0.5 rounded bg-muted text-sm font-mono">$1</code>')
  ...
  return (
    <div>
      ...
      <div className="glass-card overflow-hidden border-0 h-[calc(100vh-14rem)]">
        ...
        <div className="flex h-[calc(100%-3rem)]">
          ...
          <ArtifactSidebar
            ...
            onNewConversation={() => { const c = createConversation(); setConversations(prev => [c, ...prev]); setActiveConvId(c.id) }}
            ...
            techStack={techStack}
            onToggleTech={(tech) => setTechStack(prev => prev.includes(tech) ? prev.filter(t => t !== tech) : [...prev, tech])}
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div ref={scrollRef} className="flex-1 overflow-y-auto">
              {messages.length === 0 && !isStreaming ? (
                <ArtifactWelcomeScreen onSuggestionClick={(text) => { setInput(text); inputRef.current?.focus() }} />
              ) : (
                  ...
                  {messages.map(msg => (
                    <div key={msg.id} className={cn('group flex gap-3', msg.role === 'user' && 'justify-end')}>
                      ...
                      <div className={cn('max-w-[75%] min-w-0', msg.role === 'user' ? 'order-first' : '')}>
                        {msg.role === 'assistant' && msg.thinking && msg.thinking.length > 0 && <ThinkingDisplay steps={msg.thinking} />}
                        {msg.role === 'assistant' && msg.tools && msg.tools.length > 0 && (
                          <div className="mb-2 space-y-1">
                            {msg.tools.map(tool => (
                              <div key={tool.id} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/30 text-xs">
                                <Wrench className="h-3 w-3 text-orange-500" />
                                <span className="font-medium">{tool.name}</span>
                                <span className="text-muted-foreground">{tool.output}</span>
                                {tool.duration && <span className="text-muted-foreground ml-auto font-mono">{tool.duration}</span>}
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {msg.artifacts && msg.artifacts.length > 0 && (
                          <div className="mb-2 space-y-1">
                            {msg.artifacts.map(ref => (
                              <button
                                key={ref.id}
                                onClick={() => { setActiveArtifactId(ref.id); setArtifactPanelOpen(true) }}
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  ...
                                  <span className="text-xs text-muted-foreground ml-2">v{ref.version} &middot; {ref.type}</span>
                                </div>
                                <span className="text-xs text-violet-500">View</span>
                              </button>
                            ))}
                          </div>
                        )}
                        <div className={cn('rounded-2xl px-4 py-3', msg.role === 'user' ? 'bg-primary text-primary-foreground ml-auto' : 'bg-muted/50')}>
                          {editingMessageId === msg.id ? (
                            <div className="space-y-2">
                              <textarea value={editingText} onChange={e => setEditingText(e.target.value)} className="w-full bg-transparent border rounded-lg p-2 text-sm resize-none focus:outline-none" rows={3} />
                  ...
                  {(isThinking || activeTools.length > 0) && (
                    <div className="flex gap-3">
                      ...
                      <div className="max-w-[75%] space-y-2">
                        ...
                        {activeTools.map(t => (
                          <div key={t.id} className={cn('flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs', t.status === 'running' ? 'bg-yellow-500/10' : 'bg-muted/30')}>
                            {t.status === 'running' ? <Loader2 className="h-3 w-3 animate-spin text-yellow-500" /> : <Wrench className="h-3 w-3 text-green-500" />}
                            <span className="font-medium">{t.name}</span>
                            <span className="text-muted-foreground">{t.status === 'running' ? t.input : t.output}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="border-t p-4 bg-card/50 relative">
                ...
                <div className="absolute bottom-full left-4 mb-2 w-64 rounded-xl border bg-card shadow-xl z-50 py-1">
                  {slashCommands.map(cmd => (
                    <button
                      key={cmd.id}
                      onClick={() => {
                        if (cmd.id === 'preview') { setArtifactPanelOpen(!artifactPanelOpen); setShowSlashMenu(false) }
                        else if (cmd.id === 'export') { setShowExport(true); setShowSlashMenu(false) }
                        else { setInput(cmd.label + ' '); setShowSlashMenu(false); inputRef.current?.focus() }
                      }}
                      className="flex items-center gap-3 w-full px-3 py-2 text-sm hover:bg-muted transition-colors"
                    >
                      <span className="font-mono text-xs text-violet-500">{cmd.label}</span>
              ...
              <div className="flex items-end gap-2">
                <textarea
                  ref={inputRef}
                  value={input}
                  onChange={e => { setInput(e.target.value); setShowSlashMenu(e.target.value.endsWith('/')) }}
                  onKeyDown={e => {
              ...
              <div className="flex items-center justify-between mt-2 text-[10px] text-muted-foreground">
                <div className="flex items-center gap-2">
                  ...
                  <span>{mcpServers.filter(s => s.status === 'connected').length} MCP servers</span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => setShowExport(true)} className="flex items-center gap-1 hover:text-foreground"><Download className="h-3 w-3" /> Export</button>
                  <button onClick={() => setShowSettings(true)} className="flex items-center gap-1 hover:text-foreground"><Settings className="h-3 w-3" /> Settings</button>
                  <button onClick={() => setArtifactPanelOpen(!artifactPanelOpen)} className={cn('flex items-center gap-1 hover:text-foreground', artifactPanelOpen && 'text-violet-500')}>
      ...
      <ChatExportDialog
        open={showExport} onClose={() => setShowExport(false)} onExport={() => {}}
        formats={[
          { id: 'all', label: 'Conversation + Artifacts', description: 'Everything as structured JSON', icon: <Layers className="h-4 w-4" /> },
          { id: 'artifacts', label: 'Artifacts Only', description: 'Individual artifact files', icon: <Code className="h-4 w-4" /> },
          { id: 'markdown', label: 'Conversation Only', description: 'Chat as Markdown', icon: <FileText className="h-4 w-4" /> },
        ]}
      />
      <SettingsDialog
      ...
      >
        <div className="space-y-4">
          ...
          <ArtifactMCPManager servers={mcpServers} onServersChange={setMcpServers} />
