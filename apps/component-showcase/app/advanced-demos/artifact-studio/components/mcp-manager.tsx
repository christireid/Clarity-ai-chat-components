      ...
      {showAdd && (
        <div className="p-3 rounded-lg bg-muted/30 space-y-2 border border-dashed border-muted-foreground/20">
          ...
          <div className="flex gap-2">
            <button
              onClick={addServer}
              className="flex-1 text-xs py-1.5 rounded bg-primary text-primary-foreground hover:opacity-90 transition-opacity"
            >
              Add
            </button>
            <button
              onClick={() => setShowAdd(false)}
              className="flex-1 text-xs py-1.5 rounded bg-muted hover:bg-muted/80 transition-colors"
            >
              Cancel
            </button>
          </div>
              ...
              <div className="px-3 pb-2 pt-0 border-t space-y-2">
                ...
                <div className="flex flex-wrap gap-1">
                  {server.tools.map((tool) => (
                    <span
                      key={tool}
                      className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-500"
                    >
                      {tool}
                    </span>
                  ))}
