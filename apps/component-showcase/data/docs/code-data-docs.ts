...
console.log(greeting)\`}
    ...
    },
    {
      ...
      importPath: '@clarity-chat/react',
      usageCode: `<p>
  Use the <InlineCode>useState</InlineCode> hook to manage state.
</p>

{/* With copy-on-click */}
<InlineCode enableCopy onCopy={() => console.log('Copied!')}>
...
/>`,
    props: [
      ...
      {
        name: 'theme',
        type: "'night-owl' | 'dracula' | 'monokai' | 'github-dark' | 'one-dark' | 'nord'",
        default: '"night-owl"',
...
/>`,
  ...
  suites={[
    {
      ...
      tests: [
        ...
        { name: 'should parse JSON', status: 'pass', duration: 8 },
        {
  ...
  onTestClick={(test, suite) => console.log(test.name, suite.name)}
/>`,
    props: [
      {
        name: 'suites',
