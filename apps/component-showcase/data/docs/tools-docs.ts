...
/>`,
  ...
  tools={[
    { id: '1', name: 'web_search', status: 'success', args: { query: 'React' }, duration: 1200 },
    { id: '2', name: 'read_file', status: 'running', args: { path: '/src/index.ts' } },
    { id: '3', name: 'code_interpreter', status: 'error', error: 'Syntax error on line 5' },
  ]}
  size="sm"
  gap="sm"
  showArgs
...
/>`,
  ...
  tool={{
    ...
    status: 'executing',
    input: { query: 'React hooks best practices', max_results: 5 },
    output: null,
    startTime: new Date(),
  }}
...
/>`,
  ...
  result={{
    ...
    snippet: 'Next.js 14 includes Server Actions, Partial Prerendering, and more...',
    siteName: 'nextjs.org',
    type: 'web',
    relevance: 0.95,
  }}
...
/>`,
  ...
  toolCall={{
    ...
    args: { command: 'npm install lodash', cwd: '/project' },
    status: 'pending_approval',
    createdAt: Date.now(),
  }}
  toolDefinition={{
    name: 'run_terminal',
    description: 'Execute terminal commands in the project directory',
...
/>`,
  ...
  size="md"
  details={[
    { label: 'Command', value: 'npm install lodash', isCode: true },
    { label: 'Working Directory', value: '/project', isCode: true },
  ]}
...
/>`,
    props: [
      ...
      {
        name: 'variant',
        type: "'card' | 'inline' | 'banner' | 'compact'",
        default: '"card"',
        description:
...
const unsubscribe = registry.on((event) => {
  ...
  showSparklines
  animateValues
  onStatClick={(stat) => console.log('Clicked:', stat.label)}
/>`,
    props: [
      {
        name: 'stats',
      ...
      {
        name: 'variant',
        type: "'grid' | 'inline' | 'card' | 'compact' | 'minimal'",
        default: '"grid"',
        description:
