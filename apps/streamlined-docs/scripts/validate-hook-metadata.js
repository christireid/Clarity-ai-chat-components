...
function parseHookMetadata() {
    ...
    const hookRegex = /{\s*name:\s*['"]([^'"]+)['"],\s*href:\s*['"]([^'"]+)['"][\s\S]*?(?:relatedHooks:\s*\[([\s\S]*?)\])?[\s\S]*?}/g;
    const hooks = [];
    let match;
        ...
        if (hook.relatedHooks) {
            for (const related of hook.relatedHooks) {
                if (!hookNames.has(related)) {
                    result.warnings.push(`Hook "${hook.name}" references unknown related hook "${related}"`);
                }
            }
        }
    }
...
main();
//# sourceMappingURL=validate-hook-metadata.js.map