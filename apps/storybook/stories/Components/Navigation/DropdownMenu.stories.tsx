    ...
    await waitFor(
      async () => {
        ...
        if (menuItems.length > 0) {
          ...
          const billingItem = menuItems.find((item) =>
            item.textContent?.includes('Billing')
          )
      ...
      async () => {
        ...
        if (menuItems.length > 0) {
          ...
          const editItem = menuItems.find((item) =>
            item.textContent?.includes('Edit message')
          )
          const copyItem = menuItems.find((item) =>
            item.textContent?.includes('Copy text')
          )
          const pinItem = menuItems.find((item) =>
            item.textContent?.includes('Pin message')
          )
          const deleteItem = menuItems.find((item) =>
            item.textContent?.includes('Delete message')
          )
