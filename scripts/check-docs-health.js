...
async function countFiles(pattern) {
    try {
        const { stdout } = await execAsync(`find . -type f -name "${pattern}" -not -path "./node_modules/*" -not -path "./.git/*" | wc -l`);
...
async function findFiles(pattern) {
    try {
        const { stdout } = await execAsync(`find . -type f -name "${pattern}" -not -path "./node_modules/*" -not -path "./.git/*"`);
...
async function findEmptyFiles() {
    try {
        const { stdout } = await execAsync(`find . -type f -name "*.md" -empty -not -path "./node_modules/*"`);
        return stdout.trim().split('\n').filter(Boolean);
    }
    catch (error) {
        return [];
...
async function findTodoMarkers() {
    try {
        const { stdout } = await execAsync(`grep -rl "TODO\\|FIXME\\|TBD\\|PLACEHOLDER" --include="*.md" --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=.archive . 2>/dev/null || true`);
...
//# sourceMappingURL=check-docs-health.js.map