...
function validatePackageJson(ctx) {
    ...
    try {
        ...
        if (!pkg.types && !pkg.typings) {
            result.warnings.push('package.json: missing "types" (TypeScript declarations won\'t be found)');
        }
        ...
        if (!pkg.exports) {
            result.warnings.push('package.json: missing "exports" map (recommended for modern bundlers)');
        }
        else {
            ...
            if (typeof pkg.exports === 'object' && pkg.exports['.']) {
                ...
                if (typeof mainExport === 'object' && !mainExport.types) {
                    result.warnings.push('package.json: exports["."] is missing "types" condition');
                }
            }
        }
        // Peer dependencies for React libraries
        if (pkg.dependencies?.react && !pkg.peerDependencies?.react) {
            result.warnings.push('package.json: "react" should be a peerDependency, not a dependency');
        }
        // Side effects for tree-shaking
        if (pkg.sideEffects === undefined) {
            result.warnings.push('package.json: missing "sideEffects" (may affect tree-shaking)');
        }
        // Files array
        if (!pkg.files) {
            result.warnings.push('package.json: missing "files" (npm will include everything)');
        }
        // Repository and bugs
        if (!pkg.repository) {
...
function validateTsConfig(ctx) {
    ...
    try {
        ...
        // Strict mode
        if (!opts.strict) {
            result.warnings.push('tsconfig.json: "strict" mode is not enabled (recommended for better type safety)');
        }
        // Declaration files
        if (!opts.declaration) {
            result.errors.push('tsconfig.json: "declaration" should be true (required for library types)');
        }
        // Declaration maps for better DX
        if (!opts.declarationMap) {
            result.warnings.push('tsconfig.json: "declarationMap" recommended for better IDE navigation');
        }
        // Source maps
        if (!opts.sourceMap) {
            result.warnings.push('tsconfig.json: "sourceMap" recommended for debugging');
        }
        // Module resolution
        if (opts.moduleResolution === 'node') {
            result.warnings.push('tsconfig.json: "moduleResolution": "bundler" recommended for modern projects');
        }
        // skipLibCheck warning
        if (opts.skipLibCheck === true) {
            result.warnings.push('tsconfig.json: "skipLibCheck" is true (may hide type errors in dependencies)');
        }
        // JSX
        if (opts.jsx && !['react-jsx', 'react-jsxdev'].includes(opts.jsx)) {
            result.warnings.push('tsconfig.json: consider "jsx": "react-jsx" for React 17+ projects');
        }
    }
    catch (e) {
...
function validateEslint(ctx) {
    ...
    const found = configFiles.find((f) => fs.existsSync(path.join(ctx.rootDir, f)));
    if (!found) {
        result.warnings.push('No ESLint config found (recommended for code quality)');
    }
    return result;
}
function validatePrettier(ctx) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
    ...
    if (!found && !hasPrettierInPkg) {
        result.warnings.push('No Prettier config found (recommended for consistent formatting)');
    }
    return result;
}
function validateGitIgnore(ctx) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
...
async function main() {
    ...
    if (allValid && totalWarnings === 0) {
        ...
        console.log('⚠️  Configuration is valid, but there are warnings to address.\n');
    }
    else {
    ...
    console.log('   - See https://clarity-chat.dev/setup for recommended configuration');
    console.log('');
