 ...
 */
function classifyFile(filePath, exports) {
    const fileName = path.basename(filePath, path.extname(filePath));
    const isHook = fileName.startsWith('use') || exports.some((e) => e.startsWith('use'));
...
`;
    ...
    console.log(`✅ Generated inventory at: ${path.relative(projectRoot, outputPath)}`);
    console.log(`📊 Processed ${components.length} files`);
    console.log(`   - ${components.filter((c) => c.isComponent).length} components`);
    console.log(`   - ${components.filter((c) => c.isHook).length} hooks`);
    console.log(`   - ${components.filter((c) => !c.isComponent && !c.isHook).length} utilities\n`);
}
// Run
...
//# sourceMappingURL=generate-component-inventory.js.map