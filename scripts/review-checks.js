...
const CHECKS = {
    ...
    missingUseClient: {
        check: (content, filePath) => {
            ...
            const hasHooks = /\b(useState|useEffect|useRef|useCallback|useMemo|useReducer|useContext|useFormState|useFormStatus)\s*\(/.test(content);
        ...
        severity: 'error',
        fixable: true,
        fix: (_line) => null, // Handled specially - adds to top of file
    },
...
function parseSuppressions(content) {
    ...
    const lineSuppressions = new Map();
    const fileSuppressions = new Set();
...
function isSuppressed(suppressions, ruleName, lineNumber) {
...
// Output Formatting
// ─────────────────────────────────────────────────────────────────────────────
function printConsoleResults(result) {
