...
function parseArgs(): { name?: string; color?: string; mode?: string } {
  const args = process.argv.slice(2)
  const result: { name?: string; color?: string; mode?: string } = {}
...
async function main() {
  ...
  if (parsedArgs.name && parsedArgs.color && parsedArgs.mode) {
    ...
    mode = parsedArgs.mode as 'light' | 'dark'
  } else {
    ...
    try {
      ...
      const modeInput =
        parsedArgs.mode || (await ask(rl, 'Mode (light/dark)', 'dark'))
