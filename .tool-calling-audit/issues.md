...
This document catalogs all correctness issues, bugs, inconsistencies, and potential problems
...
**Description**: Two different `ToolRegistry` implementations exist:
...
**Description**: Three different patterns for tool execution exist:
...
**Description**: Two different `ToolCall` types with different status values:
...
**Description**: Different tool type definitions across the codebase:
...
**Description**: E2E test file uses `handler` instead of `execute` in ToolDefinition.
...
- Indicates either: a) Old test code not updated, or b) Alternative ToolDefinition format exists
...
**Description**: The streaming integration test creates a calculator tool that uses `eval()`.
...
**Description**: No clear guidance on when to use core/ToolRegistry vs agents/ToolRegistry.
...
**Description**: Three execution patterns with no clear guidance on when to use each.
...
**Description**: No test file found for `adapters/tool-formats.ts`.
...
**Description**: Test coverage unknown for utility files:
...
**Description**: Tool `execute` functions are assumed to be trusted code, but there's no sandboxing
or validation of tool implementations themselves.
...
**Description**: Different default values for `autoApprove` across implementations:
...
**Description**: No rate limiting on tool execution.
...
**Description**: Cache key generation uses JSON.stringify with sorted keys, which can have
collisions for complex objects.
...
**Description**: No limit on concurrent tool executions. Parallel execution is allowed but
unbounded.
...
2. **Support** per-tool parallelization settings (ToolDefinition has `parallelizable` flag but not
   used)
...
**Description**: Cache entries are checked for TTL on access but never proactively cleaned up.
...
**Description**: `executeBatch` utility exists but may not optimize batching.
...
**Description**: Developers using legacy `ToolRegistry` have no migration path documented.
...
**Description**: Security assumptions and best practices for tool implementations are not clearly
documented.
...
**Description**: No guidance on when to use ToolOrchestrator vs Tools Engine vs ToolExecutor.
...
- **MEDIUM**: 11 (ISSUE-003, ISSUE-004, ISSUE-005, ISSUE-007, ISSUE-008, ISSUE-012, ISSUE-013,
  ISSUE-015, ISSUE-019, ISSUE-020)
