      ...
      <aside className="w-96 border-r p-6 space-y-6">
        ...
        <div className="flex gap-2">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="h-8 w-16 bg-muted animate-pulse rounded-lg"
            />
          ))}
        </div>
        ...
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="p-4 bg-muted animate-pulse rounded-xl space-y-3"
            >
              <div className="flex gap-1">
                {[...Array(4)].map((_, j) => (
                  <div
                    key={j}
                    className="w-6 h-6 bg-muted-foreground/10 rounded-full"
                  />
                ))}
              </div>
