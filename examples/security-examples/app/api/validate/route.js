...
export async function POST(request) {
    try {
        ...
        if (!result.allowed) {
            ...
            const jailbreakCheck = result.checks?.find((c) => c.name === 'jailbreak_prevention');
            return NextResponse.json({
                allowed: false,
                reason: 'Security policy violation',
                severity: (injectionCheck?.confidence ?? 0) > 0.8 ||
        ...
        const wasRedacted = piiCheck && !piiCheck.passed && piiCheck.action === 'redact';
        return NextResponse.json({
            allowed: true,
