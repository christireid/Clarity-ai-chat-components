  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
      ...
      <div style={{ 
        ...
        {messages.map(msg => (
          <div key={msg.id} style={{ marginBottom: '8px', padding: '8px', background: '#fff', borderRadius: '4px' }}>
            <strong>{msg.role}:</strong> {msg.content}
          </div>
        ))}
      </div>
...
function Component() {
  ...
  const models = [
    ...
    { id: 'gemini', name: 'Gemini Pro', description: 'Multimodal capabilities' }
  ]
        ...
        <div style={{ marginTop: '16px' }}>
          ...
          {isError && (
            <div style={{
              ...
              ⚠️ Token limit exceeded! Please reduce text length.
            </div>
          )}
        </div>
      </div>
...
function Component() {
  ...
  const handleQuery = () => {
    ...
    // Simulate RAG pattern: retrieve context and generate response
    const mockContext = 'Retrieved context: React is a JavaScript library for building user interfaces. It uses a component-based architecture.'
    setContext(mockContext)
    
    // Simulate response generation using context
    setResponse(\`Based on the context: "\${mockContext}", here's the answer to your question: "\${query}"\`)
  }

  return (
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '20px' }}>
      <h2>RAG Pattern (Retrieval-Augmented Generation)</h2>
      ...
      <div style={{ marginTop: '20px' }}>
        ...
        <button
          ...
          style={{
            marginTop: '12px',
            padding: '10px 24px',
            background: query.trim() ? '#007bff' : '#ccc',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
