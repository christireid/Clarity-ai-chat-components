...
import { PlaygroundProvider, usePlayground, usePlaygroundState, usePlaygroundActions, usePlaygroundCode, usePlaygroundTheme, usePlaygroundSettings, usePlaygroundConsole, } from './PlaygroundContext';
// Mock sonner
vi.mock('sonner', () => ({
    toast: {
        success: vi.fn(),
...
function wrapper({ children }) {
    return _jsx(PlaygroundProvider, { children: children });
}
describe('PlaygroundContext', () => {
    ...
    describe('usePlaygroundActions', () => {
        ...
        it('addConsoleEntry adds entry with id and timestamp', () => {
            ...
            expect(result.current.state.consoleEntries[0].message).toBe('Test message');
            expect(result.current.state.consoleEntries[0].id).toBeDefined();
            expect(result.current.state.consoleEntries[0].timestamp).toBeInstanceOf(Date);
        });
...
//# sourceMappingURL=PlaygroundContext.test.js.map