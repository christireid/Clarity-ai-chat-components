...
export class EnvVarMissingError extends ClarityError {
    constructor(varName, originalError) {
        ...
        super('ENV_VAR_MISSING', `Environment variable ${varName} is not set`, `The required environment variable ${varName} is missing`, solutions, {
...
export class InvalidConfigError extends ClarityError {
    constructor(configKey, expectedType, actualValue, originalError) {
        const solutions = [
            {
        ...
        super('INVALID_CONFIG', `Invalid configuration for ${configKey}`, `Expected ${expectedType}, got ${typeof actualValue}: ${JSON.stringify(actualValue)}`, solutions, {
  ...
  apps: [{
    ...
    env: {
      PORT: ${port + 1}  // Use next available port
        ...
        super('PORT_IN_USE', `Port ${port} is already in use`, `Cannot start server on port ${port} because another process is using it`, solutions, {
...
export class FileNotFoundError extends ClarityError {
    constructor(filePath, expectedLocation, originalError) {
        const solutions = [
            {
        ...
        super('FILE_NOT_FOUND', `File not found: ${filePath}`, `The system cannot find the file at ${filePath}`, solutions, {
...
export class DependencyMissingError extends ClarityError {
    constructor(packageName, originalError) {
        const solutions = [
            {
        ...
        super('DEPENDENCY_MISSING', `Required package '${packageName}' is not installed`, `Cannot find module '${packageName}'. You need to install it first.`, solutions, {
...
// Helper function
function getConfigExample(key, type) {
    const examples = {
        number: `// config.ts\nexport const config = {\n  ${key}: 3000\n}`,
        string: `// config.ts\nexport const config = {\n  ${key}: "value"\n}`,
        boolean: `// config.ts\nexport const config = {\n  ${key}: true\n}`,
