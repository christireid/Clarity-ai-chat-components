...
import { formatError, logError, handleError, createErrorHandler, withErrorHandling, assert, tryCatch } from '../utils';
import { ClarityError } from '../base-error';
import { APIKeyMissingError } from '../api-errors';
...
describe('logError', () => {
    ...
    it('should log regular Error with stack', () => {
        ...
        expect(calls.some((c) => c.includes('Unexpected Error'))).toBe(true);
    });
    it('should log stack trace for regular errors', () => {
        const error = new Error('Error with stack');
        logError(error);
        const calls = consoleErrorSpy.mock.calls.map((c) => c[0]);
        expect(calls.some((c) => c.includes('Stack trace'))).toBe(true);
    });
});
describe('handleError', () => {
    ...
    it('should return 400 for validation errors', () => {
        const error = new ValidationError('Invalid', ['error']);
...
describe('createErrorHandler', () => {
    ...
    it('should use correct status code for ClarityErrors', async () => {
        const handler = createErrorHandler();
        const mockHandler = vi.fn().mockRejectedValue(new APIKeyMissingError('openai'));
        const mockRes = {
            status: vi.fn().mockReturnThis(),
            json: vi.fn()
        };
