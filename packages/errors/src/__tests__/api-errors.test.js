...
import { APIKeyMissingError, APIRateLimitError, APIAuthenticationError, APINetworkError, APIResponseError } from '../api-errors';
...
describe('APIAuthenticationError', () => {
    ...
    it('should suggest generating new key', () => {
        const error = new APIAuthenticationError('anthropic');
        const newKeySolution = error.solutions.find((s) => s.description.includes('Generate a new'));
...
describe('APINetworkError', () => {
    ...
    it('should suggest checking service status', () => {
        const error = new APINetworkError('openai');
        const statusSolution = error.solutions.find((s) => s.description.includes('service is down'));
    ...
    it('should include proxy configuration example', () => {
        const error = new APINetworkError('openai');
        const proxySolution = error.solutions.find((s) => s.description.includes('proxy'));
