 ...
 */
export function getLicenseFeatures(tier) {
    switch (tier) {
        case 'free':
...
export function hasFeature(tier, feature) {
    ...
    if (features.components.includes(feature))
        return true;
    if (features.themes.includes(feature))
        return true;
    if (features.aiProviders.includes(feature))
        return true;
    if (features.analyticsProviders.includes(feature))
        return true;
    if (features.errorProviders.includes(feature))
        return true;
    // Check boolean features
    if (feature === 'enterprise' && features.enterpriseFeatures)
        return true;
    if (feature === 'white-label' && features.whiteLabelAllowed)
        return true;
    if (feature === 'saas' && features.saasAllowed)
        return true;
    return false;
}
/**
 ...
 */
export function isComponentAvailable(tier, component) {
    const features = getLicenseFeatures(tier);
 ...
 */
export function isThemeAvailable(tier, theme) {
    const features = getLicenseFeatures(tier);
 ...
 */
export function isAIProviderAvailable(tier, provider) {
    const features = getLicenseFeatures(tier);
 ...
 */
export function getUpgradeDiscount(currentTier, targetTier, remainingMonths) {
