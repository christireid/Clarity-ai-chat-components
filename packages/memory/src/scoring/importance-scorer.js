...
export class ImportanceScorer {
    ...
    constructor(config) {
        this.config = {
            recencyHalfLife: config?.recencyHalfLife ?? DEFAULT_CONFIG.recencyHalfLife,
            maxFrequencyAccesses: config?.maxFrequencyAccesses ?? DEFAULT_CONFIG.maxFrequencyAccesses,
            weights: {
                base: config?.weights?.base ?? DEFAULT_CONFIG.weights.base,
                recency: config?.weights?.recency ?? DEFAULT_CONFIG.weights.recency,
                frequency: config?.weights?.frequency ?? DEFAULT_CONFIG.weights.frequency,
                relevance: config?.weights?.relevance ?? DEFAULT_CONFIG.weights.relevance,
            },
        };
    }
    /**
    ...
    score(memory, query) {
        ...
        const final = base * this.config.weights.base +
    ...
    calculateRecency(memory) {
        ...
        if (!timestamp)
            return 0.5; // Default for memories without timestamp
        const ageMs = Date.now() - new Date(timestamp).getTime();
        if (ageMs < 0 || isNaN(ageMs))
            return 1; // Future or invalid dates get max recency
