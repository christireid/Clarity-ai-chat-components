...
import type { Memory, MemoryType, MemoryScope } from '../core/types'
...
export function MemoryInspector({
  ...
  return (
    <div className={`memory-inspector ${className}`} style={{ fontFamily: 'monospace', fontSize: '12px' }}>
      {showStats && stats && (
        <div className="memory-stats" style={{ marginBottom: '16px', padding: '12px', background: '#f5f5f5', borderRadius: '4px' }}>
          <h3 style={{ margin: '0 0 8px 0', fontSize: '14px', fontWeight: 'bold' }}>Memory Statistics</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
            ...
            <div>
              <strong>Avg Importance:</strong> {stats.averageImportance.toFixed(2)}
            </div>
            <div>
              <strong>Compression:</strong> {(stats.compressionRatio * 100).toFixed(1)}%
            </div>
          </div>
          ...
          <div style={{ marginTop: '12px' }}>
            ...
            <div style={{ marginLeft: '12px', marginTop: '4px' }}>
              {Object.entries(stats.byScope).map(([scope, count]) => (
                <div key={scope}>{scope}: {count as number}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      {showMemories && (
        <div className="memory-list">
          <h3 style={{ margin: '0 0 8px 0', fontSize: '14px', fontWeight: 'bold' }}>Memories ({memories.length})</h3>
          {loading ? (
            <div>Loading...</div>
          ) : memories.length === 0 ? (
            <div style={{ color: '#666', fontStyle: 'italic' }}>No memories found</div>
          ) : (
      ...
      {selectedMemory && (
        ...
        >
          <h4 style={{ margin: '0 0 8px 0', fontSize: '13px', fontWeight: 'bold' }}>Memory Details</h4>
          <div style={{ fontSize: '11px', lineHeight: '1.6' }}>
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              <div style={{ marginTop: '4px', padding: '8px', background: '#fff', borderRadius: '2px' }}>
