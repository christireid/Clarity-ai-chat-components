...
import type { MemoryServiceConfig, MemoryType, MemoryScope } from '../src/types'

// Test metadata types for type safety validation
...
describe('TypedMemoryService - Type Safety', () => {
  let service: TypedMemoryService<TestMetadata>
  ...
  beforeEach(() => {
    ...
    service = new TypedMemoryService<TestMetadata>(config)
  })
  ...
  describe('Type-Safe Memory Addition', () => {
    ...
    it('should handle metadata with different types correctly', async () => {
      const chatService = new TypedMemoryService<ChatMetadata>(config)
      ...
      const memory = await chatService.addMemory(
        'Assistant provided helpful response',
        'episodic',
        'thread',
        chatMetadata
      )
  ...
  describe('Type-Safe Memory Retrieval', () => {
    it('should retrieve memory with correct types', async () => {
      ...
      const addedMemory = await service.addMemory(
        'Test memory for retrieval',
        'episodic',
        'session',
        metadata
      )
  ...
  describe('Performance Optimizations', () => {
    it('should handle large numbers of memories efficiently', async () => {
      ...
      const memoryPromises: Promise<TypedMemoryItem<TestMetadata>>[] = []
  ...
  describe('Error Handling with Type Safety', () => {
    it('should handle empty content with proper error types', async () => {
      await expect(
        service.addMemory('', 'episodic', 'session', {
          userId: 'user123',
          sessionId: 'session456',
          tags: [],
          confidence: 0.5,
    ...
    it('should handle service shutdown gracefully', async () => {
      ...
      await expect(
        service.addMemory('Test after shutdown', 'episodic', 'session', {
          userId: 'user123',
          sessionId: 'session456',
          tags: [],
          confidence: 0.5,
