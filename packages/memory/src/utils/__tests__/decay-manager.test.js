...
import { DecayManager, createDecayManager, DEFAULT_DECAY_CONFIG, } from '../decay-manager';
...
describe('DecayManager', () => {
    ...
    describe('findDecayCandidates', () => {
        ...
        it('should sort by decay score descending', () => {
            ...
            for (let i = 1; i < candidates.length; i++) {
                expect(candidates[i - 1].decayScore).toBeGreaterThanOrEqual(candidates[i].decayScore);
            }
        });
    });
    ...
    describe('setExpiration', () => {
        ...
        it('should set expiration from milliseconds offset', () => {
            ...
            expect(updated.expiresAt).toBeInstanceOf(Date);
            expect(updated.expiresAt.getTime()).toBeGreaterThanOrEqual(before + 60000);
    ...
    describe('DEFAULT_DECAY_CONFIG', () => {
        it('should have sensible defaults', () => {
            ...
            expect(DEFAULT_DECAY_CONFIG.defaultPolicy.ttl).toBe(7 * 24 * 60 * 60 * 1000); // 7 days
        ...
        it('should have type-specific policies', () => {
            expect(DEFAULT_DECAY_CONFIG.byType?.semantic?.ttl).toBe(30 * 24 * 60 * 60 * 1000); // 30 days
            expect(DEFAULT_DECAY_CONFIG.byType?.profile?.enabled).toBe(false);
            expect(DEFAULT_DECAY_CONFIG.byType?.['short-term']?.ttl).toBe(60 * 60 * 1000); // 1 hour
        });
...
//# sourceMappingURL=decay-manager.test.js.map