...
export interface HealthCheckResult {
  ...
  checks: {
    ...
    embedding: { status: 'ok' | 'warning' | 'error'; message: string }
    tokenBudget: { status: 'ok' | 'warning' | 'error'; message: string }
    compression: { status: 'ok' | 'warning' | 'error'; message: string }
  }
 ...
 */
export async function healthCheck(memory: ClarityMemory): Promise<HealthCheckResult> {
