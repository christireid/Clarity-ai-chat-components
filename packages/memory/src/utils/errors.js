/**
 * Clarity Memory - Error Utilities
 *
 * Error handling utilities and helpers.
 */
import { MemoryError, MemoryErrorCodes } from '../types';
/**
 * Create a store error
 */
export function createStoreError(message, cause) {
    return new MemoryError(message, MemoryErrorCodes.STORE_ERROR, cause);
}
/**
 * Create an embedding error
 */
export function createEmbeddingError(message, cause) {
    return new MemoryError(message, MemoryErrorCodes.EMBEDDING_ERROR, cause);
}
/**
 * Create a token budget error
 */
export function createTokenBudgetError(requested, available) {
    return new MemoryError(`Token budget exceeded: requested ${requested}, available ${available}`, MemoryErrorCodes.TOKEN_BUDGET_EXCEEDED);
}
/**
 * Create an invalid config error
 */
export function createInvalidConfigError(message) {
    return new MemoryError(message, MemoryErrorCodes.INVALID_CONFIG);
}
/**
 * Create a memory not found error
 */
export function createMemoryNotFoundError(id) {
    return new MemoryError(`Memory not found: ${id}`, MemoryErrorCodes.MEMORY_NOT_FOUND);
}
/**
 * Check if error is a MemoryError
 */
export function isMemoryError(error) {
    return error instanceof MemoryError;
}
/**
 * Get error message safely
 */
export function getErrorMessage(error) {
    if (error instanceof Error) {
        return error.message;
    }
    if (typeof error === 'string') {
        return error;
    }
    return 'Unknown error';
}
/**
 * Get error code safely
 */
export function getErrorCode(error) {
    if (isMemoryError(error)) {
        return error.code;
    }
    return undefined;
}
//# sourceMappingURL=errors.js.map