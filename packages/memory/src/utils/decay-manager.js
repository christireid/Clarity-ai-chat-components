...
export class DecayManager {
     ...
     */
    calculateDecayScore(age, policy) {
        if (policy.ttl === 0)
            return 0;
     ...
     * ```
     */
    findDecayCandidates(memories, options) {
     ...
     */
    getTimeToExpiry(memory, now = new Date()) {
     ...
     */
    setExpiration(memory, expiresAt) {
        const expiration = typeof expiresAt === 'number'
     ...
     * ```
     */
    getStats(memories, now = new Date()) {
        const stats = {
 ...
 */
export function createDecayManager(config) {
    return new DecayManager(config);
}
//# sourceMappingURL=decay-manager.js.map