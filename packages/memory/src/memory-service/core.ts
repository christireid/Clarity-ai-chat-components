...
import { VectorOperations } from './vector-ops'
import { EpisodicMemoryHandler } from './episodic'
import { SemanticMemoryHandler } from './semantic'
...
export class MemoryService {
  ...
  private episodicHandler: EpisodicMemoryHandler
  private semanticHandler: SemanticMemoryHandler
  ...
  constructor(
    ...
    this.vectorOps = new VectorOperations(
      vectorStore,
      config.persistence.vectorStoreNamespace,
      config.persistence.batchSize || 100,
      config.debug
    )
    this.episodicHandler = new EpisodicMemoryHandler(this.cache)
    this.semanticHandler = new SemanticMemoryHandler(this.cache)
  ...
  async initialize(): Promise<void> {
    ...
    if (this.config.persistence.useVectorStore) {
  ...
  async query(query: MemoryQuery): Promise<MemorySearchResult[]> {
    ...
    if (query.embedding) {
      results = await this.vectorOps.search(query, this.cache)
    } else if (query.query && this.embeddings) {
      // Generate embedding and search
      const embedding = await this.embeddings.embedText(query.query)
      results = await this.vectorOps.search({ ...query, embedding }, this.cache)
    } else {
  ...
  async updateMemory(
    ...
    if (updated.embedding) {
      try {
        await this.vectorOps.update([updated])
      } catch (error) {
  ...
  async deleteMemory(id: string): Promise<boolean> {
    ...
    try {
      await this.vectorOps.delete([id])
    } catch (error) {
  ...
  getMemoryContext(): MemoryContext {
    ...
    return {
      conversationActivity: this.episodicHandler.assessConversationActivity(),
      preferenceRichness: this.semanticHandler.assessPreferenceRichness(),
      taskComplexity: this.semanticHandler.assessTaskComplexity(),
      stats: {
