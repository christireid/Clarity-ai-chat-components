 ...
 */
function createDefaultOptimizer(_config) {
    ...
    return {
        ...
        optimizeContext: (options) => {
            ...
            const systemPrompt = truncate(options.systemPrompt, defaultAllocation.systemPrompt);
            const userPreferences = truncate(JSON.stringify(options.userPreferences), defaultAllocation.userPreferences);
            const recentContext = truncate(options.recentMessages.join('\n'), defaultAllocation.recentContext);
            const semanticMemory = truncate(options.semanticMemories.map((m) => m.content).join('\n'), defaultAllocation.semanticMemory);
            const episodicMemory = truncate(options.episodicMemories.map((m) => m.content).join('\n'), defaultAllocation.episodicMemory);
            const totalTokens = Math.ceil((systemPrompt.length +
...
export class MemoryService {
    ...
    async runDecay() {
        ...
        const candidates = this.decayManager.findDecayCandidates(memories, // Cast to Memory type
        { limit: 100 });
        let deleted = 0;
        let compressed = 0;
        let kept = 0;
        ...
        if (this.config.debug && candidates.length > 0) {
            console.log(`[MemoryService] Decay cycle complete: ` +
    ...
    getDecayStats() {
        ...
        const memories = Array.from(this.cache.values());
        return this.decayManager.getStats(memories);
    }
    /**
     * Run a single summarization cycle
     * Finds old memories and compresses them to save space
     */
    async runSummarizationCycle() {
        const now = new Date();
        const candidatesForSummarization = [];
        ...
        for (const memory of this.cache.values()) {
            ...
            if (isNaN(ageMs) || ageMs < 60 * 60 * 1000)
                continue;
        ...
        for (const memory of batch) {
            try {
                ...
                if (summary.length < memory.content.length * 0.7) {
                    ...
                    if (this.config.debug) {
                        console.log(`Summarized memory ${memory.id}: ${memory.content.length} -> ${summary.length} chars`);
                    }
                }
                else {
                    skippedCount++;
        ...
        if (this.config.debug && (summarizedCount > 0 || batch.length > 0)) {
            console.log(`[MemoryService] Summarization cycle complete: ` +
    ...
    createMemoryFromMatch(match) {
        ...
        return {
            ...
            tokens: TokenCounter.count(metadata['content'] || ''),
            accessCount: 0,
            lastAccessed: new Date(),
            createdAt: new Date(),
            updatedAt: new Date(),
    ...
    async add(content, options) {
        const memory = await this.addMemory(content, options?.type || 'episodic', options?.scope || 'session', {}, {
     ...
     */
    async recall(queryText, options) {
