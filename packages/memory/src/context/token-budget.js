...
export class TokenBudgetManager {
     ...
     */
    adjustAllocation(breakdown, context) {
        ...
        const currentTotal = adjusted.systemPrompt +
        ...
        if (currentTotal > maxTokens) {
            ...
            for (const field of reductionOrder) {
                if (excess <= 0)
                    break;
                const current = safeNonNegative(adjusted[field]);
                if (current >= excess) {
                    ;
                    adjusted[field] = current - excess;
                    excess = 0;
                }
                else {
                    excess -= current;
     ...
     */
    isWithinBudget(breakdown) {
        const total = breakdown.systemPrompt +
