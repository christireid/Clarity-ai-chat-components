...
import { LLMSummarizer, createSummarizerWithFallback, extractiveSummarize, } from '../llm-summarizer';
// Mock global fetch
...
describe('LLMSummarizer', () => {
    ...
    describe('summarizeConversation', () => {
        it('should throw for empty messages array', async () => {
            await expect(summarizer.summarizeConversation([])).rejects.toThrow('Cannot summarize empty conversation');
        });
    ...
    describe('progressiveSummarize', () => {
        it('should throw for empty messages array', async () => {
            await expect(summarizer.progressiveSummarize([])).rejects.toThrow('Cannot progressively summarize empty conversation');
        });
    ...
    describe('hierarchicalSummarize', () => {
        it('should throw for empty content', async () => {
            await expect(summarizer.hierarchicalSummarize('')).rejects.toThrow('Cannot create hierarchical summary of empty content');
        });
        it('should throw for content that is too short', async () => {
            // Content with less than 50 tokens (~200 chars)
            await expect(summarizer.hierarchicalSummarize('Short text.')).rejects.toThrow('Content too short for hierarchical summarization');
        });
    ...
    describe('custom provider', () => {
        it('should use custom handler', async () => {
            ...
            expect(customHandler).toHaveBeenCalledWith(expect.stringContaining('Test'), expect.objectContaining({ maxTokens: 100 }));
    ...
    describe('input validation', () => {
        it('should throw for empty text', async () => {
            await expect(summarizer.summarize('')).rejects.toThrow('Cannot summarize empty text');
        });
        it('should throw for whitespace-only text', async () => {
            await expect(summarizer.summarize('   \n\t  ')).rejects.toThrow('Cannot summarize empty text');
        });
        it('should throw for zero maxTokens', async () => {
            await expect(summarizer.summarize('Some text', 0)).rejects.toThrow('maxTokens must be a positive number');
        });
        it('should throw for negative maxTokens', async () => {
            await expect(summarizer.summarize('Some text', -100)).rejects.toThrow('maxTokens must be a positive number');
        });
        it('should throw for NaN maxTokens', async () => {
            await expect(summarizer.summarize('Some text', NaN)).rejects.toThrow('maxTokens must be a positive number');
        });
    });
    describe('constructor validation', () => {
        it('should throw for OpenAI provider without API key', () => {
            expect(() => new LLMSummarizer({
                provider: 'openai',
    ...
    describe('setCacheTTL validation', () => {
        it('should throw for zero TTL', () => {
            expect(() => summarizer.setCacheTTL(0)).toThrow('Cache TTL must be a positive number');
        });
        it('should throw for negative TTL', () => {
            expect(() => summarizer.setCacheTTL(-1000)).toThrow('Cache TTL must be a positive number');
        });
...
describe('createSummarizerWithFallback', () => {
    ...
    it('should NOT fallback on empty text validation error', async () => {
        ...
        await expect(summarize('', 50)).rejects.toThrow('Cannot summarize empty text');
    ...
    it('should NOT fallback on invalid maxTokens validation error', async () => {
        ...
        await expect(summarize('Some text', 0)).rejects.toThrow('maxTokens must be a positive number');
        expect(isLLMAvailable()).toBe(true);
    });
});
...
//# sourceMappingURL=llm-summarizer.test.js.map