            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 529 ||
                        response.status === 503 ||
                        response.status === 502) {
                        lastError = new Error(`Anthropic API temporarily unavailable: ${response.status}`);
                ...
                if (!data || typeof data !== 'object') {
                    throw new Error(`Invalid Anthropic response: expected object, got ${typeof data}`);
                }
                if (!Array.isArray(data.content) || data.content.length === 0) {
                    throw new Error(`Invalid Anthropic response: missing or empty content array. Response: ${JSON.stringify(data).slice(0, 200)}`);
                }
                ...
                if (!content ||
                    ...
                    throw new Error(`Invalid Anthropic response: expected text content, got ${JSON.stringify(content).slice(0, 200)}`);
                }
                return content.text;
            }
            catch (error) {
...
//# sourceMappingURL=anthropic-summarizer.js.map