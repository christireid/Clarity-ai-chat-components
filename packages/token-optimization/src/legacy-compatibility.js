...
export class TokenBudgetManager {
    config; // TokenOptimizationConfig from memory package
    allocation; // TokenAllocation from memory package
    constructor(config) {
        this.config = config;
    ...
    adjustAllocation(context) {
        ...
        return this.calculateBudgetsFromAllocation(this.config.maxContextWindow, adjusted);
    }
    /**
     * Calculate token budgets from allocation percentages
     */
    calculateBudgetsFromAllocation(maxTokens, alloc) {
        return {
     ...
     */
    optimizeMemories(memories, budget, priorities = {}) {
...
export class MemoryCompressor {
     ...
     */
    compressMemory(memory, targetRatio = 0.5) {
...
export class SemanticChunker {
    chunkSize;
    constructor(chunkSize = 200, _overlap = 50) {
     ...
     */
    chunkConversation(conversation) {
     ...
     */
    retrieveOptimalChunks(chunks, budget, relevanceScores) {
...
export class ContextOptimizer {
    ...
    constructor(config) {
        ...
        this.chunker = new SemanticChunker(config.chunkSize || 200, config.chunkOverlap || 50);
    }
    /**
    ...
    optimizeContext(options) {
        ...
        const recentContext = this.compressor.compressConversation(options.recentMessages, allocation.recentContext);
        const optimizedSemantic = this.budgetManager.optimizeMemories(options.semanticMemories, allocation.semanticMemory);
        const optimizedEpisodic = this.budgetManager.optimizeMemories(options.episodicMemories, allocation.episodicMemory);
        const semanticMemory = optimizedSemantic.map(m => m.content).join('\n');
        const episodicMemory = optimizedEpisodic.map(m => m.content).join('\n');
        const totalTokens = TokenCounter.count(systemPrompt) +
        ...
        return {
            ...
            stats: {
                totalTokens,
                allocation,
                compressionRatio: ((userPreferences.compressionRatio + recentContext.compressionRatio) / 2),
            },
        };
    }
    /**
...
//# sourceMappingURL=legacy-compatibility.js.map