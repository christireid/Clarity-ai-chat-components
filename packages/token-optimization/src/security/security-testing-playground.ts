...
export class SecurityTestingPlayground {
  ...
  async runRateLimitingTest(): Promise<void> {
    ...
    const testContext = { userId: 'rate-test-user', sessionId: 'rate-test-session', riskScore: 0.1, timestamp: new Date(), requestId: 'rate-req', trustLevel: 'medium' as const }
  ...
  async runQuarantineTest(): Promise<void> {
    ...
    const highRiskContext = { userId: 'quarantine-test-user', sessionId: 'quarantine-test-session', riskScore: 0.95, timestamp: new Date(), requestId: 'quarantine-req', trustLevel: 'low' as const }
...
export function createSecurityTestingPlayground(securityManager: EnhancedSecurityManager): SecurityTestingPlayground {
  return new SecurityTestingPlayground(securityManager)
}

// Quick test runner for CI/CD integration
export async function runSecurityTests(securityManager: EnhancedSecurityManager): Promise<boolean> {
