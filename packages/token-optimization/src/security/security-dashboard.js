...
export class SecurityDashboard extends EventEmitter {
    ...
    getRealTimeStats() {
        ...
        const recentEvents = this.metricsHistory.filter(event => event.timestamp > lastMinute);
        ...
        const topThreatTypes = Array.from(threatTypeCounts.entries())
            ...
            .slice(0, 5);
        // Active sessions
        const activeSessions = new Set(recentEvents.map(e => e.sessionId).filter((id) => !!id));
    ...
    updateDashboard() {
        ...
        // Update active users
        const recentEvents = this.metricsHistory.filter(event => Date.now() - event.timestamp.getTime() < 300000 // Last 5 minutes
        );
    ...
    cleanupOldData() {
        ...
        this.metricsHistory = this.metricsHistory.filter(event => event.timestamp > cutoff);
        // Limit data points
    ...
    calculateComplianceScore() {
        ...
        if (recentEvents.length === 0)
            return 1.0;
        const compliantEvents = recentEvents.filter(e => e.complianceChecks.length > 0 && e.riskLevel !== 'high').length;
