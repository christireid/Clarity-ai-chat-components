...
import { AccurateTokenCounter, ChatMessage, } from '../tokenizers/accurate-counter';
...
describe('QA Battle Tests', () => {
    describe('AccurateTokenCounter Stress Tests', () => {
        ...
        it('should handle very long text (100KB+)', () => {
            ...
            const longText = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. '.repeat(2000);
        ...
        it('should handle high-volume batch counting (1000 items)', () => {
            const texts = Array(1000).fill('This is a test message for batch processing.');
            const start = performance.now();
        ...
        it('should handle cache pressure gracefully', () => {
            ...
            const uniqueTexts = Array(1500)
                .fill(null)
                .map((_, i) => `Unique text number ${i} with some additional content to make it different.`);
        ...
        it('should handle unicode and emoji-heavy text', () => {
            const unicodeText = '你好世界 🌍🎉🚀 مرحبا بالعالم こんにちは世界 '.repeat(100);
    ...
    describe('TextChunker Stress Tests', () => {
        ...
        it('should handle all strategy presets with large documents', () => {
            ...
            expect(preciseResult.chunks.length).toBeGreaterThan(balancedResult.chunks.length);
            expect(balancedResult.chunks.length).toBeGreaterThan(contextResult.chunks.length);
        });
        it('should maintain chunk integrity with overlap', () => {
            ...
            const text = 'Word number one. Word number two. Word number three. '.repeat(100);
...
More paragraphs...
      `.repeat(100);
        ...
        it('should correctly detect chunking needs', () => {
            ...
            const longText = 'This is a very long sentence with many words that we need to make sure exceeds the fifty token limit by quite a significant margin. '.repeat(5);
