/**
 * TOON (Token-Oriented Object Notation) Core Algorithm
 *
 ...
 *
 * @module toon-optimizer/core
 * @description Core parser and encoder for the TOON format,
...
import {
  ...
  isPrimitiveArray,
  estimateSavings as estimateSavingsImpl,
  optimizeDataStructure as optimizeDataStructureImpl,
  calculateSavings as calculateSavingsImpl,
} from './strategies'
import type { SavingsEstimate, SavingsInfo } from './strategies'
import { validateAgainstSchema } from './validators'
import type { TOONSchema, ValidationResult } from './validators'
...
export class ToonOptimizer {
  ...
  static calculateSavings(json: string, toon: string): SavingsInfo {
    return calculateSavingsImpl(json, toon)
  }
  ...
  convertToToon(data: unknown, indent: number = 0): string {
    ...
    for (const [key, value] of Object.entries(
      ...
      if (Array.isArray(value)) {
        if (shouldUseTableFormat(value, this.config)) {
  ...
  estimateSavings(data: unknown): SavingsEstimate {
    ...
    return estimateSavingsImpl(json, toon)
  }
  ...
  optimizeDataStructure(data: unknown): unknown {
    return optimizeDataStructureImpl(data)
  }
  ...
  private convertArrayToToon(arr: unknown[], indent: number): string {
    if (shouldUseTableFormat(arr, this.config)) {
