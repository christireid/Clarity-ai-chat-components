...
export class ToonOptimizer {
     ...
     */
    convertToTable(name, arr, indent) {
        if (arr.length === 0) {
     ...
     */
    shouldUseTableFormat(arr) {
        if (!this.config.enableArrayTables)
            return false;
        if (arr.length === 0)
            return true;
        if (arr.length > this.config.maxArraySizeForTable)
            return false;
     ...
     */
    parseToonLines(lines, startIndent) {
        const result = {};
        let i = 0;
     ...
     * Parse array
     */
    parseArray(lines, startIndex, baseIndent) {
        const result = [];
        let i = startIndex;
        while (i < lines.length) {
            const line = lines[i];
...
//# sourceMappingURL=toon-optimizer.js.map