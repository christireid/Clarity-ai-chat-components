...
import { createTestRunner, getDeterministicScenarios, getReplayScenarios, } from './harness';
...
async function main() {
    ...
    let scenarioIds = flags.scenarios;
    if (!scenarioIds) {
        // Default: run all deterministic + replay scenarios (skip adversarial for speed)
        scenarioIds = [...getDeterministicScenarios(), ...getReplayScenarios()].map((s) => s.id);
    }
    console.log(`📋 Scenarios to run: ${scenarioIds.length}`);
    console.log(`   Categories: ${[...new Set(scenarioIds.map((id) => id.split('-')[0]))].join(', ')}`);
    console.log();
    ...
    if (flags.baselineOnly) {
        ...
        console.log(`Mean estimated input tokens: ${baseline.tokens.meanEstimatedInput.toFixed(1)}`);
        console.log(`Median estimated input tokens: ${baseline.tokens.medianEstimatedInput.toFixed(1)}`);
        console.log(`P90 estimated input tokens: ${baseline.tokens.p90EstimatedInput.toFixed(1)}`);
        if (baseline.tokens.meanActualInput !== undefined) {
            console.log(`\nMean actual input tokens: ${baseline.tokens.meanActualInput.toFixed(1)}`);
            console.log(`Estimation error: ${baseline.tokens.estimationErrorPercent?.toFixed(1)}%`);
        }
    }
    else if (flags.optimizedOnly) {
        console.log('🔬 Running OPTIMIZED only...');
        const optimized = await runner.runOptimized(optimizationConfig, scenarioIds, 1);
        console.log('\n--- OPTIMIZED RESULTS ---');
        console.log(`Total measurements: ${optimized.totalMeasurements}`);
        console.log(`Successful: ${optimized.successfulMeasurements}`);
        console.log(`Mean estimated input tokens: ${optimized.tokens.meanEstimatedInput.toFixed(1)}`);
        console.log(`Estimated savings: ${optimized.optimization.totalEstimatedSaved} tokens (${optimized.optimization.reductionPercentEstimated.toFixed(1)}%)`);
        if (optimized.optimization.totalActualSaved !== undefined) {
            console.log(`Actual savings: ${optimized.optimization.totalActualSaved} tokens (${optimized.optimization.reductionPercentActual?.toFixed(1)}%)`);
        }
    }
    else {
    ...
    if (comparison) {
        ...
        if (comparison.verdict.isEffective) {
            ...
            console.log(`   Estimated reduction: ${comparison.tokenDelta.percentReductionEstimated.toFixed(1)}%`);
            if (comparison.tokenDelta.percentReductionActual !== undefined) {
                console.log(`   Actual reduction: ${comparison.tokenDelta.percentReductionActual.toFixed(1)}%`);
            }
        }
        else {
