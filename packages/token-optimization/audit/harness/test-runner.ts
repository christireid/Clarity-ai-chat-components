 ...
 */
function truncateAssistantHistory(
  text: string,
  maxLength: number,
  ...
  for (const sentence of sentences) {
    if (result.length + sentence.length > maxLength * 0.8) break
...
function applyOptimizations(
  ...
  // 1. SYSTEM PROMPT COMPRESSION - NEW! (applies first for max impact)
  {
    ...
    optimizedMessages = optimizedMessages.map((msg) => {
      if (msg.role === 'system' && msg.content.length > 50) {
        const compressed = compressSystemPrompt(
          msg.content,
    ...
    if (systemCompressed) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeSystem.totalEstimatedTokens - afterSystem.totalEstimatedTokens,
        savingsPercent:
  ...
  if (isAggressive || isBalanced) {
    ...
    const lastUserIndex = optimizedMessages.reduce(
    ...
    optimizedMessages = optimizedMessages.map((msg, i) => {
      ...
      if (
        ...
        const truncated = truncateAssistantHistory(
          msg.content,
          maxAssistantLength,
          isBalanced
        )
    ...
    if (assistantTruncated) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeTrunc.totalEstimatedTokens - afterTrunc.totalEstimatedTokens,
        savingsPercent:
  ...
  // 5. URL SHORTENING - NEW!
  {
    ...
    if (urlsShortened) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeUrls.totalEstimatedTokens - afterUrls.totalEstimatedTokens,
        savingsPercent:
          ((beforeUrls.totalEstimatedTokens - afterUrls.totalEstimatedTokens) /
  ...
  if (isAggressive || isBalanced) {
    ...
    if (markdownStripped) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeMd.totalEstimatedTokens - afterMd.totalEstimatedTokens,
        savingsPercent:
          ((beforeMd.totalEstimatedTokens - afterMd.totalEstimatedTokens) /
  ...
  if (isAggressive) {
    ...
    if (codeExtracted) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeCode.totalEstimatedTokens - afterCode.totalEstimatedTokens,
        savingsPercent:
          ((beforeCode.totalEstimatedTokens - afterCode.totalEstimatedTokens) /
  ...
  if (config.enableToon) {
    ...
    if (toonApplied) {
      ...
      techniques.push({
        ...
        tokensSaved:
          beforeToon.totalEstimatedTokens - afterToon.totalEstimatedTokens,
        savingsPercent:
          ((beforeToon.totalEstimatedTokens - afterToon.totalEstimatedTokens) /
