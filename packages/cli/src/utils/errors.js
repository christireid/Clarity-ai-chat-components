...
export function handleError(error) {
    ...
    if (normalizedError instanceof CLIError) {
        if (!isJsonMode) {
            console.error('\n');
            const errorBox = boxen(pc.bold(pc.red(normalizedError.message)) +
    ...
    if (normalizedError instanceof Error) {
        if (!isJsonMode) {
            console.error('\n');
            const errorBox = boxen(pc.bold(pc.red('Unexpected Error:')) +
                '\n\n' +
    ...
    if (!isJsonMode) {
        console.error('\n');
        console.error(boxen(pc.bold(pc.red('Unknown Error')), {
            padding: 1,
            margin: 1,
            borderStyle: 'round',
            borderColor: 'red',
