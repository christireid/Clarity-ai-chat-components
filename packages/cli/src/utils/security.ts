...
export function validatePath(userPath: string, baseDir: string = process.cwd()): string {
  ...
  if (!resolvedPath.startsWith(resolvedBase)) {
    throw new ValidationError(
      `Path outside of project directory: ${userPath}`,
      ['Use relative paths within your project', 'Check for directory traversal attempts']
    )
  }
...
export async function validateProjectPath(projectPath: string): Promise<string> {
  ...
  if (!await fs.pathExists(validatedPath)) {
    throw new ValidationError(
      ...
      ['Check if the path is correct', 'Create the directory if needed']
    )
  }
 ...
 */
export async function checkFilePermissions(filePath: string, mode: 'read' | 'write'): Promise<void> {
  const fs = await import('fs-extra')
  ...
  try {
    ...
    throw new PermissionError(
      `No ${mode} permission for: ${filePath}`,
      [
 ...
 */
export async function ensureEnvInGitignore(cwd: string = process.cwd()): Promise<void> {
  const fs = await import('fs-extra')
 ...
 */
export function validateApiKeyFormat(key: string, provider: 'openai' | 'anthropic' | 'google'): boolean {
