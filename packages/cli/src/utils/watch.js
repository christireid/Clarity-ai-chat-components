...
export function watchFiles(options) {
    const { cwd = process.cwd(), patterns = ['**/*.ts', '**/*.tsx', '**/*.js', '**/*.jsx'], ignore = ['node_modules/**', 'dist/**', '.git/**'], debounce = 300, onChange, } = options;
    let timeout = null;
    const watchedFiles = new Set();
    const debouncedOnChange = (file) => {
        if (timeout) {
 ...
 */
export function watchAndRebuild(buildFn, options = {}) {
    return watchFiles({
        ...options,
        onChange: async () => {
