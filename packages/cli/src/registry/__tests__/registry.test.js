...
import { COMPONENT_REGISTRY, GENERATOR_REGISTRY, TEMPLATE_REGISTRY, getComponent, getGenerator, getTemplate, getComponentsByCategory, getCategories, searchComponents, getComponentSlugs, getGeneratorSlugs, resolveComponentDependencies, } from '../index.js';
    ...
    it('should have valid registryDependencies', () => {
        Object.values(COMPONENT_REGISTRY).forEach((component) => {
            component.registryDependencies.forEach((dep) => {
                expect(COMPONENT_REGISTRY[dep], `${component.slug} has invalid dependency: ${dep}`).toBeDefined();
            });
        });
    });
