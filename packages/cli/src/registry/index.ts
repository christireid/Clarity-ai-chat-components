...
export const COMPONENT_REGISTRY: Record<string, ComponentConfig> = {
  'chat-interface': {
    ...
    slug: 'chat-interface',
    description:
      'Full-featured chat UI with message list, input, and streaming support',
    icon: '💬',
    category: 'chat',
    files: [
      'ChatInterface.tsx',
...
/>`,
  ...
  'chat-input': {
    ...
    slug: 'chat-input',
    description:
      'Rich text input with attachments, mentions, and keyboard shortcuts',
    icon: '⌨️',
    category: 'chat',
...
export const GENERATOR_REGISTRY: Record<string, GeneratorConfig> = {
  component: {
    ...
    icon: '⚛️',
    description:
      'Create a React component with TypeScript, tests, and Storybook story',
 ...
 */
export function getGenerator(slug: string): GeneratorConfig | undefined {
 ...
 */
export function getGeneratorSlugs(): string[] {
 ...
 */
export function resolveComponentDependencies(slug: string): string[] {
  const component = COMPONENT_REGISTRY[slug]
  if (!component) return []

  const deps = new Set<string>([slug])
