...
export function box(content, options = {}) {
    const { title = '', titleAlign = 'left', padding = 1, margin = 0, borderStyle = 'rounded', borderColor = chalk.cyan, width, } = options;
...
export function table(data, columns) {
    ...
    // Top border
    output.push(chalk.gray('┌' + widths.map(w => '─'.repeat(w)).join('┬') + '┐'));
    // Header
