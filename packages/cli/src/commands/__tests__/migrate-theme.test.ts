...
function detectLegacyPreset(
  line: string
): { legacy: string; modern: string } | null {
 ...
 */
function migrateCSSVariable(line: string): string {
 ...
 */
function migrateImportPath(line: string): string {
  return line.replace(
...
describe('Theme Migration Utilities', () => {
  ...
  describe('migrateCSSVariable', () => {
    ...
    it('should migrate multiple variables in one line', () => {
      const input =
        'background: var(--clarity-chat-background); color: var(--clarity-chat-foreground);'
      const output = migrateCSSVariable(input)
      expect(output).toBe(
        'background: var(--clarity-background); color: var(--clarity-foreground);'
      )
    })
  ...
  describe('migrateImportPath', () => {
    ...
    it('should handle different import styles', () => {
      ...
      const output = migrateImportPath(input)
      expect(output).toBe(
        "import type { ThemeConfig } from '@clarity-chat/react'"
      )
    })
  })
  ...
  describe('Breaking Change Detection', () => {
    it('should detect baseTheme usage', () => {
      ...
      const hasBreaking =
        line.includes('createTheme(') && line.includes('baseTheme:')
