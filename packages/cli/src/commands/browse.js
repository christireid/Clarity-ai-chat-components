...
export async function browseCommand() {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        data.components.forEach((component, idx) => {
            ...
            // Quick actions
            console.log(pc.gray(`     💾 clarity-chat add ${component.name.toLowerCase()}`) +
                pc.dim(' | ') +
                pc.gray(`📖 docs`));
            console.log();
        });
...
export async function selectComponent() {
    ...
    allComponents.forEach((comp, idx) => {
        console.log(pc.gray(`  ${(idx + 1).toString().padStart(2)}.`) +
            ' ' +
...
export async function showComponentDetails(componentName) {
    ...
    console.log(pc.gray('  1. ') +
        pc.white('Install now') +
        pc.gray(' - Add to your project'));
    console.log(pc.gray('  2. ') + pc.white('View docs') + pc.gray(' - Open documentation'));
    console.log(pc.gray('  3. ') + pc.white('Live preview') + pc.gray(' - See it in action'));
    console.log();
    // Prompt for action
    console.log(pc.yellow('💡 Press Enter to install, D for docs, or Q to go back'));
    console.log();
}
/**
...
export async function searchComponents(query) {
    ...
    if (results.length === 0) {
        ...
        console.log(pc.gray('  Try browsing all components: ') +
            pc.cyan('clarity-chat browse'));
        console.log();
        return;
    }
    results.forEach((result, idx) => {
        console.log(pc.bold(pc.yellow(`  ${idx + 1}. ${result.name}`)) +
            pc.gray(` (${result.category})`));
        console.log(pc.gray(`     ${result.description}`));
        console.log(pc.cyan(`     $ clarity-chat add ${result.name.toLowerCase()}`) +
            pc.dim(' | ') +
