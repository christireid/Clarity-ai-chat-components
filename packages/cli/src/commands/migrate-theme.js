 ...
 */
async function scanForLegacyThemes(cwd) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            // Legacy preset names
            Object.keys(LEGACY_PRESETS).some((preset) => content.includes(`preset: '${preset}'`) ||
        ...
        spinner.succeed(`Found ${filesToMigrate.length} file(s) with legacy theme usage`);
    ...
    lines.forEach((line, index) => {
        ...
        if (line.includes("from '@clarity-chat/react/theme'")) {
            changes.push({
                ...
                original: line.trim(),
                migrated: line
                    .replace("from '@clarity-chat/react/theme'", "from '@clarity-chat/react'")
                    .trim(),
        ...
        if (line.includes('createTheme(') && line.includes('baseTheme:')) {
            ...
            changes.push({
                ...
                description: 'Breaking: Replace baseTheme with extends (manual review needed)',
            });
        }
 ...
 */
async function migrateFile(cwd, file, result, dryRun) {
    if (dryRun)
        return;
    const fullPath = path.join(cwd, file);
...
function displayAnalysis(results) {
    ...
    if (breakingFiles > 0) {
        console.log(`  ⚠️  Files with breaking changes: ${pc.yellow(breakingFiles)}`);
    }
    console.log();
    // Show details per file
    for (const result of results) {
        console.log(pc.bold(`  ${result.hasBreakingChanges ? '⚠️' : '📄'} ${result.file}`));
...
function displayMigrationGuide() {
    ...
    console.log(pc.red(`  import { ThemeProvider } from '@clarity-chat/react/theme'`));
    ...
    console.log(pc.green(`  <ThemeProvider defaultTheme={{ preset: 'vibrant' }}>`));
    console.log();
    console.log(pc.bold('  Creating Custom Themes:'));
    console.log();
    console.log(pc.gray('  // Before (legacy)'));
    console.log(pc.red(`  const theme = createTheme({ baseTheme: 'light', ... })`));
    console.log();
    console.log(pc.gray('  // After (modern)'));
...
export async function migrateThemeCommand(options) {
    try {
        ...
        if (filesToMigrate.length === 0) {
            console.log();
            console.log(successMessage('No legacy theme usage found!', {
                title: '✨ Up to date',
                borderColor: 'green',
            }));
            console.log();
            console.log(pc.gray('Your project is already using the modern theme system.'));
            return;
        }
        // Analyze each file
        ...
        if (results.length === 0) {
            console.log();
            console.log(successMessage('No migrations needed!', {
                title: '✨ Complete',
                borderColor: 'green',
            }));
            return;
        ...
        if (options.dryRun) {
            console.log();
            console.log(infoMessage('Dry run complete - no files modified', {
                title: '📝 Dry Run',
                borderColor: 'blue',
            }));
        ...
        if (hasBreaking) {
            console.log();
            console.log(warningMessage('Some files have breaking changes that require manual review.', {
        ...
        // Final summary
        console.log();
        console.log(successMessage('Theme migration complete!', {
            title: '✨ Complete',
            borderColor: 'green',
        }));
        if (hasBreaking) {
...
//# sourceMappingURL=migrate-theme.js.map