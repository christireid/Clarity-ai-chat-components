...
async function analyzeProject() {
    ...
    try {
        ...
        if (result.components.length === 0 && result.hooks.length === 0) {
            result.recommendations.push('No Clarity Chat components found. Run `clarity-chat add` to get started.');
        }
        else {
            if (result.components.length < 3) {
                result.recommendations.push('Consider exploring more components to enhance your UI.');
            }
            // Check for common patterns
            const hasChat = result.components.some(c => c.name.includes('Chat'));
            if (!hasChat) {
                result.recommendations.push('Add ChatWindow or ChatInterface for a complete chat experience.');
            }
            const hasModelSelector = result.components.some(c => c.name.includes('ModelSelector'));
            if (!hasModelSelector && hasChat) {
                result.recommendations.push('Add ModelSelector to let users choose AI models.');
            }
        }
...
function displayResults(result) {
    ...
    if (result.components.length > 0) {
        ...
        result.components.slice(0, 10).forEach((component, index) => {
            console.log(`  ${index + 1}. ${chalk.yellow(component.name)} - used in ${chalk.cyan(component.importCount)} file(s)`);
        });
    }
    // Top hooks
    if (result.hooks.length > 0) {
        ...
        result.hooks.slice(0, 5).forEach((hook, index) => {
            console.log(`  ${index + 1}. ${chalk.yellow(hook.name)} - used in ${chalk.cyan(hook.importCount)} file(s)`);
        });
    }
