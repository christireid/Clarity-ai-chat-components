...
import { showFilesCreated, showNextSteps, promptText, promptSelect, promptConfirm, runTasks, setupCancelHandler, onCleanup, clearCleanup, log, } from '../utils/prompts.js';
...
const GENERATORS = {
    ...
    'chat-component': {
        name: 'Chat Component',
        icon: '💬',
        description: 'Create an AI chat component with streaming and memory support',
...
// Main Command
// ============================================================================
export async function generateCommand(type, options) {
    ...
    if (!generator) {
        ...
        console.log();
        console.log(pc.gray('Example: clarity-chat generate component --name ChatMessage'));
        console.log();
        process.exit(1);
    }
    // Show header
    ...
    const context = {
        ...
        description: finalOptions.description || `A ${generator.name.toLowerCase()}`,
    ...
    try {
        // Generate files
        await runTasks(files.map((file, index) => ({
        ...
        // Success output
        console.log();
        showFilesCreated(files.map((f) => path.join(outputPath, f.name)), cwd);
        // Next steps
        let importPath;
        switch (type) {
            case 'hook':
        ...
        console.log();
        console.log(errorBox('Failed to generate files. Check the error above.', '✗ Error'));
        console.log();
        process.exit(1);
    }
}
