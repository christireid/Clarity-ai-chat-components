 ...
 */
function validateHSL(hslString, tokenName) {
 ...
 */
function validateThemeStructure(content) {
    const issues = [];
 ...
 */
async function scanAndValidateThemes(cwd) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            const hasThemeCode = content.includes('ThemeProvider') ||
            ...
            const colorMatches = content.matchAll(/(\w+):\s*['"](\d{1,3}\s+\d{1,3}%\s+\d{1,3}%)['"]/g);
            const colors = {};
...
function displayResults(results) {
    ...
    console.log(`  📊 Average score: ${avgScore >= 80 ? pc.green(avgScore) : avgScore >= 60 ? pc.yellow(avgScore) : pc.red(avgScore)}/100`);
    console.log();
    // Show details per file
    for (const result of results) {
        const scoreColor = result.score >= 80 ? pc.green : result.score >= 60 ? pc.yellow : pc.red;
        console.log(pc.bold(`  ${result.isValid ? '✅' : '❌'} ${result.file} (Score: ${scoreColor(result.score)})`));
...
export async function validateThemeCommand(options) {
    try {
        ...
        if (results.length === 0) {
            console.log();
            console.log(successMessage('No theme configurations found to validate!', {
                title: '✨ Complete',
                borderColor: 'green',
            }));
            return;
        ...
        if (options.strict && hasErrors) {
            console.log();
            console.log(errorMessage('Validation failed in strict mode', {
                title: '❌ Errors Found',
                borderColor: 'red',
            }));
            process.exit(1);
        ...
        if (!hasErrors) {
            console.log();
            console.log(successMessage('All theme configurations are valid!', {
                title: '✨ Passed',
                borderColor: 'green',
            }));
        }
...
//# sourceMappingURL=validate-theme.js.map