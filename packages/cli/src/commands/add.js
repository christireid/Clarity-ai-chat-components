...
import { COMPONENT_REGISTRY, getComponent, resolveComponentDependencies, } from '../registry/index.js';
        ...
        if (await fs.pathExists(templatesDir)) {
            for (const file of componentConfig.files) {
                ...
                if ((await fs.pathExists(destPath)) && !options.force) {
                    spinner.warn(`Skipping ${file} (already exists, use --force to overwrite)`);
                    continue;
                }
        ...
        if (options.deps !== false && allDependencies.length > 0) {
            ...
            try {
                ...
                if (componentConfig.devDependencies.length > 0) {
                    ...
                    await execa(packageManager, [installCmd, devFlag, ...componentConfig.devDependencies], { cwd });
                }
