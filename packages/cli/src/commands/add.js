...
export async function addCommand(component, options) {
    ...
    console.log(infoBox(`${componentConfig.icon} ${componentConfig.name}\n${componentConfig.description}`, 'Component Info'));
    console.log();
    ...
    try {
        ...
        if (await fs.pathExists(templatesDir)) {
            ...
            await fs.writeFile(path.join(targetPath, placeholderFile), `// ${componentConfig.name}\n// TODO: Implement component\n\nexport function ${componentConfig.name.replace(/\s+/g, '')}() {\n  return <div>Component placeholder</div>\n}\n`, 'utf-8');
