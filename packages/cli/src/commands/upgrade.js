...
function displayUpdates(updates) {
    ...
    if (majorUpdates.length > 0) {
        ...
        majorUpdates.forEach(update => {
            console.log(`  ${chalk.yellow(update.name)}: ${chalk.gray(update.current)} → ${chalk.green(update.latest)}`);
        });
        console.log();
    }
...
async function showChangelog(packageName, version) {
    ...
    try {
        ...
        const result = execSync(`npm view ${packageName} homepage`, { encoding: 'utf8', stdio: 'pipe' });
