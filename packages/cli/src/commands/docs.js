...
export async function docsCommand(query, options) {
    console.log();
    console.log(sectionHeader('📚 Clarity Chat Documentation'));
    console.log();
    if (options?.offline) {
        console.log(warningBox('Offline documentation not yet available\n\nVisit: https://clarity-chat.dev', '⚠ Offline Mode'));
        console.log();
        return;
    }
    ...
    if (query) {
        ...
        const matchedKey = Object.keys(DOCS_URLS).find(key => key.toLowerCase().includes(lowerQuery) || lowerQuery.includes(key));
        if (matchedKey) {
            url = DOCS_URLS[matchedKey];
    ...
    try {
        ...
        console.log();
        console.log(warningBox(`Failed to open browser automatically.\n\nManually visit: ${chalk.cyan(url)}`, '⚠ Browser Error'));
        console.log();
    }
}
