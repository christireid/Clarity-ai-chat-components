...
async function addKey(provider, envPath) {
    // Validate provider
    const validatedProvider = validate(ProviderSchema, provider, 'Invalid provider');
    const providerConfig = PROVIDERS[validatedProvider];
    if (!providerConfig) {
    ...
    if (!process.argv.includes('--json') && !process.argv.includes('--quiet')) {
        console.log();
        console.log(infoMessage(`Get your API key: ${pc.underline(pc.cyan(providerConfig.docs))}`, {
            title: '📚 Documentation',
            borderColor: 'blue',
        }));
        console.log();
    ...
    const { apiKey } = await prompts({
        ...
        validate: (value) => {
            ...
            if (!validateApiKeyFormat(value, validatedProvider)) {
...
async function listKeys(envPath) {
    if (!(await fs.pathExists(envPath))) {
    ...
    if (!process.argv.includes('--quiet')) {
        ...
        const keyList = Object.entries(PROVIDERS).map(([key, config]) => {
            ...
            return {
                ...
                value: keyInfo.configured && keyInfo.masked
                    ? keyInfo.masked
                    : 'Not configured',
        ...
        const listItems = keyList.map((item) => {
            const colorFn = item.color && typeof pc[item.color] === 'function'
                ? pc[item.color]
                : undefined;
            return {
                label: item.key,
...
async function removeKey(provider, envPath) {
...
async function validateKeys(envPath) {
    if (!(await fs.pathExists(envPath))) {
        console.log(pc.yellow('No .env.local file found'));
        return;
    }
    ...
    for (const [key, config] of Object.entries(PROVIDERS)) {
        ...
        try {
            ...
            if (response.ok || response.status === 200) {
                ...
                spinner.fail(pc.red(`${config.icon} ${config.name}: Invalid (${response.status})`));
            }
        }
        catch (error) {
