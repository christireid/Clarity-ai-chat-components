...
import { maskSensitive, validateApiKeyFormat, ensureEnvInGitignore } from '../utils/security.js';
import { success, info, warn, outputJson } from '../utils/output.js';
...
async function listKeys(envPath) {
    ...
    if (!process.argv.includes('--quiet')) {
        ...
        const listItems = keyList.map(item => {
            const colorFn = item.color && typeof chalk[item.color] === 'function'
                ? chalk[item.color]
                : undefined;
            return {
                label: item.key,
