...
function createConfigManager(schema) {
    return {
        validate: (_options) => ({
            success: true,
            errors: [],
...
export async function initCommand(rawOptions) {
    try {
        ...
        try {
            ...
            console.warn('Failed to save config file', error instanceof Error ? error : String(error));
        }
        spinner.succeed('Components installed');
        // Install dependencies
        if (config.installDeps) {
            spinner.text = pc.cyan(`Installing dependencies with ${packageManager}...`);
            try {
...
`, 'utf-8');
        ...
        if (!process.argv.includes('--json') && !process.argv.includes('--quiet')) {
            ...
            console.log();
            console.log(pc.gray('💡 Need help? Run: ') + pc.bold(pc.cyan('clarity-chat docs')));
        }
        else {
...
async function generateConfigFiles(cwd, config) {
