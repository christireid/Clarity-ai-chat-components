...
describe('LicenseLoadingSkeleton', () => {
  ...
  it('should accept custom dimensions', () => {
    render(<LicenseLoadingSkeleton width={200} height={50} />)
    const skeleton = screen.getByRole('status')
    expect(skeleton).toHaveStyle({ width: '200px', height: '50px' })
  })

  it('should accept string dimensions', () => {
    render(<LicenseLoadingSkeleton width="50%" height="2rem" />)
    const skeleton = screen.getByRole('status')
  ...
  it('should apply custom className', () => {
    render(<LicenseLoadingSkeleton className="custom-class" />)
    const skeleton = screen.getByRole('status')
    expect(skeleton).toHaveClass('custom-class')
  })

  it('should disable animation when animate=false', () => {
    render(<LicenseLoadingSkeleton animate={false} />)
    const skeleton = screen.getByRole('status')
...
describe('LicenseStatusBadge', () => {
  ...
  it('should show only icon in icon variant', async () => {
    render(<LicenseStatusBadge variant="icon" />)
    await waitFor(() => {
  ...
  it('should show plan in full variant', async () => {
    ...
    render(<LicenseStatusBadge variant="full" />)
    await waitFor(() => {
  ...
  it('should call onClick when clicked', async () => {
    ...
    render(<LicenseStatusBadge onClick={handleClick} />)
  ...
  it('should show different sizes', async () => {
    const { rerender } = render(<LicenseStatusBadge size="sm" />)
    await waitFor(() => {
      expect(screen.getByRole('button')).toHaveStyle({ fontSize: '11px' })
    })

    rerender(<LicenseStatusBadge size="lg" />)
    await waitFor(() => {
...
describe('LicenseExpiryWarning', () => {
  ...
  it('should not render when no license is set', () => {
    const { container } = render(<LicenseExpiryWarning />)
  ...
  it('should not render when license is not expiring soon', async () => {
    ...
    const { container } = render(<LicenseExpiryWarning daysThreshold={30} />)
    // Wait for hydration
    await waitFor(() => {
      expect(container.innerHTML).toBe('')
    })
  ...
  it('should render when license is expiring within threshold', async () => {
    ...
    render(<LicenseExpiryWarning daysThreshold={30} />)
    await waitFor(() => {
  ...
  it('should dismiss when dismiss button is clicked', async () => {
    ...
    render(<LicenseExpiryWarning daysThreshold={30} onDismiss={onDismiss} />)
  ...
  it('should call onRenew when renew button is clicked', async () => {
    ...
    render(<LicenseExpiryWarning daysThreshold={30} onRenew={onRenew} />)
...
describe('LicenseUpgradePrompt', () => {
  ...
  it('should render card variant by default', () => {
    render(<LicenseUpgradePrompt requiredPlan="pro" />)
    expect(screen.getByText('Upgrade to Pro')).toBeInTheDocument()
    expect(screen.getByText('Upgrade Now')).toBeInTheDocument()
  })

  it('should render minimal variant', () => {
    render(<LicenseUpgradePrompt requiredPlan="pro" variant="minimal" />)
    expect(screen.getByText(/requires/i)).toBeInTheDocument()
    expect(screen.getByText('Upgrade')).toBeInTheDocument()
  })

  it('should render inline variant', () => {
    render(<LicenseUpgradePrompt requiredPlan="pro" variant="inline" />)
  ...
  it('should show custom feature name', () => {
    ...
    expect(
      screen.getByText(/Advanced Analytics requires a Enterprise license/i)
    ).toBeInTheDocument()
  })
  ...
  it('should call onUpgrade when button is clicked', () => {
    ...
    render(<LicenseUpgradePrompt requiredPlan="pro" onUpgrade={onUpgrade} />)
  ...
  it('should show current plan when license is set', async () => {
    ...
    render(<LicenseUpgradePrompt requiredPlan="enterprise" />)
    await waitFor(() => {
  ...
  it('should apply custom className', () => {
    const { container } = render(
      <LicenseUpgradePrompt requiredPlan="pro" className="custom-class" />
    )
