...
export function useLicenseStatus() {
    const licenseKey = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
    return useMemo(() => {
 ...
 * ```
 */
export function useHasPlan(requiredPlan) {
    const status = useLicenseStatus();
    return useMemo(() => {
 ...
 * ```
 */
export function useRequireLicense(featureName = 'This feature') {
    const status = useLicenseStatus();
    if (status.status !== 'Valid') {
        throw new Error(`${featureName} requires a valid Clarity Chat license. ` +
    ...
    useEffect(() => {
        ...
        if (!isValid || !hasSufficientPlan) {
            if (isDevelopment()) {
                ...
                console.warn(`%c Clarity Chat License Warning %c\n\n` +
                    `The "${featureName}" feature requires a ${requiredPlan} license.\n\n` +
                    `${reason}\n\n` +
                    `Get your license at: https://claritychat.dev/pricing\n\n` +
                    `This warning only appears in development mode.`, 'background: #f59e0b; color: black; font-weight: bold; padding: 4px 8px;', '');
            }
        }
