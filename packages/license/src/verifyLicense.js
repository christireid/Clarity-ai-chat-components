...
export function verifyLicense(licenseKey, options = {}) {
    const { requiredPlan, checkDevExpiry = true, environment = getEnvironment(), gracePeriodDays = DEFAULT_GRACE_PERIOD_DAYS, } = options;
    // Check for missing key
    if (!licenseKey) {
        return {
            status: 'Missing',
            reason: 'No license key provided. Purchase at https://claritychat.dev/pricing',
        };
    }
    ...
    if (isExpired) {
        ...
        if (checkDevExpiry) {
            ...
            return {
                ...
                payload,
                reason: 'License expired and grace period ended. Renew at https://claritychat.dev/account for continued updates.',
            };
        }
    }
 ...
 */
export function isLicenseValid(licenseKey) {
 ...
 */
export function shouldShowWatermark(status) {
...
//# sourceMappingURL=verifyLicense.js.map