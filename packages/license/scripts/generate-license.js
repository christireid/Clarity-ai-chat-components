 ...
 */
import { generateLicenseKey, parseLicenseKey, verifyLicenseChecksum, } from '../src/generateLicense';
...
async function main() {
    ...
    if (command === 'generate') {
        ...
        if (!secret) {
            console.error('Error: CLARITY_LICENSE_SECRET environment variable is required');
            process.exit(1);
        }
        ...
        const validPlans = ['community', 'pro', 'enterprise'];
        const validScopes = ['individual', 'team', 'organization'];
        if (!validPlans.includes(options.plan)) {
            console.error(`Error: Invalid plan. Must be one of: ${validPlans.join(', ')}`);
            process.exit(1);
        }
        const scope = options.scope || 'individual';
        if (!validScopes.includes(scope)) {
            console.error(`Error: Invalid scope. Must be one of: ${validScopes.join(', ')}`);
            process.exit(1);
        }
        try {
            ...
            if (options.devs)
                console.log(`  Max Developers: ${options.devs}`);
            if (options.domains)
...
//# sourceMappingURL=generate-license.js.map