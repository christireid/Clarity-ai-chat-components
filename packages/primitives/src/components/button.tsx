...
export interface ButtonProps
  ...
  /**
   * Controlled button state (idle, loading, success, error)
   * When provided, takes precedence over `loading` prop
   */
  state?: ButtonState

  /**
   * Show ripple effect on click (Material Design pattern)
   * Automatically disabled for link variant and when button is disabled
   * @default true
   */
  ripple?: boolean

  /**
   * Custom ripple color (overrides variant-based color)
   */
  rippleColor?: string

  /**
   * Custom success message/icon to show in success state
   * @default checkmark icon
   */
  successMessage?: React.ReactNode

  /**
   * Custom error message/icon to show in error state
   * @default X icon
   */
  errorMessage?: React.ReactNode

  /**
   * Duration (ms) for success/error state before returning to idle
   * Only applies in uncontrolled mode (when `state` prop is not provided)
   * @default 2000
   */
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    // Announce state changes to screen readers
    const previousStateRef = React.useRef<ButtonState>(currentState)
    React.useEffect(() => {
      if (!announceStateChange) return
    ...
    return (
      <ShadcnButton
        ...
        // Data attributes for styling hooks and testing
        data-slot="button"
      ...
      >
        ...
        {/* State content (loading spinner, success icon, error icon) */}
        {stateContent}
