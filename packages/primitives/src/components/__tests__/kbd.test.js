...
const mockNavigator = (platform, userAgent = '') => {
...
describe('Kbd Component', () => {
    ...
    describe('Rendering', () => {
        it('should render a kbd element', () => {
            render(_jsx(Kbd, { shortcut: "k" }));
            expect(screen.getByText('K')).toBeInTheDocument();
        });
        it('should render multiple keys with separator', () => {
            mockNavigator('Win32');
            render(_jsx(Kbd, { shortcut: "ctrl+k" }));
            expect(screen.getByText('Ctrl')).toBeInTheDocument();
            expect(screen.getByText('K')).toBeInTheDocument();
        });
        it('should render array shortcuts', () => {
            mockNavigator('Win32');
            render(_jsx(Kbd, { shortcut: ['ctrl', 'shift', 'k'] }));
            expect(screen.getByText('Ctrl')).toBeInTheDocument();
    ...
    describe('Platform Detection', () => {
        it('should use Mac symbols on Mac platform', () => {
            ...
            render(_jsx(Kbd, { shortcut: "mod+k" }));
            expect(screen.getByText('K')).toBeInTheDocument();
        });
        ...
        it('should respect platform override', () => {
            ...
            render(_jsx(Kbd, { shortcut: "mod+k", platform: "mac" }));
    ...
    describe('Key Mappings', () => {
        ...
        it('should map mod to command on Mac', () => {
            render(_jsx(Kbd, { shortcut: "mod", platform: "mac", "data-testid": "kbd" }));
        ...
        it('should handle arrow keys', () => {
            render(_jsx(Kbd, { shortcut: "up" }));
            expect(screen.getByText('↑')).toBeInTheDocument();
        });
        it('should handle function keys', () => {
            render(_jsx(Kbd, { shortcut: "f1" }));
            expect(screen.getByText('F1')).toBeInTheDocument();
        });
        it('should handle single letters', () => {
            render(_jsx(Kbd, { shortcut: "a" }));
            expect(screen.getByText('A')).toBeInTheDocument();
        });
    });
    describe('Sizes', () => {
        it('should render with default md size', () => {
            render(_jsx(Kbd, { shortcut: "k", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
            expect(kbd).toHaveClass('text-xs');
        });
        it('should render with sm size', () => {
            render(_jsx(Kbd, { shortcut: "k", size: "sm", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
            expect(kbd).toHaveClass('text-[10px]');
        });
        it('should render with lg size', () => {
            render(_jsx(Kbd, { shortcut: "k", size: "lg", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
    ...
    describe('Variants', () => {
        ...
        it('should render with outline variant', () => {
            render(_jsx(Kbd, { shortcut: "k", variant: "outline", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
            expect(kbd).toHaveClass('bg-transparent');
        });
        it('should render with ghost variant', () => {
            render(_jsx(Kbd, { shortcut: "k", variant: "ghost", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
            expect(kbd).toHaveClass('bg-transparent');
        });
    });
    describe('Separator', () => {
        it('should use platform-specific separator by default', () => {
            mockNavigator('Win32');
            render(_jsx(Kbd, { shortcut: "ctrl+k", "data-testid": "kbd" }));
            expect(screen.getByText('+')).toBeInTheDocument();
        });
        it('should hide separator when set to none', () => {
            mockNavigator('Win32');
            render(_jsx(Kbd, { shortcut: "ctrl+k", separator: "none", "data-testid": "kbd" }));
            expect(screen.queryByText('+')).not.toBeInTheDocument();
        });
        it('should use custom separator', () => {
            render(_jsx(Kbd, { shortcut: "ctrl+k", separator: "-", "data-testid": "kbd" }));
            expect(screen.getByText('-')).toBeInTheDocument();
        });
    });
    describe('Custom className', () => {
        it('should accept custom className', () => {
            render(_jsx(Kbd, { shortcut: "k", className: "custom-class", "data-testid": "kbd" }));
            const kbd = screen.getByTestId('kbd');
    ...
    describe('ForwardRef', () => {
        it('should forward ref to kbd element', () => {
            ...
            render(_jsx(Kbd, { shortcut: "k", ref: ref }));
...
describe('useFormattedShortcut Hook', () => {
    const TestComponent = ({ shortcut, options, }) => {
        const formatted = useFormattedShortcut(shortcut, options);
        return _jsx("div", { "data-testid": "result", children: formatted });
    };
    it('should format shortcut string', () => {
        render(_jsx(TestComponent, { shortcut: "mod+k", options: { platform: 'windows' } }));
        expect(screen.getByTestId('result')).toHaveTextContent('Ctrl+K');
    });
    it('should format shortcut array', () => {
        render(_jsx(TestComponent, { shortcut: ['ctrl', 'shift', 'k'], options: { platform: 'windows' } }));
        expect(screen.getByTestId('result')).toHaveTextContent('Ctrl+Shift+K');
    });
    it('should use no separator on Mac', () => {
        render(_jsx(TestComponent, { shortcut: "mod+k", options: { platform: 'mac' } }));
