 ...
 */
const KEY_MAP: Record<string, { mac: string; other: string }> = {
  mod: { mac: '⌘', other: 'Ctrl' },
  cmd: { mac: '⌘', other: 'Ctrl' },
...
export type KbdPlatform = 'mac' | 'windows' | 'linux' | 'auto'
...
export interface KbdProps extends React.HTMLAttributes<HTMLElement> {
   ...
   * Can be a string like "mod+k" or an array like ["mod", "shift", "p"].
   * Use "mod" for the platform modifier key (Cmd on Mac, Ctrl elsewhere).
   */
  ...
  /**
   * Size variant for the kbd element.
   * @default 'md'
   */
  size?: KbdSize
  /**
  ...
  variant?: KbdVariant
  /**
   * Separator between keys. On Mac, typically no separator is used.
   * Set to 'auto' to use platform-specific defaults.
   * @default 'auto'
   */
  ...
  (
    ...
    return (
      <kbd
        ref={ref}
        className={cn(
          'inline-flex items-center justify-center gap-0.5 rounded font-mono',
      ...
      >
        {formattedKeys.map((key, index) => (
          <React.Fragment key={index}>
            ...
            <span className="min-w-[1ch] text-center">{key}</span>
          </React.Fragment>
        ))}
      </kbd>
    )
...
export function useFormattedShortcut(
  shortcut: string | string[],
  options?: { platform?: KbdPlatform; separator?: string | 'auto' | 'none' }
): string {
