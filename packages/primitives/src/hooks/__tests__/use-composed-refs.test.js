...
import { composeRefs, useComposedRefs, useForwardedRef, } from '../use-composed-refs';
...
describe('useComposedRefs', () => {
    ...
    it('updates when refs change', () => {
        ...
        const ref2 = vi.fn();
        const ref3 = vi.fn();
        const { result, rerender } = renderHook(({ refs }) => useComposedRefs(...refs), { initialProps: { refs: [ref1, ref2] } });
        const firstResult = result.current;
    ...
    it('works correctly in component context', () => {
        ...
        const { result } = renderHook(() => useComposedRefs(externalRef, internalRef));
        const element = document.createElement('div');
...
//# sourceMappingURL=use-composed-refs.test.js.map