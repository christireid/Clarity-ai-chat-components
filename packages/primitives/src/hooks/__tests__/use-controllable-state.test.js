...
import { useControllableState, useControllableBoolean, } from '../use-controllable-state';
describe('useControllableState', () => {
    ...
    describe('controlled mode', () => {
        ...
        it('reflects prop changes', () => {
            const { result, rerender } = renderHook(({ prop }) => useControllableState({
                prop,
    ...
    describe('mode switching warnings', () => {
        ...
        it('warns when switching from controlled to uncontrolled', () => {
            const { rerender } = renderHook(({ prop }) => useControllableState({
                prop,
            }), { initialProps: { prop: 'controlled' } });
            rerender({ prop: undefined });
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('switching from controlled to uncontrolled'));
        });
        it('warns when switching from uncontrolled to controlled', () => {
            ...
            expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('switching from uncontrolled to controlled'));
        });
    });
...
//# sourceMappingURL=use-controllable-state.test.js.map