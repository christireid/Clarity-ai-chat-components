 ...
 */
async function parseExports(filePath: string): Promise<ExportInfo[]> {
 ...
 */
async function verifyEntryPoint(fileName: string): Promise<VerificationResult> {
  ...
  const sourceMap = new Map<string, ExportInfo[]>()
  ...
  for (const [source, exps] of sourceMap) {
    ...
    if (resolved === null) {
      ...
      result.errors.push(
        `Cannot resolve import "${source}" (lines: ${lineNumbers})`
      )
      ...
      if (firstExp) {
        result.warnings.push(
          `External dependency: ${source} (line ${firstExp.line})`
        )
      }
    }
  }
  ...
  if (namespaceExports.length > 3) {
    result.warnings.push(
      `High number of namespace exports (${namespaceExports.length}) - consider explicit exports for better tree-shaking`
    )
  }
