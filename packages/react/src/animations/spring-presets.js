 ...
 * ```
 */
export function getSpring(preset, prefersReducedMotion, overrides) {
...
export const SPRING_COMBINATIONS = {
    ...
    entrance: (prefersReducedMotion, preset = 'smooth') => getSpring(preset, prefersReducedMotion),
    /** Exit animations (matching entrance) */
    exit: (prefersReducedMotion, preset = 'quick') => getSpring(preset, prefersReducedMotion),
    /** Staggered children (with delay) */
    stagger: (index, prefersReducedMotion, staggerDelay = 0.05, preset = 'quick') => getSpring(preset, prefersReducedMotion, {
 ...
 * ```
 */
export function createSpringTransition(damping, stiffness, prefersReducedMotion) {
    if (prefersReducedMotion) {
        return {
            type: 'tween',
