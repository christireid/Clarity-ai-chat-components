...
export interface UseSmartThrottleOptions {
  /** Throttle delay in ms (default: 500) */
  delay?: number
  /** Enable adaptive throttling based on input length */
  adaptive?: boolean
  /** Minimum input length before processing */
  minLength?: number
  /** Cancel on new input (debounce behavior) */
  cancelOnNew?: boolean
  /** Track token savings */
  trackSavings?: boolean
  /** Callback when throttled */
  onThrottle?: () => void
...
export interface UseSmartThrottleReturn<T> {
  ...
  /** Whether currently throttled (waiting) */
  isThrottled: boolean
  /** Set new value (will be throttled) */
 ...
 * ```
 */
export function useSmartThrottle<T = any>(
  ...
  const [throttledValue, setThrottledValue] = React.useState<T>()
