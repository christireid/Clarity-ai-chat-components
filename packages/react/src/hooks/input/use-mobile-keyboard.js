 ...
 * ```
 */
export function useMobileKeyboard(options = {}) {
    const { onKeyboardShow, onKeyboardHide, debounceDelay = 150, autoScroll = true, scrollOffset = 20, } = options;
    ...
    const previousHeightRef = React.useRef(typeof window !== 'undefined' ? window.innerHeight : 0);
     ...
     */
    const handleFocusIn = React.useCallback((e) => {
     ...
     */
    React.useEffect(() => {
        if (typeof window === 'undefined' || !state.isMobile)
            return;
 ...
 * ```
 */
export function useMobileKeyboardScrollLock(state) {
...
//# sourceMappingURL=use-mobile-keyboard.js.map