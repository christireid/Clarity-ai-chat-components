...
import { useKeyboardNavigation, useVimNavigation, useFocusScope, } from './use-keyboard-navigation';
// ============================================================================
// Hook
// ============================================================================
export function useChatKeyboardNavigation({ messages, onScrollToMessage, onCopyMessage, onRegenerateMessage, onDeleteMessage, onEditMessage, onFocusInput, onSendMessage, onNewChat, onToggleSidebar, onOpenSettings, onOpenCommandPalette, onShowShortcutsHelp, enabled = true, scope = 'chat', }) {
    ...
    const handleMessageSelect = React.useCallback((message) => {
        callbacksRef.current.onCopyMessage?.(message);
    }, []);
    const handleMessageFocus = React.useCallback((message, index) => {
        callbacksRef.current.onScrollToMessage?.(message, index);
        announceToScreenReader(`Message ${index + 1} of ${messages.length} from ${message.role}`, false);
    }, [announceToScreenReader, messages.length]);
    // Use vim navigation for messages
    const { focusedIndex, setFocusedIndex, getItemProps, navigate } = useVimNavigation({
        items: messages,
    ...
    const getMessageProps = React.useCallback((index) => {
...
// ============================================================================
export function useFocusInputShortcut(inputRef, shortcut = 'i') {
...
export function useQuickActions(actions) {
    const { registerShortcut } = useKeyboardNavigation();
    React.useEffect(() => {
        const unsubscribers = actions.map((action) => registerShortcut({
            id: action.id,
...
//# sourceMappingURL=use-chat-keyboard-navigation.js.map