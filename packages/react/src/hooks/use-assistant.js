                ...
                if (enableCache && cacheRef.current) {
                    const messageContentStr = typeof messageContent === 'string'
                        ...
                        : messageContent
                            ...
                            .map((part) => (part.type === 'text' ? part.text : ''))
                            .join('');
                    cacheRef.current.set(messageContentStr, { message: finalMessage, toolInvocations: finalMessage.toolInvocations || [] }, requestBody);
                }
                return;
            }
                ...
                onChunk: (chunk) => {
                    ...
                    const currentMessage = {
                        ...
                        toolInvocations: currentToolInvocations.map(tool => ({
                            ...
                            toolName: tool.toolName,
                            args: tool.args,
                            state: tool.state === 'error' ? 'call' : tool.state,
                            result: tool.result,
                        })),
                    };
