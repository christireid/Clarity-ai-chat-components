...
import { jsonToToon, autoOptimize, formatForLLM, parseFlexible, } from '../../utils/toon';
import { countTokens, countConversationTokens, } from '../../utils/tokenization';
...
import {} from '../../utils/optimization/prompt-compression';
import { intelligentCompress, } from '../../utils/optimization/llmlingua-compressor';
import { SmartCache } from '../../utils/optimization/smart-cache';
// Import persistent cache utilities
import { createPersistentSemanticCache, } from '../../utils/optimization/semantic-cache-persistent';
import { MODEL_REGISTRY, } from '../../utils/tokenization/model-registry';
// Import utilities from token-optimization for unified API
import { limitHistory, createThrottler, routeToModel, estimateRoutingSavings, createReference, enforceOutputLimit, createBatcher, } from '../../utils/optimization/token-optimization';
// Import new optimization utilities
import { PREFILL_TEMPLATES, } from '../../utils/optimization/response-prefilling';
import { restructurePrompt as restructurePromptUtil, } from '../../utils/optimization/prompt-structure';
/**
...
export function useTokenOptimizationEnhanced(options = {}) {
    ...
    // From basic hook
    enableHistoryLimiting = presetConfig.enableHistoryLimiting ?? false, historyLimiting = {}, summarizeMessage, calculateEmbedding, enablePIIRedaction = false, enableDynamicContext = false, enableThrottling = false, throttling = {}, enableModelRouting = presetConfig.enableModelRouting ?? false, modelRouting = {}, enableReferences = false, references = {}, enableOutputLimits = false, outputLimits = {}, enableBatching = false, batching = {}, 
    // New optimizations
    ...
    const embeddingCache = React.useRef(new Map());
     ...
     * PII Redaction
     */
    const redactPII = React.useCallback((text) => {
    ...
    const optimizeData = React.useCallback(async (data) => {
            ...
            if (enableStats && result.format === 'toon') {
                setStats((prev) => ({
                    ...
                    toon: {
                        ...
                        averageSavingsPercent: (prev.toon.averageSavingsPercent * prev.toon.conversions +
        ...
        if (enableStats) {
            setStats((prev) => ({
                ...prev,
                tokenization: {
                    accurateCount: prev.tokenization.accurateCount +
                        (tokens.method === 'accurate' ? 1 : 0),
                    estimatedCount: prev.tokenization.estimatedCount +
                        (tokens.method === 'estimated' ? 1 : 0),
                    accuracyRate: ((prev.tokenization.accurateCount +
                        (tokens.method === 'accurate' ? 1 : 0)) /
    ...
    const optimizePrompt = React.useCallback(async (prompt) => {
        ...
        if (enablePromptCompression) {
            ...
            const targetRatio = compressionLevel === 'aggressive'
                ? 0.4
                : compressionLevel === 'conservative'
                    ? 0.8
                    : 0.6;
            ...
            compressionResult = {
                ...
                tokenSavings: intelligentResult.originalTokens -
            ...
            if (enableStats) {
                setStats((prev) => ({
                    ...
                    compression: {
                        ...
                        totalTokensSaved: prev.compression.totalTokensSaved +
                            compressionResult.tokenSavings,
                        averageSavingsPercent: (prev.compression.averageSavingsPercent *
     ...
     */
    const optimizeHistory = React.useCallback(async (messages) => {
        ...
        if (historyLimiting.strategy === 'smart' && calculateEmbedding) {
            ...
            try {
                ...
                const reservedTokens = await countTokens(JSON.stringify([
                    ...(systemMessage ? [systemMessage] : []),
                    ...recentMessages,
                ]), { model }).then((r) => r.total);
                ...
                keptTurns.forEach((t) => {
                    ...
                    if (t.assistant)
                        semanticHistory.push(t.assistant);
                });
                return [
                    ...(systemMessage ? [systemMessage] : []),
                    ...semanticHistory,
        ...
        if (enableStats && limited.length < originalLength) {
            setStats((prev) => ({
                ...prev,
                historyLimiting: {
                    messagesRemoved: prev.historyLimiting.messagesRemoved +
    ...
    const routeQuery = React.useCallback((query) => {
        ...
        if (enableStats) {
            setStats((prev) => ({
                ...prev,
                modelRouting: {
                    simpleModelRoutes: prev.modelRouting.simpleModelRoutes +
                        (routedModel === modelRouting.simpleModel ? 1 : 0),
                    complexModelRoutes: prev.modelRouting.complexModelRoutes +
    ...
    React.useEffect(() => {
        ...
        setStats((prev) => ({
            ...
            overall: {
                totalTokensSaved,
                totalCostSaved,
                averageSavingsPercent: (stats.toon.averageSavingsPercent +
    ...
    return {
        ...
        stats,
        resetStats,
        isBudgetExceeded: !!(enableCostTracking &&
            budget &&
