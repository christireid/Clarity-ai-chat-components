...
let _modelPricing = null;
/** Cached MODEL_LIMITS value */
let _modelLimits = null;
/**
 * Computes MODEL_PRICING from MODEL_REGISTRY (called once on first access)
 */
function computeModelPricing() {
    return Object.fromEntries(Object.entries(MODEL_REGISTRY).map(([id, config]) => [
        id,
        {
 ...
 */
export const MODEL_PRICING = new Proxy({}, {
    get(_target, prop) {
 ...
 * ```
 */
export const MODEL_LIMITS = new Proxy({}, {
    get(_target, prop) {
...
export function useTokenTracker(options) {
    const { modelName, maxTokens: customMaxTokens, inputCostPerToken: customInputCost, outputCostPerToken: customOutputCost, warningThreshold = 0.8, criticalThreshold = 0.95, onWarning, onCritical, } = options;
    // Get model info
     ...
     */
    const canSend = React.useCallback((estimatedTokens) => {
     ...
     */
    const estimateTokens = React.useCallback((text) => {
