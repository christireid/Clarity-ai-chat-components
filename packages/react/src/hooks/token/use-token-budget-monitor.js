...
function createInitialUsage(config) {
    const effectiveMax = config.maxInputTokens -
 ...
 */
const VALID_BUDGET_MONITOR_MODELS = new Set([
    'gpt-4o',
    'gpt-4-turbo',
    'gpt-4o',
    'gpt-4o-mini',
 ...
 * ```
 */
export function isValidBudgetMonitorModel(model) {
    return VALID_BUDGET_MONITOR_MODELS.has(model);
}
/**
 * Get list of all supported budget monitor models
 */
export function getSupportedBudgetModels() {
    return Array.from(VALID_BUDGET_MONITOR_MODELS);
}
/**
...
export function createModelBudgetMonitor(model, overrides) {
    ...
    if (!isValidBudgetMonitorModel(model)) {
        ...
        throw new Error(`[createModelBudgetMonitor] Unknown model: "${model}". ` +
            `Supported models: ${supported}`);
    }
    // Derive configuration from MODEL_REGISTRY (single source of truth)
    const registryConfig = MODEL_REGISTRY[model];
    if (!registryConfig) {
        throw new Error(`[createModelBudgetMonitor] Model "${model}" not found in registry`);
    }
    return {
...
export function estimateTokenCost(usage, model) {
    ...
    const estimatedOutputCost = (usage.reservedForOutput / 1_000_000) * pricing.outputCostPer1M;
