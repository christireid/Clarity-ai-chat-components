 ...
 * ```
 */
export function useAnimatedValue(value, options = {}) {
    const { duration = 500, easing = 'easeOut', decimals = 0, prefix = '', suffix = '', animate = true, onAnimationComplete, } = options;
 ...
 * ```
 */
export function AnimatedNumber({ value, className, highlightChange = false, render, ...options }) {
    const result = useAnimatedValue(value, options);
    if (render) {
        return _jsx(_Fragment, { children: render(result) });
    }
    const highlightClass = highlightChange && result.direction !== 'none'
        ? result.direction === 'up'
            ? 'text-green-600 dark:text-green-400'
            : 'text-red-600 dark:text-red-400'
        : '';
    return (_jsx("span", { ...result.ariaProps, className: `${highlightClass} ${className || ''} ${result.isAnimating ? 'transition-colors duration-200' : ''}`.trim(), children: result.formattedValue }));
}
 ...
 * ```
 */
export function useValueChange(value, options = {}) {
 ...
 * ```
 */
export function FlashingValue({ value, className = '', flashClassName = 'bg-yellow-100 dark:bg-yellow-900/30', flashDuration = 500, children, }) {
    const [flash, setFlash] = React.useState(false);
    const previousValueRef = React.useRef(value);
    React.useEffect(() => {
    ...
    return (_jsx("span", { className: `transition-colors duration-200 ${className} ${flash ? flashClassName : ''}`.trim(), "aria-live": "polite", "aria-atomic": "true", children: children ?? value }));
}
FlashingValue.displayName = 'FlashingValue';
//# sourceMappingURL=use-animated-value.js.map