...
useMemoizedCallback, useBatchedState, 
// Effect optimization hooks
useUpdateEffect, useThrottledEffect, useDebouncedEffect, 
// Memoization hooks
useMemoizedSelector, useDeepMemo, 
// Virtual scrolling
useVirtualList, useDynamicVirtualList, 
// Event handling
useEventDelegation, useIntersectionObserver, 
// Context optimization
useContextSelector, useMultipleContexts, 
// Utility functions
measurePerformance, } from '../performance/enhanced';
// Mock SecureLogger
...
describe('Enhanced Performance Optimization Utilities', () => {
    ...
    describe('useMemoizedCallback', () => {
        ...
        it('should update callback when dependencies change', () => {
            ...
            const { result, rerender } = renderHook(({ cb, deps }) => useMemoizedCallback(cb, deps), {
    ...
    describe('useBatchedState', () => {
        it('should batch multiple state updates', () => {
            const { result } = renderHook(() => useBatchedState({ count: 0, name: 'test' }));
        ...
        it('should handle function updates in batch', () => {
            const { result } = renderHook(() => useBatchedState({ count: 0 }));
            const [, setState, startBatch, endBatch] = result.current;
            act(() => {
                startBatch();
    ...
    describe('useThrottledEffect', () => {
        it('should throttle effect execution', () => {
            const effect = vi.fn();
            const delay = 1000;
            const { rerender } = renderHook(() => useThrottledEffect(effect, [], delay));
            expect(effect).not.toHaveBeenCalled();
            act(() => {
        ...
        it('should cancel previous timeout on update', () => {
            const effect = vi.fn();
            const delay = 1000;
            const { rerender } = renderHook(() => useThrottledEffect(effect, [1], delay));
    ...
    describe('useDebouncedEffect', () => {
        it('should debounce effect execution', () => {
            const effect = vi.fn();
            const delay = 500;
            const { rerender } = renderHook(() => useDebouncedEffect(effect, [], delay));
        ...
        it('should reset timer on each update', () => {
            const effect = vi.fn();
            const delay = 500;
            const { rerender } = renderHook(() => useDebouncedEffect(effect, [1], delay));
    ...
    describe('useMemoizedSelector', () => {
        it('should memoize selector results', () => {
            ...
            const state = { count: 5 };
            const { result, rerender } = renderHook(() => useMemoizedSelector(selector, state));
    ...
    describe('useDeepMemo', () => {
        it('should perform deep comparison of dependencies', () => {
            ...
            const { result, rerender } = renderHook(() => useDeepMemo(factory, deps1));
    ...
    describe('useVirtualList', () => {
        it('should virtualize large lists', () => {
            ...
            const { result } = renderHook(() => useVirtualList(items, config));
    ...
    describe('useDynamicVirtualList', () => {
        it('should handle variable item heights', () => {
            ...
            const getItemHeight = (item) => (item % 2 === 0 ? 50 : 100);
            const config = {
                containerHeight: 200,
                overscan: 2,
            };
            const { result } = renderHook(() => useDynamicVirtualList(items, getItemHeight, config));
    ...
    describe('useEventDelegation', () => {
        it('should delegate events to container', () => {
            ...
            const { unmount } = renderHook(() => useEventDelegation('click', handler, containerRef));
    ...
    describe('useContextSelector', () => {
        it('should select specific context values', () => {
            ...
            const selector = (value) => value.count;
            const wrapper = ({ children }) => (_jsx(TestContext.Provider, { value: { count: 10, name: 'updated' }, children: children }));
            const { result } = renderHook(() => useContextSelector(TestContext, selector), { wrapper });
            expect(result.current).toBe(10);
        });
        it('should prevent unnecessary re-renders', () => {
            const TestContext = React.createContext({ count: 5, name: 'test' });
            const selector = vi.fn((value) => value.count);
            const { rerender } = renderHook(() => useContextSelector(TestContext, selector));
            expect(selector).toHaveBeenCalledTimes(1);
    ...
    describe('Integration Tests', () => {
        ...
        it('should handle edge cases gracefully', () => {
            // Empty virtual list
            const { result: emptyResult } = renderHook(() => useVirtualList([], {
                itemHeight: 50,
                containerHeight: 200,
            }));
            ...
            const { result: scrollResult } = renderHook(() => useVirtualList([1, 2, 3], {
                itemHeight: 50,
                containerHeight: 200,
            }));
            act(() => {
...
//# sourceMappingURL=enhanced.test.js.map