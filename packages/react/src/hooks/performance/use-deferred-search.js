 ...
 */
function findMatchIndices(text, query, options) {
    const indices = [];
    ...
    if (indices.length === 0) {
        if (options.wholeWord) {
            ...
            const wordRegex = new RegExp(`\\b${safeEscaped}\\b`, options.caseSensitive ? 'g' : 'gi');
            let match;
            let iterations = 0;
 ...
 * ```
 */
export function useDeferredSearch(messages, searchQuery, options) {
 ...
 * ```
 */
export function highlightText(text, indices, renderer) {
    if (indices.length === 0) {
 ...
 */
export function highlightSearchText(text, query, className) {
    if (!query.trim())
        return text;
 ...
 */
export function HighlightedText({ text, query, className, highlightClassName, }) {
    if (!query.trim()) {
    ...
    return (_jsx("span", { className: className, children: parts.map((part, index) => 
        ...
        part.toLowerCase() === queryLower ? (_jsx("mark", { className: highlightClassName ||
                'bg-yellow-200 dark:bg-yellow-900/50 text-inherit rounded-sm px-0.5', children: part }, index)) : (part)) }));
}
//# sourceMappingURL=use-deferred-search.js.map