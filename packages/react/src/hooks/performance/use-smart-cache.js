...
export function useSmartCache(options = {}) {
    const { enabled: initialEnabled = true, costPerToken = 0, onCacheHit, onCacheMiss, ...cacheOptions } = options;
    ...
    const cache = React.useMemo(() => new SmartCache(cacheOptions), [cacheOptions]);
    ...
    const get = React.useCallback(async (query) => {
        if (!isEnabled)
            return null;
    ...
    const clearByTag = React.useCallback((tag) => {
...
//# sourceMappingURL=use-smart-cache.js.map