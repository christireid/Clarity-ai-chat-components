...
export function useMemoizedSelector<T, R>(
  ...
  if (stateRef.current !== state || !resultRef.current) {
    ...
    console.log('MemoizedSelector recomputed', {
      stateChanged: stateRef.current !== state,
    })
  }
...
export function useDynamicVirtualList<T>(
  ...
  const paddingBottom =
    totalHeight - (cumulativeHeights[endIndex + 1] || totalHeight)
...
// NOTE: useIntersectionObserver has been removed from this file.
// Use the canonical version from @clarity-chat/react/hooks/ui/use-intersection-observer
// which provides a better API with isIntersecting boolean and freezeOnceVisible option.
...
export function useContextSelector<T, R>(
  ...
  const selectedValue = useMemo(
    () => selectorRef.current(contextValue),
    [contextValue]
  )
 ...
 */
export function useMultipleContexts<
  T extends readonly [React.Context<unknown>, (value: unknown) => unknown][],
>(
  contexts: T
): {
  [K in keyof T]: T[K] extends [
    React.Context<unknown>,
    (value: unknown) => infer R,
  ]
    ? R
    : never
} {
