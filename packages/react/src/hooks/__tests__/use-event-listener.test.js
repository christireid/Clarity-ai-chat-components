...
describe('useEventListener', () => {
    ...
    describe('Event type changes', () => {
        it('should reattach listener when event type changes', () => {
            ...
            const addEventListenerSpy = vi.spyOn(window, 'addEventListener');
            const removeEventListenerSpy = vi.spyOn(window, 'removeEventListener');
            const { rerender } = renderHook(({ eventName }) => useEventListener(eventName, handler), { initialProps: { eventName: 'click' } });
            // Should have added click listener
            expect(addEventListenerSpy).toHaveBeenLastCalledWith('click', expect.any(Function), undefined);
            // Change event type
            rerender({ eventName: 'keydown' });
            // Should have removed old listener and added new
            expect(removeEventListenerSpy).toHaveBeenCalledWith('click', expect.any(Function), undefined);
            expect(addEventListenerSpy).toHaveBeenLastCalledWith('keydown', expect.any(Function), undefined);
        });
    });
    describe('SSR safety', () => {
...
//# sourceMappingURL=use-event-listener.test.js.map