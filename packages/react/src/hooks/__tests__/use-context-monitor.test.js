...
import { useContextMonitor, getUtilizationColor, formatTokenCount, createContextMonitorIntegration, } from '../use-context-monitor';
// Mock the estimator
...
describe('useContextMonitor', () => {
    ...
    describe('basic functionality', () => {
        ...
        it('should analyze messages and update utilization', () => {
            const { result } = renderHook(() => useContextMonitor({ maxTokens: 1000, reservedTokens: 100 }));
            const messages = [
        ...
        it('should correctly categorize token breakdown', () => {
            const { result } = renderHook(() => useContextMonitor({ maxTokens: 10000, reservedTokens: 1000 }));
            const messages = [
    ...
    describe('recommendations generation', () => {
        it('should generate summarization recommendation for high utilization', () => {
            ...
            const hasRecommendation = result.current.recommendations.some(r => r.action === 'summarize' || r.action === 'switch-model');
    ...
    describe('history tracking', () => {
        ...
        it('should clear history', () => {
            const { result } = renderHook(() => useContextMonitor({ trackHistory: true }));
            const messages = [{ role: 'user', content: 'Hello' }];
            act(() => {
                result.current.analyzeMessages(messages);
                result.current.analyzeMessages(messages);
    ...
    describe('getUtilization (stateless)', () => {
        it('should return utilization without updating state', () => {
            const { result } = renderHook(() => useContextMonitor({ maxTokens: 1000 }));
            const messages = [
...
describe('createContextMonitorIntegration', () => {
    it('should return canSendMessage that checks projected utilization', () => {
        const { result } = renderHook(() => useContextMonitor({ maxTokens: 1000, reservedTokens: 0 }));
        const messages = [
...
//# sourceMappingURL=use-context-monitor.test.js.map