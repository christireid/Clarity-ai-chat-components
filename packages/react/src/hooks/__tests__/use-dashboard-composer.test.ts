...
describe('useDashboardComposer', () => {
  ...
  describe('loading states', () => {
    ...
    it('should set isLoading based on required sources', async () => {
      let resolveRequired: (value: unknown) => void
      let resolveOptional: (value: unknown) => void

      const requiredFetcher = vi.fn().mockImplementation(
        () =>
          new Promise((r) => {
            resolveRequired = r
          })
      )
      const optionalFetcher = vi.fn().mockImplementation(
        () =>
          new Promise((r) => {
            resolveOptional = r
          })
  ...
  describe('sequential vs parallel', () => {
    ...
    it('should fetch sequentially when parallel is false', async () => {
      ...
      expect(callOrder.indexOf('end1')).toBeLessThan(
        callOrder.indexOf('start2')
      )
    })
  })
