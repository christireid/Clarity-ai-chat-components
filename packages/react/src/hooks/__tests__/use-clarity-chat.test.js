...
describe('useClarityChat', () => {
    ...
    it('should support WebSocket transport option', () => {
        const { result } = renderHook(() => useClarityChat({ api: '/api/chat', transport: 'websocket' }));
        expect(result.current.messages).toEqual([]);
    });
...
describe('useClarityChat with MemoryProvider', () => {
    ...
    const Wrapper = ({ children }) => (_jsx(MemoryProvider, { config: mockConfig, autoStart: false, children: children }));
    ...
    it('should handle memory query errors gracefully', async () => {
        ...
        await expect(result.current.append({
            role: 'user',
            content: 'Test',
...
//# sourceMappingURL=use-clarity-chat.test.js.map