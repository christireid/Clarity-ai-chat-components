...
import { useMobileKeyboard, useMobileViewportHeight, useMobileKeyboardScrollLock, } from '../use-mobile-keyboard';
...
describe('useMobileKeyboard', () => {
    ...
    it('should handle autoScroll option', () => {
        const { result } = renderHook(() => useMobileKeyboard({ autoScroll: true, scrollOffset: 20 }));
        expect(result.current.isMobile).toBe(true);
    });
    it('should debounce resize events', () => {
        const { result } = renderHook(() => useMobileKeyboard({ debounceDelay: 300 }));
        expect(result.current.isKeyboardVisible).toBe(false);
    });
...
//# sourceMappingURL=use-mobile-keyboard.test.js.map