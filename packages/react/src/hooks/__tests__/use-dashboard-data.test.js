...
describe('useDashboardData', () => {
    describe('basic functionality', () => {
        ...
        it('should not fetch on mount when fetchOnMount is false', async () => {
            ...
            const { result } = renderHook(() => useDashboardData({ fetcher, fetchOnMount: false }));
            await act(async () => {
                await vi.runAllTimersAsync();
            });
        ...
        it('should use initialData when provided', async () => {
            ...
            const mockData = { value: 42 };
            const fetcher = vi.fn().mockResolvedValue(mockData);
            const { result } = renderHook(() => useDashboardData({ fetcher, initialData }));
    ...
    describe('error handling', () => {
        it('should set error state on fetch failure', async () => {
            ...
            const { result } = renderHook(() => useDashboardData({ fetcher, enableRetry: false }));
            await act(async () => {
                await vi.runAllTimersAsync();
            });
            expect(result.current.isLoading).toBe(false);
        ...
        it('should call onError callback on failure', async () => {
            ...
            const fetcher = vi.fn().mockRejectedValue(error);
            const onError = vi.fn();
            renderHook(() => useDashboardData({ fetcher, onError, enableRetry: false }));
            await act(async () => {
                await vi.runAllTimersAsync();
            });
...
describe('useSimpleDashboardData', () => {
    ...
    it('should refetch on dependency change', async () => {
        const fetcher = vi.fn().mockResolvedValue({ value: 42 });
        let dep = 1;
        const { result, rerender } = renderHook(() => useSimpleDashboardData(fetcher, [dep]));
        await act(async () => {
            await vi.runAllTimersAsync();
        });
        expect(fetcher).toHaveBeenCalledTimes(1);
...
//# sourceMappingURL=use-dashboard-data.test.js.map