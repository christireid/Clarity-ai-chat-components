...
import { useKeyboardShortcuts, useShortcutDisplay, useScopedKeyboardShortcuts, useFocusedKeyboardShortcuts, KeyboardShortcutsHelp, } from '../use-keyboard-shortcuts';
...
describe('useKeyboardShortcuts', () => {
    ...
    it('should handle mod key (platform-aware)', () => {
        ...
        const { unmount } = renderHook(() => useKeyboardShortcuts([{ key: 'mod+k', callback }]));
        act(() => {
            window.dispatchEvent(createKeyboardEvent('k', { ctrlKey: true }));
        });
        expect(callback).toHaveBeenCalledTimes(1);
        ...
        renderHook(() => useKeyboardShortcuts([{ key: 'mod+k', callback: callbackMac }]));
        act(() => {
    ...
    it('should not trigger disabled shortcuts', () => {
        const callback = vi.fn();
        renderHook(() => useKeyboardShortcuts([{ key: 'k', callback, enabled: false }]));
        act(() => {
            window.dispatchEvent(createKeyboardEvent('k'));
        });
        expect(callback).not.toHaveBeenCalled();
    ...
    it('should trigger in input elements when enableInInput is true', () => {
        const callback = vi.fn();
        renderHook(() => useKeyboardShortcuts([{ key: 'escape', callback, enableInInput: true }]));
        const input = document.createElement('input');
        document.body.appendChild(input);
        input.focus();
        act(() => {
    ...
    it('should prevent default when preventDefault is true', () => {
        ...
        renderHook(() => useKeyboardShortcuts([{ key: 'k', callback, preventDefault: true }]));
        act(() => {
            const event = createKeyboardEvent('k');
    ...
    it('should cleanup listeners on unmount', () => {
        const callback = vi.fn();
        const { unmount } = renderHook(() => useKeyboardShortcuts([{ key: 'k', callback }]));
        unmount();
        act(() => {
            window.dispatchEvent(createKeyboardEvent('k'));
        });
...
describe('useScopedKeyboardShortcuts', () => {
    ...
    it('should register shortcuts with priority', () => {
        ...
        const { unmount: unmountHigh } = renderHook(() => useScopedKeyboardShortcuts({
            shortcuts: [{ key: 'k', callback: highPriorityCallback }],
            priority: 10,
        }));
        // Then low priority
        const { unmount: unmountLow } = renderHook(() => useScopedKeyboardShortcuts({
    ...
    it('should fall through to lower priority when higher is disabled', () => {
        const lowPriorityCallback = vi.fn();
        const highPriorityCallback = vi.fn();
        renderHook(() => useScopedKeyboardShortcuts({
            shortcuts: [{ key: 'k', callback: highPriorityCallback }],
            priority: 10,
    ...
    it('should cleanup scope on unmount', () => {
        const callback = vi.fn();
        const { unmount } = renderHook(() => useScopedKeyboardShortcuts({
...
describe('useFocusedKeyboardShortcuts', () => {
    it('should only trigger when container has focus', () => {
        ...
        function TestComponent() {
            ...
            return (_jsx("div", { ref: containerRef, tabIndex: -1, "data-testid": "container", children: _jsx("button", { "data-testid": "button", children: "Click me" }) }));
        }
    ...
    it('should not trigger when focus is outside container', () => {
        ...
        function TestComponent() {
            const containerRef = React.useRef(null);
            useFocusedKeyboardShortcuts(containerRef, [{ key: 'k', callback }]);
            return (_jsxs(_Fragment, { children: [_jsx("div", { ref: containerRef, tabIndex: -1, "data-testid": "container", children: "Container" }), _jsx("button", { "data-testid": "outside-button", children: "Outside" })] }));
        }
        render(_jsx(TestComponent, {}));
...
describe('KeyboardShortcutsHelp', () => {
    it('should render shortcuts list', () => {
        ...
        render(_jsx(KeyboardShortcutsHelp, { shortcuts: shortcuts, onClose: vi.fn() }));
        expect(screen.getByText('Open search')).toBeInTheDocument();
        expect(screen.getByText('Close modal')).toBeInTheDocument();
    });
    it('should call onClose when Escape is pressed', () => {
        const onClose = vi.fn();
        render(_jsx(KeyboardShortcutsHelp, { shortcuts: [], onClose: onClose }));
        act(() => {
    ...
    it('should render custom title', () => {
        render(_jsx(KeyboardShortcutsHelp, { shortcuts: [], onClose: vi.fn(), title: "Custom Title" }));
        expect(screen.getByText('Custom Title')).toBeInTheDocument();
    });
    it('should have accessible dialog role', () => {
        render(_jsx(KeyboardShortcutsHelp, { shortcuts: [], onClose: vi.fn() }));
...
//# sourceMappingURL=use-keyboard-shortcuts.test.js.map