...
describe('useStreamingSSE', () => {
    ...
    it('should initialize with idle status', () => {
        const { result } = renderHook(() => useStreamingSSE({
            url: '/api/stream',
        }));
    ...
    it('should disconnect and cancel stream', async () => {
        const mockReader = {
            read: vi.fn().mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({ done: true }), 100))),
            cancel: vi.fn(),
        };
        mockFetch.mockResolvedValueOnce({
            ok: true,
        ...
        const { result } = renderHook(() => useStreamingSSE({ url: '/api/stream' }));
        result.current.connect();
...
//# sourceMappingURL=use-streaming-sse.test.js.map