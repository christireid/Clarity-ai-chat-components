...
import { useTokenBudgetMonitor, getStatusColor, formatTokenUsage, createModelBudgetMonitor, isValidBudgetMonitorModel, getSupportedBudgetModels, } from '../use-token-budget-monitor';
...
vi.mock('../../utils/tokenization', () => ({
    ...
    countConversationTokens: vi.fn(async (messages) => ({
...
describe('useTokenBudgetMonitor', () => {
    ...
    describe('initialization', () => {
        it('should initialize with safe status and zero usage', () => {
            const { result } = renderHook(() => useTokenBudgetMonitor({
...
describe('createModelBudgetMonitor', () => {
    ...
    it('should include supported models in error message', () => {
        try {
            ...
            expect(e.message).toContain('claude-sonnet-4');
        }
    });
});
...
//# sourceMappingURL=use-token-budget-monitor.test.js.map