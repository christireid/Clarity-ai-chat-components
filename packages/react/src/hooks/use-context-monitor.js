 ...
 */
function calculateInformationDensity(text) {
 ...
 */
function calculateRecencyScore(messages, currentTime, halfLifeMinutes = 30) {
 ...
 */
function analyzeContext(messages, options, currentTime = Date.now()) {
    const breakdown = {
        systemPrompt: 0,
    ...
    const totalTokens = breakdown.systemPrompt +
 ...
 */
function generateWarnings(utilization, options) {
    const warnings = [];
    const now = Date.now();
 ...
 */
function generateRecommendations(utilization, options) {
    ...
    if (utilization.efficiency.informationDensity < 0.5) {
        const potentialSavings = Math.floor(utilization.totalTokens * (1 - utilization.efficiency.informationDensity) * 0.5);
 ...
 * ```
 */
export function useContextMonitor(options = {}) {
     ...
     */
    const analyzeMessages = React.useCallback((messages) => {
     ...
     */
    const getUtilization = React.useCallback((messages) => {
 ...
 */
export function createContextMonitorIntegration(monitor) {
    return {
        canSendMessage: (estimatedTokens) => {
...
//# sourceMappingURL=use-context-monitor.js.map