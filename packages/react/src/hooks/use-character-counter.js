 ...
 * ```
 */
export function useCharacterCounter({ value, maxLength, warningThreshold = 0.8, }) {
    const charCount = React.useMemo(() => value.length, [value]);
    const isOverLimit = React.useMemo(() => (maxLength ? charCount > maxLength : false), [charCount, maxLength]);
    const isNearLimit = React.useMemo(() => (maxLength ? charCount >= maxLength * warningThreshold : false), [charCount, maxLength, warningThreshold]);
...
//# sourceMappingURL=use-character-counter.js.map