...
export class MemoryEmbeddingCache {
    ...
    async set(text, model, embedding, ttl) {
        const key = this.getCacheKey(text, model);
        const entry = {
            key,
            text,
 ...
 */
export class LocalStorageEmbeddingCache {
    storageKey = 'clarity-embeddings-cache';
    statsData = {
        hits: 0,
        misses: 0,
    };
...
export class SemanticEmbeddingCache {
    ...
    _similarityThreshold; // Reserved for future semantic similarity implementation
