...
> {
  ...
  constructor(config: Partial<SecurityServiceConfig> = {}) {
    ...
    this.validationService = new ValidationService(this.config.validation, this.logger)
    this.sanitizationService = new SanitizationService(this.config.sanitization, this.logger)
    this.rateLimitingService = new RateLimitingService(this.config.rateLimiting, this.logger)
    this.auditService = new AuditService(this.config.audit, this.logger, this.config.outputDir)
    this.jailbreakService = new JailbreakService(this.config.jailbreak, this.logger)
  }
  ...
  validateConfig(): void {
    if (this.config.rateLimiting.enabled && this.config.rateLimiting.points <= 0) {
  ...
  async process(request: SecurityRequest): Promise<SecurityProcessingResult> {
    ...
    let rateLimit: RateLimitResult = { allowed: true, remainingPoints: -1, msBeforeNext: 0 }
    if (this.config.rateLimiting.enabled && request.checkRateLimit !== false) {
      ...
      if (!rateLimit.allowed) {
        return this.finalizeResult({
          validation: { isValid: false, errors: ['Rate limit exceeded'], warnings: [] },
    ...
    let sanitization: SanitizationResult = { sanitized: request.data, wasModified: false }
    if (this.config.sanitization.enabled && request.sanitize !== false && typeof request.data === 'string') {
  ...
  private calculateRisk(result: any, score: number): 'low' | 'medium' | 'high' | 'critical' {
   ...
   */
  async validateOutput(output: string, context?: SecurityContext): Promise<any> {
  ...
  async validateInput(input: string, context?: SecurityContext): Promise<SecurityResult> {
    ...
    return {
      ...
      reason: result.blocked ? (result.validation.errors[0] || 'Blocked by security policy') : undefined,
      action: result.blocked ? 'block' : (result.validation.warnings.length > 0 ? 'warn' : 'allow'),
  ...
  getMetrics(timeRange?: { start?: number; end?: number }): SecurityMetrics {
    ...
    const filtered = timeRange 
      ? metrics.filter(m => {
          ...
          return (!timeRange.start || t >= timeRange.start) && (!timeRange.end || t <= timeRange.end)
        })
      : metrics
   ...
   */
  getEvents(filter?: { type?: any; severity?: any; userId?: string }): SecurityEvent[] {
   ...
   * Subscribe to alerts
   */
  onAlert(handler: { handle: (event: SecurityEvent) => void | Promise<void> }): void {
...
class ValidationService {
  ...
  constructor(private config: ValidationConfig, private logger: any) {}
  ...
  async validate(data: any): Promise<ValidationResult> {
    ...
    if (this.config.promptInjection.enabled && typeof data === 'string') {
      try {
        if (!this.promptInjection) {
          const { EnhancedPromptInjectionGuardrail } = await import('../safety/prompt-injection-enhanced.js')
          this.promptInjection = new EnhancedPromptInjectionGuardrail(this.config.promptInjection.config || {})
        }
    ...
    if (this.config.contentModeration.enabled && typeof data === 'string') {
      try {
        ...
        if (contentResult.flagged) {
          if (contentResult.action === 'block') errors.push('Content policy violation')
    ...
    return {
      ...
      errors,
      warnings,
      details: { promptInjection: promptResult, contentModeration: contentResult }
    }
  }
}
...
class SanitizationService {
  ...
  constructor(private config: SanitizationConfig, private logger: any) {}
...
class RateLimitingService {
  ...
  constructor(private config: RateLimitingConfig, private logger: any) {
    if (config.type !== 'memory') {
      logger.warn(`Rate limiting type '${config.type}' not fully implemented, falling back to memory`)
    }
  }
...
class JailbreakService {
  ...
  constructor(private config: JailbreakConfig, private logger: any) {}

  private async getPreventer() {
    if (!this.preventer) {
      const { JailbreakPrevention } = await import('../safety/jailbreak-prevention.js')
...
class AuditService {
  constructor(private config: AuditConfig, private logger: any, private outputDir: string) {}

  async log(request: SecurityRequest, result: any, risk: string): Promise<string> {
    ...
    if (this.config.storage === 'file' && this.outputDir) {
      try {
        const auditData = {
          ...
          request: { identifier: request.identifier, requestId: request.requestId },
          result: { blocked: result.blocked, risk, score: result.securityScore },
          details: result
        }
        // Ensure we handle potential errors if directory doesn't exist (though base class should ensure it)
        writeFileSync(join(this.outputDir, id), JSON.stringify(auditData, null, 2))
      } catch (e) {
