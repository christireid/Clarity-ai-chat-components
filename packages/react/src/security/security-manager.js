...
export class SecurityManager {
    ...
    prepareMessages(messages) {
        ...
        if (!this.jailbreakPreventer) {
            ...
            this.jailbreakPreventer = new JailbreakPrevention(this.config.jailbreakPrevention.config || {});
        }
    ...
    async detectPromptInjection(input) {
        if (!this.promptInjectionGuard) {
            ...
            this.promptInjectionGuard = new EnhancedPromptInjectionGuardrail(this.config.promptInjection?.config || {});
        }
    ...
    getMetrics(timeRange) {
        ...
        return {
            ...
            promptInjectionRate: (eventsByType['prompt_injection_detected'] || 0) / Math.max(filteredEvents.length, 1),
            piiDetectionRate: (eventsByType['pii_detected'] || 0) / Math.max(filteredEvents.length, 1),
            contentModerationRate: (eventsByType['content_moderation_triggered'] || 0) / Math.max(filteredEvents.length, 1),
...
export class ConsoleAlertHandler {
    handle(event) {
        ...
        console.warn(`${emoji} Security Alert [${event.severity.toUpperCase()}]: ${event.type}`, event.details);
    }
}
