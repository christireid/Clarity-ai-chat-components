...
export const Skeleton = ({ variant = 'shimmer', width, height, rounded = 'md', className, style, ...props }) => {
    ...
    return (_jsx(Component, { className: cn('bg-muted/60 backdrop-blur-sm', roundedClasses[rounded], className), style: {
            width,
            height,
            ...shimmerStyle,
            ...style,
        }, ...(animation && { variants: animation, animate: 'animate' }), ...props }));
};
export const SkeletonText = ({ lines = 3, lineHeight = 16, gap = 8, lastLineWidth = 70, variant = 'shimmer', className, }) => {
    return (_jsx("div", { className: cn('space-y-2', className), style: { gap: `${gap}px` }, children: Array.from({ length: lines }).map((_, index) => (_jsx(Skeleton, { variant: variant, height: lineHeight, width: index === lines - 1 ? `${lastLineWidth}%` : '100%', rounded: "sm" }, index))) }));
};
...
export const SkeletonCard = ({ showImage = true, imageHeight = 200, showHeader = true, bodyLines = 3, showFooter = true, variant = 'shimmer', className, }) => {
    return (_jsxs("div", { className: cn('rounded-xl border-2 bg-card overflow-hidden shadow-sm', className), children: [showImage && (_jsx(Skeleton, { variant: variant, height: imageHeight, rounded: "none" })), _jsxs("div", { className: "p-6 space-y-4", children: [showHeader && (_jsxs("div", { className: "space-y-2", children: [_jsx(Skeleton, { variant: variant, width: "60%", height: 24, rounded: "sm" }), _jsx(Skeleton, { variant: variant, width: "40%", height: 16, rounded: "sm" })] })), _jsx(SkeletonText, { lines: bodyLines, variant: variant }), showFooter && (_jsxs("div", { className: "flex gap-2 pt-2", children: [_jsx(Skeleton, { variant: variant, width: 80, height: 36, rounded: "md" }), _jsx(Skeleton, { variant: variant, width: 80, height: 36, rounded: "md" })] }))] })] }));
};
export const SkeletonList = ({ count = 3, showAvatar = true, lines = 2, variant = 'shimmer', className, }) => {
    return (_jsx("div", { className: cn('space-y-4', className), children: Array.from({ length: count }).map((_, index) => (_jsxs("div", { className: "flex gap-3 items-start", children: [showAvatar && _jsx(SkeletonAvatar, { size: 40, variant: variant }), _jsx("div", { className: "flex-1", children: _jsx(SkeletonText, { lines: lines, variant: variant, lastLineWidth: 80 }) })] }, index))) }));
};
