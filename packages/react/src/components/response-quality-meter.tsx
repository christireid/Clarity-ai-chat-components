      ...
      <CardContent className="space-y-6">
        {overall !== undefined && (
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border/40 bg-[hsl(var(--surface-muted))] px-4 py-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground/70">
              ...
              <Badge variant={overall >= 0.75 ? 'success' : overall >= 0.5 ? 'warning' : 'destructive'}>
                {overall >= 0.75 ? 'Excellent' : overall >= 0.5 ? 'Needs review' : 'Low confidence'}
              </Badge>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          {normalizedMetrics.map((metric) => (
            <div key={metric.id} className="space-y-2 rounded-2xl border border-border/50 bg-[hsl(var(--surface-muted))] p-4 shadow-xs">
