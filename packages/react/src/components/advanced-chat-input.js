...
export const AdvancedChatInput = React.forwardRef(({ value, onChange, onSubmit, onSuggestionRequest, onFileUpload, maxFiles = 5, acceptedFileTypes = ['image/*', 'application/pdf', '.txt', '.doc', '.docx'], savedPrompts = [], disabled = false, placeholder = 'Type a message... Use @ for prompts, / for commands', maxLength, className, }, ref) => {
    const [attachments, setAttachments] = React.useState([]);
    const [suggestions, setSuggestions] = React.useState([]);
    const [selectedIndex, setSelectedIndex] = React.useState(0);
    const [showSuggestions, setShowSuggestions] = React.useState(false);
    const [isUploading, setIsUploading] = React.useState(false);
    const [triggerChar, setTriggerChar] = React.useState(null);
    ...
    const loadSuggestions = async (query, trigger) => {
