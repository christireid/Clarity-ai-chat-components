...
export function EmptyState({ icon, title, description, action, secondaryAction, className, }) {
    return (_jsxs(motion.div, { initial: { opacity: 0, y: 20, scale: 0.96 }, animate: { opacity: 1, y: 0, scale: 1 }, transition: { duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }, className: cn('flex flex-col items-center justify-center text-center px-6 py-12 space-y-8', className), children: [icon && (_jsx(motion.div, { initial: { scale: 0, rotate: -90 }, animate: { scale: 1, rotate: 0 }, transition: { duration: 0.6, ease: [0.25, 0.1, 0.25, 1], delay: 0.1, type: 'spring', stiffness: 280, damping: 22 }, className: "inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-gradient-to-br from-primary/15 to-primary/5 shadow-lg ring-1 ring-primary/25", children: icon })), _jsxs(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.4, ease: [0.25, 0.1, 0.25, 1], delay: 0.25 }, className: "space-y-3.5 max-w-lg", children: [_jsx("h3", { className: "text-2xl font-bold text-foreground leading-tight", children: title }), description && (_jsx("p", { className: "text-sm text-muted-foreground/90 leading-relaxed", children: description }))] }), (action || secondaryAction) && (_jsxs(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.4, ease: [0.25, 0.1, 0.25, 1], delay: 0.35 }, className: "flex flex-wrap gap-3 justify-center", children: [action && (_jsx(InteractiveButton, { variant: action.variant || 'primary', onClick: action.onClick, children: action.label })), secondaryAction && (_jsx(InteractiveButton, { variant: "ghost", onClick: secondaryAction.onClick, children: secondaryAction.label }))] }))] }));
}
...
export function EmptyChatState({ onStartChat, onSuggestionSelect, suggestions = DEFAULT_STARTER_PROMPTS, showSuggestions = true, className, }) {
    ...
    return (_jsxs(motion.div, { initial: { opacity: 0, y: 20, scale: 0.96 }, animate: { opacity: 1, y: 0, scale: 1 }, transition: {
            ...
            ease: [0.25, 0.1, 0.25, 1]
                    ...
                    stiffness: 280,
                    damping: 22
                }, className: "inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-gradient-to-br from-primary/15 to-primary/5 shadow-lg ring-1 ring-primary/25", children: _jsx(BotIcon, { size: 40, className: "text-primary" }) }), _jsxs(motion.div, { initial: { opacity: 0, y: getMotionSafeValue(prefersReducedMotion, 10, 0) }, animate: { opacity: 1, y: 0 }, transition: {
                    ...
                    delay: getMotionSafeDuration(prefersReducedMotion, 0.25)
                            ...
                            : 'Send a message to begin chatting with the AI assistant. I\'m here to help with your questions and tasks.' })] }), showSuggestions && suggestions.length > 0 && (_jsx(motion.div, { initial: { opacity: 0, y: getMotionSafeValue(prefersReducedMotion, 10, 0) }, animate: { opacity: 1, y: 0 }, transition: {
                    duration: getMotionSafeDuration(prefersReducedMotion, 0.4),
                    ease: [0.25, 0.1, 0.25, 1],
                    delay: getMotionSafeDuration(prefersReducedMotion, 0.35)
                }, className: "w-full", children: _jsx(PromptSuggestions, { suggestions: suggestions, onSelect: onSuggestionSelect || (() => { }), suggestionType: "starter", layout: "cards", maxSuggestions: 6 }) })), onStartChat && !showSuggestions && (_jsx(motion.div, { initial: { opacity: 0, y: getMotionSafeValue(prefersReducedMotion, 10, 0) }, animate: { opacity: 1, y: 0 }, transition: {
                    duration: getMotionSafeDuration(prefersReducedMotion, 0.4),
                    ease: [0.25, 0.1, 0.25, 1],
                    delay: getMotionSafeDuration(prefersReducedMotion, 0.35)
