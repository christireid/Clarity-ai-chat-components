...
import { ANIMATION_DURATION, EASING_FRAMER, duration, } from '../../animations/constants';
 ...
 * ```
 */
export function Message({ message, onFeedback, onRetry, onEdit, onRegenerate, onDelete, onStopGeneration, isEditing = false, onSaveEdit, onCancelEdit, showAvatar = true, showTimestamp = true, className, isGroupStart = true, isGroupEnd = true, isGrouped = false, errorDetails, ref, }) {
    ...
    React.useEffect(() => {
        if (isEditing && !wasEditing) {
            announce('Editing message. Press Escape to cancel or use the save button.', {
                assertive: true,
            });
        }
    ...
    const handleFeedback = (type, comment) => {
    ...
    // Handle saving edits
    const handleSaveEdit = React.useCallback((newContent) => {
            ...
            pre: ({ children, ...props }) => {
                ...
                React.Children.forEach(children, (child) => {
                    if (React.isValidElement(child) && child.props) {
                        ...
                        codeString = props['data-code-string'] || '';
                        if (!codeString && props.children) {
                            ...
                            codeString = extractText(props.children);
                        }
                    }
                });
                return (_jsxs("div", { className: "relative group/code my-4", children: [_jsx("pre", { className: cn('relative overflow-x-auto p-4', 'bg-gradient-to-br from-muted/60 to-muted/40', 'border border-border/50', 'rounded-xl', 'shadow-sm', 'transition-shadow duration-200', 'group-hover/code:shadow-md'), ...props, children: children }), codeString && (_jsx(CopyButton, { text: codeString, className: "absolute top-2.5 right-2.5 opacity-0 group-hover/code:opacity-100 transition-all duration-200 translate-y-1 group-hover/code:translate-y-0" }))] }));
            },
    ...
    const messageTime = message.createdAt
        ...
        : '';
    const messageStatus = message.status === 'streaming'
        ? ', currently streaming'
    ...
    return (_jsxs(motion.div, { ref: ref, role: "article", "aria-label": ariaLabel, "aria-describedby": contentId, initial: {
            opacity: 0,
        ...
        }, onMouseEnter: () => setIsHovered(true), onMouseLeave: () => setIsHovered(false), onFocus: () => setIsFocusWithin(true), onBlur: (e) => {
            // Only set false if focus moved outside this component
            if (!e.currentTarget.contains(e.relatedTarget)) {
                setIsFocusWithin(false);
            }
        }, tabIndex: 0, className: cn('group flex gap-3.5 rounded-2xl transition-all duration-200 ease-out', 
        // Reduced padding for grouped messages
        isGrouped && !isGroupStart && !isGroupEnd ? 'px-4 py-1.5' : 'p-4', isUser && 'flex-row-reverse', isHovered && 'bg-muted/30 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)]', className), children: [showAvatar && isGroupStart ? (_jsx(motion.div, { initial: { scale: 0.8 }, animate: { scale: 1 }, transition: {
                    type: 'spring',
                    stiffness: 500,
                    damping: 25,
                    delay: 0.1,
            ...
            _jsx("div", { className: "w-10 flex-shrink-0", "aria-hidden": "true" })) : null, _jsxs("div", { className: cn('flex-1 space-y-2.5', isUser && 'flex flex-col items-end'), children: [isGroupStart && (_jsxs("div", { className: cn('flex items-center', isUser ? 'gap-2 flex-row-reverse' : 'gap-2'), children: [_jsx("span", { className: "font-semibold text-sm whitespace-nowrap", children: isUser ? 'You' : 'AI Assistant' }), showTimestamp && (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-muted-foreground/50", children: "\u00B7" }), _jsx(motion.span, { initial: { opacity: 0 }, animate: { opacity: isHovered ? 1 : 0.7 }, transition: { duration: duration('normal') }, className: "text-xs text-muted-foreground/90 whitespace-nowrap", children: formatRelativeTime(message.createdAt) })] })), message.status === 'sending' && (_jsx(Badge, { variant: "secondary", dot: true, children: "Sending" })), message.status === 'error' && (_jsx(Badge, { variant: "destructive", children: "Error" }))] })), _jsx("div", { id: contentId, className: cn(
                        ...
                        ]), children: isUser ? (isEditing ? (_jsx(EditableMessageContent, { content: message.content, isEditing: isEditing, onSave: handleSaveEdit, onCancel: handleCancelEdit })) : (_jsx("p", { className: "m-0 whitespace-pre-wrap text-primary-foreground", children: message.content }))) : (_jsxs("div", { className: cn(isStreaming && 'clarity-streaming-text'), children: [_jsx(ReactMarkdown, { remarkPlugins: remarkPlugins, rehypePlugins: rehypePlugins, components: markdownComponents, children: message.content }), isStreaming && (_jsx("span", { "aria-hidden": "true", className: "clarity-streaming-cursor" }))] })) }), message.status === 'error' && errorDetails && (_jsx("div", { role: "alert", "aria-live": "assertive", children: _jsx(ErrorMessage, { error: errorDetails, onRetry: onRetry, compact: isGrouped, maxRetryAttempts: 3 }) })), message.attachments && message.attachments.length > 0 && (_jsx("div", { className: "flex flex-wrap gap-2", children: message.attachments.map((attachment) => (_jsx(Badge, { variant: "outline", children: attachment.name }, attachment.id))) })), (isUser || isAssistant) && !isEditing && (_jsx(MessageActions, { messageContent: message.content, messageId: message.id, role: message.role, feedbackGiven: feedbackGiven, showConfetti: showConfetti, hasError: message.status === 'error', isStreaming: isStreaming, onFeedback: handleFeedback, onRetry: onRetry, onEdit: onEdit, onRegenerate: onRegenerate, onDelete: onDelete, onStopGeneration: onStopGeneration, show: isHovered || isFocusWithin || !!feedbackGiven || isStreaming, editButtonRef: editButtonRef })), _jsx(MessageMetadata, { message: message })] })] }));
}
