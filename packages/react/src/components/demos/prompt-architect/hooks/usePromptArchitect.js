...
import { compileTemplate, buildMessagesArray, getMissingVariables, } from '../utils/template-compiler';
// =============================================================================
// CONSTANTS
// =============================================================================
...
// REDUCER
// =============================================================================
function promptArchitectReducer(state, action) {
    switch (action.type) {
        ...
        case 'DELETE_VERSION':
            return {
                ...
                activeVersionId: state.activeVersionId === action.payload
                    ? null
 ...
 * ```
 */
export function usePromptArchitect(options) {
    const { autoSave = true, storageKey = DEFAULT_STORAGE_KEY, autoSaveDelay = DEFAULT_AUTO_SAVE_DELAY, onRun, } = options ?? {};
    // Initialize reducer
    const [state, dispatch] = React.useReducer(promptArchitectReducer, options, createInitialState);
    // ==========================================================================
    ...
    React.useEffect(() => {
        const timeoutId = setTimeout(() => {
            const compiledSystem = compileTemplate(state.systemPrompt, state.variableValues, { keepUnresolved: false });
            const compiledUser = compileTemplate(state.userPromptTemplate, state.variableValues, { keepUnresolved: false });
            ...
            const variableTokens = Object.values(state.variableValues).reduce((sum, value) => sum + estimateTokens(value, state.selectedModel), 0);
            const totalTokens = systemTokens + userTokens;
            const modelConfig = getModelConfig(state.selectedModel);
            const maxTokens = modelConfig?.maxTokens ?? 8192;
            const utilizationPercent = Math.min(100, Math.round((totalTokens / maxTokens) * 100));
            // Estimate output tokens as 25% of max output (reasonable estimate)
            const estimatedOutputTokens = Math.min((modelConfig?.maxOutputTokens ?? 2048) * 0.25, 2000);
            // Calculate cost estimates
            const costs = estimateCost(totalTokens, estimatedOutputTokens, state.selectedModel);
            dispatch({
    ...
    // COMPUTED VALUES
    // ==========================================================================
    const compiledSystemPrompt = React.useMemo(() => compileTemplate(state.systemPrompt, state.variableValues, {
        keepUnresolved: true,
    }), [state.systemPrompt, state.variableValues]);
    const compiledUserPrompt = React.useMemo(() => compileTemplate(state.userPromptTemplate, state.variableValues, {
        keepUnresolved: true,
    }), [state.userPromptTemplate, state.variableValues]);
    const messagesArray = React.useMemo(() => buildMessagesArray(compileTemplate(state.systemPrompt, state.variableValues, {
    ...
    const missingVariables = React.useMemo(() => {
        const systemMissing = getMissingVariables(state.systemPrompt, state.variableValues);
        const userMissing = getMissingVariables(state.userPromptTemplate, state.variableValues);
    ...
    // ACTIONS
    // ==========================================================================
    const setSystemPrompt = React.useCallback((value) => {
        dispatch({ type: 'SET_SYSTEM_PROMPT', payload: value });
    }, []);
    const setUserPromptTemplate = React.useCallback((value) => {
        dispatch({ type: 'SET_USER_PROMPT_TEMPLATE', payload: value });
    }, []);
    const setVariableValue = React.useCallback((name, value) => {
        dispatch({ type: 'SET_VARIABLE_VALUE', payload: { name, value } });
    }, []);
    const setAllVariables = React.useCallback((values) => {
    ...
    const saveVersion = React.useCallback((name, description) => {
        dispatch({ type: 'SAVE_VERSION', payload: { name, description } });
    }, []);
    const loadVersion = React.useCallback((id) => {
    ...
    const setDebugViewMode = React.useCallback((mode) => {
    ...
    const setModel = React.useCallback((model) => {
    ...
    const runPrompt = React.useCallback(async () => {
        ...
        const systemContent = compileTemplate(state.systemPrompt, state.variableValues, { keepUnresolved: false });
        ...
        const userContent = compileTemplate(state.userPromptTemplate, state.variableValues, { keepUnresolved: false });
        if (userContent.trim()) {
            dispatch({
                type: 'ADD_PREVIEW_MESSAGE',
                payload: {
        ...
        try {
            ...
            if (saved) {
                ...
                if (parsed.versions && Array.isArray(parsed.versions)) {
                    parsed.versions = parsed.versions.map((v) => ({
                        ...v,
 ...
 */
async function simulateStreamingResponse(dispatch, structuredOutputMode) {
...
//# sourceMappingURL=usePromptArchitect.js.map