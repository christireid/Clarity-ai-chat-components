 ...
 * ```
 */
export function inferVariableType(name: string): InferredFieldType {
 ...
 * ```
 */
export function extractVariables(template: string): VariableExtractionResult {
  ...
  for (const v of variables) {
    if (v.name.length === 1) {
      warnings.push(
        `Variable "${v.name}" has a very short name - consider a more descriptive name`
      )
    }
    if (v.name.length > 30) {
      warnings.push(
        `Variable "${v.name}" has a very long name - consider shortening it`
      )
    }
  }
 ...
 * ```
 */
export function extractVariablesFromMultiple(
  templates: string[]
): VariableExtractionResult {
  ...
  const variables = Array.from(allVariables.values()).sort((a, b) =>
...
export function getTemplateSegments(
  ...
  | { type: 'variable'; content: string; variableName: string }
> {
...
export function getVariablePositions(
  template: string
): Array<{ start: number; end: number; variableName: string }> {
  const positions: Array<{ start: number; end: number; variableName: string }> =
    []
