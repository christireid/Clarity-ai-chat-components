...
export const MessageList = memo(function MessageList({
  ...
  const containerVariants = useMemo(() => createStaggerContainerVariant('normal', 0), [])
  const itemVariants = useMemo(() => createStaggerChildVariant('slide', 'fast'), [])

  // Show empty state if no messages and not loading - memoized
  const showEmptyState = useMemo(() => messages.length === 0 && !isLoading && emptyState, [messages.length, isLoading, emptyState])
