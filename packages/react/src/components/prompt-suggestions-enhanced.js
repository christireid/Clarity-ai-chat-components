 ...
 * ```
 */
export function usePromptSuggestionsEnhanced(messages, config) {
    const defaultConfig = {
    ...
    const [abVariant, setAbVariant] = React.useState('control');
    // Stats tracking
    ...
    const getConversationContext = React.useCallback(() => {
        ...
        const topics = [];
        if (/(code|function|program|debug)/i.test(lastContent))
            topics.push('programming');
        if (/(design|ui|ux|interface)/i.test(lastContent))
            topics.push('design');
        if (/(data|analysis|chart|graph)/i.test(lastContent))
            topics.push('analytics');
        if (/(help|question|how|what|why)/i.test(lastContent))
            topics.push('help');
        if (/(error|bug|issue|problem)/i.test(lastContent))
        ...
        if (positiveWords.test(lastContent))
            sentiment = 'positive';
        if (negativeWords.test(lastContent))
            sentiment = 'negative';
        return {
            topics,
            entities: [],
     ...
     */
    const generateBaseSuggestions = React.useCallback(() => {
     ...
     */
    const rankSuggestionsML = React.useCallback(async (suggestions) => {
        ...
        return suggestions.map((suggestion) => {
            ...
            if (finalConfig.features.conversationContext &&
    ...
    const suggestions = React.useMemo(async () => {
        ...
        if (finalConfig.rankingModel.type === 'ml' ||
     ...
     */
    const trackInteraction = React.useCallback((suggestion, selected) => {
        const context = getConversationContext();
        const interaction = {
        ...
        if (finalConfig.trackEffectiveness) {
            ...
            setStats((prev) => {
                ...
                return {
                    ...
                    averageConfidence: (prev.averageConfidence * prev.totalInteractions + (suggestion.confidence || 0)) /
                        newTotal,
                };
            });
        }
 ...
 * ```
 */
export function PromptSuggestionsEnhanced({ messages, onSelect, config, ...promptSuggestionsProps }) {
    const { suggestions, trackInteraction } = usePromptSuggestionsEnhanced(messages, config);
    const handleSelect = React.useCallback((suggestion) => {
    ...
    return (_jsx(PromptSuggestions, { ...promptSuggestionsProps, suggestions: resolvedSuggestions, onSelect: handleSelect, messages: messages }));
}
PromptSuggestionsEnhanced.displayName = 'PromptSuggestionsEnhanced';
//# sourceMappingURL=prompt-suggestions-enhanced.js.map