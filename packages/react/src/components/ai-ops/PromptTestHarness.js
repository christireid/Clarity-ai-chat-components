...
export const PromptTestHarness = ({ datasetName, datasets = [], variants, tests, onRunAll, onRunVariant, onSelectDataset, isRunning = false, className, }) => {
    ...
    return (_jsxs(Card, { className: cn('border-border/60 bg-[hsl(var(--surface-elevated))] shadow-sm', className), children: [_jsxs(CardHeader, { className: "flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between", children: [_jsxs("div", { className: "space-y-2", children: [_jsx(CardTitle, { className: "text-lg font-semibold text-foreground", children: "Prompt regression harness" }), _jsx(CardDescription, { className: "text-sm text-muted-foreground/80", children: "Compare prompt variants across curated datasets before shipping changes." }), datasetName && (_jsxs(Badge, { variant: "outline", className: "text-[11px]", children: ["Dataset \u2022 ", datasetName] }))] }), _jsxs("div", { className: "flex flex-col gap-3 sm:items-end", children: [datasets.length > 0 && (_jsx("select", { value: selectedDataset ?? '', onChange: (event) => handleDatasetChange(event.target.value), className: "min-w-[200px] rounded-lg border border-border/60 bg-background px-3 py-2 text-sm text-foreground shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40", children: datasets.map((ds) => (_jsx("option", { value: ds.id, children: ds.name }, ds.id))) })), _jsxs("div", { className: "flex flex-wrap gap-2", children: [_jsx(Button, { variant: "surface", size: "sm", onClick: onRunAll, disabled: isRunning, children: "Run all variants" }), _jsx("select", { value: selectedVariant ?? '', onChange: (event) => handleVariantChange(event.target.value), className: "min-w-[200px] rounded-lg border border-border/60 bg-background px-3 py-2 text-sm text-foreground shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40", children: variants.map((variant) => (_jsx("option", { value: variant.id, children: variant.label }, variant.id))) })] })] })] }), _jsx(CardContent, { className: "overflow-x-auto", children: _jsxs("table", { className: "min-w-[720px] text-sm", children: [_jsx("thead", { className: "text-left text-xs uppercase tracking-wide text-muted-foreground/70", children: _jsxs("tr", { children: [_jsx("th", { className: "w-[24px] py-2", children: "#" }), _jsx("th", { className: "py-2", children: "Input" }), _jsx("th", { className: "py-2", children: "Status" }), _jsx("th", { className: "py-2", children: "Output" }), _jsx("th", { className: "py-2", children: "Expected" }), _jsx("th", { className: "py-2", children: "Latency" }), _jsx("th", { className: "py-2", children: "Cost" })] }) }), _jsx("tbody", { children: tests.map((test, index) => {
