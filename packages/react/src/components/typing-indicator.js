...
export function TypingIndicator({ showAvatar = true, avatarSrc, avatarFallback = 'AI', label = 'AI is typing', variant = 'dots', className, }) {
    ...
    return (_jsxs(motion.div, { initial: { opacity: 0, y: 10, scale: 0.95 }, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 0, y: -10, scale: 0.95 }, transition: {
            ...
            ease: [0.25, 0.1, 0.25, 1],
                ...
                }, children: _jsx(Avatar, { src: avatarSrc, alt: "AI Assistant", fallback: avatarFallback, className: "flex-shrink-0" }) })), _jsxs(motion.div, { initial: { scale: 0.9, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: {
                    duration: getMotionSafeDuration(prefersReducedMotion, 0.2),
                    delay: getMotionSafeDuration(prefersReducedMotion, 0.1),
                }, className: cn('flex items-center gap-1.5 px-4 py-3 rounded-xl bg-muted/60 border border-border/40 shadow-md', 'min-w-[70px]'), children: [variant === 'dots' && _jsx(DotsAnimation, { prefersReducedMotion: prefersReducedMotion }), variant === 'pulse' && _jsx(PulseAnimation, { prefersReducedMotion: prefersReducedMotion }), variant === 'wave' && _jsx(WaveAnimation, { prefersReducedMotion: prefersReducedMotion })] })] }));
}
TypingIndicator.displayName = 'TypingIndicator';
/**
 * Classic bouncing dots animation
 */
function DotsAnimation({ prefersReducedMotion }) {
    if (prefersReducedMotion) {
        return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx("div", { className: "w-2 h-2 rounded-full bg-muted-foreground/60" }, i))) }));
    }
    return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx(motion.div, { animate: {
                y: [-2, -6, -2],
                opacity: [0.6, 1, 0.6],
            }, transition: {
                duration: 0.6,
                ...
                delay: i * 0.15,
                ease: 'easeInOut',
            }, className: "w-2 h-2 rounded-full bg-muted-foreground" }, i))) }));
}
/**
 * Pulse animation (expanding circles)
 */
function PulseAnimation({ prefersReducedMotion }) {
    if (prefersReducedMotion) {
        return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx("div", { className: "w-2 h-2 rounded-full bg-muted-foreground/60" }, i))) }));
    }
    return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx(motion.div, { animate: {
 ...
 */
function WaveAnimation({ prefersReducedMotion }) {
    if (prefersReducedMotion) {
        return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx("div", { className: "w-2 h-2 rounded-full bg-muted-foreground/60" }, i))) }));
    }
    return (_jsx("div", { className: "flex gap-1.5", children: [0, 1, 2].map((i) => (_jsx(motion.div, { animate: {
