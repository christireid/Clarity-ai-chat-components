 ...
 */
export const CalendarIntegration = React.forwardRef(function CalendarIntegration({ initialEvents = [], actionItems: initialActionItems = [], dateRange, showActionItems = true, showAvailability = false, onEventCreate, onEventUpdate: _onEventUpdate, onEventDelete, onActionToEvent, fetchEvents, fetchAvailability, className, ...props }, ref) {
        ...
        try {
            ...
            if (isMounted.current) {
                setState(prev => ({
                    ...
                    actionItems: prev.actionItems.map(a => a.id === actionItem.id ? { ...a, status: 'completed' } : a),
                    loading: false,
                }));
            }
        }
