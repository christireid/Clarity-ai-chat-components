...
export function NetworkStatus({ status: externalStatus, show = true, position = 'top-right', showDetails = false, onStatusChange, pingEndpoint = '/api/ping', pingInterval = 30000, slowThreshold = 1000, className = '', }) {
    ...
    return (_jsx("div", { className: `fixed ${positionClasses[position]} z-[var(--z-toast)] ${className}`, role: "status", "aria-live": "polite", "aria-label": `Network status: ${config.label}`, children: _jsxs("div", { className: "flex items-center gap-2 px-3 py-2 bg-card rounded-lg shadow-xl border", children: [_jsxs("div", { className: "relative flex h-3 w-3", children: [_jsx("div", { className: `absolute h-3 w-3 ${config.color} rounded-full ${status === 'online' ? 'animate-ping opacity-75' : ''}` }), _jsx("div", { className: `relative h-3 w-3 ${config.color} rounded-full` })] }), _jsx("span", { className: `text-sm font-semibold ${config.textColor}`, children: config.label }), showDetails && (latency !== null || downlinkSpeed !== null) && (_jsxs("div", { className: "flex items-center gap-2 text-xs text-muted-foreground border-l pl-2", children: [latency !== null && (_jsxs("span", { className: "font-mono", children: [latency.toFixed(0), "ms"] })), downlinkSpeed !== null && (_jsxs("span", { className: "font-mono", children: [downlinkSpeed.toFixed(1), " Mbps"] }))] }))] }) }));
}
