...
export function TimeSeparator({ children, className }) {
    return (_jsxs(motion.div, { initial: { opacity: 0, y: -10 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.3, ease: 'easeOut' }, className: cn('relative flex items-center justify-center py-4', className), role: "separator", "aria-label": `Messages from ${children}`, children: [_jsx("div", { className: "flex-1 h-px bg-gradient-to-r from-transparent via-border to-border" }), _jsx(motion.div, { initial: { scale: 0.9, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: { delay: 0.1, duration: 0.2 }, className: cn('px-4 py-1.5 mx-4', 'text-xs font-bold uppercase tracking-wider', 'text-muted-foreground/90', 'bg-muted/60 backdrop-blur-md', 'border border-border/40', 'rounded-full shadow-md'), children: children }), _jsx("div", { className: "flex-1 h-px bg-gradient-to-l from-transparent via-border to-border" })] }));
}
