 ...
 * ```
 */
export function ChatInput({ value, onChange, onSubmit, placeholder = 'Type a message...', disabled = false, maxLength, showCharCounter = true, warningThreshold = 0.8, animateHeight = true, glowOnFocus = true, className, }) {
    // Runtime validation
    if (typeof value !== 'string') {
        throw new Error('ChatInput: "value" prop must be a string.\n\n' +
            'Example:\n' +
    ...
    if (typeof onChange !== 'function') {
        throw new Error('ChatInput: "onChange" prop must be a function.\n\n' +
            'Example:\n' +
            '  <ChatInput value={input} onChange={(val) => setInput(val)} onSubmit={handleSubmit} />\n\n' +
            'For more help, see: https://clarity-chat.dev/docs/components');
    }
    if (typeof onSubmit !== 'function') {
        throw new Error('ChatInput: "onSubmit" prop is required and must be a function.\n\n' +
            'Example:\n' +
    ...
    return (_jsxs(motion.div, { className: cn('relative flex flex-col gap-3 px-5 py-4 border-t border-border/80 bg-background/95 backdrop-blur-md shadow-[0_-2px_8px_rgba(15,23,42,0.06)]', className), initial: "idle", animate: isFocused ? 'focused' : 'idle', variants: containerVariants, children: [_jsxs("div", { className: "flex gap-3 items-end", children: [_jsxs(motion.div, { className: "flex-1 relative", layout: animateHeight, transition: { duration: 0.2, ease: 'easeOut' }, children: [_jsx(Textarea, { ref: textareaRef, value: value, onChange: handleChange, onKeyDown: handleKeyDown, onFocus: handleFocus, onBlur: handleBlur, placeholder: placeholder, disabled: disabled, maxLength: maxLength, autoResize: true, maxRows: 6, variant: isOverLimit ? 'error' : 'default', className: cn('transition-all duration-200 shadow-sm border-border/40', isFocused && glowOnFocus && 'ring-2 ring-ring/30 shadow-md border-ring/50', isOverLimit && 'animate-[shake_0.4s_ease-in-out]') }), maxLength && showCharCounter && (_jsx(AnimatePresence, { children: charCount > 0 && (_jsxs(motion.div, { initial: { opacity: 0, y: 5 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: 5 }, className: "absolute bottom-3 right-3 flex flex-col items-end gap-1.5", children: [_jsx("div", { className: "w-20 h-1.5 bg-muted/60 rounded-full overflow-hidden shadow-inner", children: _jsx(motion.div, { className: cn('h-full', progressColor), initial: { width: 0 }, animate: {
                                                    width: `${Math.min((charCount / maxLength) * 100, 100)}%`,
                                                }, transition: { duration: 0.2 } }) }), _jsxs(motion.div, { className: cn('text-xs tabular-nums', counterColor), animate: isOverLimit ? {
                                                scale: [1, 1.05, 1],
                                                transition: {
                                                    duration: 1.5,
                                                    repeat: Infinity,
                                            ...
                                            } : undefined, children: [charCount, "/", maxLength] })] })) }))] }), _jsx(Button, { onClick: handleSubmit, disabled: disabled || !hasContent || isOverLimit, state: buttonState, size: "icon", className: cn('transition-all duration-200 ease-out shrink-0 h-11 w-11 rounded-xl shadow-sm', hasContent && !isOverLimit
                            ? 'bg-primary text-primary-foreground hover:bg-primary/90 hover:shadow-md hover:scale-[1.02] active:scale-[0.98]'
                            : 'bg-muted/60 text-muted-foreground border border-border/40'), "aria-label": buttonState === 'loading'
                            ? 'Sending message...'
                            : buttonState === 'success'
                                ? 'Message sent!'
                                : buttonState === 'error'
                                    ? 'Failed to send'
                                    : 'Send message', children: _jsx(AnimatePresence, { mode: "wait", children: buttonState === 'idle' && (_jsx(motion.div, { initial: { scale: 0.8, opacity: 0 }, animate: { scale: 1, opacity: 1 }, exit: { scale: 0.8, opacity: 0 }, transition: { duration: 0.15 }, children: _jsx(SendIcon, { size: 19 }) }, "send")) }) })] }), _jsx(AnimatePresence, { children: isOverLimit && (_jsxs(motion.p, { initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: 'auto' }, exit: { opacity: 0, height: 0 }, className: "text-xs text-destructive px-1 font-medium", children: ["Message exceeds maximum length by ", charCount - (maxLength || 0), ' ', "characters"] })) }), _jsx(AnimatePresence, { children: isFocused && !hasContent && (_jsxs(motion.p, { initial: { opacity: 0, y: -5 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -5 }, className: "text-xs text-muted-foreground/80 px-1 mt-1", children: ["Press", ' ', _jsx("kbd", { className: "px-2 py-0.5 text-[10px] font-semibold border border-border/60 rounded-md bg-muted/50 shadow-sm", children: "Enter" }), ' ', "to send \u2022", ' ', _jsx("kbd", { className: "px-2 py-0.5 text-[10px] font-semibold border border-border/60 rounded-md bg-muted/50 shadow-sm", children: "Shift + Enter" }), ' ', "for new line"] })) })] }));
}
