...
const BookmarkPlusIcon = BookmarkPlus as React.ComponentType<{ className?: string }>
...
const SlidersIcon = SlidersHorizontal as React.ComponentType<{ className?: string }>
 ...
 */
export type SortOption = 'relevance' | 'newest' | 'oldest' | 'longest' | 'shortest'
    ...
    const isPresetActive = (preset: FilterPreset) => {
      return Object.entries(preset.filters).every(([key, value]) => {
        ...
        if (key === 'dateRange' && value) {
          return (
            filters.dateRange?.start?.getTime() === (value as any).start?.getTime() &&
              ...
              {enableSorting && (
                <Popover>
                  ...
                  <PopoverContent className="w-40 p-1" align="end">
                    {(['relevance', 'newest', 'oldest', 'longest', 'shortest'] as SortOption[]).map(
                      (option) => (
                        <button
                          key={option}
                              ...
                              >
                                ...
                                <div className="text-xs text-muted-foreground">
                                  ...
                                  {Object.keys(search.filters).filter(
                                    (k) => k !== 'query' && search.filters[k as keyof SearchFilters]
                                  ).length > 0 && ' + filters'}
                                </div>
                              </button>
                              <Button
            ...
            >
              ...
              {filters.model && (
                <Badge variant="secondary" className="gap-1 pr-1">
                  Model: {filters.model}
                  <button
                    onClick={() => setFilters((prev) => ({ ...prev, model: undefined }))}
                    className="ml-0.5 hover:text-destructive"
                  >
                    <XIcon className="h-3 w-3" />
                  </button>
              ...
              {filters.hasAttachments && (
                <Badge variant="secondary" className="gap-1 pr-1">
                  ...
                  <button
                    onClick={() =>
                      setFilters((prev) => ({ ...prev, hasAttachments: undefined }))
                    }
                    className="ml-0.5 hover:text-destructive"
                  >
                    <XIcon className="h-3 w-3" />
              ...
              {filters.hasErrors && (
                <Badge variant="secondary" className="gap-1 pr-1">
                  Has errors
                  <button
                    onClick={() => setFilters((prev) => ({ ...prev, hasErrors: undefined }))}
                    className="ml-0.5 hover:text-destructive"
                  >
                    <XIcon className="h-3 w-3" />
                  </button>
