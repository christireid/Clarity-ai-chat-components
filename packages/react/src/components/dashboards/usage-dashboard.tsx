        ...
        <ScrollArea className="h-full">
          <div className="space-y-6 pb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(Object.keys(stats.metrics) as Array<keyof UsageMetrics>).map(
                  (key, index) => {
                    ...
                    const MetricWrapper = prefersReducedMotion ? 'div' : motion.div
                    const IconWrapper = prefersReducedMotion ? 'span' : motion.span
            ...
            <div>
              ...
              <div className="space-y-2">
                {stats.costs.breakdown.map((item, index) => {
                  const CostWrapper = prefersReducedMotion ? 'div' : motion.div
                  return (
                    <CostWrapper
                      key={index}
                      {...(prefersReducedMotion ? {} : {
            ...
            {limits.some((l) => (l.current / l.limit) * 100 > 80) && (
              prefersReducedMotion ? (
                <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-500/30 rounded-xl shadow-[0_2px_8px_rgba(245,158,11,0.1)]">
