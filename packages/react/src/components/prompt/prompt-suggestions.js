...
export function PromptSuggestions({ suggestions, onSelect, messages: _messages = [], suggestionType = 'starter', layout = 'chips', isLoading = false, maxSuggestions = 6, emptyState, showCategories = false, className, }) {
    ...
    // Loading skeleton
    if (isLoading) {
        return (_jsx("div", { className: cn('flex flex-wrap gap-2.5', className), children: Array.from({ length: maxSuggestions }).map((_, i) => (_jsx(Skeleton, { width: 112, height: 36, rounded: "full" }, i))) }));
    }
    // Empty state
    ...
    if (layout === 'chips') {
        return (_jsxs("div", { className: cn('space-y-3.5', className), children: [showCategories && categories.length > 0 && (_jsxs("div", { className: "flex flex-wrap gap-2.5", children: [_jsx(Button, { variant: selectedCategory === 'all' ? 'default' : 'outline', size: "sm", onClick: () => setSelectedCategory('all'), children: "All" }), categories.map((cat) => (_jsx(Button, { variant: selectedCategory === cat ? 'default' : 'outline', size: "sm", onClick: () => setSelectedCategory(cat), children: cat }, cat)))] })), _jsx("div", { className: "flex flex-wrap gap-2.5", children: _jsx(AnimatePresence, { mode: "popLayout", children: processedSuggestions.map((suggestion, index) => (_jsx(motion.div, { initial: { opacity: 0, scale: 0.9, y: 10 }, animate: { opacity: 1, scale: 1, y: 0 }, exit: { opacity: 0, scale: 0.9, y: -10 }, transition: {
                                ...
                                stiffness: 300,
                                delay: index * 0.05,
                            }, children: _jsxs(Button, { variant: "outline", size: "sm", onClick: () => onSelect(suggestion), className: cn('group relative rounded-full px-4 py-2', 'bg-card/80 backdrop-blur-sm', 'border-border/50', 'hover:bg-gradient-to-r hover:from-primary hover:to-primary/90', 'hover:text-primary-foreground hover:border-primary/50', 'transition-all duration-200 ease-out', 'shadow-sm hover:shadow-[0_4px_16px_-4px_hsl(var(--primary)/0.3)]', 'hover:-translate-y-0.5 hover:scale-[1.02]', 'active:scale-[0.98] active:translate-y-0'), "aria-label": suggestion.label || suggestion.text, children: [suggestion.icon && (_jsx("span", { className: "mr-2", children: suggestion.icon })), _jsx("span", { children: suggestion.label || suggestion.text }), suggestion.confidence !== undefined && (_jsxs(Badge, { variant: "secondary", className: "ml-2 text-xs", children: [Math.round(suggestion.confidence * 100), "%"] }))] }) }, suggestion.id))) }) })] }));
    }
    // Render cards layout
    if (layout === 'cards') {
        return (_jsx("div", { className: cn('grid grid-cols-1 md:grid-cols-2 gap-3.5', className), children: _jsx(AnimatePresence, { mode: "popLayout", children: processedSuggestions.map((suggestion, index) => (_jsx(motion.div, { initial: { opacity: 0, y: 10, scale: 0.96 }, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 0, y: -10, scale: 0.96 }, transition: {
                        ...
                        stiffness: 280,
                        delay: index * 0.05,
                    }, children: _jsxs(Card, { hoverable: true, className: cn('cursor-pointer transition-all duration-200 ease-out', 'bg-gradient-to-br from-card to-card/95', 'border-border/40', 'shadow-sm', 'hover:shadow-[0_8px_30px_-8px_rgba(0,0,0,0.12)]', 'hover:border-primary/30', 'hover:-translate-y-1 hover:scale-[1.01]', 'active:scale-[0.99] active:translate-y-0', 'group overflow-hidden'), onClick: () => onSelect(suggestion), children: [_jsx("div", { className: "absolute inset-0 bg-gradient-to-br from-primary/0 to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" }), _jsx(CardContent, { className: "px-4 py-4 relative", children: _jsxs("div", { className: "flex items-start gap-3.5", children: [suggestion.icon && (_jsx(motion.div, { className: "flex-shrink-0 w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors duration-200", whileHover: { scale: 1.05, rotate: 3 }, transition: {
                                                type: 'spring',
                                                stiffness: 400,
                                                damping: 25,
                                            }, children: suggestion.icon })), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("h4", { className: "font-semibold text-sm mb-1 text-foreground group-hover:text-primary transition-colors duration-200", children: suggestion.label || suggestion.text }), suggestion.description && (_jsx("p", { className: "text-xs text-muted-foreground/80 line-clamp-2 leading-relaxed", children: suggestion.description })), suggestion.confidence !== undefined && (_jsxs(Badge, { variant: "secondary", className: "mt-2.5 text-xs bg-primary/10 text-primary border-0", children: [Math.round(suggestion.confidence * 100), "% match"] }))] }), _jsx("div", { className: "flex-shrink-0 w-6 h-6 rounded-full bg-muted/50 flex items-center justify-center opacity-0 group-hover:opacity-100 translate-x-1 group-hover:translate-x-0 transition-all duration-200", children: _jsx("svg", { className: "w-3.5 h-3.5 text-muted-foreground", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5l7 7-7 7" }) }) })] }) })] }) }, suggestion.id))) }) }));
    }
    // Render list layout
