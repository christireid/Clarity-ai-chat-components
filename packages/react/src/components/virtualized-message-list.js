...
export function VirtualizedMessageList({ messages, renderMessage, estimatedItemSize = 150, overscanCount = 3, autoScrollToBottom = true, onScroll, className, itemKey, }) {
    ...
    const handleScroll = React.useCallback(({ scrollOffset, scrollUpdateWasRequested }) => {
        if (!scrollUpdateWasRequested && listRef.current) {
            ...
            const clientHeight = list._outerRef?.clientHeight || 600;
    ...
    return (_jsx("div", { className: className, style: { height: '100%', width: '100%' }, children: _jsx(AutoSizerComponent, { children: ({ height: _height, width }) => (_jsx(ListComponent, { ref: listRef, height: _height, width: width, itemCount: messages.length, itemSize: getItemSize, itemData: {
                    messages,
                    renderMessage,
...
export function AutoVirtualizedMessageList({ messages, renderMessage, virtualizationThreshold = 100, ...props }) {
    ...
    return (_jsx("div", { className: props.className, children: messages.map((message, index) => (_jsx("div", { children: renderMessage(message, index) }, message.id || `msg-${index}`))) }));
}
// ============================================================================
// Utility Hooks
// ============================================================================
