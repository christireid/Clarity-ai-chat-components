 ...
 */
export function AdvancedChatInput({ value, onChange, onSubmit, onSuggestionRequest, onFileUpload, maxFiles = 5, acceptedFileTypes = ['image/*', 'application/pdf', '.txt', '.doc', '.docx'], savedPrompts = [], disabled = false, placeholder = 'Type a message... Use @ for prompts, / for commands', maxLength, className, ref, }) {
    ...
    useEffect(() => {
        const checkForTrigger = () => {
            ...
            if (lastAtIndex > lastSpaceIndex &&
                lastAtIndex === beforeCursor.length - 1) {
                setTriggerChar('@');
