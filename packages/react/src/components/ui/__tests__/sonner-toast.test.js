...
vi.mock('sonner', () => ({
    Toaster: ({ position, richColors, closeButton }) => (_jsx("div", { "data-testid": "sonner-toaster", "data-position": position, "data-rich-colors": String(richColors), "data-close-button": String(closeButton) })),
    toast: mockToast,
}));
...
describe('ClarityToaster', () => {
    describe('Rendering', () => {
        ...
        it('should apply custom position', () => {
            render(_jsx(ClarityToaster, { position: "top-center" }));
        ...
        it('should allow disabling rich colors', () => {
            render(_jsx(ClarityToaster, { richColors: false }));
            expect(screen.getByTestId('sonner-toaster')).toHaveAttribute('data-rich-colors', 'false');
        });
    });
...
describe('toast API', () => {
    ...
    describe('toast.success', () => {
        it('should call sonner toast.success with message', () => {
            ...
            expect(mockToast.success).toHaveBeenCalledWith('Success message', expect.any(Object));
        });
        ...
        it('should support duration option', () => {
            ...
            expect(mockToast.success).toHaveBeenCalledWith('Message', expect.objectContaining({ duration: 5000 }));
        });
    ...
    describe('toast.error', () => {
        ...
        it('should support action option', () => {
            ...
            expect(mockToast.error).toHaveBeenCalledWith('Failed', expect.objectContaining({
                action: { label: 'Retry', onClick },
            }));
        });
    });
    ...
    describe('toast.custom', () => {
        it('should call sonner toast.custom with render function', () => {
            const content = _jsx("div", { children: "Custom content" });
            toast.custom(content);
            expect(mockToast.custom).toHaveBeenCalledWith(expect.any(Function), expect.any(Object));
        });
    });
    describe('toast.message', () => {
        it('should call sonner toast.message', () => {
            toast.message('Neutral message');
            expect(mockToast.message).toHaveBeenCalledWith('Neutral message', expect.any(Object));
        });
    });
...
//# sourceMappingURL=sonner-toast.test.js.map