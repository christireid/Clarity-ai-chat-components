...
export const MultiModalPreview = ({ attachments, onOpen, onRetry, onRemove, className, title = defaultTitle, subtitle = defaultSubtitle, }) => {
    ...
    const renderThumbnail = (attachment) => {
        if (attachment.thumbnailUrl) {
            return (_jsx("img", { src: attachment.thumbnailUrl, alt: attachment.title, className: "h-16 w-16 rounded-xl object-cover" }));
        }
        return (_jsx("div", { className: "flex h-16 w-16 items-center justify-center rounded-xl bg-[hsl(var(--surface-muted))] text-muted-foreground", children: typeIcon[attachment.type] }));
    };
