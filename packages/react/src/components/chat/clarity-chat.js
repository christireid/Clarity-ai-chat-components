 ...
 * ```
 */
export function ClarityChat({ api, className, emptyState, showHeader, sessionTitle, sessionSubtitle, headerActions, showMessageCount, onExport, onClear, onMessageCopy, onMessageFeedback, onEditMessage, onRegenerateMessage, onDeleteMessage, ...hookOptions }) {
    ...
    const messages = React.useMemo(() => convertCoreMessagesToMessages(chat.messages), [chat.messages]);
    ...
    const handleDeleteMessage = React.useCallback((messageId) => {
        ...
        chat.setMessages((prevMessages) => prevMessages.filter((m) => m.id !== messageId));
    ...
    const handleEditMessage = React.useCallback((messageId) => {
    ...
    // Handle saving edits
    const handleSaveEdit = React.useCallback(async (messageId, newContent) => {
        ...
        try {
            ...
            if (needsRegeneration) {
                ...
                chat.setMessages((prevMessages) => prevMessages.map((m) => m.id === messageId ? { ...m, content: trimmedContent } : m));
    ...
    const handleRegenerateMessage = React.useCallback(async (messageId) => {
    ...
    return (_jsx(ChatWindow, { messages: messages, isLoading: chat.isLoading || isRegenerating, onSendMessage: handleSendMessage, onStopGeneration: handleStopGeneration, onMessageCopy: onMessageCopy, onMessageFeedback: onMessageFeedback, onEditMessage: handleEditMessage, onRegenerateMessage: handleRegenerateMessage, onDeleteMessage: handleDeleteMessage, editingMessageId: editingMessageId, onSaveEdit: handleSaveEdit, onCancelEdit: handleCancelEdit, className: className, emptyState: emptyState, showHeader: showHeader, sessionTitle: sessionTitle, sessionSubtitle: sessionSubtitle, headerActions: headerActions, showMessageCount: showMessageCount, onExport: onExport, onClear: onClear ? handleClear : undefined }));
}
ClarityChat.displayName = 'ClarityChat';
//# sourceMappingURL=clarity-chat.js.map