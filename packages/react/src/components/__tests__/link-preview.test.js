...
import { LinkPreview, LinkPreviewSkeleton, LinkPreviewError, LinkPreviewCompact, InlineLink, SmartLinkPreview, RichEmbed, useLinkPreview, isValidUrl, sanitizeUrl, detectEmbedType, createMetadataFetcher, createFallbackMetadata, } from '../link-preview';
...
describe('LinkPreview', () => {
    describe('rendering', () => {
        it('should render card variant with all metadata', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata }));
        ...
        it('should render compact variant correctly', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, variant: "compact" }));
            expect(screen.getByText('Example Article Title')).toBeInTheDocument();
            expect(screen.getByText('example.com')).toBeInTheDocument();
        });
        it('should render inline variant correctly', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, variant: "inline" }));
        ...
        it('should show skeleton while loading', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, loading: true }));
        ...
        it('should show fallback when provided and metadata url is missing', () => {
            ...
            render(_jsx(LinkPreview, { metadata: fallbackMetadata, fallback: _jsx("div", { "data-testid": "fallback", children: "Fallback content" }) }));
            expect(screen.getByTestId('fallback')).toBeInTheDocument();
        });
        it('should handle missing optional fields gracefully', () => {
            render(_jsx(LinkPreview, { metadata: minimalMetadata }));
            // Should show domain when no title
            expect(screen.getAllByText('example.com').length).toBeGreaterThan(0);
        });
        it('should apply custom className', () => {
            const { container } = render(_jsx(LinkPreview, { metadata: sampleMetadata, className: "custom-class" }));
    ...
    describe('image handling', () => {
        ...
        it('should hide image when showImage is false', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, showImage: false }));
            expect(screen.queryByRole('img', { name: /preview image/i })).not.toBeInTheDocument();
        });
    ...
    describe('accessibility', () => {
        it('should have proper ARIA labels when clickable', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onClick: vi.fn() }));
            expect(screen.getByRole('link', { name: /open link: example article title/i })).toBeInTheDocument();
        });
        it('should have proper ARIA labels when not clickable', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata }));
            expect(screen.getByLabelText(/link preview: example article title/i)).toBeInTheDocument();
        });
        it('should be keyboard navigable when clickable', () => {
            const handleClick = vi.fn();
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onClick: handleClick }));
        ...
        it('should support custom aria-label', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onClick: vi.fn(), "aria-label": "Custom label for the link" }));
            expect(screen.getByLabelText('Custom label for the link')).toBeInTheDocument();
        });
        it('should have accessible remove button', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onRemove: vi.fn() }));
    ...
    describe('interactions', () => {
        ...
        it('should call onRemove when remove button is clicked', async () => {
            ...
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onRemove: handleRemove }));
        ...
        it('should not trigger onClick when remove button is clicked', async () => {
            const handleClick = vi.fn();
            const handleRemove = vi.fn();
            render(_jsx(LinkPreview, { metadata: sampleMetadata, onClick: handleClick, onRemove: handleRemove }));
            fireEvent.click(screen.getByRole('button', { name: /remove/i }));
            expect(handleRemove).toHaveBeenCalledTimes(1);
    ...
    describe('display options', () => {
        it('should hide description when showDescription is false', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, showDescription: false }));
            expect(screen.queryByText('This is a sample description for testing purposes.')).not.toBeInTheDocument();
        });
        it('should hide favicon when showFavicon is false', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, showFavicon: false }));
            expect(screen.queryByRole('img', { name: '' })).not.toBeInTheDocument();
        });
        it('should hide domain badge when showDomain is false', () => {
            render(_jsx(LinkPreview, { metadata: sampleMetadata, showDomain: false }));
...
describe('LinkPreviewSkeleton', () => {
    it('should render card skeleton by default', () => {
        const { container } = render(_jsx(LinkPreviewSkeleton, {}));
        expect(container.querySelector('.animate-pulse, [class*="animate"]')).toBeInTheDocument();
    });
    it('should render compact skeleton', () => {
        const { container } = render(_jsx(LinkPreviewSkeleton, { variant: "compact" }));
        expect(container.querySelector('.animate-pulse')).toBeInTheDocument();
    });
    it('should render inline skeleton', () => {
        const { container } = render(_jsx(LinkPreviewSkeleton, { variant: "inline" }));
        expect(container.querySelector('.animate-pulse')).toBeInTheDocument();
    });
    it('should apply custom className', () => {
        const { container } = render(_jsx(LinkPreviewSkeleton, { className: "custom-skeleton" }));
...
describe('LinkPreviewError', () => {
    it('should render error state with domain', () => {
        render(_jsx(LinkPreviewError, { url: "https://failed.example.com/page" }));
        expect(screen.getByText('Failed to load preview')).toBeInTheDocument();
        expect(screen.getByText('failed.example.com')).toBeInTheDocument();
    });
    it('should display error message when provided', () => {
        render(_jsx(LinkPreviewError, { url: "https://example.com", error: "Network error occurred" }));
        expect(screen.getByText('Network error occurred')).toBeInTheDocument();
    });
    it('should render retry button when onRetry provided', () => {
        render(_jsx(LinkPreviewError, { url: "https://example.com", onRetry: vi.fn() }));
        expect(screen.getByRole('button', { name: /retry/i })).toBeInTheDocument();
    });
    it('should call onRetry when retry button clicked', async () => {
        const handleRetry = vi.fn();
        render(_jsx(LinkPreviewError, { url: "https://example.com", onRetry: handleRetry }));
        fireEvent.click(screen.getByRole('button', { name: /retry/i }));
        expect(handleRetry).toHaveBeenCalledTimes(1);
    });
    it('should not show retry button when onRetry not provided', () => {
        render(_jsx(LinkPreviewError, { url: "https://example.com" }));
...
describe('LinkPreviewCompact', () => {
    it('should render compact card with metadata', () => {
        render(_jsx(LinkPreviewCompact, { metadata: sampleMetadata }));
        expect(screen.getByText('Example Article Title')).toBeInTheDocument();
        expect(screen.getByText('example.com')).toBeInTheDocument();
    });
    ...
    it('should hide favicon when showFavicon is false', () => {
        render(_jsx(LinkPreviewCompact, { metadata: sampleMetadata, showFavicon: false }));
        expect(screen.queryByRole('img')).not.toBeInTheDocument();
    });
    it('should be keyboard accessible when clickable', () => {
        const handleClick = vi.fn();
        render(_jsx(LinkPreviewCompact, { metadata: sampleMetadata, onClick: handleClick }));
        const card = screen.getByRole('link');
        fireEvent.keyDown(card, { key: 'Enter' });
        expect(handleClick).toHaveBeenCalledTimes(1);
    });
    it('should show arrow icon when clickable', () => {
        render(_jsx(LinkPreviewCompact, { metadata: sampleMetadata, onClick: vi.fn() }));
        // Arrow icon should be present (SVG)
        const card = screen.getByRole('link');
        expect(card.querySelector('svg')).toBeInTheDocument();
    });
    it('should show domain when no title', () => {
        render(_jsx(LinkPreviewCompact, { metadata: minimalMetadata }));
        expect(screen.getByText('example.com')).toBeInTheDocument();
    });
});
// ============================================================================
...
describe('InlineLink', () => {
    it('should render link with correct attributes', () => {
        render(_jsx(InlineLink, { url: "https://example.com" }));
        const link = screen.getByRole('link');
    ...
    it('should render children as link text', () => {
        render(_jsx(InlineLink, { url: "https://example.com", children: "Click here" }));
    ...
    it('should call onPreview and prevent default when provided', async () => {
        ...
        render(_jsx(InlineLink, { url: "https://example.com", onPreview: handlePreview }));
        fireEvent.click(screen.getByRole('link'));
        expect(handlePreview).toHaveBeenCalledWith('https://example.com');
    });
    it('should apply custom className', () => {
        render(_jsx(InlineLink, { url: "https://example.com", className: "custom-link" }));
        const link = screen.getByRole('link');
        expect(link).toHaveClass('custom-link');
    });
    it('should not show preview when showHoverPreview is false', async () => {
        render(_jsx(InlineLink, { url: "https://example.com", showHoverPreview: false }));
        const link = screen.getByRole('link');
...
describe('SmartLinkPreview', () => {
    it('should fetch metadata on mount', async () => {
        render(_jsx(SmartLinkPreview, { url: "https://example.com" }));
    ...
    it('should call onLoad when metadata is loaded', async () => {
        ...
        render(_jsx(SmartLinkPreview, { url: "https://example.com", onLoad: handleLoad }));
        await waitFor(() => {
    ...
    it('should show error state when fetch fails', async () => {
        ...
        render(_jsx(SmartLinkPreview, { url: "https://example.com", fetchFn: mockFetch }));
        await waitFor(() => {
            expect(screen.getByText('Failed to load preview')).toBeInTheDocument();
        });
    });
    it('should call onError when fetch fails', async () => {
        ...
        const mockFetch = vi.fn().mockRejectedValue(new Error('Network error'));
        render(_jsx(SmartLinkPreview, { url: "https://example.com", fetchFn: mockFetch, onError: handleError }));
        await waitFor(() => {
    ...
    it('should show fallback when provided and fetch fails', async () => {
        const mockFetch = vi.fn().mockRejectedValue(new Error('Network error'));
        render(_jsx(SmartLinkPreview, { url: "https://example.com", fetchFn: mockFetch, fallback: _jsx("div", { "data-testid": "fallback", children: "Custom fallback" }) }));
        await waitFor(() => {
            expect(screen.getByTestId('fallback')).toBeInTheDocument();
        });
    });
    it('should support different variants', async () => {
        render(_jsx(SmartLinkPreview, { url: "https://example.com", variant: "compact" }));
...
describe('Domain Extraction', () => {
    it('should extract domain from URL', () => {
        render(_jsx(LinkPreview, { metadata: { url: 'https://www.example.com/path' } }));
        expect(screen.getByText('example.com')).toBeInTheDocument();
    });
    it('should remove www prefix from domain', () => {
        render(_jsx(LinkPreview, { metadata: { url: 'https://www.test.example.com/path' } }));
        expect(screen.getByText('test.example.com')).toBeInTheDocument();
    });
    it('should handle invalid URLs gracefully', () => {
        render(_jsx(LinkPreview, { metadata: { url: 'not-a-valid-url' } }));
...
describe('createMetadataFetcher', () => {
    ...
    it('should call API endpoint with URL parameter', async () => {
        ...
        expect(mockFetch).toHaveBeenCalledWith('https://api.example.com/metadata?url=https%3A%2F%2Fexample.com', expect.any(Object));
...
describe('RichEmbed', () => {
    it('should render YouTube embed', () => {
        render(_jsx(RichEmbed, { url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ", embedType: "youtube" }));
    ...
    it('should render Vimeo embed', () => {
        render(_jsx(RichEmbed, { url: "https://vimeo.com/123456789", embedType: "vimeo" }));
    ...
    it('should render Spotify embed', () => {
        render(_jsx(RichEmbed, { url: "https://open.spotify.com/track/abc123", embedType: "spotify" }));
    ...
    it('should render generic fallback for unsupported types', () => {
        render(_jsx(RichEmbed, { url: "https://example.com", embedType: "generic" }));
        // Should show a link or preview card, not an iframe
        expect(screen.queryByRole('link')).toBeInTheDocument();
    });
    it('should apply custom className', () => {
        const { container } = render(_jsx(RichEmbed, { url: "https://www.youtube.com/watch?v=test", embedType: "youtube", className: "custom-embed" }));
        expect(container.querySelector('.custom-embed')).toBeInTheDocument();
    });
    it('should have accessible iframe titles', () => {
        render(_jsx(RichEmbed, { url: "https://www.youtube.com/watch?v=test", embedType: "youtube" }));
...
describe('LRU Cache Behavior', () => {
    it('should evict oldest entries when cache is full', async () => {
        const mockFetch = vi
            .fn()
            .mockImplementation((url) => Promise.resolve({ url, title: `Title for ${url}` }));
        const { result } = renderHook(() => useLinkPreview({ fetchFn: mockFetch }));
...
describe('Expandable Description', () => {
    ...
    const metadataWithLongDescription = {
        ...sampleMetadata,
        description: longDescription,
    };
    it('should truncate long descriptions by default', () => {
        render(_jsx(LinkPreview, { metadata: metadataWithLongDescription }));
    ...
    it('should expand description when expandableDescription is enabled and clicked', async () => {
        render(_jsx(LinkPreview, { metadata: metadataWithLongDescription, expandableDescription: true }));
    ...
    it('should not show expand button for short descriptions', () => {
        ...
        render(_jsx(LinkPreview, { metadata: shortMetadata, expandableDescription: true }));
        expect(screen.queryByRole('button', { name: /show more|expand/i })).not.toBeInTheDocument();
    });
});
//# sourceMappingURL=link-preview.test.js.map