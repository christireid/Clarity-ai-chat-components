...
import { HistoryManager, HistoryMessageRow, TokenUsageBar, HistoryToolbar, } from '../history-manager';
...
describe('HistoryManager', () => {
    describe('rendering', () => {
        it('should render empty state when no messages', () => {
            ...
            render(_jsx(HistoryManager, { messages: [], onMessagesChange: onChange, maxTokens: 4096 }));
            expect(screen.getByText('No messages in history')).toBeDefined();
        });
        it('should render messages with token counts', () => {
            const messages = createMessages(3);
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096, showTokenCounts: true }));
        ...
        it('should show token usage bar', () => {
            ...
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 1000, currentTokens: 500 }));
        ...
        it('should show message count summary', () => {
            const messages = createMessages(5);
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096 }));
    ...
    describe('deletion', () => {
        it('should delete selected messages', () => {
            ...
            const newMessages = onChange.mock.calls[0][0];
        ...
        it('should call onPrune when messages are deleted', () => {
            ...
            const onChange = vi.fn();
            const onPrune = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, onPrune: onPrune, maxTokens: 4096 }));
    ...
    describe('compact mode', () => {
        it('should truncate content in compact mode', () => {
            ...
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096, compact: true }));
    ...
    describe('options', () => {
        it('should hide bulk delete when allowBulkDelete is false', () => {
            const messages = createMessages(3);
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096, allowBulkDelete: false }));
        ...
        it('should hide token counts in message rows when showTokenCounts is false', () => {
            ...
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096, showTokenCounts: false }));
        ...
        it('should hide delete button when allowIndividualDelete is false', () => {
            ...
            const onChange = vi.fn();
            render(_jsx(HistoryManager, { messages: messages, onMessagesChange: onChange, maxTokens: 4096, allowIndividualDelete: false }));
        ...
        it('should clean up stale selectedIds when messages change', () => {
            ...
            const onChange = vi.fn();
            const { rerender } = render(_jsx(HistoryManager, { messages: initialMessages, onMessagesChange: onChange, maxTokens: 4096 }));
            // Select first two messages
            const checkboxes = screen.getAllByRole('checkbox');
            fireEvent.click(checkboxes[1]);
            fireEvent.click(checkboxes[2]);
            ...
            rerender(_jsx(HistoryManager, { messages: remainingMessages, onMessagesChange: onChange, maxTokens: 4096 }));
        ...
        it('should warn about duplicate IDs in development', () => {
            ...
            render(_jsx(HistoryManager, { messages: messagesWithDuplicates, onMessagesChange: vi.fn(), maxTokens: 4096 }));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Duplicate message IDs detected'));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('dup-1'));
...
describe('TokenUsageBar', () => {
    it('should show correct percentage', () => {
        render(_jsx(TokenUsageBar, { current: 500, max: 1000, warningThreshold: 0.8, criticalThreshold: 0.95 }));
        expect(screen.getByText('50.0%')).toBeDefined();
        expect(screen.getByText('500 / 1,000 tokens')).toBeDefined();
    });
    it('should show warning at warning threshold', () => {
        render(_jsx(TokenUsageBar, { current: 850, max: 1000, warningThreshold: 0.8, criticalThreshold: 0.95 }));
    ...
    it('should show critical warning at critical threshold', () => {
        render(_jsx(TokenUsageBar, { current: 960, max: 1000, warningThreshold: 0.8, criticalThreshold: 0.95 }));
        expect(screen.getByText(/Critical/)).toBeDefined();
        const progressBar = screen.getByRole('progressbar');
        expect(progressBar.className).toContain('bg-red-500');
    });
    it('should cap at 100%', () => {
        render(_jsx(TokenUsageBar, { current: 1500, max: 1000, warningThreshold: 0.8, criticalThreshold: 0.95 }));
        expect(screen.getByText('100.0%')).toBeDefined();
    });
    it('should handle max=0 without division by zero', () => {
        render(_jsx(TokenUsageBar, { current: 500, max: 0, warningThreshold: 0.8, criticalThreshold: 0.95 }));
        // Should not crash and should show some percentage
        expect(screen.getByRole('progressbar')).toBeDefined();
    });
    it('should handle warningThreshold >= criticalThreshold gracefully', () => {
        render(_jsx(TokenUsageBar, { current: 900, max: 1000, warningThreshold: 0.95, criticalThreshold: 0.8 }));
...
describe('HistoryToolbar', () => {
    it('should show selection count', () => {
        render(_jsx(HistoryToolbar, { selectedCount: 3, totalCount: 10, onDeleteSelected: vi.fn(), onKeepLast: vi.fn(), onClear: vi.fn() }));
        expect(screen.getByText('3 of 10 selected')).toBeDefined();
    });
    it('should disable delete when none selected', () => {
        render(_jsx(HistoryToolbar, { selectedCount: 0, totalCount: 10, onDeleteSelected: vi.fn(), onKeepLast: vi.fn(), onClear: vi.fn() }));
        const deleteButton = screen.getByText(/Delete Selected/);
        expect(deleteButton).toBeDisabled();
    });
    it('should disable all buttons when disabled prop is true', () => {
        render(_jsx(HistoryToolbar, { selectedCount: 5, totalCount: 10, onDeleteSelected: vi.fn(), onKeepLast: vi.fn(), onClear: vi.fn(), disabled: true }));
        const deleteButton = screen.getByText(/Delete Selected/);
        const keepLastButton = screen.getByText('Keep Last...');
        const clearButton = screen.getByText('Clear All');
        expect(deleteButton).toBeDisabled();
...
describe('HistoryMessageRow', () => {
    ...
    it('should render message content', () => {
        render(_jsx(HistoryMessageRow, { ...defaultProps }));
    ...
    it('should show token count', () => {
        render(_jsx(HistoryMessageRow, { ...defaultProps, showTokenCount: true }));
        expect(screen.getByText('10 tokens')).toBeDefined();
    });
    it('should call onSelect when checkbox clicked', () => {
        const onSelect = vi.fn();
        render(_jsx(HistoryMessageRow, { ...defaultProps, onSelect: onSelect }));
    ...
    it('should call onDelete when delete button clicked', () => {
        ...
        render(_jsx(HistoryMessageRow, { ...defaultProps, onDelete: onDelete }));
        const deleteButton = screen.getByLabelText('Delete message from user');
        fireEvent.click(deleteButton);
        expect(onDelete).toHaveBeenCalled();
    });
    it('should highlight when selected', () => {
        const { container } = render(_jsx(HistoryMessageRow, { ...defaultProps, selected: true }));
        const row = container.firstChild;
        expect(row.className).toContain('bg-accent');
    });
    it('should truncate long content in compact mode', () => {
        const longContent = 'A'.repeat(200);
        render(_jsx(HistoryMessageRow, { ...defaultProps, message: { ...defaultProps.message, content: longContent }, compact: true }));
        const content = screen.getByText(/A+\.\.\./);
        expect(content.textContent.length).toBeLessThan(200);
    });
});
//# sourceMappingURL=history-manager.test.js.map