...
import { OutputPreferenceSelector, UncontrolledOutputPreferenceSelector, useOutputPreference, } from '../output-preference-selector';
import { renderHook, act } from '@testing-library/react';
describe('OutputPreferenceSelector', () => {
    describe('rendering', () => {
        it('should render all three preference options', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange }));
        ...
        it('should render descriptions in expanded mode', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, displayMode: "expanded" }));
        ...
        it('should render compact mode with short labels when size is sm', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, displayMode: "compact", size: "sm" }));
        ...
        it('should render compact mode with full labels when size is md or lg', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, displayMode: "compact", size: "md" }));
            expect(screen.getByText('Concise')).toBeDefined();
            expect(screen.getByText('Balanced')).toBeDefined();
            expect(screen.getByText('Detailed')).toBeDefined();
        });
        it('should show token estimates when enabled', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, showTokenEstimate: true, modelCapacity: 128000, inputTokens: 5000 }));
        ...
        it('should indicate selected option', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "concise", onChange: onChange }));
            const conciseButton = screen.getByRole('radio', { name: /concise/i });
            expect(conciseButton.getAttribute('aria-checked')).toBe('true');
        });
        it('should apply custom className', () => {
            const onChange = vi.fn();
            const { container } = render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, className: "custom-class" }));
    ...
    describe('interaction', () => {
        it('should call onChange with correct values when option selected', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 128000, inputTokens: 5000 }));
            const conciseButton = screen.getByRole('radio', { name: /concise/i });
        ...
        it('should emit different maxTokens for different preferences', () => {
            const onChange = vi.fn();
            const { rerender } = render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 128000, inputTokens: 5000 }));
            // Click concise
        ...
        it('should not call onChange when disabled', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, disabled: true }));
            const conciseButton = screen.getByRole('radio', { name: /concise/i });
            fireEvent.click(conciseButton);
    ...
    describe('keyboard navigation', () => {
        it('should navigate with arrow keys', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, displayMode: "compact" }));
            const balancedButton = screen.getByRole('radio', { name: /balanced/i });
            fireEvent.keyDown(balancedButton, { key: 'ArrowRight' });
            expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ mode: 'detailed' }));
        });
        it('should wrap around when navigating past last option', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "detailed", onChange: onChange, displayMode: "compact" }));
            const detailedButton = screen.getByRole('radio', { name: /detailed/i });
            fireEvent.keyDown(detailedButton, { key: 'ArrowRight' });
            expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ mode: 'concise' }));
        });
        ...
        it('should navigate to last option with End key', () => {
            const onChange = vi.fn();
            render(_jsx(OutputPreferenceSelector, { value: "concise", onChange: onChange, displayMode: "compact" }));
            const conciseButton = screen.getByRole('radio', { name: /concise/i });
    ...
    describe('token calculation', () => {
        it('should calculate lower tokens for concise preference', () => {
            ...
            // Get concise tokens
            fireEvent.click(screen.getByRole('radio', { name: /concise/i }));
            const conciseResult = onChange.mock.calls[0]?.[0];
            // Get balanced tokens
            onChange.mockClear();
            fireEvent.click(screen.getByRole('radio', { name: /balanced/i }));
            const balancedResult = onChange.mock.calls[0]?.[0];
        ...
        it('should respect task type in calculation', () => {
            const onChange = vi.fn();
            const { rerender } = render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 128000, inputTokens: 5000, taskType: "classification" }));
            fireEvent.click(screen.getByRole('radio', { name: /balanced/i }));
            const classificationTokens = onChange.mock.calls[0]?.[0]?.maxTokens;
            onChange.mockClear();
            rerender(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 128000, inputTokens: 5000, taskType: "generation" }));
            fireEvent.click(screen.getByRole('radio', { name: /balanced/i }));
...
describe('UncontrolledOutputPreferenceSelector', () => {
    it('should manage its own state', () => {
        ...
        render(_jsx(UncontrolledOutputPreferenceSelector, { defaultValue: "balanced", onValueChange: onValueChange }));
        ...
        const conciseButton = screen.getByRole('radio', { name: /concise/i });
        expect(conciseButton.getAttribute('aria-checked')).toBe('true');
        expect(onValueChange).toHaveBeenCalledWith(expect.objectContaining({ mode: 'concise' }));
    });
    it('should expose getValue via ref', () => {
        const ref = React.createRef();
        render(_jsx(UncontrolledOutputPreferenceSelector, { ref: ref, defaultValue: "detailed", modelCapacity: 128000, inputTokens: 5000 }));
    ...
    it('should allow setting value via ref', () => {
        const ref = React.createRef();
        render(_jsx(UncontrolledOutputPreferenceSelector, { ref: ref, defaultValue: "balanced" }));
        act(() => {
...
describe('edge cases', () => {
    it('should handle modelCapacity=0 without crashing', () => {
        ...
        // Should not throw
        expect(() => {
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 0, inputTokens: 100 }));
        }).not.toThrow();
    ...
    it('should handle negative inputTokens without crashing', () => {
        const onChange = vi.fn();
        expect(() => {
            render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 128000, inputTokens: -1000 }));
        }).not.toThrow();
        fireEvent.click(screen.getByRole('radio', { name: /detailed/i }));
    ...
    it('should handle invalid value prop gracefully', () => {
        ...
        expect(() => {
            render(_jsx(OutputPreferenceSelector
            // @ts-expect-error - testing runtime behavior with invalid value
            , { 
                // @ts-expect-error - testing runtime behavior with invalid value
                value: "invalid", onChange: onChange }));
        }).not.toThrow();
    ...
    it('should handle inputTokens > modelCapacity', () => {
        const onChange = vi.fn();
        render(_jsx(OutputPreferenceSelector, { value: "balanced", onChange: onChange, modelCapacity: 1000, inputTokens: 5000 }));
        fireEvent.click(screen.getByRole('radio', { name: /concise/i }));
...
describe('useOutputPreference hook', () => {
    ...
    it('should provide brevity instruction', () => {
        const { result } = renderHook(() => useOutputPreference('concise', {
            modelCapacity: 128000,
            inputTokens: 5000,
        }));
...
//# sourceMappingURL=output-preference-selector.test.js.map