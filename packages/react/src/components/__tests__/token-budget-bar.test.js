...
vi.mock('framer-motion', () => ({
    motion: {
        div: ({ children, className, style, ...props }) => (_jsx("div", { className: className, style: style, ...props, children: children })),
...
describe('TokenBudgetBar', () => {
    const createUsage = (current, max, reserved = 4096) => {
        const effectiveMax = max - reserved;
        const utilizationPercent = effectiveMax > 0 ? (current / effectiveMax) * 100 : 0;
        const exceededPercent = utilizationPercent > 100 ? utilizationPercent - 100 : 0;
        let status = 'safe';
    ...
    describe('renders correctly', () => {
        it('renders with safe status', () => {
            ...
            render(_jsx(TokenBudgetBar, { usage: usage }));
        ...
        it('shows calculating state', () => {
            const usage = createUsage(5000, 128000);
            render(_jsx(TokenBudgetBar, { usage: usage, isCalculating: true }));
    ...
    describe('compact mode', () => {
        it('renders only the bar in compact mode', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, compact: true }));
            // Should not show visible labels in compact mode (but sr-only description is ok)
            expect(screen.queryByText(/5,000 \/ 123,904 tokens/)).not.toBeInTheDocument();
    ...
    describe('size variants', () => {
        it('renders small size', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, size: "sm" }));
            expect(container.querySelector('.text-xs')).toBeInTheDocument();
        });
        it('renders large size', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, size: "lg" }));
    ...
    describe('interactions', () => {
        it('calls onClick when clicked', () => {
            ...
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, onClick: handleClick }));
        ...
        it('shows tooltip on hover', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, showTooltip: true }));
        ...
        it('hides tooltip when not hovering', () => {
            ...
            if (focusable) {
                ...
                expect(screen.queryByText('Token Budget Details')).not.toBeInTheDocument();
            }
        });
    });
    describe('accessibility', () => {
        it('has aria-live for status updates', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage }));
        ...
        it('supports custom aria label', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, ariaLabel: "Chat Token Budget" }));
            const progressbar = container.querySelector('[role="progressbar"]');
    ...
    describe('without labels', () => {
        it('hides visible labels when showLabel is false', () => {
            const usage = createUsage(5000, 128000);
            const { container } = render(_jsx(TokenBudgetBar, { usage: usage, showLabel: false }));
    ...
    describe('cost estimation', () => {
        it('shows cost estimate when model and showCost are provided', () => {
            ...
            render(_jsx(TokenBudgetBar, { usage: usage, model: "gpt-4o", showCost: true }));
            // Should show approximately formatted cost with ~ prefix
            expect(screen.getByText(/~\$/)).toBeInTheDocument();
        });
        it('does not show cost when showCost is false', () => {
            const usage = createUsage(10000, 128000);
            render(_jsx(TokenBudgetBar, { usage: usage, model: "gpt-4o", showCost: false }));
            expect(screen.queryByText(/~\$/)).not.toBeInTheDocument();
        });
        it('does not show cost when model is not provided', () => {
            const usage = createUsage(10000, 128000);
            render(_jsx(TokenBudgetBar, { usage: usage, showCost: true }));
            expect(screen.queryByText(/~\$/)).not.toBeInTheDocument();
        });
    });
});
describe('TokenBudgetIndicator', () => {
    const createUsage = (utilizationPercent) => ({
        current: 5000,
        max: 128000,
        available: 118904,
        utilizationPercent,
    ...
    it('renders percentage correctly', () => {
        ...
        render(_jsx(TokenBudgetIndicator, { usage: usage }));
        expect(screen.getByText('46%')).toBeInTheDocument();
    });
    it('renders with correct status color', () => {
        const usage = { ...createUsage(100), status: 'exceeded' };
        const { container } = render(_jsx(TokenBudgetIndicator, { usage: usage }));
    ...
    it('has accessible role and label', () => {
        ...
        expect(status).toHaveAttribute('aria-label', 'Token usage: 50% used, normal');
    });
    it('reports warning status in aria-label', () => {
        const usage = { ...createUsage(85), status: 'warning' };
        const { container } = render(_jsx(TokenBudgetIndicator, { usage: usage }));
        const status = container.querySelector('[role="status"]');
    ...
    it('supports custom aria label', () => {
        const usage = createUsage(50);
        const { container } = render(_jsx(TokenBudgetIndicator, { usage: usage, ariaLabel: "Chat budget" }));
        const status = container.querySelector('[role="status"]');
        expect(status).toHaveAttribute('aria-label', 'Chat budget: 50% used, normal');
    });
    it('applies custom className', () => {
        const usage = createUsage(50);
        const { container } = render(_jsx(TokenBudgetIndicator, { usage: usage, className: "custom-class" }));
        expect(container.querySelector('.custom-class')).toBeInTheDocument();
    });
});
//# sourceMappingURL=token-budget-bar.test.js.map