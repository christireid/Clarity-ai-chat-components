...
describe('ChatWindow Component', () => {
    ...
    describe('Rendering', () => {
        it('should render the chat window with messages', () => {
            render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: mockOnSendMessage }));
            expect(screen.getByText('Hello')).toBeInTheDocument();
            expect(screen.getByText('Hi there! How can I help you?')).toBeInTheDocument();
        });
        it('should render empty state with no messages', () => {
            render(_jsx(ChatWindow, { messages: [], onSendMessage: mockOnSendMessage }));
            // Should still render the input area
            const textarea = screen.getByRole('textbox');
            expect(textarea).toBeInTheDocument();
        });
        it('should render thinking indicator when loading', () => {
            render(_jsx(ChatWindow, { messages: mockMessages, isLoading: true, aiStatus: { stage: 'thinking', startedAt: new Date() }, onSendMessage: mockOnSendMessage }));
    ...
    describe('Message Input', () => {
        ...
        it('should disable input when loading', () => {
            render(_jsx(ChatWindow, { messages: mockMessages, isLoading: true, onSendMessage: mockOnSendMessage }));
            const textarea = screen.getByRole('textbox');
    ...
    describe('Message List Display', () => {
        it('should display messages in order', () => {
            const { container } = render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: mockOnSendMessage }));
        ...
        it('should update when new messages are added', () => {
            const { rerender } = render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: mockOnSendMessage }));
            const newMessages = [
                ...mockMessages,
                {
                    id: '3',
            ...
            rerender(_jsx(ChatWindow, { messages: newMessages, onSendMessage: mockOnSendMessage }));
    ...
    describe('Loading State', () => {
        ...
        it('should hide thinking indicator when not loading', () => {
            render(_jsx(ChatWindow, { messages: mockMessages, isLoading: false, aiStatus: { stage: 'thinking', startedAt: new Date() }, onSendMessage: mockOnSendMessage }));
            expect(screen.queryByText(/thinking|processing/i)).not.toBeInTheDocument();
        });
        it('should toggle thinking indicator correctly', () => {
            const { rerender } = render(_jsx(ChatWindow, { messages: mockMessages, isLoading: false, aiStatus: { stage: 'thinking', startedAt: new Date() }, onSendMessage: mockOnSendMessage }));
            expect(screen.queryByText(/thinking|processing/i)).not.toBeInTheDocument();
            rerender(_jsx(ChatWindow, { messages: mockMessages, isLoading: true, aiStatus: { stage: 'thinking', startedAt: new Date() }, onSendMessage: mockOnSendMessage }));
            expect(screen.getByText(/thinking/i)).toBeInTheDocument();
        });
    });
    ...
    describe('Custom className', () => {
        it('should apply custom className to container', () => {
            const { container } = render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: mockOnSendMessage, className: "custom-chat-window" }));
    ...
    describe('Integration', () => {
        ...
        it('should handle async onSendMessage', async () => {
            ...
            render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: asyncOnSend }));
            const user = userEvent.setup();
            const textarea = screen.getByRole('textbox');
        ...
        it('should clear input even if onSendMessage throws', async () => {
            ...
            render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: errorOnSend }));
            const user = userEvent.setup();
            const textarea = screen.getByRole('textbox');
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle empty messages array', () => {
            expect(() => render(_jsx(ChatWindow, { messages: [], onSendMessage: mockOnSendMessage }))).not.toThrow();
        });
        it('should handle undefined isLoading', () => {
            expect(() => render(_jsx(ChatWindow, { messages: mockMessages, onSendMessage: mockOnSendMessage }))).not.toThrow();
        });
    });
    describe('Performance', () => {
        it('should handle large message lists', () => {
            ...
            const { container } = render(_jsx(ChatWindow, { messages: manyMessages, onSendMessage: mockOnSendMessage }));
