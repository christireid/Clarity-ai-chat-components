...
export const WithHighlightedLines = {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Highlight specific lines to draw attention. Use comma-separated values and ranges (e.g., "2,5-7,10").',
            },
        },
    },
};
...
+  console.log(\`Hello, \${name}!\`);
   return name;
    ...
    parameters: {
        docs: {
            description: {
                story: 'Show diff-style highlighting with added (green) and removed (red) lines.',
            },
        },
    },
};
...
export const LongCodeCollapsible = {
    args: {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Long code blocks can be collapsed with a maximum height. Users can expand to see all content.',
            },
        },
    },
};
...
export const ThemeShowcase = {
    render: () => (_jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: getDarkThemes()
            .slice(0, 6)
            .map((theme) => (_jsx(CodeBlock, { language: "typescript", theme: theme, title: CODE_THEMES[theme].name, showLineNumbers: true, children: `const greet = (name: string) => {
  return \`Hello, \${name}!\`;
};` }, theme))) })),
    parameters: {
        docs: {
...
export const LightThemes = {
    render: () => (_jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-100 p-4 rounded-lg", children: getLightThemes()
            .slice(0, 4)
            .map((theme) => (_jsx(CodeBlock, { language: "typescript", theme: theme, title: CODE_THEMES[theme].name, showLineNumbers: true, children: `const greet = (name: string) => {
  return \`Hello, \${name}!\`;
};` }, theme))) })),
