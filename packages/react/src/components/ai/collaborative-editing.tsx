 ...
 */
function transformOperation(op1: EditOperation, op2: EditOperation): EditOperation {
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    {operations.length > 0 && (
                      <Badge variant="outline">{operations.length} changes</Badge>
                    )}
                  </div>
                </div>
 ...
 */
export function useCollaborativeSession(conversationId: string, currentUser: CollaborativeUser) {
  ...
  // Update user status
  const updateUserStatus = React.useCallback((userId: string, status: PresenceStatus) => {
    setSession(prev => {
      const newUsers = new Map(prev.users)
  ...
  const requestLock = React.useCallback((messageId: string, userId: string): boolean => {
  ...
  // Get message lock
  const getMessageLock = React.useCallback((messageId: string): MessageLock | undefined => {
