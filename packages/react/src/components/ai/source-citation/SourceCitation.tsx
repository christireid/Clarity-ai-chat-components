...
export function SourceCitation({
          ...
          {Array.from(groups.entries()).map(([domain, domainSources]) => (
            <div key={domain}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={cn(sizeConfig.title, 'text-foreground')}>
                  {domain}
                </span>
                ...
                <div className="grid gap-2 pl-6">
                  {domainSources.map((source, index) => (
                    <CardSourceItem
                      ...
                      onClick={() =>
                        onSourceClick?.(source, sources.indexOf(source))
                      }
  ...
  if (variant === 'inline') {
    return (
      ...
      >
        {title && (
          <span
            className={cn(sizeConfig.snippet, 'text-muted-foreground mr-1')}
          >
            {title}:
          </span>
        )}
  ...
  return (
    ...
    >
      {title && (
        <h3
          className={cn('font-semibold text-foreground mb-2', sizeConfig.title)}
        >
          {title}
        </h3>
      )}
