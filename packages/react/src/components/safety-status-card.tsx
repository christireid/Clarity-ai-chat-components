  ...
  return (
    <Card className={cn('border-border/60 bg-[hsl(var(--surface-elevated))] shadow-sm', className)}>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-border/40 bg-[hsl(var(--surface-muted))] p-4 text-xs font-medium text-muted-foreground/80">
        ...
        <ul className="space-y-3">
          {checks.map((check) => (
            ...
            >
              ...
              {check.remediation && (
                <div className="rounded-xl border border-dashed border-border/40 bg-background/60 p-3 text-xs text-muted-foreground/80">
                  {check.remediation}
                </div>
              )}
