...
/**
 * Metrics needed for significance calculation
 */
export interface VariantMetricsInput {
  /** Number of impressions/views */
  impressions: number
  /** Number of conversions */
  conversions: number
  /** Conversion rate (optional, calculated if not provided) */
...
export function calculateSignificance(
  control: VariantMetricsInput,
  variant: VariantMetricsInput,
  options: {
    confidenceLevel?: number
    minEffect?: number
  } = {}
  ...
  const isSignificant =
    pValue < 1 - confidenceLevel && Math.abs(effectSize) >= minEffect
 ...
 * ```
 */
export function useStatisticalSignificance(
  control: VariantMetricsInput | null,
  variant: VariantMetricsInput | null,
  options: {
    confidenceLevel?: number
    minEffect?: number
  } = {}
