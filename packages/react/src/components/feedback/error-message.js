...
import { AlertCircleIcon, AlertTriangleIcon, RefreshIcon, InfoIcon, } from '../ui/icons';
...
export function ErrorMessage({ error, onRetry, onDismiss, showTechnicalDetails = false, maxRetryAttempts = 3, className, compact = false, }) {
    ...
    if (compact) {
        return (_jsxs(motion.div, { initial: {
            ...
            }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, scale: 0.95 }, transition: {
                duration: getMotionSafeDuration(prefersReducedMotion, 0.2),
            }, className: cn('flex items-center gap-2.5 p-2.5 rounded-lg border shadow-sm', config.bgColor, config.borderColor, className), children: [_jsx(IconComponent, { size: 16, className: cn(config.iconColor, 'shrink-0') }), _jsx("span", { className: "text-sm text-foreground/90 flex-1", children: errorDetails.message }), errorDetails.canRetry && onRetry && (_jsx("button", { onClick: onRetry, className: "p-1.5 rounded-lg hover:bg-accent/50 transition-colors", "aria-label": "Retry", children: _jsx(RefreshIcon, { size: 14 }) }))] }));
    }
    return (_jsxs(motion.div, { initial: {
            opacity: 0,
            y: getMotionSafeValue(prefersReducedMotion, 10, 0),
            scale: 0.98,
        }, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 0, scale: 0.95 }, transition: {
            // Framer Motion 12: Spring entrance for full error displays
            type: 'spring',
            damping: 25,
            stiffness: 280,
        }, className: cn('relative p-4 rounded-lg border shadow-sm space-y-4', config.bgColor, config.borderColor, className), role: "alert", "aria-live": "assertive", children: [_jsxs("div", { className: "flex items-start gap-3.5", children: [_jsx(motion.div, { initial: { scale: 0, rotate: -90 }, animate: { scale: 1, rotate: 0 }, transition: {
                ...
                }, className: "space-y-2.5", children: [_jsx("p", { className: "text-xs font-semibold text-foreground/90 uppercase tracking-wide", children: "Suggested Actions" }), _jsx("ul", { className: "space-y-2", children: errorDetails.suggestions.map((suggestion, index) => (_jsxs(motion.li, { initial: {
                                opacity: 0,
                                x: getMotionSafeValue(prefersReducedMotion, -10, 0),
                            }, animate: { opacity: 1, x: 0 }, transition: {
                                delay: getMotionSafeDuration(prefersReducedMotion, 0.1 + index * 0.05),
                                duration: getMotionSafeDuration(prefersReducedMotion, 0.2),
                            }, className: "flex items-start gap-2.5 text-sm text-foreground/90", children: [_jsx("span", { className: "text-primary mt-0.5", children: "\u2022" }), _jsx("span", { children: suggestion })] }, index))) })] })), showTechnicalDetails && errorDetails.technicalDetails && (_jsxs("div", { className: "space-y-2.5", children: [_jsxs("button", { onClick: () => setShowDetails(!showDetails), className: "flex items-center gap-1.5 text-xs font-semibold text-muted-foreground/90 hover:text-foreground transition-colors", children: [_jsx(motion.svg, { className: "w-3 h-3", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", animate: { rotate: showDetails ? 90 : 0 }, transition: { duration: durations.normal }, children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 5l7 7-7 7" }) }), "Technical Details"] }), _jsx(AnimatePresence, { children: showDetails && (_jsx(motion.pre, { initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: 'auto' }, exit: { opacity: 0, height: 0 }, className: "text-xs bg-muted/50 rounded p-3 overflow-x-auto font-mono", children: errorDetails.technicalDetails })) })] })), errorDetails.canRetry && onRetry && (_jsx(motion.div, { initial: {
                    opacity: 0,
                    y: getMotionSafeValue(prefersReducedMotion, 10, 0),
                }, animate: { opacity: 1, y: 0 }, transition: {
                    delay: getMotionSafeDuration(prefersReducedMotion, 0.2),
                }, children: _jsx(RetryButton, { onRetry: onRetry, errorType: errorDetails.type, maxAttempts: maxRetryAttempts, buttonText: errorDetails.retryButtonText, size: "sm" }) }))] }));
}
