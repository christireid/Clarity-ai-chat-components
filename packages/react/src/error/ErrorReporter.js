...
export function ErrorReporterProvider({ children, config }) {
    ...
    const { providers, enabled = true, autoReport = true, captureConsole = false, sampleRate = 1.0, beforeSend, onError, globalTags = {}, globalContext = {}, } = config;
    ...
    const updateStats = useCallback((report) => {
        setErrorStats((prev) => {
            ...
            newStats.byHour = prev.byHour.map((h) => h.hour === hour ? { ...h, count: h.count + 1 } : h);
            // Update top errors
