...
describe('Architect Hooks', () => {
    describe('useArchitectWorkflow', () => {
        ...
        it('should complete audit phase and advance to planning', () => {
            ...
            const { result } = renderHook(() => useArchitectWorkflow({ onPhaseComplete }));
        ...
        it('should complete implementation phase and advance to review', () => {
            ...
            act(() => {
                result.current.completePlanning(createStrategicPlan({
                    chainOfThought: 'T',
                    steps: [
                        createPlanningStep({
                            stepNumber: 1,
        ...
        it('should complete review phase and finish workflow', () => {
            ...
            const { result } = renderHook(() => useArchitectWorkflow({ onWorkflowComplete }));
            act(() => {
                result.current.completeAudit(createAuditResult({}));
            });
            act(() => {
            ...
            act(() => {
                result.current.completeImplementation(createImplementationOutput({
                    code: 'x',
                    language: 'typescript',
                }));
            });
    ...
    describe('useSecurityAudit', () => {
        ...
        it('should add findings', () => {
            ...
            const { result } = renderHook(() => useSecurityAudit({ onFindingDetected }));
            act(() => {
                result.current.startAudit();
            });
            act(() => {
                result.current.addFinding('A03_INJECTION', 'high', 'SQL injection', 'Use params');
            });
    ...
    describe('useDesignPatterns', () => {
        ...
        it('should select pattern', () => {
            ...
            const { result } = renderHook(() => useDesignPatterns({ onPatternSelected }));
            act(() => {
        ...
        it('should add recommendation', () => {
            const { result } = renderHook(() => useDesignPatterns());
            act(() => {
                result.current.addRecommendation('STRATEGY', 'Allows flexible algorithm selection', ['Pro 1', 'Con 1'], 'Use interfaces');
            });
