...
import { OWASP_VULNERABILITIES, CODE_SMELL_PATTERNS, COMMON_CONTEXT_QUESTIONS, createContextRequirement, createSecurityFinding, createCodeSmellFinding, createTechnicalDebtItem, calculateRiskScore, createAuditResult, formatAuditResultAsMarkdown, } from '../phases/phase1-audit';
describe('Phase 1: Analysis & Audit', () => {
    ...
    describe('createContextRequirement', () => {
        it('should create a context requirement with defaults', () => {
            const req = createContextRequirement('Project dependencies', 'To check for vulnerabilities');
        ...
        it('should respect explicit priority', () => {
            const req = createContextRequirement('Test', 'Reason', 'required');
            expect(req.priority).toBe('required');
        });
    });
    describe('createSecurityFinding', () => {
        it('should create a security finding with required fields', () => {
            const finding = createSecurityFinding('A03_INJECTION', 'high', 'SQL injection in user input', 'Use parameterized queries');
        ...
        it('should include optional fields when provided', () => {
            const finding = createSecurityFinding('A03_INJECTION', 'critical', 'SQL injection', 'Use prepared statements', {
    ...
    describe('createCodeSmellFinding', () => {
        it('should create a code smell finding', () => {
            const smell = createCodeSmellFinding('HIGH_CYCLOMATIC_COMPLEXITY', 'high', 'Function has 15 decision points', 'Extract methods to reduce complexity');
        ...
        it('should include optional location and effort', () => {
            const smell = createCodeSmellFinding('GOD_OBJECT', 'medium', 'Class has 50 methods', 'Split into smaller classes', {
    ...
    describe('createTechnicalDebtItem', () => {
        it('should create a technical debt item with defaults', () => {
            const debt = createTechnicalDebtItem('ARCHITECTURAL_DEBT', 'high', 'No separation of concerns', 'Changes ripple across codebase', 'Implement clean architecture');
        ...
        it('should respect explicit interest rate', () => {
            const debt = createTechnicalDebtItem('TEST_DEBT', 'critical', 'No tests', 'Cannot refactor safely', 'Add test coverage', 'critical');
    ...
    describe('calculateRiskScore', () => {
        ...
        it('should weight security findings highest', () => {
            const securityFinding = createSecurityFinding('A03_INJECTION', 'critical', 'Test', 'Test');
            const scoreWithSecurity = calculateRiskScore([securityFinding], [], []);
            const scoreWithSmell = calculateRiskScore([], [createCodeSmellFinding('DEAD_CODE', 'critical', 'Test', 'Test')], []);
            expect(scoreWithSecurity).toBeGreaterThan(scoreWithSmell);
        });
        it('should cap at 100', () => {
            const criticalFindings = Array(10)
                .fill(null)
                .map(() => createSecurityFinding('A03_INJECTION', 'critical', 'Test', 'Test'));
    ...
    describe('COMMON_CONTEXT_QUESTIONS', () => {
        it('should contain common questions', () => {
            ...
            const hasRequired = COMMON_CONTEXT_QUESTIONS.some((q) => q.priority === 'required');
            expect(hasRequired).toBe(true);
        });
    });
});
//# sourceMappingURL=phase1-audit.test.js.map