 ...
 */
export function createSelfCorrectionItem(
  constraint: string,
  passed: boolean,
  details?: string,
 ...
 */
export function generateSelfCorrectionChecklist(
  checks: Array<{
    constraint: string
    passed: boolean
    details?: string
 ...
 */
export function createDefaultSelfCorrection(): SelfCorrectionItem[] {
 ...
 */
export function generateDRYSuggestions(
  violations: DRYViolationType[]
): Array<{ type: DRYViolationType; description: string; suggestion: string }> {
 ...
 */
export function formatADRAsMarkdown(
...
export function createReviewResult(params: {
  ...
  const qualityScore = calculateQualityScore(
    params.selfCorrections,
    params.dryCheck,
    params.securityRecheck
  )
 ...
 */
export function generateReviewBlockTemplate(): string {
  return `<REVIEW>
## Self-Correction Checklist
- [ ] All required functionality implemented
- [ ] Code is DRY (no duplication)
 ...
 */
export function isADRNeeded(decisionType: string): boolean {
  const adrTriggers = [
    'new technology',
    'new framework',
    'architecture change',
