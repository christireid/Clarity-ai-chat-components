 ...
 */
export const STYLE_GUIDE_PRESETS = {
    'typescript-airbnb': {
        language: 'typescript',
        styleGuide: 'airbnb',
        naming: {
 ...
 */
export function getStyleGuidePreset(language, styleGuide = 'airbnb') {
    const key = `${language}-${styleGuide}`;
    if (STYLE_GUIDE_PRESETS[key]) {
        return STYLE_GUIDE_PRESETS[key];
    }
    // Fallback to language default
    const languageDefaults = Object.keys(STYLE_GUIDE_PRESETS).find((k) => k.startsWith(`${language}-`));
    if (languageDefaults) {
 ...
 */
export const LEGACY_IMPROVEMENT_PATTERNS = {
    ADD_TYPES: {
        name: 'Add Types',
 ...
 */
export function createLegacyImprovement(type, description, options) {
    return {
        type,
        description,
 ...
 */
export function formatImplementationAsMarkdown(output) {
    const lines = [];
 ...
 */
export function generateCodeBlockTemplate(language) {
 ...
 */
export function validateAgainstStyleGuide(code, rules) {
    const issues = [];
 ...
 */
export function getDocumentationRequirements(language) {
    return {
 ...
 */
export function generateDocTemplate(language, functionInfo) {
    switch (language) {
        case 'typescript':
        case 'javascript':
...
//# sourceMappingURL=phase3-implementation.js.map