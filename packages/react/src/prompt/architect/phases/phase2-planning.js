 ...
 */
export const DESIGN_PATTERN_CATALOG = {
    // Creational Patterns
    SINGLETON: {
        name: 'Singleton',
        category: 'creational',
 ...
 */
export function createPatternRecommendation(pattern, rationale, customTradeoffs, implementationNotes) {
 ...
 */
export function createRejectedAlternative(name, description, rejectionReason, preferredWhen) {
    return {
        name,
        description,
        rejectionReason,
 ...
 */
export function calculateTotalComplexity(steps) {
    ...
    const totalWeight = steps.reduce((sum, step) => sum + complexityWeights[step.complexity], 0);
 ...
 */
export function formatStrategicPlanAsMarkdown(plan) {
    const lines = [];
    ...
    if (plan.testPlan.length > 0) {
        ...
        for (const test of plan.testPlan) {
            lines.push(`| ${test.priority} | ${test.strategy} | ${test.target} | ${test.description} |`);
        }
        lines.push('');
    }
    return lines.join('\n');
 ...
 */
export function generatePlanningBlockTemplate() {
    return `<PLANNING>
## Understanding
 ...
 */
export function getPatternIdsByCategory(category) {
    return Object.keys(DESIGN_PATTERN_CATALOG).filter((pattern) => DESIGN_PATTERN_CATALOG[pattern].category === category);
}
/**
 * Suggest patterns for a use case
 */
export function suggestPatternsForUseCase(useCase) {
    ...
    for (const [pattern, info] of Object.entries(DESIGN_PATTERN_CATALOG)) {
        ...
        for (const uc of info.useCases) {
            ...
            if (normalizedUseCase.includes(ucNormalized) ||
        ...
        if (score > 0) {
            matches.push({ pattern: pattern, score });
        }
    }
    return matches
...
//# sourceMappingURL=phase2-planning.js.map