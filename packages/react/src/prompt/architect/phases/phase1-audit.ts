 ...
 */
export const OWASP_VULNERABILITIES: Record<
  OWASPVulnerability,
  {
    name: string
    description: string
...
> = {
  ...
  A05_SECURITY_MISCONFIGURATION: {
    ...
    description:
      'Missing or incorrect security hardening across application stack',
    patterns: [
      'Default credentials',
 ...
 */
export const CODE_SMELL_PATTERNS: Record<
  CodeSmellType,
  {
    name: string
    description: string
 ...
 */
export function createContextRequirement(
  requirement: string,
  reason: string,
 ...
 */
export function createCodeSmellFinding(
  type: CodeSmellType,
  severity: DebtSeverity,
  description: string,
...
export function createTechnicalDebtItem(
  ...
  impact: string,
  remediation: string,
  interestRate: 'low' | 'medium' | 'high' | 'critical' = 'medium'
): TechnicalDebtItem {
  return {
    type,
    severity,
...
function generateAuditSummary(
  ...
  if (securityFindings.length > 0) {
    ...
    parts.push(
      `Security: ${securityFindings.length} findings (${critical} critical, ${high} high)`
    )
  }
 ...
 */
export function formatAuditResultAsMarkdown(result: AuditResult): string {
  const lines: string[] = []
  ...
  if (result.securityFindings.length > 0) {
    ...
    for (const finding of result.securityFindings) {
      lines.push(
        `| ${finding.severity.toUpperCase()} | ${finding.type} | ${finding.description} | ${finding.recommendation} |`
      )
    }
    lines.push('')
  }
 ...
 */
export const COMMON_CONTEXT_QUESTIONS: ContextRequirement[] = [
  createContextRequirement(
    'Project dependencies and versions',
    'To check for known vulnerabilities and compatibility',
    'required'
  ),
  createContextRequirement(
    'Target environment (browser, Node.js, etc.)',
    'To ensure code works in the target runtime',
    'required'
  ),
  createContextRequirement(
    'Existing test coverage',
    'To understand what testing is needed',
    'recommended'
  ),
  createContextRequirement(
    'Authentication/authorization requirements',
    'To implement proper security controls',
    'recommended'
  ),
  createContextRequirement(
    'Performance requirements',
    'To make appropriate optimization decisions',
    'optional'
  ),
  createContextRequirement(
    'Coding style guide in use',
    'To maintain consistency with existing code',
    'recommended'
  ),
]
