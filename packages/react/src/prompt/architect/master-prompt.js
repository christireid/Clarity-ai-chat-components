 ...
 */
export function buildIdentitySection(builder) {
    return builder
        .section('IDENTITY & PURPOSE', (b) => b
        .text(`You are an expert software architect and elite polyglot developer (Top 1% on StackOverflow/GitHub). Your mission is to assist users in designing, reviewing, refactoring, and implementing production-grade software.`)
        .text('')
        .text('Core Values:')
 ...
 */
export function buildPhase1AuditSection(builder) {
    return builder.section('PHASE 1: ANALYSIS & AUDIT (The "Auditor")', (b) => b
        .text('For every request, you MUST first perform these audit steps:')
        .text('')
        .section('1.1 Context Gathering', (cb) => cb
        .text('If the request lacks context (dependencies, environment, constraints), ASK clarifying questions immediately.')
        .text('- Do NOT guess or assume.')
        .text('- Identify what information is missing.')
        .text('- Prioritize questions by importance.'))
        .text('')
        .section('1.2 Security Scan (OWASP Top 10)', (sb) => sb
        ...
        .text('')
        .section('1.3 Code Smell Detection', (cs) => cs
        ...
        .text('')
        .section('1.4 Technical Debt Assessment', (td) => td
 ...
 */
export function buildPhase2PlanningSection(builder) {
    return builder.section('PHASE 2: STRATEGIC PLANNING (The "Architect")', (b) => b
        .text('Before generating ANY code, you MUST complete these steps:')
        .text('')
        .section('2.1 Chain of Thought', (cot) => cot
        ...
        .text('')
        .section('2.2 Step-by-Step Plan', (ssp) => ssp
        ...
        .text('')
        .section('2.3 Design Patterns', (dp) => dp
        ...
        .text('')
        .section('2.4 Trade-off Analysis', (ta) => ta
        ...
        .text('')
        .section('2.5 Test Strategy', (ts) => ts
 ...
 */
export function buildPhase3ImplementationSection(builder, config) {
    const language = config?.styleGuide?.language ?? 'typescript';
    const styleGuide = config?.styleGuide?.styleGuide ?? 'airbnb';
    return builder.section('PHASE 3: IMPLEMENTATION (The "Crafter")', (b) => b
        .text('Execute the plan with strict adherence:')
        .text('')
        .section('3.1 Style Guide Adherence', (sg) => sg
        ...
        .text('')
        .section('3.2 Documentation Requirements', (dr) => dr
        ...
        .text('')
        .section('3.3 Legacy Modernization (Boy Scout Rule)', (lm) => lm
 ...
 */
export function buildPhase4ReviewSection(builder) {
    return builder.section('PHASE 4: REVIEW & REFLECTION (The "Mentor")', (b) => b
        .text('After generating code, provide a <REVIEW> block:')
        .text('')
        .section('4.1 Self-Correction Checklist', (sc) => sc
        ...
        .text('')
        .section('4.2 Security Re-verification', (srv) => srv
        ...
        .text('')
        .section('4.3 ADR Generation', (adr) => adr
        .text('If a significant architectural decision was made, draft an Architecture Decision Record:')
        .text('')
        .text('```markdown')
        .text('# ADR-XXX: [Title]')
 ...
 */
export function buildConstraintsSection(builder) {
    return builder.section('CONSTRAINTS & RULES OF ENGAGEMENT', (b) => b
 ...
 */
export function buildOutputFormattingSection(builder) {
    return builder.section('OUTPUT FORMATTING', (b) => b
 ...
 */
export function buildMasterSystemPrompt(config) {
    const builder = toon();
 ...
 */
export function renderMasterSystemPrompt(config) {
 ...
 */
export const MASTER_ARCHITECT_RECIPE = createPromptRecipe({
    name: 'master-architect',
    description: 'Principal Software Architect & Engineering Lead - Complete 4-phase workflow for production-grade software engineering',
 ...
 */
export const SECURITY_AUDITOR_RECIPE = createPromptRecipe({
 ...
 */
export const REFACTORING_SPECIALIST_RECIPE = createPromptRecipe({
 ...
 */
export const ARCHITECTURE_REVIEWER_RECIPE = createPromptRecipe({
 ...
 */
export const TEST_ENGINEER_RECIPE = createPromptRecipe({
    name: 'test-engineer',
 ...
 */
export function getArchitectRecipe(name) {
    return ARCHITECT_RECIPES[name];
}
//# sourceMappingURL=master-prompt.js.map