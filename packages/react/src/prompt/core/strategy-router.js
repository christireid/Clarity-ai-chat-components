 ...
 */
export function chooseOptimizationStrategy(currentTokens, targetTokens, modelProfile) {
 ...
 */
export async function shouldSwitchModel(currentModel, targetTokens, costBudget, availableModels) {
    return shouldSwitchModelSync(currentModel, targetTokens, costBudget, availableModels);
}
/**
 * Determine if model should be switched (sync version)
 */
export function shouldSwitchModelSync(currentModel, targetTokens, costBudget, availableModels = MODEL_PROFILES) {
    ...
    if (costBudget !== undefined) {
        ...
        if (estimatedCost > costBudget) {
            ...
            if (cheaperModels.length > 0) {
                ...
                const savings = (targetTokens / 1000) *
 ...
 */
export function analyzeTokenCapacity(currentTokens, modelProfile) {
 ...
 */
export function getRoutingRecommendation(estimatedInputTokens, estimatedOutputTokens, currentModel, preferences = {}) {
    const currentProfile = typeof currentModel === 'string'
        ? getModelProfileOrDefault(currentModel)
        : currentModel;
    const totalTokens = estimatedInputTokens + estimatedOutputTokens;
    // Filter models that can handle the request
    const viableModels = Object.values(MODEL_PROFILES).filter((p) => p.maxTokens >= totalTokens && p.maxOutputTokens >= estimatedOutputTokens);
    ...
    if (preferences.preferCost) {
        ...
        sortedModels = [...viableModels].sort((a, b) => a.costPer1K - b.costPer1K // Cheaper usually means faster
        );
    }
    else if (preferences.preferQuality) {
        // Prefer larger, more capable models
        sortedModels = [...viableModels].sort((a, b) => b.costPer1K - a.costPer1K // More expensive usually means better
        );
    }
    else {
        ...
        sortedModels = [...viableModels].sort((a, b) => {
            // Prefer same family
            if (a.family === currentProfile.family && b.family !== currentProfile.family)
                return -1;
            if (b.family === currentProfile.family && a.family !== currentProfile.family)
                return 1;
            // Then by cost
...
//# sourceMappingURL=strategy-router.js.map