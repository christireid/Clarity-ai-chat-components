...
export async function optimizePrompt(
  ...
  const {
    ...
    targetTokens: targetTokensInput,
  ...
  const modelProfile =
    typeof modelProfileInput === 'string'
  ...
  let messages = await lexingStage(
    toonDefinition,
    variables,
    inputMessages,
    memoryContext
  ...
  const strategySelection = chooseOptimizationStrategy(
    currentTokens,
    targetTokens,
    modelProfile
  )
  ...
  if (applyStyleTransformation) {
    ...
    if (currentTokens <= targetTokens) {
      return createResult(
        ...
        stages,
        true,
        'Style transformation was sufficient'
      )
    }
  }
  ...
  if (getEmbedding && userIntent) {
    ...
    if (currentTokens <= targetTokens) {
      return createResult(
        ...
        stages,
        true,
        'Semantic prioritization was sufficient'
      )
    }
  }
  ...
  if (currentTokens < beforeTokens) {
    stages.push({
      ...
      tokensAfter: currentTokens,
      tokensSaved: beforeTokens - currentTokens,
      details: compressionResult.logs.map(
        (l) => `${l.strategy}: ${l.description}`
      ),
    })
  }
 ...
 * Stage 1: Lexing - Convert inputs to messages
 */
async function lexingStage(
  toonDefinition: ToonNode[] | undefined,
  variables: Record<string, unknown>,
  inputMessages: CoreMessage[],
  memoryContext: string | CoreMessage[] | undefined
): Promise<CoreMessage[]> {
 ...
 */
async function structuringStage(
  messages: CoreMessage[],
...
async function analysisStage(
  ...
  return scoredMessages
    .sort((a, b) => {
      ...
      if (b.message.role === 'system' && a.message.role !== 'system') return 1
