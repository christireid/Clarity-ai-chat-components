 ...
 */
export class ApproximateTokenizer {
    name = 'approximate';
    charsPerToken;
 ...
 */
export function getTokenizerForModel(modelName, customTokenizer) {
    if (customTokenizer)
 ...
 */
export function estimateSingleMessageTokens(message, tokenizer = defaultTokenizer) {
 ...
 */
export function estimateMessageTokens(messages, tokenizer = defaultTokenizer) {
 ...
 */
export function estimatePromptTokens(input, variables = {}, tokenizer = defaultTokenizer) {
 ...
 */
function estimateToonNodeTokens(node, variables, tokenizer) {
    switch (node.type) {
        case 'text':
        ...
        case 'section': {
            let total = 0;
            if (node.title)
                total += tokenizer.estimate(node.title) + 2; // Title + newlines
 ...
 */
export function estimateCost(inputTokens, outputTokens, modelMetadata) {
