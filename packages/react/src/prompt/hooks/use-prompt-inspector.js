 ...
 */
export function usePromptInspector(options) {
    const { messages, modelMetadata: modelMetadataOption, detailed = false } = options;
    ...
    const breakdown = useMemo(() => {
        ...
        return messages.map(msg => {
            ...
            return {
                ...
                role: msg.role,
                tokens: msgTokens,
                content: detailed ? content : content.slice(0, 100) + (content.length > 100 ? '...' : ''),
    ...
    const formattedView = useMemo(() => {
        ...
        for (const [role, data] of Object.entries(byRole)) {
            details.push(`  ${role}: ${data.tokens} tokens (${data.count} messages, ${data.percentage.toFixed(1)}%)`);
        }
        if (detailed) {
            details.push('');
            details.push('Per-message breakdown:');
            for (const item of breakdown) {
                details.push(`  [${item.role}] ${item.tokens} tokens (${item.percentage.toFixed(1)}%): ${item.content.slice(0, 50)}...`);
            }
        }
...
//# sourceMappingURL=use-prompt-inspector.js.map