 ...
 */
export function usePromptDebugger(options) {
    const { diagnostics, routingDecision, messagesBefore = [], messagesAfter = [], detailed = true, } = options;
    ...
    const compressionLogs = useMemo(() => {
        const logs = [];
        for (const msg of messagesAfter) {
            if (msg.metadata?.compressed) {
                const originalLength = msg.metadata.originalTokens || msg.metadata.originalLength || 0;
                const compressedLength = msg.metadata.compressedTokens ||
    ...
    const dslTransformations = useMemo(() => {
        ...
        for (const msg of messagesAfter) {
            if (msg.metadata?.styleTransformed) {
                ...
                const before = beforeMsg
                    ...
                    : '';
                const after = typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content);
    ...
    const summary = useMemo(() => {
        ...
        const totalDuration = diagnostics.stages.reduce((sum, stage) => sum + (stage.duration || 0), 0);
        const compressionRatio = diagnostics.originalTokens > 0
    ...
    const formattedView = useMemo(() => {
        ...
        const stages = diagnostics
            ? diagnostics.stages.map((stage) => `${stage.name}: ${stage.tokensBefore} → ${stage.tokensAfter} tokens ` +
                `(${stage.tokensSaved > 0 ? '-' : '+'}${Math.abs(stage.tokensSaved)} tokens) ` +
                `[${stage.duration || 0}ms]`)
            : [];
        const compressionDetails = compressionLogs.map((log) => `Message ${log.messageId}: ${log.originalLength} → ${log.compressedLength} tokens ` +
...
//# sourceMappingURL=use-prompt-debugger.js.map