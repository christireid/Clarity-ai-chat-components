 ...
 * ```
 */
export function SupportBot({ botName: _botName = 'Support Bot', // Reserved for future use
botAvatar: _botAvatar, // Reserved for future use
welcomeMessage = "Hi! I'm here to help. What can I assist you with today?", quickReplies = defaultQuickReplies, knowledgeBase = defaultKnowledgeBase, escalationThreshold = 5, onEscalate, className = '', }) {
    ...
    return (_jsxs("div", { className: `support-bot-container ${className}`, children: [_jsx(ChatWindow, { messages: messages, onSendMessage: handleSendMessage, onEditMessage: handleEdit, onDeleteMessage: handleDelete, onExport: handleExport }), showQuickReplies && messages.length > 0 && (_jsxs("div", { className: "p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900", children: [_jsx("p", { className: "text-xs text-gray-500 dark:text-gray-500 mb-2", children: "Quick actions:" }), _jsx("div", { className: "flex flex-wrap gap-2", children: quickReplies.map((reply, index) => (_jsx("button", { onClick: () => handleQuickReply(reply.action), className: "px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-full text-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors", children: reply.text }, index))) })] }))] }));
}
//# sourceMappingURL=support-bot.js.map