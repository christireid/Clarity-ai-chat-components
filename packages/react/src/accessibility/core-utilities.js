 ...
 */
export function createLiveRegion(id = 'live-region') {
    if (typeof document === 'undefined') {
 ...
 */
export function meetsContrastRequirements(foreground, background, level = 'AAA') {
    const ratio = getContrastRatio(foreground, background);
 ...
 */
export function getAccessibleDuration(normalDuration, reducedDuration = 0) {
    if (typeof window === 'undefined')
 ...
 */
export function createAriaLabel(componentName, props) {
    const parts = [componentName];
    if (props.variant)
        parts.push(String(props.variant));
    if (props.size)
        parts.push(String(props.size));
    if (props.state)
        parts.push(String(props.state));
    if (props.label)
        parts.push(String(props.label));
    if (props.title)
        parts.push(String(props.title));
    return parts.join(' ').trim();
}
/**
 * Create accessibility props for buttons
 */
export function createButtonAccessibility(props, additionalProps = {}) {
    const accessibility = {
        ...additionalProps,
    };
    if (props.variant === 'loading') {
        ...
        accessibility['aria-label'] =
            props['aria-label'] || 'Button is loading';
    }
    if (props.disabled) {
    ...
    if (props.variant === 'destructive') {
        accessibility['aria-label'] =
            props['aria-label'] || 'Destructive action';
    }
 ...
 */
export function createInputAccessibility(props, additionalProps = {}) {
    const accessibility = {
        ...additionalProps,
    };
    if (props.required)
 ...
 */
export function createNavigationAccessibility(currentPath, itemPath, additionalProps = {}) {
    const accessibility = {
        ...additionalProps,
    };
 ...
 */
export function createModalAccessibility(isOpen, additionalProps = {}) {
    const accessibility = {
        ...additionalProps,
        role: 'dialog',
        'aria-modal': true,
