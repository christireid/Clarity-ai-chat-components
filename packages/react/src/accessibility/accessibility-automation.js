 ...
 */
export function useAutoAccessibility(options, config = {}) {
    const { componentType, content, ariaLabel, ariaLabelledBy, ariaDescribedBy, role, tabIndex, disabled, expanded, selected, checked, hasPopup, level, value, min, max, step, required, invalid, busy, live, atomic, relevant, } = options;
    const { componentMap = DEFAULT_COMPONENT_MAP, autoGenerateIds = true, validateOnMount = true, announceChanges = true, keyboardNavigation = true, focusManagement = true, } = config;
    ...
    const accessibilityAttributes = useMemo(() => {
        ...
        if (value !== undefined) {
            ...
            if (step !== undefined)
                attributes['aria-valuetext'] = `Step ${step}`;
        }
        // Form attributes
    ...
    const keyboardHandlers = useMemo(() => {
        const handlers = {};
        const componentConfig = componentMap[componentType];
        if (!componentConfig || !keyboardNavigation)
            return handlers;
        // Focus management
    ...
    const accessibilityReport = useMemo(() => {
        ...
        if (componentConfig.labelSource === 'aria-label' &&
            !ariaLabel &&
            !ariaLabelledBy) {
            report.warnings.push('Component should have aria-label or aria-labelledby');
        }
        if (componentConfig.labelSource === 'content' &&
            !content &&
            !ariaLabel &&
            !ariaLabelledBy) {
 ...
 */
export function validateAccessibility(element, componentType, config = {}) {
    const report = {
        isValid: true,
        warnings: [],
        errors: [],
 ...
 */
export function useAccessibilityAnnouncement(message, priority = 'polite') {
    useEffect(() => {
        if (!message)
            return;
 ...
 */
export function useFocusManagement(isEnabled = true) {
...
//# sourceMappingURL=accessibility-automation.js.map