...
export class VectorStoreConfigError extends Error {
    provider;
    field;
    constructor(message, provider, field) {
        super(message);
 ...
 */
function validateRequiredString(value, fieldName, provider) {
    if (typeof value !== 'string' || value.trim() === '') {
        throw new VectorStoreConfigError(`${fieldName} is required and must be a non-empty string`, provider, fieldName);
    }
}
/**
 * Validate Pinecone configuration
 */
function validatePineconeConfig(config) {
    ...
    if (config.environment !== undefined &&
        ...
        throw new VectorStoreConfigError('environment must be a string', 'pinecone', 'environment');
    }
}
/**
...
function validateQdrantConfig(config) {
    ...
    if (config.endpoint !== undefined) {
        ...
        if (!config.endpoint.startsWith('http://') &&
            ...
            throw new VectorStoreConfigError('endpoint must be a valid URL starting with http:// or https://', 'qdrant', 'endpoint');
        }
    }
    ...
    if (config.dimension !== undefined) {
        if (typeof config.dimension !== 'number' ||
            ...
            throw new VectorStoreConfigError('dimension must be a positive integer', 'qdrant', 'dimension');
        }
    }
}
/**
 ...
 */
function validateWeaviateConfig(config) {
    validateRequiredString(config.indexName, 'indexName', 'weaviate');
    validateRequiredString(config.endpoint, 'endpoint', 'weaviate');
    // Validate endpoint is a valid URL
    if (!config.endpoint.startsWith('http://') &&
        !config.endpoint.startsWith('https://')) {
        throw new VectorStoreConfigError('endpoint must be a valid URL starting with http:// or https://', 'weaviate', 'endpoint');
    }
}
/**
 * Validate Chroma configuration
 */
function validateChromaConfig(config) {
    validateRequiredString(config.indexName, 'indexName', 'chroma');
    // Endpoint validation (optional - defaults to localhost)
    if (config.endpoint !== undefined) {
        if (typeof config.endpoint !== 'string' || config.endpoint.trim() === '') {
            throw new VectorStoreConfigError('endpoint must be a non-empty string', 'chroma', 'endpoint');
        }
    }
}
/**
...
export function createVectorStore(config) {
    ...
    if (!config || typeof config !== 'object') {
        throw new VectorStoreConfigError('Configuration object is required', 'unknown');
    }
    switch (config.provider) {
        ...
        case 'custom':
            throw new VectorStoreConfigError('Custom vector store requires manual implementation. ' +
                'Please implement the VectorStore interface directly.', 'custom');
        default: {
            const _exhaustiveCheck = config;
            throw new VectorStoreConfigError(`Unsupported vector store provider: "${_exhaustiveCheck.provider}". ` +
                'Supported providers: pinecone, qdrant, weaviate, chroma', _exhaustiveCheck.provider);
        }
    }
}
