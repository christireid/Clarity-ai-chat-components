...
export function AnalyticsProvider({ children, config, }) {
    ...
    const currentUserId = React.useRef(undefined);
    ...
    React.useEffect(() => {
        ...
        const initializeProviders = async () => {
            if (config.debug) {
                console.debug('[Analytics] Initializing providers:', config.providers?.map((p) => p.name));
            }
            try {
                await Promise.all(config.providers.map((provider) => provider.init?.()));
                setIsInitialized(true);
                if (config.debug) {
    ...
    React.useEffect(() => {
        if (!isEnabled || !config.autoTrackPageViews || !isInitialized)
            return;
        const trackPageView = () => {
            const pageView = {
    ...
    // Auto-track errors
    React.useEffect(() => {
        if (!isEnabled || !config.autoTrackErrors || !isInitialized)
            return;
    ...
    // Identify function
    const identify = React.useCallback((user) => {
        if (!isEnabled || !isInitialized)
            return;
    ...
    // Page function
    const page = React.useCallback((pageView) => {
        if (!isEnabled || !isInitialized)
            return;
        if (config.debug) {
...
export function useAnalytics() {
    ...
    if (!context) {
        throw new Error('[useAnalytics] AnalyticsProvider is not available. ' +
