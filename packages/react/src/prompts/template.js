...
export class PromptTemplateEngine {
    ...
    render(template, options) {
        ...
        const variablePattern = new RegExp(`${this.escapeRegExp(delimiter.start)}\\s*([\\w.]+)\\s*${this.escapeRegExp(delimiter.end)}`, 'g');
        const foundVariablesSet = new Set();
        let match;
        while ((match = variablePattern.exec(templateStr)) !== null) {
            const varName = match[1];
            if (varName)
                foundVariablesSet.add(varName);
        }
        ...
        for (const varName of foundVariables) {
            ...
            if (value !== undefined) {
                ...
                const varPattern = new RegExp(`${this.escapeRegExp(delimiter.start)}\\s*${this.escapeRegExp(varName)}\\s*${this.escapeRegExp(delimiter.end)}`, 'g');
    ...
    extractVariables(template) {
        const pattern = new RegExp(`${this.escapeRegExp(this.delimiter.start)}\\s*([\\w.]+)\\s*${this.escapeRegExp(this.delimiter.end)}`, 'g');
