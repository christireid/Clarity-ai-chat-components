...
export class ComponentError extends Error {
    ...
    cause;
    constructor(options) {
        const { code, component, message, received, expected, example, docsPath, cause, } = options;
 ...
 * ```
 */
export function invariant(condition, options) {
    if (!condition) {
 ...
 * ```
 */
export function createComponentInvariant(component) {
    return function componentInvariant(condition, code, message, details) {
        if (!condition) {
            throw new ComponentError({
                code,
                component,
 ...
 * ```
 */
export function throwMissingProviderError(component, provider) {
    throw new ComponentError({
 ...
 * ```
 */
export function throwInvalidPropError(component, propName, received, expected) {
    throw new ComponentError({
        code: 'INVALID_PROP',
        component,
 ...
 * ```
 */
export function throwMissingPropError(component, propName) {
    throw new ComponentError({
        code: 'MISSING_PROP',
        component,
 ...
 * ```
 */
export function createErrorHandler(component, onError) {
    return (error, errorInfo) => {
        // Enhance error with component context if not already a ComponentError
        if (!(error instanceof ComponentError)) {
            console.error(`[Clarity Chat] ${component} encountered an error:`, error, '\n\nComponent Stack:', errorInfo.componentStack);
        }
        else {
 ...
 * ```
 */
export function withComponentErrorHandling(component, fn) {
    return ((...args) => {
        try {
