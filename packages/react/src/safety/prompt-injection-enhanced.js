...
export class EnhancedPromptInjectionGuardrail {
    ...
    detectHeuristic(message) {
        ...
        // Instruction override
        if (/ignore|disregard|forget|override/i.test(message) &&
    ...
    detectMultiTurnAttack() {
        ...
        const roleShiftCount = recentMessages.filter((msg) => /you\s+are|your\s+role|act\s+as/i.test(msg.content)).length;
        if (roleShiftCount >= 2) {
        ...
        const instructionOverrides = recentMessages.filter((msg) => /ignore|disregard|forget|override|instead/i.test(msg.content)).length;
    ...
    async detectLLMJudge(message) {
        ...
        try {
            const prompt = this.config.llmJudge.prompt ||
...
Respond with JSON:
{
  ...
  "explanation": "brief explanation"
     ...
     */
    aggregateResults(results, originalMessage) {
        if (results.length === 0) {
            return {
                safe: true,
                confidence: 0,
 ...
 */
export async function detectPromptInjection(message, config) {
    const guard = new EnhancedPromptInjectionGuardrail(config);
    return guard.check(message);
}
//# sourceMappingURL=prompt-injection-enhanced.js.map