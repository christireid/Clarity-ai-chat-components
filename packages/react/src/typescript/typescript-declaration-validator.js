...
export class TypeScriptDeclarationValidator extends EventEmitter {
     ...
     * Generate validation issues
     */
    generateIssues(bundleAnalysis, tsConfig, memoryAnalysis) {
        const issues = [];
     ...
     * Generate recommendations
     */
    generateRecommendations(bundleAnalysis, tsConfig, memoryAnalysis) {
        const recommendations = [];
     ...
     */
    calculateScore(bundleAnalysis, issues) {
        let score = 100;
        ...
        const totalSizeKB = bundleAnalysis.totalSize / 1024;
        if (totalSizeKB > this.config.bundleSizeLimit) {
            const excessRatio = (totalSizeKB - this.config.bundleSizeLimit) /
...
export class DeclarationGenerator extends EventEmitter {
     ...
     */
    async generateDeclarations(projectPath, outputPath) {
     ...
     */
    async runTypeScriptCompiler(projectPath, outputPath) {
        try {
...
export class BundleSizeAnalyzer extends EventEmitter {
     ...
     * Analyze bundle size and composition
     */
    async analyzeBundleSize(outputPath) {
...
//# sourceMappingURL=typescript-declaration-validator.js.map