...
export class PromptCompressor {
    ...
    compress(text, options = {}) {
        const { targetRatio = 8, preserveKeywords = true, preserveStructure = true, minLength = 100, } = options;
    ...
    compressMessages(messages, options = {}) {
        ...
        const totalOriginal = messages.reduce((sum, msg) => sum + this.countTokens(msg.content), 0);
        const totalCompressed = compressedMessages.reduce((sum, msg) => sum + this.countTokens(msg.content), 0);
        return {
            ...
            result: {
                ...
                semanticRetention: this.estimateSemanticRetention(totalOriginal / totalCompressed),
            },
        };
    }
    /**
     ...
     */
    removeFillerWords(text, preserveKeywords) {
        const fillerWords = [
            'actually',
            'basically',
            'essentially',
     ...
     */
    summarizeSentences(text, targetRatio) {
        ...
        const sampleSize = Math.max(1, Math.floor(sentences.length / targetRatio));
     ...
     */
    aggressiveCompression(text, targetRatio) {
        ...
        const compressedSentences = sentences
            ...
            const hasKeywords = sentenceWords.some((w) => topKeywords.some((kw) => w.includes(kw)));
            if (hasKeywords) {
     ...
     */
    estimateSemanticRetention(compressionRatio) {
