 ...
 */
export function useMemoryService(config) {
...
export function useMemories(userId, options) {
    ...
    const retrieve = useCallback(async (retrievalOptions) => {
        setIsLoading(true);
        setError(null);
        try {
    ...
    const store = useCallback(async (memory, storageOptions) => {
        setIsLoading(true);
        setError(null);
        try {
 ...
 */
export function useMemoryStats(userId) {
    const service = useMemoryService();
 ...
 */
export function useMemoryStorage(userId) {
    const { store, isLoading, error } = useMemories(userId);
    const storeFromMessage = useCallback(async (message, options) => {
        return store({
            userId,
