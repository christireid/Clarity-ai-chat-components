 ...
 * - Entity density
 */
export function calculateMessageImportance(message, options) {
 ...
 * ```
 */
export function reorderForAttention(messages, options = { strategy: 'importance-weighted' }) {
    ...
    let orderedNonSystem = [];
    const positions = new Array(messages.length).fill('middle');
 ...
 * ```
 */
export function addStructuralMarkers(messages, options = {}) {
    ...
    for (const msg of systemMessages) {
        const section = sections[0] || { name: 'system', type: 'context' };
    ...
    for (let i = 0; i < nonSystemMessages.length; i++) {
        ...
        const section = nonSystemSections[sectionIdx] || { name: 'conversation', type: 'history' };
        ...
        if (addNumberedRefs) {
            ...
            const markedSentences = sentences.map(sentence => {
                const isKeyFact = /\d/.test(sentence) || // Contains numbers
 ...
 */
function wrapInSection(content, sectionName) {
 ...
 * ```
 */
export function optimizeContextLayout(messages, options = {}) {
    const { maxTokens, strategy = 'importance-weighted', addStructuralMarkers: useMarkers = false, recencyWeight = 0.3, } = options;
    ...
    let finalMessages = ordered.messages;
    if (useMarkers) {
...
export function createContextOrderingHook() {
    return function useContextOrdering(messages, options = {}) {
 ...
 */
export function estimateAttentionLoss(messages) {
...
//# sourceMappingURL=context-ordering.js.map