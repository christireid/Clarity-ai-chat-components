 ...
 */
export class FIFOTruncation {
    name = 'fifo';
    truncate(messages, options) {
 ...
 */
export class SlidingWindowTruncation {
    windowSize;
    name = 'sliding-window';
...
export class SmartTruncation {
    ...
    truncate(messages, options) {
        ...
        for (let i = turns.length - 1; i >= 0; i--) {
            ...
            if (!turn)
                continue;
            const turnTokens = turn.reduce((sum, msg) => sum + options.countTokens(msg.content), 0);
 ...
 */
export class SummarizationTruncation {
    summarize;
    summarizeAfter;
    name = 'summarization';
    constructor(summarize, summarizeAfter = 10) {
...
export class ContextWindowManager {
     ...
     */
    fitsInWindow(messages) {
        const totalTokens = messages.reduce((sum, msg) => sum + this.options.countTokens(msg.content), 0);
    ...
    countTokens(messages) {
        return messages.reduce((sum, msg) => sum + this.options.countTokens(msg.content), 0);
    }
    /**
