 ...
 */
function calculateBoost(token, context) {
    let boost = 1.0;
 ...
 */
export function calculateTokenImportance(tokens, context) {
...
function extractPreservedBlocks(text) {
    ...
    while ((match = inlineCodeRegex.exec(text)) !== null) {
        ...
        const isInsideBlock = blocks.some((b) => match.index >= b.startIndex && match.index < b.endIndex);
        if (!isInsideBlock) {
            blocks.push({
                type: 'code',
                content: match[0],
 ...
 */
function compressWithImportance(text, options) {
    const { targetRatio = 0.5, useImportanceScoring = true, preserveSentences = true, } = options;
    ...
    const keptTokens = importanceScores.filter((s) => keptTokenSet.has(s.token.toLowerCase()));
 ...
 * ```
 */
export function compressPromptSemantic(prompt, options = {}) {
    const { targetRatio = 0.5, preserveStructure = true, minTokensToCompress = 100, } = options;
 ...
 */
export function compressPromptCombined(prompt, options = {}) {
 ...
 */
export function estimateCompressibility(prompt) {
...
//# sourceMappingURL=prompt-compression-advanced.js.map