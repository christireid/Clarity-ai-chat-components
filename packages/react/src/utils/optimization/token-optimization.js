...
export function limitHistoryFIFO(messages, options = {}) {
    ...
    let tokensUsed = lastMessages.reduce((sum, msg) => sum +
        estimateTokens(typeof msg.content === 'string'
            ? msg.content
            : JSON.stringify(msg.content)), 0);
    // Add system message tokens
    tokensUsed += systemMessages.reduce((sum, msg) => sum +
        estimateTokens(typeof msg.content === 'string'
            ? msg.content
            : JSON.stringify(msg.content)), 0);
    ...
    for (let i = remainingMessages.length - 1; i >= 0; i--) {
        ...
        if (!msg)
            continue;
        const msgTokens = estimateTokens(typeof msg.content === 'string'
            ? msg.content
...
export function limitHistorySmart(messages, options = {}) {
    ...
    let tokensUsed = systemMessages.reduce((sum, msg) => sum +
        estimateTokens(typeof msg.content === 'string'
            ? msg.content
            : JSON.stringify(msg.content)), 0);
    ...
    for (let i = turns.length - 1; i >= 0; i--) {
        ...
        if (!turn)
            continue;
        const turnTokens = turn.reduce((sum, msg) => sum +
            estimateTokens(typeof msg.content === 'string'
                ? msg.content
                : JSON.stringify(msg.content)), 0);
