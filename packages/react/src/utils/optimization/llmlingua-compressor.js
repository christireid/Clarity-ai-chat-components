 ...
 */
function calculatePerplexityScore(word, context, position, totalTokens) {
 ...
 */
function compressWithPerplexity(text, targetRatio, preserveFirst, preserveLast) {
    ...
    const targetTokenCount = Math.max(Math.ceil(tokens.length * targetRatio), preserveFirst + preserveLast);
 ...
 */
function iterativeCompress(text, targetRatio, iterations, preserveFirst, preserveLast) {
    let currentText = text;
    const iterationRatio = Math.pow(targetRatio, 1 / iterations);
    for (let i = 0; i < iterations; i++) {
        const result = compressWithPerplexity(currentText, iterationRatio, Math.floor(preserveFirst / (i + 1)), Math.floor(preserveLast / (i + 1)));
...
export class LLMLinguaCompressor {
    ...
    async compress(text, options) {
        ...
        if (config.iterativeMode) {
            ...
            compressed = iterativeCompress(text, config.targetRatio, config.iterations, config.preserveFirst, config.preserveLast);
            preservedSegments = []; // Not tracked in iterative mode
        }
        else {
            // Single-pass compression
            const result = compressWithPerplexity(text, config.targetRatio, config.preserveFirst, config.preserveLast);
     ...
     */
    async compressWithSegments(text, segments) {
        const startTime = performance.now();
        // Input validation
        if (text == null || typeof text !== 'string') {
            return {
        ...
        for (const part of parts) {
            if (part.preserve) {
                ...
                const result = compressWithPerplexity(part.text, this.config.targetRatio, 0, // No first/last preservation within segment
                0);
 ...
 * ```
 */
export async function compressRAGContext(ragResults, config = {}, existingCompressor) {
    const compressor = existingCompressor ?? new LLMLinguaCompressor(config);
    const compressedResults = await Promise.all(ragResults.map(async (text) => {
 ...
 */
export async function intelligentCompress(text, options = {}) {
    const { targetRatio = 0.25, preserveCode = true, preserveFirst = 100, preserveLast = 50, } = options;
    const startTime = performance.now();
 ...
 */
export function createLLMLinguaCompressor(config) {
