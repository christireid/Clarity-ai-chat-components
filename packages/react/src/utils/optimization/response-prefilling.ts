...
export interface PrefillConfig {
  ...
  prefill: string
  /** Whether this prefill forces structured output (e.g., JSON) */
 ...
 */
export const PREFILL_TEMPLATES: Record<string, PrefillTemplate> = {
 ...
 * ```
 */
export function createAnthropicPrefill(
 ...
 * ```
 */
export function createOpenAIPrefill(
  messages: Array<{ role: string; content: string }>,
  config: PrefillConfig
): {
  messages: Array<{ role: string; content: string }>
 ...
 */
function getFormatInstructions(config: PrefillConfig): string {
 ...
 */
export function extractPrefillContent(
  response: string,
  config: PrefillConfig
): string {
 ...
 */
export function estimatePrefillSavings(config: PrefillConfig): {
  minSavedTokens: number
  maxSavedTokens: number
  averageSavedTokens: number
 ...
 */
export function choosePrefillTemplate(query: string): PrefillTemplate | null {
 ...
 */
export function createCustomPrefill(
  prefill: string,
  options: {
    structuredOutput?: boolean
    contentType?: PrefillConfig['contentType']
  } = {}
): PrefillConfig {
  return {
    prefill,
