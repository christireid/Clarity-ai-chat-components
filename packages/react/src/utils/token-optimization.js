 ...
 */
function removeFillers(text, preserveKeywords = []) {
 ...
 * ```
 */
export function shortenPrompt(prompt, options = {}) {
    const { removeDuplicates = true, removeFillers: shouldRemoveFillers = true, simplifySentences: shouldSimplifySentences = true, targetReduction = 0.2, preserveKeywords = [], } = options;
 ...
 */
export function calculateTokenSavings(original, shortened) {
 ...
 */
export function limitHistorySlidingWindow(messages, options = {}) {
 ...
 */
export function limitHistoryFIFO(messages, options = {}) {
    ...
    let tokensUsed = lastMessages.reduce((sum, msg) => sum + estimateTokens(typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)), 0);
    // Add system message tokens
    tokensUsed += systemMessages.reduce((sum, msg) => sum + estimateTokens(typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)), 0);
 ...
 */
export function limitHistorySmart(messages, options = {}) {
    ...
    let tokensUsed = systemMessages.reduce((sum, msg) => sum + estimateTokens(typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)), 0);
    ...
    for (let i = turns.length - 1; i >= 0; i--) {
        ...
        if (!turn)
            continue;
        const turnTokens = turn.reduce((sum, msg) => sum + estimateTokens(typeof msg.content === 'string' ? msg.content : JSON.stringify(msg.content)), 0);
 ...
 */
export function classifyQueryComplexity(query, threshold = 50) {
 ...
 */
export function routeToModel(query, options = {}) {
    const { complexityThreshold = 50, simpleModel = 'gpt-3.5-turbo', complexModel = 'gpt-4', } = options;
 ...
 */
export function estimateRoutingSavings(query, options = {}) {
    const { complexityThreshold = 50, simpleModel = 'gpt-3.5-turbo', complexModel = 'gpt-4', simpleModelCost = 0.0000005, complexModelCost = 0.00003, } = options;
    const tokens = estimateTokens(query);
    const complexity = classifyQueryComplexity(query, complexityThreshold);
 ...
 */
export function shouldUseReference(data, options = {}) {
 ...
 */
export function createReference(data, options = {}) {
 ...
 */
export function enforceOutputLimit(output, options = {}) {
 ...
 */
export function findSimilarCached(query, cache, threshold = 0.7) {
