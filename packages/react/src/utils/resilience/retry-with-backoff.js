 ...
 * ```
 */
import { calculateRetryDelay, } from '../api/rate-limit-headers';
/**
 ...
 */
function calculateBackoffDelay(attempt, baseDelay, maxDelay, jitter, rateLimitInfo) {
...
export async function retryWithBackoff(fn, options = {}) {
    const { maxRetries = 3, baseDelay = 1000, maxDelay = 30000, jitter = 0.5, shouldRetry = defaultShouldRetry, onRetry, signal, getRateLimitInfo, } = options;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        try {
            ...
            const rateLimitInfo = getRateLimitInfo?.(lastError) ??
                lastError.rateLimitInfo;
            // Calculate delay
            const delay = calculateBackoffDelay(attempt, baseDelay, maxDelay, jitter, rateLimitInfo);
            // Notify callback
 ...
 */
export const AI_API_RETRY_OPTIONS = {
    maxRetries: 3,
    baseDelay: 1000,
 ...
 */
export const STREAMING_RETRY_OPTIONS = {
