 ...
 */
export function createLimitedPrompt(userPrompt, config = {}) {
    const { maxTokens, maxCharacters, enforceFormat = 'none', stopSequences = [], temperature, brevityLevel = 0.5, } = config;
    const constraints = [];
    const instructions = [];
    ...
    const brevityKey = brevityLevel < 0.25 ? 'low' :
 ...
 */
export function enforceResponseLimits(response, config = {}) {
...
export const RESPONSE_PRESETS = {
    ...
    ultraBrief: {
        maxTokens: 100,
        enforceFormat: 'bullet-points',
        brevityLevel: 0.9,
    },
    /** Brief but complete */
    brief: {
        maxTokens: 300,
        enforceFormat: 'concise',
        brevityLevel: 0.6,
    },
