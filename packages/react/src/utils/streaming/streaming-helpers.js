...
class SSEEventParser {
    ...
    feed(line) {
        ...
        if (!line.trim()) {
            if (this.currentEvent.event === undefined &&
        ...
        if (!parsed)
            return null;
        if (parsed.event)
            this.currentEvent.event = parsed.event;
        if (parsed.id)
...
export async function processStream(stream, options = {}) {
    if (!stream ||
        ...
        throw new Error('[processStream] Invalid stream: expected a ReadableStream<Uint8Array>.');
    }
    const { signal, format = 'sse', onChunk, onData, onProgress, onComplete, onError, maxChunkSize = 65536, // 64KB
     } = options;
 ...
 */
function processChunkByFormat(chunk, format) {
    switch (format) {
        case 'sse': {
 ...
 * ```
 */
export async function* createStreamReader(stream, signal) {
    const reader = stream.getReader();
    const decoder = new TextDecoder();
    try {
        while (true) {
 ...
 */
export async function retryStream(fn, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(baseDelay * Math.pow(2, attempt - 1) * (0.5 + Math.random()), maxDelay);
 ...
 */
export async function mergeStreams(streams, signal) {
    const results = await Promise.all(streams.map((stream) => processStream(stream, { signal })));
 ...
 */
export async function transformStream(stream, transform, signal) {
    const results = [];
    for await (const chunk of createStreamReader(stream, signal)) {
 ...
 */
export async function* filterStream(stream, predicate, signal) {
    for await (const chunk of createStreamReader(stream, signal)) {
        if (predicate(chunk)) {
            yield chunk;
        }
 ...
 */
export async function* bufferStream(stream, windowMs, signal) {
    let buffer = [];
...
//# sourceMappingURL=streaming-helpers.js.map