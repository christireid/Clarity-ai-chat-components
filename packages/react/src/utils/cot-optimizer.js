 ...
 */
export function analyzeCoTPrompt(prompt) {
    // Detect exemplars
    let exemplarCount = 0;
 ...
 * ```
 */
export function optimizeCoTPrompt(prompt, options = {}) {
    const { preferZeroShot = true, zeroShotTrigger = COT_TRIGGERS.standard, modelStrength = 'strong', preserveFormatExemplars = false, maxExemplars = 0, } = options;
    const analysis = analyzeCoTPrompt(prompt);
    const originalTokens = estimateTokens(prompt);
    let optimized = prompt;
    const appliedOptimizations = [];
    // Decide whether to convert to zero-shot
    const shouldConvert = preferZeroShot &&
        modelStrength === 'strong' &&
    ...
    else if (analysis.hasFewShotExemplars &&
 ...
 * ```
 */
export function addZeroShotCoT(prompt, trigger = COT_TRIGGERS.standard) {
...
export function recommendCoTApproach(params) {
    ...
    const cotBenefitTasks = ['math', 'logic', 'coding', 'analysis'];
 ...
 */
export function estimateCoTSavings(exemplarCount, averageExemplarLength = 100) {
...
//# sourceMappingURL=cot-optimizer.js.map