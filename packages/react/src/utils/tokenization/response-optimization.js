...
export class ResponseLengthPredictor {
     ...
     */
    async predictResponseLength(prompt, model, context) {
        ...
        let predictedTokens = this.basePrediction(promptTokens, promptAnalysis, model);
        // Adjust based on historical patterns
        if (historicalPattern) {
            predictedTokens = this.adjustPrediction(predictedTokens, historicalPattern);
        }
        ...
        const confidence = this.calculateConfidence(promptAnalysis, historicalPattern);
     ...
     */
    basePrediction(promptTokens, analysis, model) {
     ...
     */
    getHistoricalPattern(model, context) {
        const key = `${model}_${context?.conversationId || 'global'}`;
        const history = this.historicalData.get(key) || [];
        if (history.length < 3)
            return null;
        const recent = history.slice(-5);
        const avgTokens = recent.reduce((sum, h) => sum + h.actualTokens, 0) / recent.length;
     ...
     */
    adjustPrediction(predictedTokens, historicalPattern) {
        if (!historicalPattern)
     ...
     */
    adjustForContext(predictedTokens, context) {
     ...
     */
    calculateConfidence(promptAnalysis, historicalPattern) {
        let confidence = 0.5; // Base confidence
        // Increase confidence with more prompt information
        if (promptAnalysis.hasConstraints)
            confidence += 0.2;
        if (promptAnalysis.hasExamples)
            confidence += 0.1;
        // Increase confidence with historical data
        if (historicalPattern?.consistency > 0.8)
            confidence += 0.2;
        if (historicalPattern?.trend !== 0)
            confidence += 0.1;
     ...
     */
    calculateOptimalLength(predictedTokens, model) {
     ...
     */
    estimateQuality(predictedTokens, optimalLength) {
     ...
     */
    estimateCost(predictedTokens, model) {
    ...
    calculateComplexity(text) {
        ...
        const technicalTerms = (text.match(/\b(algorithm|methodology|framework)\b/gi) || []).length;
        let complexity = 0.5;
    ...
    detectQuestionType(prompt) {
        ...
        if (/\b(what|when|where|who)\b/.test(lowerPrompt))
            return 'factual';
        if (/\b(how|why|explain|describe)\b/.test(lowerPrompt))
            return 'explanatory';
        if (/\b(create|write|generate|imagine)\b/.test(lowerPrompt))
            return 'creative';
        if (/\b(analyze|compare|evaluate|assess)\b/.test(lowerPrompt))
            return 'analytical';
        if (/\b(code|function|implement|build)\b/.test(lowerPrompt))
            return 'code';
        return 'general';
    }
    /**
     ...
     */
    async recordActualResponse(prompt, actualResponse, model, context) {
        const key = `${model}_${context?.conversationId || 'global'}`;
        const history = this.historicalData.get(key) || [];
     ...
     */
    getAccuracy(model, conversationId) {
...
export class ResponseOptimizer {
     ...
     */
    async optimizeResponse(prompt, model, config, context) {
        // Predict optimal response length
        const prediction = await this.predictor.predictResponseLength(prompt, model, context);
        // Generate optimized prompt
        const optimizedPrompt = this.generateOptimizedPrompt(prompt, prediction, config, context);
        // Calculate metrics
        const metrics = await this.calculateResponseMetrics(prompt, optimizedPrompt, prediction, model);
        return {
            optimizedPrompt,
     ...
     */
    generateOptimizedPrompt(originalPrompt, prediction, config, context) {
        let optimizedPrompt = originalPrompt;
        // Add length control instructions
        if (config.strategy === 'length_control' ||
     ...
     */
    async calculateResponseMetrics(originalPrompt, optimizedPrompt, prediction, model) {
    ...
    calculateInformationDensity(text) {
        ...
        let density = 0.5;
        if (sentences > 0)
            density += Math.min(0.3, words / (sentences * 20));
        if (keyTerms > 0)
            density += Math.min(0.2, keyTerms / 5);
        return Math.min(1, density);
    }
    /**
     * Estimate response time
     */
    estimateResponseTime(tokens, model) {
     ...
     */
    async optimizeStreamingResponse(prompt, model, chunkSize = 100) {
     ...
     */
    async controlResponseBudget(prompt, model, maxTokens, minQuality = 0.7) {
        const prediction = await this.predictor.predictResponseLength(prompt, model);
     ...
     */
    async optimizeMultiTurnResponse(prompt, model, context, config) {
        const prediction = await this.predictor.predictResponseLength(prompt, model, context);
        // Analyze conversation state
        const contextPreservation = this.calculateContextPreservation(context);
        const conversationStrategy = this.selectConversationStrategy(context, config);
     ...
     */
    calculateContextPreservation(context) {
        if (context.turnCount === 0)
            return 0;
        const recentResponses = context.historicalResponses.slice(-3);
        const avgLength = recentResponses.reduce((sum, r) => sum + (r.tokens || 0), 0) /
            Math.max(recentResponses.length, 1);
        const consistency = context.averageResponseLength > 0
            ? 1 -
     ...
     */
    selectConversationStrategy(context, config) {
        if (context.turnCount < 3)
            return 'exploratory';
        if (context.contextWindowUsage > 0.8)
            return 'compressed';
     ...
     */
    async cacheResponse(prompt, response, model, metadata) {
     ...
     * Get cached response
     */
    async getCachedResponse(prompt, model, similarityThreshold = 0.8) {
        const cacheKey = this.generateCacheKey(prompt, model);
        ...
        if (similarityThreshold > 0) {
            const similarResponse = await this.findSimilarResponse(prompt, model, similarityThreshold);
     ...
     */
    async findSimilarResponse(prompt, model, threshold) {
     ...
     * Get prediction accuracy
     */
    getPredictionAccuracy(model, conversationId) {
...
export async function optimizeResponse(prompt, model, config, context) {
    return responseOptimizer.optimizeResponse(prompt, model, config, context);
}
export async function predictResponseLength(prompt, model, context) {
    return responseLengthPredictor.predictResponseLength(prompt, model, context);
}
export async function controlResponseBudget(prompt, model, maxTokens, minQuality) {
    return responseOptimizer.controlResponseBudget(prompt, model, maxTokens, minQuality);
}
export function getResponsePredictionAccuracy(model, conversationId) {
    return responseLengthPredictor.getAccuracy(model, conversationId);
}
//# sourceMappingURL=response-optimization.js.map