...
export class DynamicOptimizer {
  ...
  private async applyAggressiveOptimization(
    ...
    const maxTokens = Math.floor((await this.tokenCounter.count(text)) * targetRatio);
    
    // Use extractive summarization for aggressive reduction
    const { truncateText } = await import('./smart-truncation.js');
    optimized = await truncateText(optimized, maxTokens, 'extractive', contentContext.type);
  ...
  private async applyConservativeOptimization(
    ...
    if (config.budgetTokens && losslessResult.compressedTokens > config.budgetTokens) {
  ...
  private async applyQualityFirstOptimization(
    ...
    const targetTokens = config.budgetTokens || Math.floor((await this.tokenCounter.count(text)) * 0.8);
   ...
   */
  private calculateComplexity(text: string, contentContext: ContentContext): number {
    const words = text.split(/\s+/).length;
    const sentences = text.split(/[.!?]+/).length;
   ...
   */
  private calculateCostSavings(originalTokens: number, optimizedTokens: number, modelContext: ModelContext): number {
  ...
  private selectStrategyByRules(
    ...
    if (modelContext.maxContextWindow > 100000 && modelContext.inputTokenCost > 0.01) {
      return 'moderate';
    }
  ...
  private async compressTextAggressively(text: string, contentContext: ContentContext): Promise<string> {
  ...
  private strategyFromHistoricalData(data: HistoricalData): OptimizationStrategy {
