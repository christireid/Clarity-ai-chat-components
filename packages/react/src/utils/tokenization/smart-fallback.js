...
export class SmartTokenCounter {
    ...
    initializeStrategies() {
        this.strategies = [
            ...
            {
                name: 'word-based',
                shouldUse: (text, context) => context.textLength < 10000 &&
                    !context.hasSpecialChars &&
                    !context.isCode,
                count: (text, context) => {
                    const words = text.split(/\s+/).filter((word) => word.length > 0);
                    const avgTokensPerWord = this.getTokensPerWord(context.language || 'en');
            ...
            {
                name: 'unicode-aware',
                shouldUse: (text, context) => context.hasSpecialChars && /[^\u0020-\u007E]/.test(text), // Contains non-printable ASCII or non-ASCII characters
                count: (text, context) => {
            ...
            // Strategy 5: Streaming-optimized counting
            {
                ...
                shouldUse: (text, context) => context.isStreaming === true && context.textLength < 1000,
                count: (text, context) => {
     ...
     */
    isValidCount(count, text, options) {
        if (count < 0)
            return false;
        if (count > text.length * 2)
            return false; // Sanity check
        if (options.maxTokens && count > options.maxTokens * 1.5)
            return false;
        return true;
    }
    /**
    ...
    isCode(text) {
        ...
        const codeScore = codeIndicators.reduce((score, pattern) => score + (pattern.test(text) ? 1 : 0), 0);
        return codeScore >= 2;
    }
    /**
     ...
     * Log fallback usage
     */
    logFallback(strategy, text, primaryCount, fallbackCount) {
        if (process.env.NODE_ENV === 'development') {
            console.log(`[TokenCounter] Using ${strategy} fallback. Primary: ${primaryCount}, Fallback: ${fallbackCount}, Text: "${text.substring(0, 50)}..."`);
        }
    }
    /**
...
export function smartCountTokens(text, options) {
