...
export class SmartTokenCounter {
  ...
  private initializeStrategies() {
    this.strategies = [
      ...
      // Strategy 5: Streaming-optimized counting
      {
        ...
        shouldUse: (text, context) => context.isStreaming && context.textLength < 1000,
        count: (text, context) => {
   ...
   */
  public smartCount(text: string, options: {
    model?: string;
    maxTokens?: number;
    isStreaming?: boolean;
    strategy?: string;
   ...
   * Log fallback usage
   */
  private logFallback(strategy: string, text: string, primaryCount: number, fallbackCount: number) {
