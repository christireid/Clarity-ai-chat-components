...
export class ModelPresets {
   ...
   */
  public static getPresetsByProvider(provider: ModelPreset['provider']): ModelPreset[] {
  ...
  public static compareModels(
    ...
    const models = modelIds 
      ? modelIds.map(id => this.getPreset(id)).filter(Boolean) as ModelPreset[]
      : this.getAllPresets();
...
export function compareModelsForText(text: string, modelIds?: string[]): ModelComparison[] {
  return ModelPresets.compareModels(text, modelIds);
}

export function recommendModelForText(
  text: string,
  priorities?: { cost?: number; speed?: number; accuracy?: number; context?: number; }
): ModelPreset {
...
): { inputCost: number; outputCost: number; totalCost: number; tokens: number; } {
