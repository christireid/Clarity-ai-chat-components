...
export class SemanticCompressor {
  ...
  async compressSemantic(
    ...
    const targetTokens = config.budgetTokens || Math.floor(originalTokens * (config.targetRatio || 0.5));
  ...
  async compressBudgetControlled(
    ...
    const strategies = ['semantic', 'extractive', 'hybrid'] as CompressionStrategy[];
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (result.compressedTokens <= budgetTokens && result.estimatedQuality > bestQuality) {
          bestResult = result;
    ...
    return bestResult || this.createFallbackResult(text, budgetTokens, originalTokens);
  }
  ...
  private async scoreTokenImportance(tokens: string[], context: string): Promise<Map<string, number>> {
    ...
    for (const token of tokens) {
      ...
      const positionScore = position < context.length * 0.2 || position > context.length * 0.8 ? 1.5 : 1.0;
      score *= positionScore;
  ...
  private estimateQuality(compressionRatio: number, config: CompressionConfig): number {
  ...
  private extractPreservedTerms(original: string[], selected: string[]): string[] {
    return selected.filter(token => original.includes(token));
  }

  private extractRemovedContent(original: string[], selected: string[]): string[] {
  ...
  private scoreSentenceByKeyphrases(sentence: string, keyphrases: string[]): number {
  ...
  private selectSentencesByScore(
    scoredSentences: Array<{sentence: string, score: number, tokens: Promise<number>}>,
    targetTokens: number
  ): string[] {
  ...
  private createFallbackResult(text: string, budgetTokens: number, originalTokens: number): CompressionResult {
...
// Export the main compressor class for advanced usage
export { SemanticCompressor };