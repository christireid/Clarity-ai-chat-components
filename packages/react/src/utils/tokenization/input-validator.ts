...
export class InputValidator {
  ...
  static validateTextInput(
    ...
    if (text.length < minLength) {
      errors.push(
        `Text length (${text.length}) is below minimum (${minLength})`
      )
    }
    ...
    try {
      ...
      errors.push(
        `Text contains invalid encoding: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  ...
  static validateCompressionRatio(ratio: number): ValidationResult {
    ...
    if (ratio < 0.1) {
      warnings.push(
        'Very low compression ratio may not achieve significant savings'
      )
    }
  ...
  static validateQualityThreshold(threshold: number): ValidationResult {
    ...
    if (threshold > 0.95) {
      warnings.push(
        'Very high quality threshold may prevent effective optimization'
      )
    }
  ...
  static validateBudget(budget: number): ValidationResult {
    ...
    return {
      ...
      errors,
      warnings,
      sanitized: Math.floor(
        Math.max(0, Math.min(this.MAX_TOKEN_COUNT, budget))
      ),
    }
  }
  ...
  static validateOptimizationConfig(config: any): ValidationResult {
    ...
    if (config.compressionRatio !== undefined) {
      const ratioValidation = this.validateCompressionRatio(
    ...
    if (config.qualityThreshold !== undefined) {
      const qualityValidation = this.validateQualityThreshold(
