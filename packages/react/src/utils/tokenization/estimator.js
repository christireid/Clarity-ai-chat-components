 ...
 * ```
 */
export async function estimateTokensAsync(text, model, options) {
    if (!text)
        return 0;
 ...
 */
export async function estimateMessagesTokensAsync(messages, model, options) {
    const TOKENS_PER_MESSAGE = 4;
    let total = 0;
    for (let i = 0; i < messages.length; i++) {
        ...
        const tokens = msg.content.length > ASYNC_THRESHOLD
 ...
 */
export function shouldUseAsyncEstimation(text) {
...
export function createCancellableEstimator() {
    ...
    return {
         ...
         */
        async estimate(text, model, onProgress) {
