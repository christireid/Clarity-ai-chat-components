...
export class OptimizedTokenCounter {
  ...
  public countBatch(texts: string[]): number[] {
    ...
    try {
      ...
      this.recordMetrics('batch_count', performance.now() - startTime, 
   ...
   */
  public countWithMetrics(text: string): { count: number; metrics: PerformanceMetrics } {
    const startTime = performance.now();
    ...
    const metrics: PerformanceMetrics = {
      ...
      outputSize: count,
      timestamp: Date.now(),
      memoryUsage: startMemory > 0 ? (performance.memory?.usedJSHeapSize || 0) - startMemory : undefined
    };
