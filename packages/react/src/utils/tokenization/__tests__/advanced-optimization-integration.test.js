...
import { compressWithLLMLingua, compressWithSelectiveContext, compressAdaptive, compressEnsemble, compressIncremental, optimizeTokens, advancedCompressor, } from '../advanced-compression';
import { optimizeTokensAdaptively, adaptiveOptimizer, updateConversationState, getAdaptiveAnalytics, } from '../adaptive-optimizer';
import { getCachedTokenCount, getCachedCompression, setCachedCompression, getCacheAnalytics, tokenCache, } from '../intelligent-caching';
import { optimizeResponse, predictResponseLength, controlResponseBudget, responseLengthPredictor, getResponsePredictionAccuracy, } from '../response-optimization';
import { optimizeTokens as middlewareOptimizeTokens, getOptimizationMetrics, getOptimizationHistory, tokenMiddleware, } from '../optimization-middleware';
...
// We should optimize it using memoization or dynamic programming`,
    ...
    describe('Advanced Compression', () => {
        ...
        it('should compress incrementally', async () => {
            ...
            expect(result).toBeTruthy();
            expect(result.length).toBeLessThanOrEqual(sampleTexts.technical.length * 0.6);
        }, 10000);
    });
    describe('Adaptive Optimization', () => {
        it('should optimize tokens adaptively', async () => {
            const result = await optimizeTokensAdaptively(sampleTexts.technical, 'gpt-4', { strategy: 'moderate', targetReduction: 0.4 });
    ...
    describe('Intelligent Caching', () => {
        ...
        it('should cache compression results', async () => {
            const originalText = 'This is a longer text that should be compressed and cached';
            const strategy = 'adaptive';
            // Cache compression result
            const compressedText = await compressWithAdvanced(originalText, strategy, 0.3, 0.8);
    ...
    describe('Integration Flow Tests', () => {
        it('should handle complete optimization workflow', async () => {
            ...
            const compressed = await compressAdaptive(originalText, model, 'technical', 0.4);
    ...
    describe('Performance Benchmarks', () => {
        it('should optimize large text efficiently', async () => {
            ...
            const result = await compressAdaptive(largeText, 'gpt-4', 'technical', 0.3);
...
//# sourceMappingURL=advanced-optimization-integration.test.js.map