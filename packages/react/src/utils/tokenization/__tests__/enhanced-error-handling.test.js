...
import { TokenOptimizationErrorHandler, ErrorCategory, ErrorSeverity, } from '../enhanced-error-handling';
describe('Enhanced Error Handling Unit Tests', () => {
    ...
    describe('Error Creation', () => {
        it('should create structured errors with all required properties', () => {
            const error = errorHandler.createError('Test error message', 'TEST_ERROR', ErrorCategory.VALIDATION, ErrorSeverity.LOW, {
        ...
        it('should categorize errors correctly', () => {
            const timeoutError = errorHandler.createError('Operation timed out', 'TIMEOUT_001', ErrorCategory.TIMEOUT, ErrorSeverity.MEDIUM);
            expect(timeoutError.category).toBe(ErrorCategory.TIMEOUT);
            const resourceError = errorHandler.createError('Out of memory', 'RESOURCE_001', ErrorCategory.RESOURCE, ErrorSeverity.HIGH);
            expect(resourceError.category).toBe(ErrorCategory.RESOURCE);
        });
        it('should determine severity correctly', () => {
            const criticalError = errorHandler.createError('Out of memory error', 'CRITICAL_001', ErrorCategory.RESOURCE, ErrorSeverity.CRITICAL);
    ...
    describe('Error Recovery', () => {
        it('should attempt recovery for recoverable errors', async () => {
            const validationError = errorHandler.createError('Invalid input format', 'VALIDATION_001', ErrorCategory.VALIDATION, ErrorSeverity.LOW, {
                operation: 'validateInput',
                input: 'invalid input',
                timestamp: Date.now(),
            });
            const result = await errorHandler.handleError(validationError, { operation: 'test', timestamp: Date.now() }, { attemptRecovery: true, fallbackValue: 'fallback result' });
            expect(result).toBe('fallback result');
        });
        it('should use fallback value when recovery fails', async () => {
            const error = new Error('Unrecoverable error');
            const result = await errorHandler.handleError(error, { operation: 'test', timestamp: Date.now() }, {
    ...
    describe('Error Statistics', () => {
        it('should track error history', () => {
            ...
            for (let i = 0; i < 5; i++) {
                errorHandler.createError(`Test error ${i}`, `TEST_${i}`, ErrorCategory.VALIDATION, ErrorSeverity.LOW);
            }
        ...
        it('should calculate recoverable rate correctly', () => {
            ...
            errorHandler.createError('Recoverable error', 'RECOVERABLE_001', ErrorCategory.VALIDATION, ErrorSeverity.LOW);
            errorHandler.createError('Non-recoverable critical error', 'CRITICAL_001', ErrorCategory.RESOURCE, ErrorSeverity.CRITICAL);
            const stats = errorHandler.getErrorStats();
    ...
    describe('Error Categorization', () => {
        it('should categorize timeout errors correctly', () => {
            ...
            const normalizedError = errorHandler.normalizeError(timeoutError, {
        ...
        it('should categorize resource errors correctly', () => {
            ...
            const normalizedError = errorHandler.normalizeError(resourceError, {
        ...
        it('should categorize validation errors correctly', () => {
            ...
            const normalizedError = errorHandler.normalizeError(validationError, {
                operation: 'validateInput',
                timestamp: Date.now(),
            });
...
//# sourceMappingURL=enhanced-error-handling.test.js.map