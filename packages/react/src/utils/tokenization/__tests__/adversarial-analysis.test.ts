...
import { countConversationTokens, countTokens } from '../accurate-counter.js';
import { InputValidator } from '../input-validator.js';
import { errorHandler } from '../enhanced-error-handling.js';
...
describe('Adversarial Analysis - Input Validation & Error Handling', () => {
  ...
  describe('Input Validation Edge Cases', () => {
    ...
    it('should handle invalid message structures', async () => {
      ...
      await expect(countConversationTokens([
        { role: 'user' } as any
      ])).rejects.toThrow();
  ...
  describe('Error Handling & Recovery', () => {
    ...
    it('should provide meaningful error messages', async () => {
      try {
        await countConversationTokens(null as any);
      } catch (error: any) {
    ...
    it('should track error statistics', async () => {
      ...
      try {
        await estimateTokens(null as any);
      } catch (error) {
        // Expected
      }
  ...
  describe('Race Conditions & Concurrent Operations', () => {
    it('should handle concurrent token counting', async () => {
      ...
      const promises = texts.map(text => 
        countConversationTokens([{ role: 'user', content: text }])
      );
    ...
    it('should handle mixed valid/invalid concurrent requests', async () => {
      ...
      const promises = mixedInputs.map(input => 
        countConversationTokens(input as any).catch(error => ({ error }))
      );
