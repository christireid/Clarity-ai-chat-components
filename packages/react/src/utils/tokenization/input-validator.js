...
export class InputValidator {
    static MAX_TEXT_LENGTH = 10 * 1024 * 1024; // 10MB
    static MAX_TOKEN_COUNT = 1000000; // 1M tokens
    static VALID_MODEL_PATTERN = /^[a-zA-Z0-9\-_.]+$/;
    /**
     * Validate text input for token counting operations
     */
    static validateTextInput(input, constraints = {}) {
        const errors = [];
        const warnings = [];
        const { maxLength = this.MAX_TEXT_LENGTH, minLength = 0, allowEmpty = true, requiredType = 'string', } = constraints;
        // Type validation
        if (input === null || input === undefined) {
            if (allowEmpty) {
                return {
                    valid: true,
                    errors: [],
                    warnings: ['Input is null/undefined, treating as empty string'],
                };
            }
            else {
                return {
        ...
        try {
            ...
            return {
                valid: false,
                errors: [
                    `Failed to convert input to string: ${error instanceof Error ? error.message : String(error)}`,
                ],
                warnings: [],
            };
        }
        ...
        try {
            ...
            errors.push(`Text contains invalid encoding: ${error instanceof Error ? error.message : String(error)}`);
        }
        return {
     ...
     */
    static validateCompressionRatio(ratio) {
        const errors = [];
        const warnings = [];
     ...
     */
    static validateQualityThreshold(threshold) {
        const errors = [];
        const warnings = [];
