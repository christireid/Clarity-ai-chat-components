...
export class TokenOptimizationMonitor {
     ...
     */
    async recordTokenUsage(originalText, optimizedText, modelName, strategy, qualityScore, latency) {
     ...
     */
    updateMetrics(tokensSaved, compressionRatio, qualityScore, latency) {
     ...
     * Update model-specific metrics
     */
    updateModelMetrics(modelName, originalTokens, optimizedTokens, compressionRatio, qualityScore, latency) {
     ...
     */
    recordHistoricalData(originalTokens, optimizedTokens, compressionRatio, qualityScore, latency) {
        const dataPoint = {
            timestamp: Date.now(),
...
export class TokenOptimizationAnalytics {
    ...
    calculateEffectivenessScore(metrics) {
        ...
        return (factors.compression * weights.compression +
...
export function createTokenAnalytics(monitor) {
    return new TokenOptimizationAnalytics(monitor);
}
export async function recordTokenUsage(monitor, originalText, optimizedText, modelName, strategy, qualityScore, latency) {
    return monitor.recordTokenUsage(originalText, optimizedText, modelName, strategy, qualityScore, latency);
}
//# sourceMappingURL=optimization-dashboard.js.map