...
export class ModelPresets {
    static presets = new Map([
        [
            'gpt-4',
            {
                ...
                maxTokens: 8192,
                description: 'Most capable GPT-4 model with broad general knowledge and domain expertise',
                useCases: ['Complex reasoning', 'Creative writing', 'Code generation', 'Analysis'],
        ...
        [
            'gpt-4-turbo',
            {
                ...
                estimatedCharsPerToken: 4.0,
                maxTokens: 128000,
                description: 'Improved GPT-4 with larger context window and faster responses',
                useCases: ['Long documents', 'Code review', 'Complex analysis', 'Multi-turn conversations'],
        ...
        [
            'gpt-4o',
            {
                ...
                estimatedCharsPerToken: 4.0,
                maxTokens: 128000,
                description: 'Omni model with vision capabilities and improved performance',
                useCases: ['Vision tasks', 'Multimodal', 'General purpose', 'Cost-effective'],
                advantages: ['Vision support', 'Fast', 'Cost-effective', 'Large context'],
        ...
        [
            'gpt-3.5-turbo',
            {
                ...
                useCases: ['Simple queries', 'Data extraction', 'Basic analysis', 'Chat'],
        ...
        [
            'claude-3-opus',
            {
                ...
                useCases: ['Complex analysis', 'Creative writing', 'Code review', 'Research'],
        ...
        [
            'claude-3-haiku',
            {
                ...
                useCases: ['Quick responses', 'Simple analysis', 'Data extraction', 'Chat'],
        ...
        [
            'gemini-pro',
            {
                ...
                useCases: ['Multimodal', 'Code generation', 'Reasoning', 'Creative tasks'],
                advantages: ['Multimodal', 'Large context', 'Good performance'],
                limitations: ['Google ecosystem', 'Variable quality', 'Limited availability']
            }
        ],
        [
            'deepseek-chat',
            {
                ...
                useCases: ['Code generation', 'Technical tasks', 'Cost-effective solutions'],
                advantages: ['Very cheap', 'Good coding', 'Large context'],
                limitations: ['Less known', 'Variable availability', 'Limited features']
            }
        ],
        [
            'llama-3-70b',
            {
                ...
                useCases: ['Open source', 'Research', 'Custom deployment', 'Privacy-focused'],
                advantages: ['Open source', 'Customizable', 'Privacy', 'No vendor lock-in'],
     ...
     */
    static recommendModel(text, priorities = {}) {
        ...
        comparisons.forEach(comparison => {
            const costScore = comparison.estimatedCost < 0.01 ? 1 : 1 / (comparison.estimatedCost * 100);
            const speedScore = comparison.model.name.includes('turbo') || comparison.model.name.includes('haiku') ? 1 : 0.7;
            const accuracyScore = comparison.model.name.includes('gpt-4') || comparison.model.name.includes('opus') ? 1 : 0.8;
            const contextScore = comparison.contextUsage < 50 ? 1 : 0.5;
            const score = (costScore * weights.cost) +
     ...
     */
    static estimateCost(text, modelId, options = {}) {
     ...
     */
    static validateModelChoice(text, modelId, constraints = {}) {
        const model = this.getPreset(modelId);
        if (!model) {
            return {
                isValid: false,
        ...
        if (constraints.maxCost && costEstimate.totalCost > constraints.maxCost) {
            issues.push(`Cost exceeds limit: $${costEstimate.totalCost.toFixed(4)} > $${constraints.maxCost}`);
            suggestions.push('Consider using a cheaper model like GPT-3.5 Turbo or Claude Haiku');
            alternatives.push(this.getPreset('gpt-3.5-turbo'), this.getPreset('claude-3-haiku'));
        }
        // Check token constraint
        if (constraints.maxTokens && tokens > constraints.maxTokens) {
            issues.push(`Token count exceeds limit: ${tokens} > ${constraints.maxTokens}`);
            suggestions.push('Consider truncating the text or using a model with larger context');
        }
        // Check context window
        if (tokens > model.maxTokens) {
            issues.push(`Text exceeds model context window: ${tokens} > ${model.maxTokens}`);
            suggestions.push('Consider using a model with larger context window like GPT-4 Turbo or Claude');
            alternatives.push(this.getPreset('gpt-4-turbo'), this.getPreset('claude-3-sonnet'));
        }
        return {
...
export function compareModelsForText(text, modelIds) {
    return ModelPresets.compareModels(text, modelIds);
}
export function recommendModelForText(text, priorities) {
    return ModelPresets.recommendModel(text, priorities);
}
export function estimateTextCost(text, modelId, options) {
