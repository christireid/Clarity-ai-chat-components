...
import type { InputConstraints, ValidationResult } from './input-validator'
...
export function useTokenValidator() {
  ...
  const validateText = useCallback(
    (text: string, constraints?: Partial<InputConstraints>) => {
  ...
  const allWarnings = useMemo(() => {
    return Object.values(validationState)
      .filter(
        (result): result is ValidationResult => result?.warnings !== undefined
      )
...
export type { ValidationResult, InputConstraints } from './input-validator'
