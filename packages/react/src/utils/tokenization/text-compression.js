...
export class SemanticCompressor {
    ...
    async compressSemantic(text, config) {
        ...
        const selectedTokens = this.selectImportantTokens(tokens, tokenScores, targetTokens, config);
     ...
     */
    async compressExtractive(text, config) {
        const originalTokens = await TokenCounter.count(text);
        ...
        const targetTokens = config.budgetTokens ||
            Math.floor(originalTokens * (config.targetRatio || 0.5));
        const selectedSentences = this.selectSentencesByScore(sentenceScores, targetTokens);
        ...
        return {
            ...
            estimatedQuality: this.estimateQuality(compressedTokens / originalTokens, config),
     ...
     */
    async compressHybrid(text, config) {
        ...
        if (losslessResult.compressionRatio > (config.targetRatio || 0.5)) {
            const semanticConfig = {
                ...config,
                targetRatio: (config.targetRatio || 0.5) / losslessResult.compressionRatio,
            };
            const semanticResult = await this.compressSemantic(losslessResult.compressedText, semanticConfig);
            // Combine results
     ...
     */
    async compressBudgetControlled(text, budgetTokens, config) {
        const originalTokens = await TokenCounter.count(text);
        ...
        for (const strategy of strategies) {
            try {
                ...
                if (result.compressedTokens <= budgetTokens &&
    ...
    async scoreTokenImportance(tokens, context) {
        ...
        for (const token of tokens) {
            ...
            const positionScore = position < context.length * 0.2 || position > context.length * 0.8
                ? 1.5
                : 1.0;
            score *= positionScore;
    ...
    selectImportantTokens(tokens, scores, targetCount, config) {
    ...
    extractPreservedTerms(original, selected) {
        return selected.filter((token) => original.includes(token));
    }
    extractRemovedContent(original, selected) {
    ...
    createFallbackResult(text, budgetTokens, originalTokens) {
...
export async function compressText(text, config) {
    ...
    switch (config.strategy) {
        ...
        case 'budget-controlled':
            return compressor.compressBudgetControlled(text, config.budgetTokens || Math.floor(text.length / 4), config);
        default:
            return compressor.compressHybrid(text, config);
    }
}
// Convenience functions for common use cases
export async function compressForBudget(text, maxTokens, options) {
...
export async function compressForRatio(text, targetRatio, options) {
    return compressText(text, {
        strategy: 'hybrid',
        targetRatio,
        preserveStructure: true,
...
export async function compressSemanticOnly(text, targetRatio) {
    return compressText(text, {
        strategy: 'semantic',
        targetRatio,
...
export async function compressMultiStrategy(text, strategies, config) {
    const results = [];
    for (const strategy of strategies) {
        try {
...
//# sourceMappingURL=text-compression.js.map