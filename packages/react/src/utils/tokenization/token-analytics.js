...
export class TokenAnalyticsMonitor {
    ...
    getUsageStats() {
        ...
        const recentTotal = recentDays.reduce((sum, day) => sum + (daily[day]?.totalTokens || 0), 0);
        const olderTotal = olderDays.reduce((sum, day) => sum + (daily[day]?.totalTokens || 0), 0);
        let usageTrend = 'stable';
        if (recentTotal > olderTotal * 1.2)
            usageTrend = 'increasing';
        if (recentTotal < olderTotal * 0.8)
    ...
    checkAlerts(event) {
        ...
        if (event.tokens > this.config.alertThresholds.perRequest) {
            this.createAlert('threshold_exceeded', 'high', `Large token usage detected: ${event.tokens} tokens`, event.tokens, this.config.alertThresholds.perRequest, { event });
        }
        ...
        if (hourlyTokens > this.config.alertThresholds.hourly) {
            this.createAlert('threshold_exceeded', 'medium', `Hourly token threshold exceeded: ${hourlyTokens} tokens`, hourlyTokens, this.config.alertThresholds.hourly);
        }
    ...
    checkUnusualPatterns(event) {
        ...
        if (deviation > 2) {
            ...
            this.createAlert('unusual_usage', 'low', `Unusual token usage detected: ${event.tokens} tokens (avg: ${average.toFixed(0)})`, event.tokens, average * 2, { event, average, deviation });
        }
    }
    /**
     * Create alert
     */
    createAlert(type, severity, message, value, threshold, context) {
        const alert = {
     ...
     */
    calculateAnalytics(tokenCounts) {
        if (tokenCounts.length === 0) {
            return this.getEmptyAnalytics();
        }
        const sortedCounts = [...tokenCounts].sort((a, b) => a - b);
    ...
    calculateCostForEvents(events) {
        ...
        Object.entries(modelEvents).forEach(([model, modelEventsList]) => {
            ...
            if (pricing && modelEventsList.length > 0) {
                const totalTokens = modelEventsList.reduce((sum, e) => sum + e.tokens, 0);
                const firstEvent = modelEventsList[0];
                const costPerToken = firstEvent?.type === 'output' ? pricing.output : pricing.input;
...
export function recordTokenUsage(tokens, model, type = 'total') {
