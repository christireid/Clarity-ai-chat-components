...
import { AdvancedCompressionOrchestrator, compressWithAdvanced } from './advanced-compression.js';
...
const MODEL_EFFICIENCY_PROFILES: Record<string, ModelEfficiencyProfile> = {
  'gpt-4': {
    ...
    outputEfficiency: 0.80,
    contextEfficiency: 0.75,
    reasoningEfficiency: 0.90,
...
export class AdaptiveTokenOptimizer {
  ...
  async optimizeTokensAdaptively(
    ...
    const conversationState = conversationId ? 
      this.conversationStates.get(conversationId) : undefined;
  ...
  private async analyzeContext(text: string): Promise<ContextProfile> {
    ...
    const tokenDensity = await this.tokenCounter.count(text) / Math.max(characters, 1);
  ...
  private detectDomain(text: string): string {
    if (/\b(function|class|import|export|const|let|var)\b/.test(text)) return 'programming';
    if (/\b(medical|health|patient|diagnosis|treatment)\b/.test(text)) return 'medical';
    if (/\b(legal|law|court|contract|regulation)\b/.test(text)) return 'legal';
    if (/\b(financial|investment|market|trading|economy)\b/.test(text)) return 'financial';
    return 'general';
  }
  ...
  getPerformanceAnalytics(
    ...
    const averageReduction = history.reduce((sum, h) => 
      sum + (1 - h.metrics.compressionEfficiency), 0
    ) / totalOptimizations;
    
    const averageQuality = history.reduce((sum, h) => 
      sum + h.strategy.qualityThreshold, 0
    ) / totalOptimizations;
