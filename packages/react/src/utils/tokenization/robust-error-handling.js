...
export class RobustTokenCounter {
     ...
     */
    async countTokensWithFallback(text, options = {}) {
        ...
        try {
            ...
            if (!validation.valid && validation.reason) {
                return this.handleInvalidInput({ valid: false, reason: validation.reason }, mergedOptions);
            }
     ...
     */
    async countWithTimeout(text, options) {
     ...
     */
    handleInvalidInput(validation, options) {
        const error = this.createError('INVALID_INPUT', `Invalid input: ${validation.reason}`, { input: validation, options });
     ...
     */
    handleError(error, text, options, startTime) {
     ...
     */
    async retryCount(text, options, originalError) {
        for (let attempt = 1; attempt <= options.maxRetries; attempt++) {
            try {
                ...
                if (attempt === options.maxRetries) {
                    const finalError = this.createErrorFromAny(retryError, text, options, 'MAX_RETRIES_EXCEEDED');
                    if (options.throwOnError) {
                        throw finalError;
                    }
                    return {
     ...
     * Try fallback strategies
     */
    tryFallbackStrategies(text, options, error) {
        // Try smart fallback
     ...
     */
    characterBasedFallback(text, model) {
        const modelRatios = {
            'gpt-4': 4.0,
            'gpt-3.5-turbo': 4.0,
            'claude-3-opus': 3.8,
     ...
     */
    createErrorFromAny(error, text, options, code) {
...
export async function countTokensRobust(text, options) {
    const result = await robustTokenCounter.countTokensWithFallback(text, options);
    return result.count;
}
//# sourceMappingURL=robust-error-handling.js.map