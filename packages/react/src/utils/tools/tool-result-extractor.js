 ...
 */
export function extractToolResultsByName(messages, toolName) {
    return extractToolResults(messages).filter(({ toolCall }) => toolCall.function?.name === toolName);
}
/**
 ...
 */
export function getLatestToolResult(messages, toolName) {
    const results = extractToolResultsByName(messages, toolName);
    const lastResult = results[results.length - 1];
    return lastResult ?? null;
}
//# sourceMappingURL=tool-result-extractor.js.map