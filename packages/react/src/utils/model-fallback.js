        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    const delay = calculateDelay(options.retryDelay ?? 1000, attempt, options.exponentialBackoff ?? true, options.jitter ?? true);
 ...
 */
function calculateDelay(baseDelay, attempt, exponential, jitter = true) {
    let delay;
    if (exponential) {
