 ...
 */
export function validateRequiredString(value, propName, componentName, example) {
    if (!value) {
        throw new Error(`[${componentName}] Missing required prop "${propName}". ` +
            (example ? `Example: ${example}` : `Please provide a value for "${propName}".`));
    }
    if (typeof value !== 'string') {
        throw new Error(`[${componentName}] Invalid "${propName}" prop. ` +
 ...
 */
export function validateEnum(value, propName, componentName, allowedValues, defaultValue) {
 ...
 */
export function validateProvider(contextValue, providerName, hookName) {
    if (!contextValue) {
        throw new Error(`[${hookName}] ${providerName} is not available. ` +
 ...
 */
export function validateApiEndpoint(api, componentName) {
    ...
    if (api.startsWith('http://localhost') || api.startsWith('http://127.0.0.1')) {
        console.warn(`[${componentName}] Using localhost API endpoint. ` +
 ...
 */
export function validateStorageKey(key, hookName) {
    if (key === undefined || key === null) {
        return; // Optional, will use default
    }
    if (typeof key !== 'string' || key.trim() === '') {
        console.warn(`[${hookName}] Invalid "storageKey" option. ` +
