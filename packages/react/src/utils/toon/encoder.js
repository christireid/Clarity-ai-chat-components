 ...
 * ```
 */
export function jsonToToon(data, options = {}) {
    const { indent = 2, compact = false, preserveNulls = true, quoteStrings = false, } = options;
    return encodeValue(data, 0, indent, compact, preserveNulls, quoteStrings);
}
/**
 * Encode a value to TOON format
 */
function encodeValue(value, depth, indent, compact, preserveNulls, quoteStrings) {
    // Null/undefined
 ...
 * Encode string value
 */
function encodeString(str, quoteStrings) {
 ...
 */
function encodeArray(arr, depth, indent, compact, preserveNulls, quoteStrings) {
    if (arr.length === 0)
        return '[]';
    ...
    const items = arr.map(item => `${indentStr}- ${encodeValue(item, depth + 1, indent, compact, preserveNulls, quoteStrings)}`);
    return items.join('\n');
}
/**
 * Encode array of uniform objects as table (CSV-style)
 */
function encodeUniformObjectArray(arr, depth, indent, compact, preserveNulls, quoteStrings) {
    const indentStr = ' '.repeat(depth * indent);
 ...
 */
function encodePrimitiveArray(arr, quoteStrings) {
 ...
 */
function encodeObject(obj, depth, indent, compact, preserveNulls, quoteStrings) {
 ...
 */
export function estimateToonSavings(data) {
 ...
 */
export function isSuitableForToon(data) {
    // TOON works best for:
    ...
    if (typeof data === 'object' && data !== null && !Array.isArray(data)) {
        ...
        const simpleValues = values.filter(v => isPrimitive(v) || (Array.isArray(v) && isUniformObjectArray(v)));
 ...
 * ```
 */
export function wrapWithToonInstructions(toonData, options = {}) {
    const { includeInstructions = true, instructionLevel = 'brief', instructionPlacement = 'before', instructionsAlreadySent = false, } = options;
 ...
 * ```
 */
export function createToonConversationWrapper(defaultOptions = {}) {
    let instructionsSent = false;
    return {
        wrap: (toonData, options = {}) => {
