...
export function extractToolCalls(message) {
    ...
    if (message.toolInvocations) {
        toolCalls.push(...message.toolInvocations.map((inv) => ({
 ...
 */
export function mergeStreamingChunks(existing, chunk) {
 ...
 */
export function createSystemMessage(content, options) {
    return {
        id: options?.id,
        role: 'system',
        content,
 ...
 */
export function createToolResultMessage(toolCallId, toolName, result, options) {
    return {
        id: options?.id,
        role: 'tool',
        content: [
 ...
 */
export function getLastMessageByRole(messages, role) {
 ...
 */
export function truncateMessagesToTokenLimit(messages, maxTokens) {
    let totalTokens = 0;
    const truncated = [];
