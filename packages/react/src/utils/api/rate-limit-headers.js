    ...
    if (retryAfter) {
        ...
        if (!isNaN(seconds)) {
            ...
            if (!isNaN(date.getTime())) {
                info.retryAfter = Math.max(0, Math.ceil((date.getTime() - Date.now()) / 1000));
            }
        }
    }
    // OpenAI headers
 ...
 */
export function calculateRetryDelay(info, defaultDelay = 1000) {
 ...
 */
export function shouldThrottle(info, threshold = 5) {
