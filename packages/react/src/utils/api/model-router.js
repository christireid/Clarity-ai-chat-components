...
const COMPLEXITY_PATTERNS = {
    ...
    complex: [
        ...
        {
            pattern: /\b(explain|describe|elaborate)\b.*\b(detail|depth|comprehensive)\b/i,
            weight: 0.3,
 ...
 */
export function analyzeComplexity(query, context) {
...
export function routeQuery(query, options = {}) {
    const { availableModels = COMMON_MODELS, context, preferProvider, maxCost, forceModel, } = options;
    // Analyze complexity
    ...
    if (forceModel) {
        ...
        if (forced) {
            const estimatedCost = complexity.estimatedTokens * forced.inputCost +
    ...
    const targetTier = complexity.level === 'simple'
        ? 'simple'
    ...
    const maxCostEstimate = complexity.estimatedTokens * mostExpensive.inputCost +
...
export class ModelRouter {
    ...
    options;
    history = [];
    constructor(availableModels = COMMON_MODELS, options = {}) {
     ...
     */
    recordFeedback(index, actualCost, userSatisfaction) {
    ...
    getStats() {
        ...
        const totalEstimatedCost = this.history.reduce((sum, h) => sum + h.decision.estimatedCost, 0);
        const totalActualCost = this.history.reduce((sum, h) => sum + (h.actualCost || 0), 0);
        const averageSavings = this.history.reduce((sum, h) => sum + h.decision.savingsPercent, 0) /
