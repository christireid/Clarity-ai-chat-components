...
// Mock IndexedDB
const mockStore = new Map<string, CachedResponse>()
const mockIndexedDB = {
  open: vi.fn(),
}

// Simple mock embedding function
const mockEmbedFunction = vi.fn(async (text: string): Promise<number[]> => {
...
const mockEmbedFunctionWithSimilarity = vi.fn(
    ...
    const mockDB = {
      transaction: vi.fn(() => ({
        objectStore: vi.fn(() => ({
          ...
          put: vi.fn((entry: CachedResponse) => {
          ...
          index: vi.fn(() => ({
            get: vi.fn((query: string) => {
              const match = Array.from(mockStore.values()).find(
                (e) => e.query === query
              )
              return {
                result: match,
...
describe('usePersistentSemanticCacheConfig', () => {
  it('should create config object', async () => {
    const { usePersistentSemanticCacheConfig } = await import(
