...
import { PromptCacheManager, createAnthropicCachedMessages, estimateCacheSavings, calculateCacheBreakpoints, applyBreakpointsToMessages, createCacheWarmingRequests, calculateWarmingSchedule, calculateTTLRefreshSchedule, } from '../prompt-caching/cache-manager';
// =============================================================================
...
describe('createAnthropicCachedMessages', () => {
    ...
    it('respects custom minCacheLength', () => {
        const messages = createAnthropicCachedMessages('x'.repeat(500), [], 100 // Lower threshold
        );
        expect(messages[0]?.cache_control).toEqual({ type: 'ephemeral' });
    });
...
describe('calculateCacheBreakpoints', () => {
    ...
    it('prefers assistant message boundaries', () => {
        ...
        const assistantBreakpoint = breakpoints.find(bp => messages[bp.index]?.role === 'assistant');
    ...
    it('includes long user context', () => {
        ...
        const userBreakpoint = breakpoints.find(bp => messages[bp.index]?.role === 'user' && bp.priority === 'medium');
    ...
    it('returns no critical/high priority breakpoints for short conversation', () => {
        ...
        const meaningfulBreakpoints = breakpoints.filter(bp => bp.priority === 'critical' || bp.priority === 'high');
...
describe('applyBreakpointsToMessages', () => {
    it('adds cache_control at breakpoint indices', () => {
        ...
        const breakpoints = [
            { index: 0, cumulativeTokens: 100, priority: 'critical', reason: 'System' },
            { index: 2, cumulativeTokens: 200, priority: 'high', reason: 'Turn' },
        ];
    ...
    it('preserves message content', () => {
        ...
        const breakpoints = [
            { index: 0, cumulativeTokens: 100, priority: 'critical', reason: 'Test' },
        ];
        const result = applyBreakpointsToMessages(messages, breakpoints);
...
//# sourceMappingURL=prompt-cache-manager.test.js.map