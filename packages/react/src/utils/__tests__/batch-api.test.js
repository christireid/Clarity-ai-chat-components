...
import { BatchRequestManager, estimateBatchSavings, shouldUseBatch, createOpenAIBatchFile, createAnthropicBatchFile, parseOpenAIBatchResults, } from '../batch-api';
// Mock the estimator
...
describe('BatchRequestManager', () => {
    ...
    describe('addRequest', () => {
        ...
        it('should auto-submit when maxBatchSize reached', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                promises.push(manager.addRequest({
                    id: `req${i}`,
                    model: 'gpt-4o-mini',
                    messages: [{ role: 'user', content: 'Hello' }],
                    priority: 'normal',
...
describe('shouldUseBatch', () => {
    it('should not recommend batch for high urgency', () => {
        const result = shouldUseBatch({ messages: [{ role: 'user', content: 'Hello' }] }, { urgency: 'high' });
    ...
    it('should not recommend batch for small requests', () => {
        const result = shouldUseBatch({ messages: [{ role: 'user', content: 'Hi' }] }, // Very small
        { urgency: 'low' });
        expect(result.useBatch).toBe(false);
        expect(result.reason).toContain('too small');
    });
    it('should recommend batch for low urgency, large requests', () => {
        const result = shouldUseBatch({ messages: [{ role: 'user', content: 'A'.repeat(1000) }] }, { urgency: 'low' });
    ...
    it('should not recommend batch if wait time unacceptable', () => {
        const result = shouldUseBatch({ messages: [{ role: 'user', content: 'A'.repeat(1000) }] }, { maxWaitAcceptable: 60000 } // 1 minute (less than 1 hour)
        );
        expect(result.useBatch).toBe(false);
...
//# sourceMappingURL=batch-api.test.js.map