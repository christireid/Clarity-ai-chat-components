...
import { countTokens, countConversationTokens, truncateToTokenBudget, chunkByTokens, getTokenizerStats, clearTokenCache, } from '../tokenization/accurate-counter';
import { estimateTokens, estimateTokensByProvider, getCharsPerToken, estimateMessagesTokens, validateEstimation, estimateTokensDebug, } from '../tokenization/estimator';
import { calculateCost, calculateCacheSavings, estimateConversationCost, compareModelCosts, recommendModel, MODEL_PRICING, } from '../tokenization/model-pricing';
// =============================================================================
...
describe('Accurate Counter', () => {
    ...
    describe('countTokens', () => {
        ...
        it('throws for unknown models', async () => {
            await expect(countTokens('test', { model: 'unknown-model' })).rejects.toThrow('Unknown model');
        });
    ...
    describe('countConversationTokens', () => {
        ...
        it('adds extra tokens for named messages', async () => {
            ...
            const resultWithName = await countConversationTokens(messagesWithName, { model: 'gpt-4' });
    ...
    describe('cache management', () => {
        ...
        it('evicts oldest entries when cache is full', async () => {
            ...
            const promises = Array.from({ length: 1001 }, (_, i) => countTokens(`text-${i}`, { model: 'gpt-4', cache: true }));
...
describe('Token Estimator', () => {
    ...
    describe('estimateTokensByProvider', () => {
        ...
        it('handles CJK text correctly for consistency with estimateTokens', () => {
            const chineseText = '你好世界'; // "Hello world" in Chinese
            const latinText = 'Hello'; // Similar length Latin text
            const chineseTokensOpenAI = estimateTokensByProvider(chineseText, 'openai');
...
describe('Tokenization Integration', () => {
    ...
    it('message estimation matches individual token estimation', () => {
        ...
        const individualTokens = estimateTokens('Hello', 'gpt-4') +
