...
describe('TokenCounter Comprehensive Tests', () => {
  ...
  describe('Multilingual Support', () => {
    Object.entries(TEST_CASES.multilingual).forEach(
      ([text, expectedTokens]) => {
  ...
  describe('Performance Tests', () => {
    ...
    it('should handle concurrent counting', async () => {
      ...
      const results = await Promise.all(
        texts.map((text) => Promise.resolve(TokenCounter.count(text)))
      )
  ...
  describe('Model-Specific Accuracy', () => {
    const testText =
      'The quick brown fox jumps over the lazy dog. This is a standard test sentence for token counting across different AI models.'
    const charCount = testText.length

    Object.entries(MODEL_EXPECTATIONS).forEach(
      ([model, { ratio, tolerance }]) => {
