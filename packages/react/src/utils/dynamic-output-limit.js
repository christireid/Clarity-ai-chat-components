 ...
 * ```
 */
export function calculateDynamicOutputLimit(config) {
    const { modelCapacity, inputTokenCount, userPreference = 'balanced', taskType = 'chat', uiConfiguredMax, minOutputTokens = 50, includeBrevityInstruction = true, } = config;
    // Input validation
    if (modelCapacity <= 0) {
    ...
    const taskBasedLimit = Math.min(taskConfig.base, Math.floor(absoluteMaxTokens * taskConfig.multiplier));
 ...
 * ```
 */
export function injectBrevityInstruction(systemPrompt, instruction) {
    if (!instruction)
        return systemPrompt;
 ...
 */
export function getPreferenceConfig(preference) {
    const configs = {
        concise: {
            multiplier: 0.5,
 ...
 */
export function createModelOutputConfig(model, inputTokenCount, overrides) {
 ...
 */
export function getMaxTokens(modelCapacity, inputTokenCount, preference = 'balanced', taskType = 'chat') {
    const result = calculateDynamicOutputLimit({
        modelCapacity,
        inputTokenCount,
...
//# sourceMappingURL=dynamic-output-limit.js.map