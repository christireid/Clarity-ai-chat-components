...
export interface PromptStructureOptions {
  /** Include section labels/headers */
  includeSectionHeaders?: boolean
  /** Maximum total tokens */
  maxTokens?: number
  /** Section separator */
  separator?: string
  /** Force question at end */
...
export function buildStructuredPrompt(
  ...
  options: PromptStructureOptions = {}
): StructuredPrompt {
  const {
  ...
  const beginning: PromptSection[] = []
  const middle: PromptSection[] = []
  ...
  const sortByPriority = (a: PromptSection, b: PromptSection) =>
  ...
  const sectionBreakdown: StructuredPrompt['sections'] = []
  ...
  for (const section of allSections) {
    ...
    if (beginning.includes(section)) position = 'beginning'
    else if (middle.includes(section)) position = 'middle'
    else position = 'end'
 ...
 * ```
 */
export function restructurePrompt(
  prompt: string,
  options: PromptStructureOptions = {}
): StructuredPrompt {
 ...
 */
function detectSections(prompt: string): PromptSection[] {
 ...
 */
function classifyParagraph(text: string): PromptSection {
...
export function createFewShotPrompt(params: {
  ...
  query: string
  exampleFormat?: (input: string, output: string) => string
}): StructuredPrompt {
  const {
    instruction,
    examples,
    query,
    exampleFormat = (i, o) => `Input: ${i}\nOutput: ${o}`,
  } = params
 ...
 */
export function estimateAttentionDistribution(prompt: StructuredPrompt): {
  beginning: { tokens: number; attention: number; sections: string[] }
