 ...
 */
import { EnhancedEnterpriseFeature, CLIError, ExitCode, } from '../enterprise/enterprise-feature-base';
import { formatBytes, calculatePercentage, deepMerge, } from '../internal/helpers';
/**
...
export class BundleAnalyzer extends EnhancedEnterpriseFeature {
    ...
    validateConfig() {
        ...
        if (thresholds.total <= 0) {
            throw new CLIError('Total threshold must be positive', ExitCode.VALIDATION_ERROR, ['Set total threshold to a positive value', 'Check configuration file']);
        }
        if (thresholds.perChunk <= 0 || thresholds.perAsset <= 0) {
            throw new CLIError('Per-chunk and per-asset thresholds must be positive', ExitCode.VALIDATION_ERROR, ['Set thresholds to positive values', 'Check configuration file']);
        }
        // Validate asset groups
        if (!this.config.assetGroups || this.config.assetGroups.length === 0) {
            throw new CLIError('Asset groups configuration is required', ExitCode.VALIDATION_ERROR, ['Define asset groups in configuration', 'Use default asset groups']);
        }
        // Validate output directory
        if (!this.config.outputDir || this.config.outputDir.trim() === '') {
            throw new CLIError('Output directory is required', ExitCode.VALIDATION_ERROR, [
    ...
    async process(webpackStats) {
        ...
        try {
            ...
            if (!webpackStats || typeof webpackStats !== 'object') {
                throw new CLIError('Invalid webpack stats provided', ExitCode.VALIDATION_ERROR, ['Provide valid webpack stats object', 'Check webpack configuration']);
            }
            // Process assets
     ...
     */
    findAssetGroup(assetName) {
        return this.config.assetGroups.find((group) => group.patterns.some((pattern) => {
     ...
     */
    generateWarnings(assets, chunks) {
        const warnings = [];
        // Check asset sizes
        ...
        chunks.forEach((chunk) => {
            if (chunk.size > this.config.thresholds.perChunk) {
                warnings.push({
                    type: 'chunk-size',
                    severity: chunk.size > this.config.thresholds.perChunk * 1.5
                        ? 'high'
                        : 'medium',
     ...
     */
    generateRecommendations(assets, chunks, warnings) {
        ...
        const uncompressedAssets = assets.filter((asset) => asset.compressedSize > asset.size * 0.8 && asset.size > 10 * 1024);
        if (uncompressedAssets.length > 0) {
            const totalSavings = uncompressedAssets.reduce((sum, asset) => sum + (asset.size - asset.compressedSize), 0);
            recommendations.push({
                type: 'compression',
                priority: 'medium',
        ...
        const largeImages = assets.filter((asset) => asset.type === 'image' && asset.size > 50 * 1024);
        if (largeImages.length > 0) {
            const totalSavings = largeImages.reduce((sum, image) => sum + Math.floor(image.size * 0.3), 0);
            recommendations.push({
                type: 'optimization',
                priority: 'medium',
     ...
     */
    calculateScore(assets, chunks, warnings) {
        let score = 100;
        // Penalize warnings
