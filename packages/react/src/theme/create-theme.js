...
function normalizeColors(colors) {
    ...
    for (const [key, value] of Object.entries(colors)) {
        if (value) {
            normalized[key] = toHSLString(value);
        }
    }
    return normalized;
}
 ...
 */
function getRadiusValue(preset) {
    const radiusMap = {
        none: '0',
        sm: '0.25rem',
        md: '0.375rem',
...
function getThemeByName(name) {
    ...
    if (process.env.NODE_ENV !== 'production') {
        console.warn(`[Clarity Chat] Unknown theme preset "${name}". ` +
...
export function createTheme(config) {
    ...
    const baseThemeName = config.extends ?? (config.mode === 'dark' ? 'default-dark' : 'default');
 ...
 */
export function removeThemeVariables(element) {
