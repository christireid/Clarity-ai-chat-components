...
import { hexToHSLString, hslStringToHex, getContrastingForeground, } from './color-utils';
...
export const ColorMixins = {
     ...
     */
    surfaces: (cardBg, popoverBg, mutedBg) => {
...
export class ThemeBuilder {
    ...
    fixContrast(colors) {
        ...
        try {
            ...
            const adjustedPrimary = autoAdjustForContrast(primaryHex, bgHex, this.contrastTarget);
            ...
            if (colors.destructive) {
                ...
                const adjustedDestructive = autoAdjustForContrast(destructiveHex, bgHex, this.contrastTarget);
 ...
 */
export function composeThemes(...themes) {
    let result = { ...modernThemes['default'] };
    for (const theme of themes) {
        if (typeof theme === 'string') {
            result = { ...result, ...modernThemes[theme] };
        }
        else {
            result = deepMergeTheme(result, theme);
        }
    }
    return result;
}
 ...
 */
function deepMergeTheme(base, override) {
    return {
 ...
 */
export function createVariant(base, variantFn) {
 ...
 */
export function createBrandTheme(brandColor, options = {}) {
 ...
 */
export function createMultiColorTheme(colors, options = {}) {
    const builder = new ThemeBuilder({
        base: options.base || (options.mode === 'dark' ? 'default-dark' : 'default'),
        mode: options.mode || 'light',
    })
    ...
    if (colors.success || colors.warning || colors.error) {
        builder.colors(ColorMixins.states({
...
//# sourceMappingURL=theme-composer.js.map