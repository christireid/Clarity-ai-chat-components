...
function getModernPreset(name: ModernThemePresetName) {
...
describe('Theme Visual Regression', () => {
  ...
  describe('Preset Completeness', () => {
    ...
    it.each(presetNames)(
      'preset %s should have all required color tokens',
      (presetName) => {
        ...
        requiredColors.forEach((colorKey) => {
          expect(theme.colors[colorKey as keyof ColorTokens]).toBeDefined()
          expect(theme.colors[colorKey as keyof ColorTokens]).toBeTruthy()
        })
      }
    )
    ...
    it.each(presetNames)(
      'preset %s should have valid border radius config',
      (presetName) => {
        const theme = getModernPreset(presetName)
