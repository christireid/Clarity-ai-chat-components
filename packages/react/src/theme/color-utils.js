 ...
 */
export function isHSLString(color) {
...
export function toHSLString(color) {
    if (!color || typeof color !== 'string') {
        if (process.env.NODE_ENV !== 'production') {
            console.warn(`[Clarity Chat] Invalid color value: ${color}. Using fallback.`);
        }
        return FALLBACK_HSL;
    }
    if (isHexColor(color)) {
    ...
    if (process.env.NODE_ENV !== 'production') {
        console.warn(`[Clarity Chat] Unrecognized color format: "${color}". ` +
...
export function adjustSaturation(hslString, amount) {
    const [h, s, l] = hslString.split(' ');
    const newS = Math.max(0, Math.min(100, parseFloat(s?.replace('%', '') ?? '0') + amount));
    return `${h ?? 0} ${newS}% ${l ?? '0%'}`;
}
/**
 ...
 */
export function meetsContrastRequirement(foreground, background, level = 'AA', isLargeText = false) {
 ...
 */
export function getContrastingForeground(background) {
 ...
 */
export function generatePaletteFromBrandColor(brandColor) {
