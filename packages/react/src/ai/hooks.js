 ...
 * ```
 */
export function useSuggestions(context, options) {
...
export function useModeration() {
    ...
    const moderateMessage = React.useCallback(async (content, context) => {
...
export function useSentimentAnalysis(options) {
    ...
    const analyzeFeedback = React.useCallback((text) => {
...
export function useAutoComplete(options) {
    ...
    const { suggestions, isLoading } = useSuggestions({ input }, {
        debounceMs: options?.debounceMs,
        minInputLength: options?.minInputLength,
    });
    // Filter to only completion suggestions
    const completions = React.useMemo(() => suggestions.filter((s) => s.type === 'completion'), [suggestions]);
    const selectCompletion = React.useCallback((suggestion) => {
