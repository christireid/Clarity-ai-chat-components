 ...
 */
export function createQuickReplyProvider(replies) {
    return (context) => {
        ...
        const suggestions = replies
            ...
            if (!reply.triggers?.length)
                return true;
            // Check if any trigger matches
            return reply.triggers.some(trigger => input.includes(trigger.toLowerCase()));
        })
            .map((reply, index) => ({
 ...
 */
export function createCommandProvider(commands) {
    return (context) => {
 ...
 */
export function createCompletionProvider(completions) {
    return (context) => {
        const input = context.input.toLowerCase().trim();
 ...
 */
export function createContextAwareProvider() {
    return (context) => {
 ...
 */
export function createProfanityFilter(bannedWords = []) {
 ...
 */
export function createSimpleSentimentAnalyzer() {
    const positiveWords = [
 ...
 */
export function combineModerationProviders(providers) {
    return async (content, context) => {
        const results = await Promise.all(providers.map(provider => provider(content, context)));
