 ...
 */
function calculateDelayWithJitter(baseDelay, attempt, jitterFactor) {
 ...
 * ```
 */
export function useStreamingError(options = {}) {
    const { maxRetries = 3, retryDelay = 1000, jitterFactor = 0.3, onRetry, onMaxRetriesExceeded, circuitBreakerThreshold = 5, circuitBreakerResetTime = 30000, onCircuitOpen, ...errorOptions } = options;
    const { handleError: baseHandleError, clearError: baseClearError, error: baseError, } = useEnhancedErrorHandler(errorOptions);
    ...
    const retryCallbackRef = React.useRef(null);
    const retryTimerRef = React.useRef(null);
    const countdownIntervalRef = React.useRef(null);
    const circuitTimerRef = React.useRef(null);
    // Extract streaming error or null
    const error = isStreamingError(baseError) ? baseError : null;
    const canRetry = error?.recoverable === true &&
    ...
    const handleStreamError = React.useCallback((rawError, opts) => {
        ...
        const streamError = rawError instanceof StreamingError
            ? rawError
            : new StreamingError(rawError instanceof Error ? rawError.message : String(rawError), {
    ...
    const setRetryCallback = React.useCallback((callback) => {
    ...
    const retry = React.useCallback(() => {
        ...
        if (!canRetry) {
            console.warn('[useStreamingError] Cannot retry: max retries exceeded or error not recoverable');
            return;
        }
        if (!retryCallbackRef.current) {
            console.warn('[useStreamingError] Cannot retry: no retry callback registered');
            return;
        }
        setIsRetrying(true);
    ...
    const resumeStream = React.useCallback(async (customPayload) => {
        if (!error?.hasPartialContent && !customPayload) {
            console.warn('[useStreamingError] Cannot resume: no partial content available');
            return;
        }
        if (!retryCallbackRef.current) {
            console.warn('[useStreamingError] Cannot resume: no retry callback registered');
            return;
        }
        setIsRetrying(true);
        const resumePayload = customPayload ?? {
            partialContent: error?.partialContent,
            lastEventId: lastEventId ?? error?.context?.['lastEventId'],
        };
        try {
...
//# sourceMappingURL=useStreamingError.js.map