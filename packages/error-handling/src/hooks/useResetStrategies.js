 ...
 * ```
 */
export function useResetStrategies(options = {}) {
    const { resetKeys, resetAfterTimeout, resetOnNetworkRestore, resetOnVisibilityChange, resetOnCustomEvent, onReset, } = options;
    const hasError = useRef(false);
    const timeoutRef = useRef(undefined);
    const prevKeysRef = useRef(undefined);
    const reset = useCallback((reason = 'manual') => {
    ...
    useEffect(() => {
        ...
        const keysChanged = resetKeys.length !== prevKeys.length ||
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener(resetOnCustomEvent, handleCustomEvent);
 ...
 * ```
 */
export function dispatchResetEvent(eventName) {
 ...
 */
export function useRouteChangeReset(onRouteChange, customEventName) {
    useEffect(() => {
...
//# sourceMappingURL=useResetStrategies.js.map