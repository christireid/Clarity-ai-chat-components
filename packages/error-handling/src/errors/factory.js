...
export const createConfigError = {
    missingApiEndpoint: () => new ConfigurationError('Missing API endpoint configuration', {
    ...
    invalidModel: (model, validModels) => new ConfigurationError(`Invalid model: ${model}`, {
        code: 'INVALID_MODEL',
    ...
    missingApiKey: () => new ConfigurationError('Missing API key', {
    ...
    invalidConfiguration: (field, value, expected) => new ConfigurationError(`Invalid configuration for ${field}`, {
    ...
    missingRequiredProp: (propName, componentName) => new ConfigurationError(`Missing required prop: ${propName}`, {
...
export const createApiError = {
    requestFailed: (endpoint, statusCode, statusText) => new APIError(`API request failed: ${endpoint}`, {
    ...
    invalidResponse: (endpoint, details) => new APIError(`Invalid API response from: ${endpoint}`, {
    ...
    serverError: (endpoint, statusCode) => new APIError(`Server error: ${endpoint}`, {
        code: 'SERVER_ERROR',
        statusCode,
    ...
    notFound: (endpoint) => new APIError(`API endpoint not found: ${endpoint}`, {
        code: 'API_NOT_FOUND',
        statusCode: 404,
...
export const createAuthError = {
    missingApiKey: () => new AuthenticationError('API key is missing', {
        code: 'MISSING_API_KEY',
        solution: 'Provide an API key via the apiKey prop or CLARITY_API_KEY environment variable',
        docs: 'https://docs.claritychat.dev/authentication',
    }),
    invalidApiKey: () => new AuthenticationError('API key is invalid', {
    ...
    insufficientPermissions: (action) => new AuthenticationError(`Insufficient permissions for action: ${action}`, {
...
export const createNetworkError = {
    connectionFailed: (endpoint) => new NetworkError(`Failed to connect to: ${endpoint}`, {
    ...
    requestTimeout: (endpoint, timeout) => new TimeoutError(`Request timed out: ${endpoint}`, {
    ...
    offline: () => new NetworkError('No internet connection', {
        code: 'OFFLINE',
        solution: 'Check your internet connection and try again',
    }),
    dnsLookupFailed: (hostname) => new NetworkError(`DNS lookup failed for: ${hostname}`, {
...
export const createValidationError = {
    invalidInput: (field, value, expected) => new ValidationError(`Invalid input for ${field}`, {
        code: 'INVALID_INPUT',
        field,
        value,
        expected,
    ...
    requiredField: (field) => new ValidationError(`Required field missing: ${field}`, {
    ...
    valueTooLong: (field, maxLength, actualLength) => new ValidationError(`Value too long for ${field}`, {
        code: 'VALUE_TOO_LONG',
        field,
    ...
    invalidFormat: (field, format) => new ValidationError(`Invalid format for ${field}`, {
        code: 'INVALID_FORMAT',
        field,
        expected: format,
...
export const createStreamError = {
    connectionLost: () => new StreamError('Stream connection lost', {
        code: 'STREAM_CONNECTION_LOST',
        solution: 'The stream connection was interrupted. It will automatically retry',
    }),
    malformedData: (data) => new StreamError('Received malformed stream data', {
    ...
    streamAborted: () => new StreamError('Stream was aborted', {
        code: 'STREAM_ABORTED',
        solution: 'The stream was manually aborted by the user or application',
    }),
    streamTimeout: (timeout) => new StreamError('Stream timed out', {
        code: 'STREAM_TIMEOUT',
...
export const createRateLimitError = {
    exceeded: (retryAfter, limit) => new RateLimitError('Rate limit exceeded', {
    ...
    quotaExceeded: (limit, used) => new RateLimitError('API quota exceeded', {
        code: 'QUOTA_EXCEEDED',
        limit,
...
export const createTokenLimitError = {
    messageTooLong: (limit, actual) => new TokenLimitError('Message exceeds token limit', {
    ...
    contextTooLong: (limit, actual) => new TokenLimitError('Conversation context exceeds token limit', {
