        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (message.includes('rate limit')) {
                    return errorResponse(new ApiError('Rate limit exceeded', {
                ...
                if (message.includes('unauthorized') ||
                    ...
                    return errorResponse(new ApiError('Unauthorized', {
                ...
                if (message.includes('not found')) {
                    return errorResponse(new ApiError('Not found', {
 ...
 * ```
 */
export function streamingApiHandler(handler) {
 ...
 * ```
 */
export async function writeErrorToStream(writer, error) {
    const encoder = new TextEncoder();
    const errorData = isClarityError(error)
        ? error.toJSON()
        : { message: error instanceof Error ? error.message : 'An error occurred' };
...
function sanitizeValue(value, depth = 0) {
    ...
    if (typeof value === 'object') {
        ...
        for (const key of keysToProcess) {
            ...
            sanitizedObj[sanitizedKey] = sanitizeValue(value[key], depth + 1);
        }
 ...
 * ```
 */
export function parseSSEError(data) {
    // Input validation
