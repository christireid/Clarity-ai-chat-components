/**
 * Beautiful table formatting utilities for dev-tools
...
export function table(data, columns, options = {}) {
    const { border = true, padding = 1, headerColor = chalk.bold.cyan, compact = false, } = options;
    const dim = chalk.dim;
...
export function keyValueTable(data, options = {}) {
    const { labelColor = chalk.dim, valueColor = (t) => t } = options;
