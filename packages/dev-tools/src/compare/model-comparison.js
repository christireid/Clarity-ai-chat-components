...
export class ModelComparator {
     ...
     */
    compare(promptId, prompt) {
        const responses = this.responses.get(promptId);
        if (!responses || responses.length === 0) {
            return null;
        }
        // Find best performers
        const fastest = responses.reduce((prev, curr) => curr.metadata.latency < prev.metadata.latency ? curr : prev);
        const cheapest = responses.reduce((prev, curr) => curr.metadata.cost < prev.metadata.cost ? curr : prev);
        const mostTokens = responses.reduce((prev, curr) => curr.metadata.totalTokens > prev.metadata.totalTokens ? curr : prev);
        // Calculate averages
     ...
     */
    generateRecommendations(responses, fastest, cheapest) {
        const recommendations = [];
        // Speed recommendation
        if (fastest.metadata.latency < 1000) {
            recommendations.push(`For real-time applications, use ${fastest.provider}/${fastest.model} (${fastest.metadata.latency}ms)`);
        }
        // Cost recommendation
        const avgCost = responses.reduce((sum, r) => sum + r.metadata.cost, 0) / responses.length;
        if (cheapest.metadata.cost < avgCost * 0.5) {
            recommendations.push(`For cost efficiency, use ${cheapest.provider}/${cheapest.model} ($${cheapest.metadata.cost.toFixed(4)} vs avg $${avgCost.toFixed(4)})`);
        }
        ...
        const longest = responses.reduce((prev, curr) => curr.content.length > prev.content.length ? curr : prev);
        if (longest.content.length > avgLength * 1.2) {
            recommendations.push(`For detailed responses, consider ${longest.provider}/${longest.model} (${longest.content.length} chars vs avg ${Math.round(avgLength)})`);
        }
        ...
        const bestBalance = costPerMs.reduce((prev, curr) => curr.ratio < prev.ratio ? curr : prev);
        recommendations.push(`For best cost/performance balance, use ${bestBalance.model}`);
...
export function scoreResponseQuality(prompt, response, expectedKeywords) {
    ...
    const coherence = Math.min(100, Math.max(0, (avgSentenceLength > 20 && avgSentenceLength < 150) ? 80 : 50));
    ...
    const completeness = Math.min(100, (responseWordCount / promptWordCount) * 20);
    // Relevance: keyword matching
    let relevance = 70; // Default
    if (expectedKeywords && expectedKeywords.length > 0) {
        const matchedKeywords = expectedKeywords.filter(kw => response.toLowerCase().includes(kw.toLowerCase()));
 ...
 */
export async function compareModels(prompt, modelConfigs, options) {
