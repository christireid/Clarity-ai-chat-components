...
export class DevNotifications {
    ...
    notify(options) {
        ...
        if (this.enabled) {
            ...
            if (this.config.groupSimilar) {
                const similar = this.notifications.find(n => !n.dismissed &&
     ...
     */
    performance(title, message, options) {
     ...
     */
    subscribe(listener, channel = 'all') {
    ...
    notifyListeners(notification) {
        ...
        this.channels.forEach((channel, channelId) => {
            ...
            if (!channel.enabled)
                return;
            if (channel.filter && !channel.filter(notification))
                return;
 ...
 */
export function createDevNotifications(config) {
...
export const devNotify = {
    info: (title, message) => getDevNotifications().info(title, message),
    success: (title, message) => getDevNotifications().success(title, message),
    warning: (title, message) => getDevNotifications().warning(title, message),
    error: (title, message) => getDevNotifications().error(title, message),
    performance: (title, message) => getDevNotifications().performance(title, message),
    build: (title, message) => getDevNotifications().build(title, message),
};
export default DevNotifications;
//# sourceMappingURL=dev-notifications.js.map