...
export interface TokenEvent {
  ...
  provider: string
  model: string
  operation: 'chat' | 'completion' | 'embedding' | 'image' | 'audio' | 'other'
  usage: TokenUsage
  cost: TokenCost
...
export class TokenTracker {
  ...
  private budget: BudgetConfig = { currency: 'USD' }
  private listeners: Set<(event: TokenEvent, stats: UsageStats) => void> = new Set()
  ...
  track(options: {
    provider: string
    model: string
    operation?: TokenEvent['operation']
    usage: TokenUsage
    conversationId?: string
    metadata?: Record<string, unknown>
  }): TokenEvent {
   ...
   */
  calculateCost(model: string, usage: TokenUsage): TokenCost {
   ...
   */
  estimateCost(model: string, estimatedPromptTokens: number, estimatedCompletionTokens: number): TokenCost {
   ...
   */
  private updateUsageTracking(event: TokenEvent): void {
    const dateKey = event.timestamp.toISOString().split('T')[0]
    const monthKey = event.timestamp.toISOString().slice(0, 7)

    this.dailyUsage.set(
      dateKey,
      (this.dailyUsage.get(dateKey) ?? 0) + event.cost.totalCost
    )

    this.monthlyUsage.set(
      monthKey,
      (this.monthlyUsage.get(monthKey) ?? 0) + event.cost.totalCost
    )
  }
  ...
  private checkBudget(event: TokenEvent): void {
    ...
    if (this.budget.perRequestLimit && event.cost.totalCost > this.budget.perRequestLimit) {
      console.warn(
        chalk.yellow(`⚠️ Request cost (${this.formatCost(event.cost.totalCost)}) exceeds per-request limit (${this.formatCost(this.budget.perRequestLimit)})`)
      )
    }
    ...
    if (this.budget.dailyLimit) {
      ...
      if (dailyCost > this.budget.dailyLimit) {
        console.warn(
          chalk.red(`🚨 Daily budget exceeded! Used: ${this.formatCost(dailyCost)}, Limit: ${this.formatCost(this.budget.dailyLimit)}`)
        )
      } else if (dailyCost > this.budget.dailyLimit * threshold) {
        console.warn(
          chalk.yellow(`⚠️ Approaching daily budget limit. Used: ${this.formatCost(dailyCost)} (${((dailyCost / this.budget.dailyLimit) * 100).toFixed(1)}%)`)
        )
      }
    }
    ...
    if (this.budget.monthlyLimit) {
      ...
      if (monthlyCost > this.budget.monthlyLimit) {
        console.warn(
          chalk.red(`🚨 Monthly budget exceeded! Used: ${this.formatCost(monthlyCost)}, Limit: ${this.formatCost(this.budget.monthlyLimit)}`)
        )
      } else if (monthlyCost > this.budget.monthlyLimit * threshold) {
        console.warn(
          chalk.yellow(`⚠️ Approaching monthly budget limit. Used: ${this.formatCost(monthlyCost)} (${((monthlyCost / this.budget.monthlyLimit) * 100).toFixed(1)}%)`)
        )
      }
    }
  }
   ...
   */
  getDailyUsage(days: number = 30): Array<{ date: string; tokens: number; cost: number }> {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const dayEvents = this.events.filter(e =>
        e.timestamp.toISOString().split('T')[0] === dateKey
      )
  ...
  getOptimizationRecommendations(): string[] {
    ...
    if (expensiveModelUsage > stats.totalCost * 0.8) {
      recommendations.push(
        'Consider using smaller models (e.g., GPT-4o-mini, Claude 3 Haiku) for simpler tasks to reduce costs.'
      )
    }
    ...
    if (stats.avgTokensPerRequest > 5000) {
      recommendations.push(
        'Average tokens per request is high. Consider summarizing context or implementing conversation memory management.'
      )
    }
    ...
    if (promptRatio > 0.8) {
      recommendations.push(
        'Prompts account for most of your token usage. Consider caching system prompts or using prompt compression techniques.'
      )
    }
    ...
    if (providers.length === 1) {
      recommendations.push(
        'Consider using multiple providers for redundancy and cost optimization.'
      )
    }
    ...
    if (duplicatePromptRatio > 0.2) {
      recommendations.push(
        `${(duplicatePromptRatio * 100).toFixed(0)}% of recent prompts appear similar. Implement response caching to reduce costs.`
      )
    }
   ...
   */
  private detectDuplicatePrompts(events: TokenEvent[]): number {
   ...
   * Add event listener
   */
  addListener(listener: (event: TokenEvent, stats: UsageStats) => void): () => void {
   ...
   * Get all events
   */
  getEvents(limit?: number): TokenEvent[] {
