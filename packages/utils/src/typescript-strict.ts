...
export type StrictValidation<T> = StrictValidationSuccess<T> | StrictValidationError;
 ...
 */
export function isStrictFunction<T extends (...args: any[]) => any = (...args: any[]) => any>(
  value: unknown
): value is T {
 ...
 */
export function isStrictPromise<T = unknown>(value: unknown): value is Promise<T> {
  return (
    value !== null &&
    typeof value === 'object' &&
 ...
 */
export function strictAssertNever(
  value: never,
  message?: string
): never {
...
export function validateStrictUnion<T extends readonly unknown[]>(
  value: unknown,
  validators: readonly [...{ [K in keyof T]: (value: unknown) => StrictValidation<T[K]> }]
 ...
 */
export function strictPropertyAccess<T extends Record<string, any>, K extends keyof T>(
 ...
 * Negate a type guard
 */
export function not<T>(guard: StrictTypeGuard<T>): StrictTypeGuard<Exclude<unknown, T>> {
 ...
 */
export function optional<T>(guard: StrictTypeGuard<T>): StrictTypeGuard<T | null | undefined> {
 ...
 */
export function strictMeasurePerformance<T>(
  name: string,
  fn: () => T
): T {
