 ...
 *
 * Errors for API operations including authentication, rate limiting, and network issues.
 */
...
function getExampleKey(provider) {
    const map = {
        openai: 'sk-...',
        anthropic: 'sk-ant-...',
        google: 'AI...',
...
function getProviderSignupUrl(provider) {
    const map = {
...
function getProviderDashboardUrl(provider) {
    const map = {
...
function getProviderStatusUrl(provider) {
    const map = {
...
/**
 * Error thrown when an API key is missing or not configured
 *
 * @example
 * ```ts
...
export class APIKeyMissingError extends ClarityError {
    constructor(provider, originalError) {
        ...
        super('API_KEY_MISSING', `${provider.toUpperCase()} API key is not configured`, `The environment variable ${getEnvKeyName(provider)} is missing or empty`, solutions, {
 ...
 *   throw new APIRateLimitError('openai', response.headers.get('Retry-After'))
 * }
 * ```
 */
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      throw error
        ...
        super('API_RATE_LIMIT', `${provider} rate limit exceeded`, `You've exceeded your API rate limit. ${retryAfter ? `Retry after ${retryAfter} seconds.` : ''}`, solutions, {
...
export class APIAuthenticationError extends ClarityError {
    constructor(provider, originalError) {
        ...
        super('API_AUTHENTICATION_FAILED', `Authentication failed for ${provider}`, 'Your API key is invalid, expired, or revoked', solutions, {
...
/**
 * Error thrown when network connection to API fails
 */
...
const client = new OpenAI({
  httpAgent: agent
        ...
        super('API_NETWORK_ERROR', `Network error connecting to ${provider}`, 'Failed to establish connection to the API server', solutions, {
...
export class APIResponseError extends ClarityError {
    statusCode;
    constructor(provider, statusCode, responseBody, originalError) {
        const solutions = [
            {
        ...
        super('API_RESPONSE_ERROR', `${provider} API returned an error (${statusCode})`, `The API request failed with status ${statusCode}`, solutions, {
 ...
 */
export class APITimeoutError extends ClarityError {
    timeout;
    constructor(provider, timeout, originalError) {
        super('API_TIMEOUT', `Request to ${provider} timed out after ${timeout}ms`, `The API request did not complete within the timeout period`, [
            {
                description: 'Increase the timeout or retry',
                steps: [
            ...
            },
            {
                ...
                steps: [
                    `Visit ${getProviderStatusUrl(provider)}`,
                    'Check for slow response times or incidents',
                ],
            },
        ], {
            location: 'API request',
