...
describe('EnvVarMissingError', () => {
  ...
  it('should include description if provided', () => {
    const error = new EnvVarMissingError(
      'MY_VAR',
      'This variable is used for authentication'
    )
...
describe('PortAlreadyInUseError', () => {
  ...
  it('should suggest finding the process', () => {
    ...
    const findSolution = error.solutions.find((s) =>
      s.description.includes('Find')
    )
  ...
  it('should suggest using different port', () => {
    const error = new PortAlreadyInUseError(3000)
    const alternateSolution = error.solutions.find((s) =>
      s.description.includes('different port')
    )
...
describe('FileNotFoundError', () => {
  ...
  it('should suggest creating the file', () => {
    ...
    expect(
      error.solutions[0].steps?.some((s) => s.includes('/app/config.ts'))
    ).toBe(true)
  })

  it('should suggest checking configuration', () => {
    const error = new FileNotFoundError('/some/path.js')
    const configSolution = error.solutions.find((s) =>
      s.description.includes('configuration')
    )
...
describe('DependencyMissingError', () => {
  ...
  it('should suggest installing the package', () => {
    ...
    expect(
      error.solutions[0].steps?.some((s) => s.includes('npm install express'))
    ).toBe(true)
  })
...
describe('ConfigParseError', () => {
  ...
  it('should suggest validation', () => {
    const error = new ConfigParseError('package.json', 'JSON', 'Parse error')
    const validateSolution = error.solutions.find((s) =>
      s.steps?.some((step) => step.toLowerCase().includes('valid'))
    )
