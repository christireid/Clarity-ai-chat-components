...
import { APIKeyMissingError, APIRateLimitError, APIAuthenticationError, APINetworkError, APIResponseError, APITimeoutError, } from '../api.js';
...
describe('APIRateLimitError', () => {
    ...
    it('should include exponential backoff solution', () => {
        const error = new APIRateLimitError('openai');
        const backoffSolution = error.solutions.find((s) => s.description.toLowerCase().includes('backoff'));
...
describe('APIAuthenticationError', () => {
    ...
    it('should include verification steps', () => {
        ...
        expect(error.solutions[0].steps?.some((s) => s.includes('typos'))).toBe(true);
    });
    ...
    it('should suggest generating new key', () => {
        const error = new APIAuthenticationError('anthropic');
        const newKeySolution = error.solutions.find((s) => s.description.includes('Generate'));
...
describe('APINetworkError', () => {
    ...
    it('should suggest checking internet connection', () => {
        ...
        const connectionSolution = error.solutions.find((s) => s.description.includes('internet connection'));
        expect(connectionSolution).toBeDefined();
    });
    it('should suggest checking service status', () => {
        const error = new APINetworkError('openai');
        const statusSolution = error.solutions.find((s) => s.description.includes('service'));
    ...
    it('should include proxy configuration example', () => {
        const error = new APINetworkError('openai');
        const proxySolution = error.solutions.find((s) => s.description.toLowerCase().includes('proxy'));
        expect(proxySolution?.example).toContain('proxy');
    });
});
describe('APIResponseError', () => {
    it('should create error with status code', () => {
        const error = new APIResponseError('openai', 500, {
    ...
    it('should include docs URL for error codes', () => {
        ...
        expect(error.solutions[0].docsUrl).toContain('error');
    });
    it('should preserve original error', () => {
...
describe('APITimeoutError', () => {
    it('should create timeout error with provider', () => {
        const error = new APITimeoutError('openai', 30000);
        expect(error.code).toBe('API_TIMEOUT');
        expect(error.userMessage).toContain('timed out');
        expect(error.context.data?.timeout).toBe(30000);
    });
    it('should include timeout duration in message', () => {
        const error = new APITimeoutError('anthropic', 60000);
        expect(error.userMessage).toContain('60000');
    });
    it('should suggest increasing timeout', () => {
        const error = new APITimeoutError('google', 5000);
        const timeoutSolution = error.solutions.find((s) => s.description.toLowerCase().includes('timeout'));
        expect(timeoutSolution).toBeDefined();
    });
    it('should be instance of ClarityError', () => {
        const error = new APITimeoutError('openai', 10000);
        expect(error).toBeInstanceOf(ClarityError);
    });
});
