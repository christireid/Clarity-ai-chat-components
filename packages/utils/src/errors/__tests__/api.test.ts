...
describe('APIAuthenticationError', () => {
  ...
  it('should include verification steps', () => {
    ...
    expect(error.solutions[0].steps?.some((s) => s.includes('typos'))).toBe(
      true
    )
  })
...
describe('APINetworkError', () => {
  ...
  it('should suggest checking service status', () => {
    ...
    const statusSolution = error.solutions.find((s) =>
      s.description.includes('service')
    )
  ...
  it('should include proxy configuration example', () => {
    ...
    const proxySolution = error.solutions.find((s) =>
      s.description.toLowerCase().includes('proxy')
    )
...
describe('APITimeoutError', () => {
  ...
  it('should suggest increasing timeout', () => {
    ...
    const timeoutSolution = error.solutions.find((s) =>
      s.description.toLowerCase().includes('timeout')
    )
