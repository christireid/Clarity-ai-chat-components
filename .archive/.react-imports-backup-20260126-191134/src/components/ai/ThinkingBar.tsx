...
const IndeterminateProgress: React.FC<IndeterminateProgressProps> = ({
  ...
  <motion.div
    ...
    animate={
      prefersReducedMotion
        ? {
            ...ANIMATION_PRESETS.fadeIn.animate,
            x: '0%',
            width: '100%',
            opacity: [0.5, 1, 0.5],
          }
...
export function ThinkingBar({
  ...
  const isIndeterminate =
    indeterminate ??
    (progress === undefined && status !== 'complete' && status !== 'idle')
  ...
  const computedAriaLabel = React.useMemo(() => {
    ...
    const timeText = estimatedTime
      ? `, ${formatEstimatedTime(estimatedTime)} remaining`
      : ''
  ...
  if (variant === 'detailed') {
    return (
      ...
      >
        ...
        <div
          className={cn(
            'h-2 overflow-hidden rounded-full bg-muted/40',
            progressClassName
          )}
        >
          {isIndeterminate ? (
...
export function useThinkingBar({
  ...
  const [status, setStatusState] =
    React.useState<ThinkingBarStatus>(initialStatus)
