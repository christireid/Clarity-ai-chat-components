...
export class TokenAnalyticsMonitor {
  ...
  public getUsageStats(): {
    ...
    const recentTotal = recentDays.reduce(
      (sum, day) => sum + (daily[day]?.totalTokens || 0),
      0
    )
    const olderTotal = olderDays.reduce(
      (sum, day) => sum + (daily[day]?.totalTokens || 0),
      0
    )
  ...
  private checkUnusualPatterns(event: TokenUsageEvent): void {
    ...
    const average =
      recentTokens.reduce((sum, t) => sum + t, 0) / recentTokens.length
  ...
  private calculateCostForEvents(events: TokenUsageEvent[]): number {
    ...
    Object.entries(modelEvents).forEach(([model, modelEventsList]) => {
      ...
      if (pricing && modelEventsList.length > 0) {
        const totalTokens = modelEventsList.reduce(
          (sum, e) => sum + e.tokens,
          0
        )
        const firstEvent = modelEventsList[0]
        const costPerToken =
          firstEvent?.type === 'output' ? pricing.output : pricing.input
