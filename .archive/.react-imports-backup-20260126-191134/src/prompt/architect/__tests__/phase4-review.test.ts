...
describe('Phase 4: Review & Reflection', () => {
  ...
  describe('DRY_VIOLATION_PATTERNS', () => {
    it('should have all violation types', () => {
      ...
      for (const type of types) {
        expect(DRY_VIOLATION_PATTERNS[type as keyof typeof DRY_VIOLATION_PATTERNS]).toBeDefined()
      }
    })
  ...
  describe('createSelfCorrectionItem', () => {
    it('should create a passed self-correction item', () => {
      const item = createSelfCorrectionItem(
        'Code is DRY',
        true
      )
  ...
  describe('calculateQualityScore', () => {
    it('should return 100 for perfect code', () => {
      const score = calculateQualityScore(
        createDefaultSelfCorrection(),
        createDRYCheckResult({ isDRY: true }),
        []
      )
    ...
    it('should not go below 0', () => {
      ...
      const score = calculateQualityScore(
        [],
        createDRYCheckResult({ isDRY: false, duplications: Array(10).fill({ description: '', locations: [], suggestion: '' }) }),
        manyFindings
      )
